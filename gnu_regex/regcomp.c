    ...
    if (type == CHARACTER) {
      ...
      if ((bufp->syntax & RE_ICASE) && dfa->mb_cur_max > 1) {
        ...
        *p++ = dfa->nodes[node].opr.c;
        while (++node < dfa->nodes_len && dfa->nodes[node].type == CHARACTER &&
      ...
      if (dfa->mb_cur_max > 1 && (cset->nchar_classes || cset->non_match
#ifdef _LIBC
                                  || cset->nequiv_classes
#endif /* _LIBC */
                                  )) {
...
#ifdef _LIBC
libc_freeres_fn(free_mem) { __regfree(&re_comp_buf); }
#endif
...
static bin_tree_t *parse_expression(re_string_t *regexp, regex_t *preg,
  ...
  case OP_NOTWORD:
    tree = build_charclass_op(
        dfa, regexp->trans, (const unsigned char *)"alnum",
        (const unsigned char *)"_", token->type == OP_NOTWORD, err);
    if (BE(*err != REG_NOERROR && tree == NULL, 0))
      return NULL;
    break;
  case OP_SPACE:
  case OP_NOTSPACE:
    tree = build_charclass_op(
        dfa, regexp->trans, (const unsigned char *)"space",
        (const unsigned char *)"", token->type == OP_NOTSPACE, err);
    if (BE(*err != REG_NOERROR && tree == NULL, 0))
      return NULL;
    break;
  case OP_ALT:
...
static bin_tree_t *parse_bracket_exp(re_string_t *regexp, re_dfa_t *dfa,
  ...
  while (1) {
    ...
    if (is_range_exp == 1) {
      ...
      ret = parse_bracket_element(&end_elem, regexp, &token2, token_len2, dfa,
                                  syntax, 1);
      if (BE(ret != REG_NOERROR, 0)) {
        *err = ret;
        goto parse_bracket_exp_free_return;
...
#else
#ifdef RE_ENABLE_I18N
      *err = build_range_exp(sbcset, dfa->mb_cur_max > 1 ? mbcset : NULL,
    ...
    for (ch = 0; ch < SBC_MAX; ++ch) {
      ...
      */
      ...
      if (len == weights[idx2 & 0xffffff] && (idx1 >> 24) == (idx2 >> 24)) {
        ...
        while (cnt <= len && weights[(idx1 & 0xffffff) + 1 + cnt] ==
...
build_charclass(RE_TRANSLATE_TYPE trans, bitset_t sbcset, re_charset_t *mbcset,
                int *char_class_alloc, const unsigned char *class_name,
                reg_syntax_t syntax)
#else  /* not RE_ENABLE_I18N */
