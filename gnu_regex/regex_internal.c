...
build_wcs_upper_buffer(re_string_t *pstr) {
  ...
  if (!pstr->map_notascii && pstr->trans == NULL && !pstr->offsets_needed) {
    while (byte_idx < end_idx) {
      ...
      prev_st = pstr->cur_state;
      mbclen = __mbrtowc(
          &wc, ((const char *)pstr->raw_mbs + pstr->raw_mbs_idx + byte_idx),
    ...
    if (BE(offset < pstr->valid_raw_len, 1)) {
      ...
      if (pstr->mb_cur_max > 1) {
          ...
          if (isascii(*p) && BE(pstr->trans == NULL, 1)) {
            ...
            for (; p >= end; --p)
              if ((*p & 0xc0) != 0x80) {
                ...
                mbclen = __mbrtowc(&wc2, (const char *)p, mlen, &cur_state);
        ...
        if (wc == WEOF)
          ...
          pstr->tip_context =
              ((BE(pstr->word_ops_used != 0, 0) && IS_WIDE_WORD_CHAR(wc))
                   ? CONTEXT_WORD
...
#endif /* RE_ENABLE_I18N */
      {
        ...
        pstr->tip_context =
            (bitset_contain(pstr->word_char, c)
                 ? CONTEXT_WORD
                 : ((IS_NEWLINE(c) && pstr->newline_anchor) ? CONTEXT_NEWLINE
                                                            : 0));
      }
    }
    if (!BE(pstr->mbs_allocated, 0))
...
static int internal_function re_dfa_add_node(re_dfa_t *dfa, re_token_t token) {
  ...
  if (BE(dfa->nodes_len >= dfa->nodes_alloc, 0)) {
    ...
    if (BE(new_nexts == NULL || new_indices == NULL || new_edests == NULL ||
               new_eclosures == NULL,
           0))
      return -1;
    dfa->nexts = new_nexts;
