 ...
 *
 *   This module contains code for generating tags for Windows PowerShell
 * scripts (https://en.wikipedia.org/wiki/PowerShell).
 */
...
static const char *const accessTypes[] = {ACCESS_UNDEFINED, "global", "local",
                                          "script", "private"};

typedef enum { K_FUNCTION, K_VARIABLE, COUNT_KIND } powerShellKind;

static kindDefinition PowerShellKinds[COUNT_KIND] = {
    {true, 'f', "function", "functions"}, {true, 'v', "variable", "variables"}};
...
static void initPowerShellEntry(tagEntryInfo *const e,
...
static const char *parsePowerShellScope(tokenInfo *const token) {
  ...
  if (powershellScopeEnd) {
    ...
    memmove(token->string->buffer,
            ...
            /* +1 for the skipped leading colon and - 1 to include the trailing
               \0 byte */
...
static bool parseFunction(tokenInfo *const token) {
  ...
  if (token->type == TOKEN_OPEN_PAREN) {
    ...
    readToken(token);
             ...
             TOKEN_OPEN_CURLY) { /* filters doesn't need to have an arglist */
...
static bool parseVariable(tokenInfo *const token) {
  ...
  if (token->type == TOKEN_EQUAL_SIGN) {
    if (token->parentKind !=
        K_FUNCTION) { /* ignore local variables (i.e. within a function) */
