...
static bool isValue(int c) { return (c != '\0'); }

static iniconfSubparser *maySwitchLanguage(const char *section, const char *key,
                                           const char *value) {
  ...
  foreachSubparser(sub, false) {
    ...
    if ((sub->direction & SUBPARSER_BASE_RUNS_SUB) && s->probeLanguage) {
      bool r;
...
static void makeIniconfTagMaybe(const char *section, const char *key,
...
static void findIniconfTags(void) {
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (*cp == '[') {
      ...
      makeIniconfTagMaybe(vStringValue(name), NULL, NULL, &sectionCorkIndex);
    ...
    while (*cp != '\0') {
      /*  We look for any sequence of identifier characters following a white
       * space */
      if (possible && isIdentifier((int)*cp)) {
        ...
        if (*cp == '=' || *cp == ':') {
          ...
          makeIniconfTagMaybe(
              (vStringLength(scope) > 0) ? vStringValue(scope) : NULL,
              vStringValue(name), vStringValue(val), &sectionCorkIndex);
          if (!sub)
            sub = maySwitchLanguage(
                (vStringLength(scope) > 0) ? vStringValue(scope) : NULL,
                vStringValue(name), vStringValue(val));
          if (sub) {
            enterSubparser((subparser *)sub);
            sub->newDataNotify(
                sub, (vStringLength(scope) > 0) ? vStringValue(scope) : NULL,
                vStringValue(name), vStringValue(val));
            leaveSubparser();
          }
          vStringClear(val);
