 ...
 *
 *   This module contains functions for generating tags for property list
 * defined in http://www.apple.com/DTDs/PropertyList-1.0.dtd.
 */
...
static void plistFindTagsUnderKey(xmlNode *node, const char *xpath,
...
static tagXpathTable plistXpathMainTable[] = {
    {"///plist//dict/key", LXPATH_TABLE_DO_RECUR,
     .spec.recurSpec = {plistFindTagsUnderKey}},
};

static tagXpathTable plistXpathTextTable[] = {
    {"text()", LXPATH_TABLE_DO_MAKE,
     .spec.makeTagSpec = {K_KEY, ROLE_DEFINITION_INDEX, makeTagWithScope}},
};
...
static bool isCompoundElement(xmlNode *node) {
  return !!(node->name && ((strcmp((char *)(node->name), "dict") == 0) ||
...
static void
plistFindTagsUnderKey(xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
                      const struct sTagXpathRecurSpec *spec CTAGS_ATTR_UNUSED,
                      xmlXPathContext *ctx, void *userData CTAGS_ATTR_UNUSED) {
  xmlNode *current;
  xmlNode *prev;
  stringList *queue;
  vString *path;
  ...
  for (current = node; current; current = current->parent) {
    if (isCompoundElement(current) && (prev = getPrevKeyElement(current))) {
  ...
  findXMLTags(ctx, node, TABLE_TEXT,
...
static void findPlistTags(void) {
...
extern parserDefinition *PlistXMLParser(void) {
