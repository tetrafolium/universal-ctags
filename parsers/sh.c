...
static kindDefinition ShKinds[] = {
    ...
    {true, 's', "script", "script files", .referenceOnly = true,
     ATTACH_ROLES(ShScriptRoles)},
    {true, 'h', "heredoc", "label for here document", .referenceOnly = false,
     ATTACH_ROLES(ShHeredocRoles)},
};
...
static bool isIdentChar(int c) { return (isalnum(c) || c == '_' || c == '-'); }
...
static void hdocStateMakePromiseMaybe(struct hereDocParsingState *hstate) {
  if (hstate->sublang != LANG_IGNORE)
    makePromise(getLanguageName(hstate->sublang), hstate->startLine, 0,
                getInputLineNumber(), 0, 0);
  hstate->sublang = LANG_IGNORE;
}

static void
hdocStateRecordStartlineFromDestfileMaybe(struct hereDocParsingState *hstate) {
...
static void hdocStateUpdateTag(struct hereDocParsingState *hstate,
...
static void findShTags(void) {
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (hereDocDelimiter) {
      ...
      if ((strncmp((const char *)cp, vStringValue(hereDocDelimiter),
                   vStringLength(hereDocDelimiter)) == 0) &&
        ...
        if (!vStringIsEmpty(hereDocDelimiter))
          makeSimpleRefTag(hereDocDelimiter, K_HEREDOCLABEL,
                           R_HEREDOC_ENDMARKER);
    ...
    while (*cp != '\0') {
      ...
      if (!check_char((int)*cp)) {
        ...
        int d = hdocStateReadDestfileName(&hstate, cp, hereDocDelimiter);
        if (d > 0)
          cp += d;
        else if (*cp != '\0')
          ++cp;
...
extern parserDefinition *ShParser(void) {
  static const char *const extensions[] = {"sh",  "SH",  "bsh", "bash",
                                           "ksh", "zsh", "ash", NULL};
  static const char *const aliases[] = {"sh", "bash", "ksh", "zsh", "ash",
