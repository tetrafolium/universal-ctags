...
static kindDefinition DtdKinds[] = {
    ...
    {true, 'e', "element", "elements", .referenceOnly = false,
     ATTACH_ROLES(DtdElementRoles)},
    {
        true,
        'a',
        "attribute",
...
static void copyToken(tokenInfo *dest, tokenInfo *src,
...
static tokenInfo *newDtdToken(void) { return newToken(&dtdTokenInfoClass); }
...
static void copyToken(tokenInfo *dest, tokenInfo *src,
                      void *data CTAGS_ATTR_UNUSED) {
  TOKENX(dest, tokenExtra)->scopeIndex = TOKENX(src, tokenExtra)->scopeIndex;
}
...
retry:
  ...
  default:
    if (isIdentifierChar(c)) {
      ...
      token->keyword = lookupKeyword(vStringValue(token->string), Lang_dtd);
...
static void parseEntity(tokenInfo *const token) {
  ...
  if (token->type == '%') {
    ...
    if (tokenIsType(token, IDENTIFIER))
      index =
          makeDtdTagMaybe(&e, token, K_PARAMETER_ENTITY, ROLE_DEFINITION_INDEX);
  } else if (tokenIsType(token, IDENTIFIER))
    index = makeDtdTagMaybe(&e, token, K_ENTITY, ROLE_DEFINITION_INDEX);
...
static void parseElement(tokenInfo *const token, bool skipToClose) {
  ...
  if (token->type == '%') {
    ...
    if (identifier) {
      makeDtdTagMaybe(&e, identifier, K_PARAMETER_ENTITY,
                      DTD_PARAMETER_ENTITY_ELEMENT_NAME);
      tokenDelete(identifier);
    }
  } else if (tokenIsType(token, IDENTIFIER))
...
static void parseAttDefs(tokenInfo *const token) {
  ...
  do {
    ...
    if (tokenIsType(token, IDENTIFIER)) {
      tagEntryInfo e;
      makeDtdTagMaybe(&e, token, K_ATTRIBUTE, ROLE_DEFINITION_INDEX);
...
static void parseAttlist(tokenInfo *const token) {
  ...
  if (token->type == '%') {
    ...
    if (tokenIsType(token, IDENTIFIER)) {
      tokenInfo *identifier = parserParameterEntityRef(token);
      if (identifier) {
        index = makeDtdTagMaybe(&e, identifier, K_ENTITY,
                                DTD_PARAMETER_ENTITY_ELEMENT_NAME);
        tokenDelete(identifier);
    ...
    index = makeDtdTagMaybe(&e, element, K_ELEMENT, DTD_ELEMENT_ATT_OWNER);
    tokenDelete(element);
...
static void parseNotation(tokenInfo *const token) {
  ...
  tokenRead(token);
  if (tokenIsType(token, IDENTIFIER))
    index = makeDtdTagMaybe(&e, token, K_NOTATION, ROLE_DEFINITION_INDEX);
  ...
  if (tokenIsKeyword(token, IGNORE))
    ...
    if (tokenIsKeyword(token, INCLUDE)) {
      ...
      if (condition) {
        tagEntryInfo e;
        unsigned int index;
        index = makeDtdTagMaybe(&e, condition, K_PARAMETER_ENTITY,
                                DTD_PARAMETER_ENTITY_CONDITION);
