 ...
 *   DATA DEFINITIONS
 */
typedef enum { K_PROCEDURE, K_NAMESPACE } tclKind;
...
#define TOKEN_PSTATE(TOKEN) TOKENX(TOKEN, struct tokenExtra)->pstate
...
static void readToken0(tokenInfo *const token, struct sTclParserState *pstate) {
  ...
  bool escaped;
  bool bol =
      (pstate->lastTokenType == TOKEN_TCL_EOL || pstate->lastTokenType == ';' ||
...
static bool tokenIsEOL(tokenInfo *const token) {
  if (token->type == ';' || tokenIsType(token, TCL_EOL) || tokenIsEOF(token))
    return true;
  return false;
}
...
static void parseProc(tokenInfo *const token, unsigned int parent) {
  int index = CORK_NIL;
  ...
  if (tokenIsType(token, TCL_IDENTIFIER)) {
    ...
    if (last) {
      ...
      if (isAbsoluteIdentifier(token)) {
        ...
        const char *sep = scopeSeparatorFor(getInputLanguage(), K_PROCEDURE,
                                            e_parent->kindIndex);
      ...
      if (isXtagEnabled(XTAG_QUALIFIED_TAGS)) {
        const char *sep = scopeSeparatorFor(
            getInputLanguage(), K_PROCEDURE,
            vStringIsEmpty(ns) ? KIND_GHOST_INDEX : K_NAMESPACE);
...
static void parseNamespace(tokenInfo *const token, unsigned int parent) {
  tokenRead(token);
  if (tokenIsEOF(token))
    return;
