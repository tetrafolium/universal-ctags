...
typedef enum eAdaException { EXCEPTION_NONE, EXCEPTION_EOF } adaException;
...
static const char *AdaKeywords[] = {
    "accept", "begin",    "body",      "case",      "constant",   "declare",
    "do",     "else",     "elsif",     "end",       "entry",      "exception",
    "for",    "function", "generic",   "if",        "in",         "is",
    "loop",   "new",      "not",       "or",        "overriding", "package",
    "pragma", "private",  "procedure", "protected", "record",     "renames",
    "select", "separate", "subtype",   "task",      "then",       "type",
...
static struct cmpKeywordOrWordDataElt *
skipPastKeywordOrWord(struct cmpKeywordOrWordDataElt *elt, int count);
...
#define isAdaComment(buf, pos, len)                                            \
  (((pos) == 0 || (!isalnum((buf)[(pos)-1]) && (buf)[(pos)-1] != '_')) &&      \
   (pos) < (len) && strncasecmp(&(buf)[(pos)], "--", strlen("--")) == 0)
...
static bool cmp(const char *buf, int len, const char *match) {
  ...
  if ((strncasecmp(buf, match, matchLen) == 0) &&
      (matchLen == len ||
       (matchLen < len && (isspace(buf[matchLen]) || buf[matchLen] == '(' ||
...
skipPastKeywordOrWord(struct cmpKeywordOrWordDataElt *elt, int count) {
  struct cmpKeywordOrWordData data = {
      .found = NULL, .count = count, .elt = elt};
...
static adaTokenInfo *adaParseBlock(adaTokenInfo *parent, adaKind kind) {
  ...
  } /* while(token != NULL) - while the end of spec, or beginning of body not
       found */
...
static adaTokenInfo *adaParseSubprogram(adaTokenInfo *parent, adaKind kind) {
  ...
  while (exception != EXCEPTION_EOF && token != NULL) {
    ...
    if (adaKeywordCmp(ADA_KEYWORD_IS)) {
      ...
      /* nothing found, move to the next word */
      movePos(
          1); /* make sure to advance even if we aren't actually on a word */
      skipPastWord();
    }
  } /* while(exception != EXCEPTION_EOF && token != NULL) */
...
static adaTokenInfo *adaParseVariables(adaTokenInfo *parent, adaKind kind) {
  ...
  if (varEndPos != -1) {
    ...
    if (tokenStart != -1) {
      token =
          newAdaToken(&buf[tokenStart], i - tokenStart, kind, false, parent);
...
static adaTokenInfo *adaParseLoopVar(adaTokenInfo *parent) {
  ...
  for (i = 1; (pos + i) < lineLen && !isspace(line[pos + i]); i++)
    ;
  token =
      newAdaToken(&line[pos], i, ADA_KIND_AUTOMATIC_VARIABLE, false, parent);
  movePos(i);
  ...
  while (exception == EXCEPTION_NONE) {
    ...
    case ADA_ROOT:
      if (adaKeywordCmp(ADA_KEYWORD_PACKAGE)) {
        ...
        if (exception != EXCEPTION_EOF && line[pos] == '(') {
          ...
          token = newAdaToken(&line[pos], i, ADA_KIND_SEPARATE, false, parent);
    ...
    case ADA_GENERIC:
      ...
      if (adaKeywordCmp(ADA_KEYWORD_PACKAGE)) {
        ...
        appendAdaToken(
            &genericParamsRoot,
            newAdaToken(&line[pos], i, ADA_KIND_FORMAL, false, NULL));
    ...
    case ADA_DECLARATIONS:
      ...
      if (token != NULL) {
        ...
        if (token->kind == ADA_KIND_PACKAGE ||
            ...
            token->kind == ADA_KIND_TASK || token->kind == ADA_KIND_PROTECTED) {
          /* if any generic params have been gathered, attach them to
           * token */
          appendAdaTokenList(token, &genericParamsRoot.children);
        }
       ...
       * one word keywords and then the start <stuff> end statements */
        ...
        for (i = 1; (pos + i) < lineLen; i++) {
          ...
          if (!isalnum(line[pos + i]) && line[pos + i] != '_' &&
            ...
            token =
                newAdaToken(&line[pos], i, ADA_KIND_IDENTIFIER, false, parent);
            break;
          }
        } /* for(i = 1; (pos + i) < lineLen; i++) */
...
static void storeAdaTags(adaTokenInfo *token, const char *parentScope) {
  ...
  if ((token->kind > ADA_KIND_UNDEFINED) && (token->kind < ADA_KIND_COUNT) &&
      (AdaKinds[token->kind].enabled == true) && (token->name != NULL) &&
    ...
    if ((isXtagEnabled(XTAG_QUALIFIED_TAGS) == true) &&
        ...
        (token->kind != ADA_KIND_FORMAL) && (token->kind != ADA_KIND_LABEL) &&
