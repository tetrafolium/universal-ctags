...
static tagRegexTable myrddinTagRegexTable[] = {
    ...
    /* var foo = initializer */
    {"^([ \t]*extern)?[ \t]*var[ \t]+([a-zA-Z_][a-zA-Z0-9_]*)", "\\2",
     "v,var,variables", NULL},
    /* type foo = name */
    {"^[ \t]*type[ \t]+([a-zA-Z_][a-zA-Z0-9_]*)[ \t]*=", "\\1", "t,type,types",
     NULL},
    /* trait foo = trait-defn ;; */
    {"^[ \t]*trait[ \t]+([a-zA-Z_][a-zA-Z0-9_]*)[ \t]*=", "\\1",
     "r,trait,traits", NULL},
    /* pkg foo = declarations ;; */
    {"^[ \t]*pkg[ \t]+([a-zA-Z_][a-zA-Z0-9_]*)", "\\1", "p,pkg,packages", NULL},
};
