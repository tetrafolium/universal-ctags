 ...
 *version.
 *
 *	 This module contains functions for generating tags for TypeScript
 *language files.
 *
 *	 Reference:
...
static kindDefinition TsKinds[] = {
    ...
    {false, 'z', "parameter",
     "function parameters inside function definitions"},
...
typedef void (*Parser)(const int c, tokenInfo *const, parserState *state,
                       parserResult *const);
...
static int emitTag(const tokenInfo *const token, const tsKind kind) {
  ...
  static const char *const access[3] = {"private", "protected", "public"};
...
CTAGS_INLINE void parseOneChar(const int c, tokenInfo *const token,
                               const char expected, const tokenType type,
                               parserResult *const result) {
  if (c != expected) {
    result->status = PARSER_FAILED;
    return;
...
CTAGS_INLINE void parseChar(const int c, tokenInfo *const token, void *state,
                            parserResult *const result, const char *chars,
...
CTAGS_INLINE void parseWord(const int c, tokenInfo *const token,
                            const char *word, int *parsed,
                            parserResult *const result) {
  if (word[*parsed] == '\0') {
    if (isIdentChar(c)) {
      result->status = PARSER_FAILED;
...
CTAGS_INLINE void parseWordToken(const int c, tokenInfo *const token,
                                 const char *word, const tokenType type,
                                 int *parsed, parserResult *const result) {
  if (c == word[*parsed]) {
    *parsed += 1;
...
CTAGS_INLINE void parseComment(const int c, tokenInfo *const token,
  ...
  if (state->comment.parsed < 2) {
    ...
    if (result->status == PARSER_FAILED) {
      parseWordToken(c, token, "/*", TOKEN_COMMENT_BLOCK,
                     &state->comment.parsed, result);
  ...
  if (c == EOF)
    ...
    parseWordToken(c, token, "*/", TOKEN_COMMENT_BLOCK,
                   &state->comment.blockParsed, result);
...
CTAGS_INLINE void parseString(const int c, tokenInfo *const token,
                              const char quote, char *prev,
                              parserResult *const result) {
  if (*prev == '\0') {
    if (c == quote) {
      *prev = c;
...
CTAGS_INLINE void parseBlock(const int c, tokenInfo *const token,
                             tokenType const ttype, const char start,
                             const char end, blockState *state,
                             parserResult *const result) {
  if (state->parsed == 0) {
    if (c != start) {
      result->status = PARSER_FAILED;
...
CTAGS_INLINE void parseIdentifierCommon(const int c, tokenInfo *const token,
                                        int *parsed, parserResult *const result,
                                        bool acceptFqName) {
...
CTAGS_INLINE void parseIdentifier(const int c, tokenInfo *const token,
                                  int *parsed, parserResult *const result) {
  parseIdentifierCommon(c, token, parsed, result, false);
}

CTAGS_INLINE void parseFQIdentifier(const int c, tokenInfo *const token,
                                    int *parsed, parserResult *const result) {
...
CTAGS_INLINE bool tryParser(Parser parser, tokenInfo *const token,
                            bool skipWhite) {
...
static void parseDecorator(tokenInfo *const token) {
  ...
  do {
    ...
    parsed =
        tryInSequence(token, false, parseDecoratorChars, parseComment, NULL);
  } while (parsed && token->type != TOKEN_PARENS);

  do {
    clearPoolToken(token);

    parsed = tryInSequence(token, false, parseNewLine, parseComment,
...
MULTI_CHAR_PARSER_DEF(InterfaceBodyChars, "}:;.,|&", TOKEN_CLOSE_CURLY,
                      TOKEN_COLON, TOKEN_SEMICOLON, TOKEN_PERIOD, TOKEN_COMMA,
                      TOKEN_PIPE, TOKEN_AMPERSAND)
static void parseInterfaceBody(const int scope, tokenInfo *const token) {
  bool parsed;

  do {
    parsed = tryInSequence(token, true, parseComment, parseTemplate,
                           parseStringSQuote, parseStringDQuote,
                           parseStringTemplate, parseStringRegex, parseParens,
                           parseSquares, parseOpenCurly, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(
        token, true, parseTemplate, parseComment, parseStringSQuote,
        parseStringDQuote, parseStringTemplate, parseStringRegex, parseParens,
        parseSquares, parseCurlies, parseNumber, parsePrivateKeyword,
        parseProtectedKeyword, parsePublicKeyword, parseReadonlyKeyword,
        parseStaticKeyword, parseArrow, parseTypeofKeyword,
        parseInterfaceBodyChars, parseIdentifier, NULL);
...
MULTI_CHAR_PARSER_DEF(TypeChars, ":;,=|&", TOKEN_COLON, TOKEN_SEMICOLON,
                      TOKEN_COMMA, TOKEN_EQUAL_SIGN, TOKEN_PIPE,
                      TOKEN_AMPERSAND)
static void parseType(const int scope, tokenInfo *const token) {
  ...
  do {
    clearPoolToken(token);
    parsed = tryInSequence(
        token, false, parseComment, parseTemplate, parseStringSQuote,
        parseStringDQuote, parseStringTemplate, parseStringRegex, parseParens,
        parseSquares, parseCurlies, parseArrow, parseNumber, parseTypeofKeyword,
        parseTypeChars, parseIdentifier, NULL);
    if (parsed) {
      switch (token->type) {
      case TOKEN_KEYWORD:
        switch (token->keyword) {
...
static void parseEnumBody(const int scope, tokenInfo *const token) {
  ...
  do {
    parsed = tryInSequence(token, true, parseTemplate, parseComment,
                           parseStringSQuote, parseStringDQuote,
                           parseStringTemplate, parseStringRegex, parseParens,
                           parseSquares, parseOpenCurly, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(
        token, true, parseTemplate, parseComment, parseStringSQuote,
        parseStringDQuote, parseStringTemplate, parseStringRegex, parseParens,
        parseSquares, parseCurlies, parseNumber, parseEnumBodyChars,
        parseNewKeyword, parseIdentifier, NULL);
...
static void parseEnum(const int scope, tokenInfo *const token) {
  ...
  do {
    clearPoolToken(token);
    parsed = tryInSequence(token, false, parseComment, parseIdentifier, NULL);
  } while (parsed && token->type != TOKEN_IDENTIFIER);
...
MULTI_CHAR_PARSER_DEF(VariableChars, "|&=?[]{}()\n:;,.=-+/^<>*", TOKEN_PIPE,
                      TOKEN_AMPERSAND, TOKEN_EQUAL_SIGN, TOKEN_QUESTION_MARK,
                      TOKEN_OPEN_SQUARE, TOKEN_CLOSE_SQUARE, TOKEN_OPEN_CURLY,
                      TOKEN_CLOSE_CURLY, TOKEN_OPEN_PAREN, TOKEN_CLOSE_PAREN,
                      TOKEN_NL, TOKEN_COLON, TOKEN_SEMICOLON, TOKEN_COMMA,
                      TOKEN_PERIOD, TOKEN_EQUAL_SIGN, TOKEN_MINUS, TOKEN_PLUS,
                      TOKEN_DIV, TOKEN_POWER, TOKEN_GREATER, TOKEN_STAR)
static void parseVariable(bool constVar, bool localVar, const int scope,
  ...
  tsKind varKind =
      constVar ? TSTAG_CONSTANT : (localVar ? TSTAG_LOCAL : TSTAG_VARIABLE);

  do {
    clearPoolToken(token);
    parsed = tryInSequence(
        token, false, parseTemplate, parseComment, parseStringRegex,
        parseStringSQuote, parseStringDQuote, parseStringTemplate, parseNumber,
        parseArrow, parseAsKeyword, parseAwaitKeyword, parseForKeyword,
        parseFunctionKeyword, parseWhileKeyword, parseThisKeyword,
        parseEnumKeyword, parseOfKeyword, parseInKeyword, parseNewKeyword,
        parseTypeofKeyword, parseVariableChars, parseIdentifier, NULL);
  ...
  } while (parsed && !((token->type == TOKEN_SEMICOLON ||
...
MULTI_CHAR_PARSER_DEF(FunctionArgsAfterParenChars, "|&=?[]{})\n:,.@",
                      TOKEN_PIPE, TOKEN_AMPERSAND, TOKEN_EQUAL_SIGN,
                      TOKEN_QUESTION_MARK, TOKEN_OPEN_SQUARE,
                      TOKEN_CLOSE_SQUARE, TOKEN_OPEN_CURLY, TOKEN_CLOSE_CURLY,
                      TOKEN_CLOSE_PAREN, TOKEN_NL, TOKEN_COLON, TOKEN_COMMA,
                      TOKEN_PERIOD, TOKEN_AT)
static void parseFunctionArgs(const int scope, tokenInfo *const token) {
  ...
  do {
    clearPoolToken(token);
    parsed = tryInSequence(token, false, parseTemplate, parseComment,
                           parseFunctionArgsChars, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(token, false, parseTemplate, parseComment,
                           parseStringSQuote, parseStringDQuote,
                           parseStringTemplate, parseStringRegex, parseParens,
                           parseNumber, parseFunctionArgsAfterParenChars,
                           parseArrow, parseIdentifier, NULL);
...
MULTI_CHAR_PARSER_DEF(FunctionBodyChars, "{}", TOKEN_OPEN_CURLY,
                      TOKEN_CLOSE_CURLY)
static void parseFunctionBody(const int scope, tokenInfo *const token) {
  ...
  do {
    ...
    parsed =
        tryInSequence(token, true, parseOpenCurly, parseComment,
                      parseStringSQuote, parseStringDQuote, parseStringTemplate,
                      parseStringRegex, parseTemplate, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(
        token, true, parseFunctionBodyChars, parseComment, parseStringSQuote,
        parseStringDQuote, parseStringTemplate, parseStringRegex, parseTemplate,
        parseVarKeyword, parseLetKeyword, parseConstKeyword, NULL);
...
static void parseFunction(const int scope, tokenInfo *const token) {
  ...
  do {
    clearPoolToken(token);
    parsed = tryInSequence(token, false, parseComment, parseStar,
                           parseIdentifier, NULL);
  ...
  const int nscope =
      emitTag(token, isGenerator ? TSTAG_GENERATOR : TSTAG_FUNCTION);

  parseFunctionArgs(nscope, token);
  parseFunctionBody(nscope, token);
}

MULTI_CHAR_PARSER_DEF(PropertyTypeChars, "\n;|&,)", TOKEN_NL, TOKEN_SEMICOLON,
                      TOKEN_PIPE, TOKEN_AMPERSAND, TOKEN_COMMA,
                      TOKEN_CLOSE_PAREN)
static void parsePropertyType(tokenInfo *const token) {
  ...
  do {
    ...
    if (parsedIdentifier) {
      parsed = tryInSequence(token, false, parsePropertyTypeChars, parseArrow,
                             parseTemplate, parseParens, parseComment,
                             parseStringSQuote, parseStringDQuote,
                             parseStringTemplate, parseStringRegex,
                             parseSquares, parseCurlies, NULL);
      ...
      parsed = tryInSequence(token, false, parsePropertyTypeChars, parseArrow,
                             ...
                             parseStringSQuote, parseStringDQuote,
                             parseStringTemplate, parseStringRegex,
                             parseSquares, parseCurlies, parseIdentifier, NULL);
    ...
    if (isType(token, TOKEN_ARROW))
      parseReturnValue = true;
           ...
           !isType(token, TOKEN_SEMICOLON) && !isType(token, TOKEN_COMMA) &&
           !(parseReturnValue &&
             (isType(token, TOKEN_PARENS) || isType(token, TOKEN_CURLIES) ||
...
MULTI_CHAR_PARSER_DEF(ConstructorParamsAfterParenChars, "\n:,)@", TOKEN_NL,
                      TOKEN_COLON, TOKEN_COMMA, TOKEN_CLOSE_PAREN, TOKEN_AT)
static void parseConstructorParams(const int classScope, const int constrScope,
  ...
  do {
    ...
    parsed = tryInSequence(token, false, parseConstructorParamsChars,
                           parseComment, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(
        token, false, parseConstructorParamsAfterParenChars, parseComment,
        parsePrivateKeyword, parseProtectedKeyword, parsePublicKeyword,
        parseReadonlyKeyword, parseStaticKeyword, parseIdentifier, NULL);
...
static void parseClassBody(const int scope, tokenInfo *const token) {
  ...
  do {
    ...
    parsed = tryInSequence(token, false, parseClassBodyChars, parseTemplate,
                           parseComment, parseExtendsKeyword,
                           parseImplementsKeyword, parseIdentifier, NULL);
  ...
  do {
    ...
    parsed = tryInSequence(
        token, false, parseComment, parseStringSQuote, parseStringDQuote,
        parseStringTemplate, parseStringRegex, parseAsyncKeyword,
        parseConstructorKeyword, parseNewKeyword, parsePrivateKeyword,
        parseProtectedKeyword, parsePublicKeyword, parseReadonlyKeyword,
        parseStaticKeyword, parseNumber, parseTemplate, parseTypeofKeyword,
        parseClassBodyAfterCurlyChars, parseIdentifier, NULL);
    ...
    if (parsed) {
      ...
      case TOKEN_OPEN_PAREN:
        ...
        const int nscope =
            emitTag(member, isGenerator ? TSTAG_GENERATOR : TSTAG_METHOD);
...
MULTI_CHAR_PARSER_DEF(NamespaceBodyAfterCurlyChars, "@{}()[]", TOKEN_AT,
                      TOKEN_OPEN_CURLY, TOKEN_CLOSE_CURLY, TOKEN_OPEN_PAREN,
...
static void parseNamespaceBody(const int scope, tokenInfo *const token) {
  ...
  // parse until {
  do {
    parsed = tryInSequence(token, false, parseNamespaceBodyChars, parseComment,
                           NULL);
  } while (parsed && token->type != TOKEN_OPEN_CURLY);
  ...
  do {
    ...
    parsed =
        tryInSequence(token, true, parseComment, parseTemplate,
                      parseStringSQuote, parseStringDQuote, parseStringTemplate,
                      parseStringRegex, parseInterfaceKeyword, parseTypeKeyword,
                      parseEnumKeyword, parseFunctionKeyword, parseClassKeyword,
                      parseVarKeyword, parseLetKeyword, parseConstKeyword,
                      parseNamespaceBodyAfterCurlyChars, NULL);
  ...
  } while (parsed && !(isType(token, TOKEN_CLOSE_CURLY) && parenLvl <= 0 &&
                       squareLvl <= 0 && curlyLvl <= 0));
}
...
static void parseTsFile(tokenInfo *const token) {
  ...
  do {
    ...
    parsed = tryInSequence(
        token, true, parseComment, parseTemplate, parseStringSQuote,
        parseStringDQuote, parseStringTemplate, parseStringRegex,
        parseInterfaceKeyword, parseTypeKeyword, parseEnumKeyword,
        parseFunctionKeyword, parseClassKeyword, parseNamespaceKeyword,
        parseVarKeyword, parseLetKeyword, parseConstKeyword, parseAt, NULL);
...
static void initialize(const langType language) {
  ...
  TokenPool =
      objPoolNew(16, newPoolToken, deletePoolToken, clearPoolToken, NULL);
}
