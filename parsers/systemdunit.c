...
static kindDefinition SystemdUnitKinds[] = {
    {true, 'u', "unit", "units", .referenceOnly = true,
     ATTACH_ROLES(SystemdUnitUnitRoles)},
};
...
static void newDataCallback(iniconfSubparser *s CTAGS_ATTR_UNUSED,
                            const char *section CTAGS_ATTR_UNUSED,
...
static void findSystemdUnitTags(void) { scheduleRunningBaseparser(0); }

extern parserDefinition *SystemdUnitParser(void) {
  static const char *const extensions[] = {
      "service", "socket", "device", "mount",    "automount", "swap",
      "target",  "path",   "timer",  "snapshot", "slice",     NULL};
