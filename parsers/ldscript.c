...
static kindDefinition LdScriptKinds[] = {
    ...
    {true, 's', "symbol", "symbols", .referenceOnly = false,
     ATTACH_ROLES(LdScriptSymbolRoles)},
    {true, 'v', "version", "versions"},
    {true, 'i', "inputSection", "input sections", .referenceOnly = true,
     ATTACH_ROLES(LdScriptInputSectionRoles)},
};
...
typedef enum { F_ASSIGNMENT, COUNT_FIELD } ldScriptField;
...
static int makeLdScriptTagMaybe(tagEntryInfo *const e, tokenInfo *const token,
  ...
  if ((kind == K_SYMBOL) && LdScriptFields[F_ASSIGNMENT].enabled) {
...
static void readToken(tokenInfo *const token, void *data CTAGS_ATTR_UNUSED) {
  ...
  default:
    if (cppIsdigit(c)) {
      ...
      token->keyword =
          lookupKeyword(vStringValue(token->string), Lang_ldscript);
...
static void parseProvide(tokenInfo *token) {
  ...
  if (tokenSkipToType(token, '(')) {
    ...
    if (tokenIsType(token, IDENTIFIER)) {
      ...
      makeLdScriptTagMaybe(&e, token, K_SYMBOL, ROLE_DEFINITION_INDEX);
      TOKENX(token, struct tokenExtra)->assignment = KEYWORD_NONE;
    }
    tokenSkipToType(token, ')');
  }
...
static void parseInputSections(tokenInfo *const token) {
  ...
  do {
    ...
    if (token->type == ')')
      ...
      makeLdScriptTagMaybe(&e, token, K_INPUT_SECTION,
                           TOKENX(token, struct tokenExtra)->scopeIndex ==
                                   CORK_NIL
                               ? LD_SCRIPT_INPUT_SECTION_DISCARDED
                               : LD_SCRIPT_INPUT_SECTION_MAPPED);
...
static void parseOutputSectionCommands(tokenInfo *const token, int terminator) {
  ...
  do {
    ...
    } else if (tokenIsType(token, IDENTIFIER) || tokenIsKeyword(token, LOC)) {
      tagEntryInfo e;
...
static void parseSection(tokenInfo *name) {
  ...
  if (tokenIsType(token, ASSIGNMENT_OP)) {
    if (!tokenIsKeyword(name, LOC))
      makeLdScriptTagMaybe(&e, name, K_SYMBOL, ROLE_DEFINITION_INDEX);
    tokenSkipToType(token, ';');
  } else {
  retry:
    ...
    if (token->type == ':') {
      ...
      if (tokenIsType(name, DISCARD))
        ...
        scope_index =
            makeLdScriptTagMaybe(&e, name, K_SECTION, ROLE_DEFINITION_INDEX);
...
static void parseVersion(tokenInfo *const token) {
  tagEntryInfo e;
  makeLdScriptTagMaybe(&e, token, K_VERSION, ROLE_DEFINITION_INDEX);
...
static void initialize(const langType language) { Lang_ldscript = language; }
