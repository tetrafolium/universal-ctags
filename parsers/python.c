...
#define isIdentifierChar(c) (isalnum(c) || (c) == '_' || (c) >= 0x80)
...
static const char *const PythonAccesses[COUNT_ACCESS] = {"private", "protected",
                                                         "public"};

typedef enum { F_DECORATORS, COUNT_FIELD } pythonField;
...
static roleDefinition PythonModuleRoles[] = {
    {true, "imported", "imported modules"},
    {true, "namespace",
     "namespace from where classes/variables/functions are imported"},
    {true, "indirectlyImported", "module imported in alternative name"},
};
...
static kindDefinition PythonKinds[COUNT_KIND] = {
    ...
    {true, 'i', "module", "modules", .referenceOnly = true,
     ATTACH_ROLES(PythonModuleRoles)},
    {true, 'x', "unknown",
     "name referring a class/variable/function/module defined in other module",
...
static const keywordTable PythonKeywordTable[] = {
    /* keyword			keyword ID */
    {"as", KEYWORD_as},         {"async", KEYWORD_async},
    {"cdef", KEYWORD_cdef},     {"cimport", KEYWORD_import},
    {"class", KEYWORD_class},   {"cpdef", KEYWORD_cpdef},
    {"def", KEYWORD_def},       {"extern", KEYWORD_extern},
    {"from", KEYWORD_from},     {"import", KEYWORD_import},
    {"inline", KEYWORD_inline}, {"lambda", KEYWORD_lambda},
    {"pass", KEYWORD_pass},     {"return", KEYWORD_return},
};
...
#define PY_NL(nl)                                                              \
  ((struct pythonNestingLevelUserData *)nestingLevelGetUserData(nl))
...
static accessType accessFromIdentifier(const vString *const ident,
  ...
  /* "__...__": magic methods */
  else if (kind == K_FUNCTION && parentKind == K_CLASS && len > 3 &&
           p[1] == '_' && p[len - 2] == '_' && p[len - 1] == '_')
    return ACCESS_PUBLIC;
  /* "__...": name mangling */
...
static int makeClassTag(const tokenInfo *const token,
  ...
  if (PythonKinds[K_CLASS].enabled) {
    ...
    e.extensionFields.inheritance =
        inheritance ? vStringValue(inheritance) : "";
...
static int makeSimplePythonTag(const tokenInfo *const token,
...
static int makeSimplePythonRefTag(const tokenInfo *const token,
                                  ...
                                  pythonKind const kind, int roleIndex,
                                  xtagType xtag) {
  if (isXtagEnabled(XTAG_REFERENCE_TAGS) &&
      PythonKinds[kind].roles[roleIndex].enabled) {
    tagEntryInfo e;

    initRefTagEntry(&e, vStringValue(altName ? altName : token->string), kind,
                    roleIndex);
...
static void reprCat(vString *const repr, const tokenInfo *const token) {
  if (token->type != TOKEN_INDENT && token->type != TOKEN_WHITESPACE) {
  ...
  while (token->type != TOKEN_EOF && token->type != ':' &&
    ...
    if (token->type == '(')
      ...
      readNext = skipOverPair(token, '{', '}', repr, true);
             ...
             KEYWORD_lambda) { /* handle lambdas in a default value */
      if (repr)
        reprCat(repr, token);
      readTokenFull(token, true);
...
static void readQualifiedName(tokenInfo *const nameToken) {
  ...
  if (nameToken->type == TOKEN_IDENTIFIER || nameToken->type == '.') {
    vString *qualifiedName = vStringNew();
    tokenInfo *token = newToken();

    while (nameToken->type == TOKEN_IDENTIFIER || nameToken->type == '.') {
...
static bool readCDefName(tokenInfo *const token, pythonKind *kind) {
  ...
  if (token->keyword == KEYWORD_extern || token->keyword == KEYWORD_import) {
    readToken(token);
...
static bool parseClassOrDef(tokenInfo *const token,
                            const vString *const decorators, pythonKind kind,
                            bool isCDef) {
  ...
  if (token->keyword == KEYWORD_import) {
    ...
    if (fromModule) {
      ...
      makeSimplePythonRefTag(fromModule, NULL, K_MODULE,
                             PYTHON_MODULE_NAMESPACE, XTAG_UNKNOWN);
    }
      ...
      if (token->type == TOKEN_IDENTIFIER) {
        ...
        if (token->keyword == KEYWORD_as) {
          ...
          if (fromModule) {
            ...
            /* Y */
            makeSimplePythonRefTag(name, NULL, K_UNKNOWN,
                                   PYTHON_UNKNOWN_IMPORTED, XTAG_UNKNOWN);
            /* x.Y */
            if (isXtagEnabled(XTAG_QUALIFIED_TAGS)) {
              vString *fq = vStringNewCopy(fromModule->string);
              vStringPut(fq, '.');
            ...
            makeSimplePythonRefTag(name, NULL, K_MODULE, PYTHON_MODULE_IMPORTED,
                                   XTAG_UNKNOWN);
          }
        }
...
static bool parseVariable(tokenInfo *const token, const pythonKind kind) {
  ...
  while (token->type == TOKEN_IDENTIFIER &&
    ...
    if (token->type == '.') {
      ...
      do {
        readToken(token);
      } while (token->type == TOKEN_IDENTIFIER || token->type == '.');
    }
...
static void findPythonTags(void) {
  ...
  NextToken = NULL;
  PythonNestingLevels =
      nestingLevelsNew(sizeof(struct pythonNestingLevelUserData));
  ...
  while (token->type != TOKEN_EOF) {
    ...
    if (token->type == TOKEN_INDENT)
      setIndent(token);
    else if (token->keyword == KEYWORD_class || token->keyword == KEYWORD_def) {
    ...
    if (iterationTokenType != TOKEN_INDENT && iterationTokenType != '@' &&
        PythonFields[F_DECORATORS].enabled) {
...
extern parserDefinition *PythonParser(void) {
  static const char *const extensions[] = {"py",    "pyx",  "pxd", "pxi",
                                           "scons", "wsgi", NULL};
