...
static kindDefinition NsisKinds[] = {
    ...
    {true, 'i', "script", "NSIS scripts", .referenceOnly = true,
     ATTACH_ROLES(NsisScriptRoles)},
};
...
static int makeSimpleTagWithScope(vString *name, int kindIndex,
                                  int parentCorkIndex) {
  tagEntryInfo e;
...
static const unsigned char *parseSection(const unsigned char *cp, vString *name,
  ...
  if (strpbrk((const char *)cp, "'`\"")) {
    ...
    if (*cp == terminator) {
       ...
       */
      anonGenerate(name,
                   (kindIndex == K_SECTION ? "AnonymousSection"
...
static const unsigned char *parseLangString(const unsigned char *cp,
                                            vString *name) {
  cp = skipWhitespace(cp);
...
extern parserDefinition *NsisParser(void) {
  static const char *const extensions[] = {"nsi", "nsh", NULL};
