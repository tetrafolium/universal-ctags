...
static void yamlFini(yaml_parser_t *yaml) { yaml_parser_delete(yaml); }
...
static kindDefinition YamlKinds[] = {
    {true, 'a', "anchor", "anchors", .referenceOnly = false,
     ATTACH_ROLES(YamlAnchorRoles)},
};
...
static void handlYamlToken(yaml_token_t *token) {
  ...
  if (token->type == YAML_ANCHOR_TOKEN) {
    initTagEntry(&tag, (char *)token->data.anchor.value, K_ANCHOR);
    attachYamlPosition(&tag, token, false);
    makeTagEntry(&tag);
  } else if (token->type == YAML_ALIAS_TOKEN) {
    initRefTagEntry(&tag, (char *)token->data.alias.value, K_ANCHOR,
                    R_ANCHOR_ALIAS);
    attachYamlPosition(&tag, token, false);
    makeTagEntry(&tag);
  }
...
static void findYamlTags(void) {
  ...
  while (!done) {
    ...
    verbose("yaml token:%s<%d>@Line:%" PRIuPTR "\n", tokenTypeName[token.type],
            token.type, token.start_mark.line + 1);
