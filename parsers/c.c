 ...
 *
 *   This module contains functions for parsing and scanning C, C++, C#, D and
 * Java source files.
 */
...
#define insideAnnotationBody(st)                                               \
  ((st)->parent == NULL                                                        \
       ? false                                                                 \
       : (bool)((st)->parent->declaration == DECL_ANNOTATION))
#define insideInterfaceBody(st)                                                \
  ((st)->parent == NULL ? false                                                \
                        : (bool)((st)->parent->declaration == DECL_INTERFACE))
#define isSignalDirection(token)                                               \
  (bool)(((token)->keyword == KEYWORD_INPUT) ||                                \
         ((token)->keyword == KEYWORD_OUTPUT) ||                               \
         ((token)->keyword == KEYWORD_INOUT))
#define isExternCDecl(st, c)                                                   \
  (bool)((c) == STRING_SYMBOL && !(st)->haveQualifyingName &&                  \
         (st)->scope == SCOPE_EXTERN)

#define isOneOf(c, s) (bool)(strchr((s), (c)) != NULL)

#define isHighChar(c)                                                          \
  ((c) != EOF && (unsigned int)(c) >= 0xc0 && (unsigned int)(c) <= 0xff)
...
static kindDefinition JavaKinds[] = {
    ...
    {false, 'l', "local", "local variables"},
    {true, 'm', "method", "methods"},
    {true, 'p', "package", "packages", .referenceOnly = false,
     ATTACH_ROLES(JavaPackageRoles)},
};
...
static bool isMember(const statementInfo *const st) {
  ...
  if (isType(st->context, TOKEN_NAME))
    ...
    result = (bool)(st->parent != NULL && isContextualStatement(st->parent));
  return result;
}
...
static void initMemberInfo(statementInfo *const st) {
  ...
  if (st->parent != NULL)
    ...
    case DECL_ENUM:
      accessDefault =
          (isInputLanguage(Lang_java) ? ACCESS_PUBLIC : ACCESS_UNDEFINED);
      break;
    case DECL_NAMESPACE:
...
static csharpKind csharpTagKindFull(const tagType type,
                                    const bool with_assert) {
...
static dKind dTagKindFull(const tagType type, bool with_assert) {
  ...
  case TAG_CLASS:
    ...
    break;
  case TAG_ENUM:
    result = DK_ENUMERATION;
    break;
  case TAG_ENUMERATOR:
    result = DK_ENUMERATOR;
    break;
  case TAG_EXTERN_VAR:
    result = DK_EXTERN_VARIABLE;
    break;
  case TAG_FUNCTION:
    result = DK_FUNCTION;
    break;
...
static bool findScopeHierarchy(vString *const string,
    ...
    for (s = st->parent; s != NULL; s = s->parent) {
      if (isContextualStatement(s) || s->declaration == DECL_NAMESPACE ||
        ...
        if (s->declaration == DECL_PRIVATE ||
            s->declaration == DECL_PROTECTED || s->declaration == DECL_PUBLIC) {
          continue;
        }
...
static void makeExtraTagEntry(const tagType type, tagEntryInfo *const e,
                              vString *const scope) {
  if (isXtagEnabled(XTAG_QUALIFIED_TAGS) && scope != NULL &&
...
static int makeTag(const tokenInfo *const token, const statementInfo *const st,
...
static int qualifyCompoundTag(const statementInfo *const st,
  ...
  if (isType(nameToken, TOKEN_NAME)) {
    ...
    const bool fileScoped =
        (bool)(!(isInputLanguage(Lang_java) || isInputLanguage(Lang_csharp) ||
...
static int qualifyVariableTag(const statementInfo *const st,
  ...
  if (!isType(nameToken, TOKEN_NAME))
    ...
    corkIndex = makeTag(nameToken, st,
                        (bool)(st->member.access == ACCESS_PRIVATE), TAG_EVENT);
  ...
  if (!isInputLanguage(Lang_java)) {
    ...
    if (macro) {
      if (macro->hasParameterList) {
        // This old parser does not support macro parameters: we simply assume
        // them to be empty
...
static void readIdentifier(tokenInfo *const token, const int firstChar) {
  ...
  do {
    ...
    c = cppGetc();
           ...
           ((isInputLanguage(Lang_java) || isInputLanguage(Lang_csharp)) &&
            (isHighChar(c) || c == '.')));
...
static void setAccess(statementInfo *const st, const accessType access) {
  ...
  if (isMember(st)) {
    if (isInputLanguage(Lang_cpp)) {
      ...
      if (st->parent != NULL && (st->parent->declaration == DECL_PRIVATE ||
...
static void processToken(tokenInfo *const token, statementInfo *const st) {
  ...
  case KEYWORD_CHAR:
    ...
    break;
  case KEYWORD_CLASS:
    checkIsClassEnum(st, DECL_CLASS);
    break;
  case KEYWORD_CONST:
    st->declaration = DECL_BASE;
    break;
  ...
  case KEYWORD_STRING:
    ...
    break;
  case KEYWORD_STRUCT:
    checkIsClassEnum(st, DECL_STRUCT);
    break;
  case KEYWORD_TASK:
  ...
  case KEYWORD_TEMPLATE:
    ...
    break;
  case KEYWORD_NAMESPACE:
    readPackageOrNamespace(st, DECL_NAMESPACE, false);
    break;
  case KEYWORD_MODULE:
  case KEYWORD_PACKAGE:
    readPackageOrNamespace(st, DECL_PACKAGE, false);
    break;
  ...
  do {
    ...
    default:
      if (cppIsident1(c)) {
        ...
        default:
          /* "override" and "final" are only keywords in the declaration of a
           * virtual member function, so need to be handled specially, not as
           * keywords */
...
static void analyzeParens(statementInfo *const st) {
  ...
  if (st->inFunction && !st->assignment &&
      !(
          ...
          isInputLanguage(Lang_cpp) && isType(prev, TOKEN_NAME) &&
...
static bool inheritingDeclaration(declType decl) {
  ...
  return (bool)(decl == DECL_CLASS || decl == DECL_STRUCT ||
                decl == DECL_INTERFACE);
}
...
static void processColon(statementInfo *const st) {
  ...
  if (doubleColon) {
    ...
    cppUngetc(c);
    if ((isInputLanguage(Lang_cpp) || isInputLanguage(Lang_csharp) ||
         isInputLanguage(Lang_d)) &&
...
static int tagCheck(statementInfo *const st) {
  ...
  case TOKEN_NAME:
    ...
    if (isInputLanguage(Lang_vera) && insideInterfaceBody(st)) {
      ...
      */
      if ((isType(prev, TOKEN_KEYWORD) && isSignalDirection(prev)) ||
          (isType(prev2, TOKEN_KEYWORD) && isSignalDirection(prev)))
  ...
  case TOKEN_BRACE_OPEN:
    ...
    } else if (isContextualStatement(st) || st->declaration == DECL_VERSION ||
               st->declaration == DECL_PROGRAM) {
  ...
  case TOKEN_KEYWORD:
    ...
    if (token->keyword == KEYWORD_DEFAULT && isType(prev, TOKEN_ARGS) &&
        insideAnnotationBody(st)) {
  ...
  case TOKEN_COMMA:
    ...
    if (isInputLanguage(Lang_java) && token->type == TOKEN_SEMICOLON &&
        insideEnumBody(st)) {
...
static rescanReason findCTags(const unsigned int passCount) {
  ...
  if (isInputLanguage(Lang_c) || isInputLanguage(Lang_cpp)) {
    ...
    kind_for_param = CK_MACRO_PARAM, role_for_macro_undef = CR_MACRO_UNDEF;
    ...
    kind_for_param = VK_MACRO_PARAM, role_for_macro_undef = VR_MACRO_UNDEF;
  ...
  cppInit((bool)(passCount > 1), isInputLanguage(Lang_csharp),
          isInputLanguage(Lang_cpp), isInputLanguage(Lang_vera),
