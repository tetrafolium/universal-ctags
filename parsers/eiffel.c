...
static kindDefinition EiffelKinds[] = {{true, 'c', "class", "classes"},
...
getNextChar:
  ...
  default:
    if (isalpha(c)) {
      ...
      token->keyword =
          lookupCaseKeyword(vStringValue(token->string), Lang_eiffel);
...
static void parseGeneric(tokenInfo *const token,
                         bool declaration CTAGS_ATTR_UNUSED) {
...
static void parseLocal(tokenInfo *const token) {
  ...
  while (!isKeyword(token, KEYWORD_do) && !isKeyword(token, KEYWORD_once) &&
         !isKeyword(token, KEYWORD_attribute) && !isType(token, TOKEN_EOF)) {
...
static void parseExport(tokenInfo *const token) {
  ...
  if (isType(token, TOKEN_OPEN_BRACE)) {
    ...
    while (!isType(token, TOKEN_CLOSE_BRACE) && !isType(token, TOKEN_EOF)) {
      if (isType(token, TOKEN_IDENTIFIER))
    ...
    if (!isKeyword(token, KEYWORD_feature) &&
      ...
      if (!parseFeature(token))
        readToken(token);
           ...
           !isKeyword(token, KEYWORD_invariant) &&
           !isKeyword(token, KEYWORD_note) && !isType(token, TOKEN_EOF));
}
