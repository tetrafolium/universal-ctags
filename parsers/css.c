...
#define isSelectorChar(c)                                                      \
  ...
  (isalnum(c) || (c) == '_' || /* allowed char */                              \
   ...
   (c) == ')' || (c) == '.' || /* allow classes and selectors */               \
   (c) == ':' ||               /* allow pseudo classes */                      \
   (c) == '*' ||               /* allow globs as P + * */                      \
   (c) == '#')                 /* allow ids */

typedef enum eCssKinds { K_CLASS, K_SELECTOR, K_ID } cssKind;

static kindDefinition CssKinds[] = {{true, 'c', "class", "classes"},
  ...
  do {
    ...
    if (token.type ==
      ...
      readNextToken = useContents && token.type == '{';
               ...
               TOKEN_SELECTOR) { /* collect selectors and make a tag */
