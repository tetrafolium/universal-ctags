...
static kindDefinition M4Kinds[] = {
    {true, 'd', "macro", "macros", .referenceOnly = false,
     ATTACH_ROLES(M4MacroRoles)},
    {true, 'I', "macrofile", "macro files", .referenceOnly = true,
     ATTACH_ROLES(M4MacrofileRoles)},
};
...
#define ENTRY(K)                                                               \
  {#K, KEYWORD_##K}, { "m4_" #K, KEYWORD_##K }
    ENTRY(define),   ENTRY(undefine),    ENTRY(include),
    ENTRY(sinclude), ENTRY(changequote),
};
...
static void readQuotedWord(vString *const name) {
  ...
  for (; c != EOF; c = getcFromInputFile()) {
    /* don't allow embedded NULs, and prevents to match when quote == 0 (aka
     * none) */
    if (c == 0)
      break;
...
static m4Subparser *maySwitchLanguage(const char *token) {
  ...
  foreachSubparser(tmp, false) {
    ...
    enterSubparser(tmp);
    if (m4tmp->probeLanguage && m4tmp->probeLanguage(m4tmp, token)) {
...
static bool doesQuoteStart(int c) { return (c == m4QuoteOpen); }
...
static struct newMacroResult newMacroM4(const char *token) {
  ...
  if ((!isXtagEnabled(XTAG_REFERENCE_TAGS)) && (role != ROLE_DEFINITION_INDEX))
    return result;
