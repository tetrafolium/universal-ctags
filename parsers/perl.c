...
#define TRACE                                                                  \
  if (TRACE_PERL_C)                                                            \
  printf("perl.c:%d: ", __LINE__), printf
...
static kindDefinition PerlKinds[] = {
    ...
    {false, 'M', "module", "modules", .referenceOnly = true,
     ATTACH_ROLES(PerlModuleRoles)},
};
...
static bool isIdentifier1(int c) { return (bool)(isalpha(c) || c == '_'); }

static bool isIdentifier(int c) { return (bool)(isalnum(c) || c == '_'); }
...
static bool isPodWord(const char *word) {
  ...
  switch (len) {
  case 3:
    return 0 == strncmp(word, "end", 3) || 0 == strncmp(word, "for", 3) ||
           0 == strncmp(word, "pod", 3);
  case 4:
    return 0 == strncmp(word, "back", 4) || 0 == strncmp(word, "item", 4) ||
           0 == strncmp(word, "over", 4);
  case 5:
    return 0 == strncmp(word, "begin", 5) || 0 == strncmp(word, "head1", 5) ||
           0 == strncmp(word, "head2", 5) || 0 == strncmp(word, "head3", 5) ||
 ...
 */
static enum const_state parseConstantsFromLine(const char *cp, vString *name,
                                               vString *package) {
  while (1) {
 ...
 */
static int parseConstantsFromHashRef(const unsigned char *cp, vString *name,
                                     vString *package) {
  while (1) {
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (skipPodDoc) {
      if (strncmp((const char *)line, "=cut", (size_t)4) == 0) {
        ...
        if (podStart != 0UL) {
          notifyLeavingPod();
          makePromise("Pod", podStart, 0, getInputLineNumber(), 0,
                      getSourceLineNumber());
          podStart = 0UL;
        }
      }
    ...
    if (strncmp((const char *)cp, "sub", (size_t)3) == 0) {
      ...
      spaceRequired = false;
      qualified = true;
    } else if (strncmp((const char *)cp, "no", (size_t)2) == 0 &&
               isspace(cp[2])) {
      cp += 3;
      while (isspace(*cp))
        cp++;
    ...
    if (kind != KIND_PERL_NONE) {
      ...
      if (KIND_PERL_SUBROUTINE == kind) {
        ...
        if (isXtagEnabled(XTAG_QUALIFIED_TAGS) && qualified &&
            KIND_PERL_PACKAGE != kind && package != NULL &&
...
extern parserDefinition *PerlParser(void) {
  static const char *const extensions[] = {"pl",  "pm",   "ph",
                                           "plx", "perl", NULL};
  static selectLanguage selectors[] = {selectByPickingPerlVersion, NULL};
