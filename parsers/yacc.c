...
static tagRegexTable yaccTagRegexTable[] = {
    {"^([A-Za-z][A-Za-z_0-9]+)[ \t]*:", "\\1", "l,label,labels", NULL,
     &parserState.not_in_grammar_rules},
    {"^([A-Za-z][A-Za-z_0-9]+)[ \t]*$", "\\1", "l,label,labels", NULL,
     &parserState.not_in_grammar_rules},
};
...
static bool change_section(const char *line CTAGS_ATTR_UNUSED,
                           ...
                           unsigned int count CTAGS_ATTR_UNUSED, void *data) {
...
static void initializeYaccParser(langType language) {
  ...
  */
  ...
  addLanguageCallbackRegex(language, "^%\\{", "{exclusive}", enter_c_prologue,
                           NULL, &parserState);
  addLanguageCallbackRegex(language, "^%\\}", "{exclusive}", leave_c_prologue,
                           NULL, &parserState);

  addLanguageCallbackRegex(language, "^%%", "{exclusive}", change_section, NULL,
                           &parserState);

  addLanguageCallbackRegex(language, "^%union", "{exclusive}", enter_union,
                           NULL, &parserState);
  addLanguageCallbackRegex(language, "^}", "{exclusive}", leave_union, NULL,
                           &parserState);
}
...
static void runYaccParser(void) {
  ...
  while (readLineFromInputFile() != NULL) {
    if (last_area == YACC_TOP_LEVEL && parserState.area == YACC_C_PROLOGUE) {
      ...
      c_input = 0;
      c_source = 0;
    } else if (last_area == YACC_TOP_LEVEL && parserState.area == YACC_UNION) {
      c_input = getInputLineNumber();
      c_source = getInputLineNumber();
    } else if (last_area == YACC_UNION && parserState.area == YACC_TOP_LEVEL) {
      unsigned long c_end = getInputLineNumber();
      makePromise("C", c_input, strlen("%"), c_end, strlen("}"), c_source);
      c_input = 0;
      c_source = 0;
