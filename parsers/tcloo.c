...
static kindDefinition TclOOKinds[] = {
    ...
    {true, 'm', "method", "methods", ATTACH_SEPARATORS(TclOOGenericSeparators)},
};
...
static void parseSuperclass(tokenInfo *token, int this_class) {
  ...
  if (tokenIsType(token, TCL_IDENTIFIER)) {
    ...
    if (e) {
      if (e->extensionFields
              .inheritance) { /* superclass is used twice in a class. */
...
static int commandNotify(tclSubparser *s, char *command, int parentIndex,
  ...
  if ((tcloo->foundTclOONamespaceImported && (strcmp(command, "class") == 0)) ||
...
static void namespaceImportNotify(tclSubparser *s, char *namespace,
  ...
  if (strcmp(namespace, "oo::*") == 0 || strcmp(namespace, "oo::class") == 0)
