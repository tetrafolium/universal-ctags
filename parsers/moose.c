 ...
 *
 *   This module contains functions for generating tags for Moose perl
 * extension. https://metacpan.org/pod/Moose
 *
 *   This module can gather tags for Moo perl extension, too.
 *   https://metacpan.org/pod/Moo
 *
 *   From the output of this parser, a client cannot know which extension is
 * used in the input source file. However, perl parser can give you hints about
 * the extension; with "--extras=+r --kinds-Perl=+M" options, the perl parser
 * tags module names specified as arguments for `use' perl built-in function.
 *   You don't need the options if you just want to run This parser.
 *   This parser just needs --extras=+s option that run subparsers including
 * this parser.
 *
 */
 ...
 *
 * A: capturing 'foo' as a reference tag with 'method' kind and 'wrapped' role,
 * and B: capturing 'foo' as a definition tag with 'wrapper' kind.
 *
...
static fieldDefinition MooseFields[] = {
    {
        .name = "wrapping",
        .description =
            "how a wrapper wrapping the method (around, after, or before)",
        .enabled = true,
    },
};
...
static void makeTagEntryNotify(subparser *s, const tagEntryInfo *tag,
                               int corkIndex);
...
static void makeTagEntryNotify(subparser *s, const tagEntryInfo *tag,
  ...
  if (tag->kindIndex == KIND_PERL_PACKAGE)
    ...
    if (isRoleAssigned(tag, ROLE_PERL_MODULE_USED)) {
      if (strcmp(tag->name, "Moose") == 0 || strcmp(tag->name, "Moo") == 0)
        enterMoose(moose);
...
static void parseExtendsClass(const char *input, size_t input_len,
                              vString *inherits, bool *notContinuousLine) {
  int i = 0;
  do {
    if (input[i] == ',')
      i++;
...
static bool findExtendsClass(const char *line, const regexMatch *matches,
...
static const char *parseAttributeOrWrapper(const char *str, int parentCorkIndex,
                                           int extraTerminator, int kind,
                                           enum Wrapping wrapping) {
  int i;

  for (i = 0; str[i] && str[i] != extraTerminator &&
...
static void solveKindAndWrappng(const char *str, int *kind,
                                enum Wrapping *wrapping) {
  *kind = K_WRAPPER;
  *wrapping = W_UNKNOWN;
  switch (str[0]) {
...
static bool findAttributeOrWrapperOne(const char *line,
  ...
  parseAttributeOrWrapper(line + matches[2].start, moose->classCork, -1, kind,
                          wrapping);
  return true;
}
...
static bool findAttributeOrWrapperMulit(const char *line,
  ...
  while ((str = parseAttributeOrWrapper(str, moose->classCork, terminator, kind,
                                        wrapping))) {
    int i;
...
static void initializeMooseParser(langType language) {
  addLanguageCallbackRegex(language, "^[ \t]*extends *(.+)", "{exclusive}",
                           findExtendsClass, &mooseSubparser.notInMoose,
                           &mooseSubparser);
  addLanguageCallbackRegex(
      language,
      "^[ \t]*(has|after|before|around|override) +"
      /* foo => ()
       * 'foo' => ()
       * "foo" => ()
       * ( foo => ())
      ...
      "{exclusive}", findAttributeOrWrapperOne, &mooseSubparser.notInMoose,
      &mooseSubparser);
  addLanguageCallbackRegex(
      language,
      "^[ \t]*(has|after|before|around) +\\(?\\[qw([^ \t])[ \t]*"
      /* [qw/foo bar/] => ()
       * ([qw/foo bar/] => ()) */
      "([a-zA-Z_][a-zA-Z0-9_]*([ \t][a-zA-Z_][a-zA-Z0-9_]*)*).*=>",
      "{exclusive}", findAttributeOrWrapperMulit, &mooseSubparser.notInMoose,
      &mooseSubparser);
  addLanguageCallbackRegex(
      language, "^[ \t]*(.+)", "{exclusive}", findExtendsClassContinuation,
      &mooseSubparser.notContinuousExtendsLines, &mooseSubparser);
}
