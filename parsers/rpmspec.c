...
static kindDefinition RpmSpecKinds[] = {
    {true, 't', "tag", "tags"},
    {true, 'm', "macro", "macros", .referenceOnly = false,
     ATTACH_ROLES(RpmSpecMacroRoles)},
...
static bool found_macro_cb_full(const char *line, const regexMatch *matches,
                                unsigned int count, bool global, bool undef,
                                void *userData) {
  ...
  TRACE_PRINT("Line %04d continuation: %d", getInputLineNumber(),
              ctx->rejecting);
  ...
  if (count > 0) {
    vString *signature =
        ((count > 1) && (matches[2].length > 0)) ? vStringNew() : NULL;
...
static bool found_macro_cb(const char *line, const regexMatch *matches,
                           unsigned int count, void *userData) {
  return found_macro_cb_full(line, matches, count, false, false, userData);
}

static bool found_global_cb(const char *line, const regexMatch *matches,
                            unsigned int count, void *userData) {
  return found_macro_cb_full(line, matches, count, true, false, userData);
}

static bool found_undef_cb(const char *line, const regexMatch *matches,
                           unsigned int count, void *userData) {
  return found_macro_cb_full(line, matches, count, false, true, userData);
}

static bool found_tag_cb(const char *line, const regexMatch *matches,
                         unsigned int count, void *userData) {
  if (count > 0) {
    vString *name = vStringNew();
    vStringNCopyS(name, line + matches[1].start, matches[1].length);
...
static bool found_package_cb(const char *line, const regexMatch *matches,
                             unsigned int count, void *userData) {
  if (count > 0) {
    vString *name = vStringNew();
    tagEntryInfo tag;
...
static bool check_line_continuation(const char *line, const regexMatch *matches,
                                    unsigned int count, void *userData) {
  struct rpmSpecCtx *ctx = (struct rpmSpecCtx *)userData;
  bool rejecting = ctx->rejecting;
  ctx->rejecting = (line && is_line_continued(line));

  TRACE_PRINT("Line %04d continuation: %d -> %d", getInputLineNumber(),
              rejecting, ctx->rejecting);
...
static void findRpmSpecTags(void) {
  ...
  rpmSpecCtx.macro = (struct macro_cb_data){.kindex = K_MACOR,
                                            .rindex = ROLE_DEFINITION_INDEX};
