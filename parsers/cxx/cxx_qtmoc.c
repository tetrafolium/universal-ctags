...
static const keywordTable QtMocKeywordTable[] = {
    /* keyword			keyword ID */
    {"Q_OBJECT", KEYWORD_QOBJECT}, {"Q_SIGNALS", KEYWORD_SIGNALS},
    {"signals", KEYWORD_SIGNALS},  {"Q_SLOTS", KEYWORD_SLOTS},
    {"slots", KEYWORD_SLOTS},      {"Q_PROPERTY", KEYWORD_PROPERTY},
};
...
static bool cxxParserSkipToClosingParenthesisOrEOF(void) {
  if (cxxTokenTypeIsOneOf(g_cxx.pToken,
                          CXXTokenTypeClosingParenthesis | CXXTokenTypeEOF))
    return true;

  return cxxParserParseUpToOneOf(
      CXXTokenTypeClosingParenthesis | CXXTokenTypeEOF, false);
}

static void qtMocMakeTagForProperty(CXXToken *pToken, const char *pszType) {
  tagEntryInfo tag;

  initTagEntry(&tag, vStringValue(pToken->pszWord), K_PROPERTY);
...
static bool qtMocParseProperty(void) {
  ...
  if (!cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeIdentifier)) {
    CXX_DEBUG_LEAVE_TEXT("Found no identifier after Q_PROPERTY(%s",
                         pszPropType);
    cxxParserSkipToClosingParenthesisOrEOF();
    eFree(pszPropType);
    return false;
...
static void makeTagEntryNotify(subparser *s, const tagEntryInfo *entry,
                               int corkIndex) {
  struct sQtMocSubparser *pQtMoc = (struct sQtMocSubparser *)s;
  ...
  if ((pQtMoc->eMemberMarker != QtMocMemberMarkerNone) &&
    ...
    parasiteTag.kindIndex =
        (pQtMoc->eMemberMarker == QtMocMemberMarkerSlot) ? K_SLOT : K_SIGNAL;
...
static void
foundExtraIdentifierAsAccessSpecifier(struct sCxxSubparser *pSubparser,
                                      CXXToken *pToken) {
  struct sQtMocSubparser *pQtMoc = (struct sQtMocSubparser *)pSubparser;
  keywordId keyword = lookupKeyword(vStringValue(pToken->pszWord), Lang_QtMoc);
...
static void findQtMocTags(void) { scheduleRunningBaseparser(0); }

static void initialize(langType lang) { Lang_QtMoc = lang; }
