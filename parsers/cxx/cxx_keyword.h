...
typedef enum _CXXKeyword {
  ...
  CXXKeywordEXTERN,
  CXXKeywordFALSE,
  CXXKeywordFINAL, // not really a keyword, has meanings in some specific
                   // contexts
  CXXKeywordFLOAT,
  CXXKeywordFOR,
  CXXKeywordFRIEND,
  ...
  // CXXKeywordOR,
  // CXXKeywordOR_EQ,
  // CXXKeywordOVERRIDE, // not really a keyword, has meanings in some specific
  // contexts
  CXXKeywordPRIVATE,
  CXXKeywordPROTECTED,
  CXXKeywordPUBLIC,
  ...
  // CXXKeywordXOR,
  // CXXKeywordXOR_EQ,
  // WARNING: There is a table in cxx_keyword.c that must match order in this
  // enumeration
} CXXKeyword;
...
// However when parsing .h files we don't know if they belong to a C program or
// C++ one and thus for safety we parse them as C++. If our guess is wrong then
// the parser may become confused and in some cases even bail out.
//
...
//
// This function returns the previous state of the public/protected/private
// keywords enabled flag so it can be easily restored.
bool cxxKeywordEnablePublicProtectedPrivate(bool bEnableIt);

//
// "final" keyword
//
// This is actually special at C++ level: it's a keyword only within a specific
// part of a class declaration. In other contexts it's not a keyword.
