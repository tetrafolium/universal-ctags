...
bool cxxParserParseAndCondenseCurrentSubchain(
    unsigned int uInitialSubchainMarkerTypes, bool bAcceptEOF,
  ...
  bool bRet = cxxParserParseAndCondenseSubchainsUpToOneOf(
      uTokenTypes, uInitialSubchainMarkerTypes, bCanReduceInnerElements);
...
bool cxxParserParseAndCondenseSubchainsUpToOneOf(
    unsigned int uTokenTypes, unsigned int uInitialSubchainMarkerTypes,
    bool bCanReduceInnerElements) {
  CXX_DEBUG_ENTER_TEXT("Token types = 0x%x(%s), reduce = %d", uTokenTypes,
                       cxxDebugTypeDecode(uTokenTypes),
  ...
  for (;;) {
    ...
    if (cxxTokenTypeIsOneOf(g_cxx.pToken, uTokenTypes)) {
      ...
      CXX_DEBUG_LEAVE_TEXT("Got terminator token '%s' 0x%x",
                           vStringValue(g_cxx.pToken->pszWord),
                           g_cxx.pToken->eType);
      return true;
    }
    ...
    if (cxxTokenTypeIsOneOf(g_cxx.pToken, uInitialSubchainMarkerTypes)) {
      // subchain
      CXX_DEBUG_PRINT("Got subchain start token '%s' 0x%x",
                      vStringValue(g_cxx.pToken->pszWord), g_cxx.pToken->eType);
      ...
      if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeOpeningBracket) &&
        ...
        if (!cxxParserParseAndCondenseCurrentSubchain(
                uInitialSubchainMarkerTypes, (uTokenTypes & CXXTokenTypeEOF),
                bCanReduceInnerElements)) {
          CXX_DEBUG_LEAVE_TEXT("Failed to parse subchain of type 0x%x",
                               g_cxx.pToken->eType);
          return false;
        }
      }
      ...
      if (cxxTokenTypeIsOneOf(g_cxx.pToken, uTokenTypes)) {
        ...
        CXX_DEBUG_LEAVE_TEXT("Got terminator subchain token 0x%x",
                             g_cxx.pToken->eType);
        return true;
      }
    ...
    if (cxxTokenTypeIsOneOf(g_cxx.pToken, uFinalSubchainMarkerTypes)) {
      CXX_DEBUG_LEAVE_TEXT("Got mismatched subchain terminator 0x%x",
                           g_cxx.pToken->eType);
...
// ([...],(...),{...}). Within the subchain analysis of uTokenTypes is
// completely disabled. Subchains do nest.
//
...
// This is usually what you want, unless you're really expecting a scope to
// begin in the current statement.
//
...
//
// Attempts to skip to either a semicolon or an EOF, ignoring anything in
// between. May be also used to recovery from certain forms of syntax errors.
// This function works also if the current token is a semicolon or an EOF.
//
bool cxxParserSkipToSemicolonOrEOF(void) {
  if (cxxTokenTypeIsOneOf(g_cxx.pToken,
...
bool cxxParserParseToEndOfQualifedName(void) {
  ...
  CXX_DEBUG_ASSERT(
      cxxTokenTypeIsOneOf(g_cxx.pToken,
                          CXXTokenTypeMultipleColons | CXXTokenTypeIdentifier),
      "This function should be called when pointing to a double-colon or an "
      "identifier");
  ...
  CXX_DEBUG_ASSERT(g_cxx.pToken->pPrev,
                   "There should be a previous token here");
  CXX_DEBUG_ASSERT(cxxTokenTypeIs(g_cxx.pToken->pPrev, CXXTokenTypeIdentifier),
...
void cxxParserSetEndLineForTagInCorkQueue(int iCorkQueueIndex,
                                          unsigned long lEndLine) {
  CXX_DEBUG_ASSERT(iCorkQueueIndex > CORK_NIL,
                   "The cork queue index is not valid");
...
// Make sure that the token chain contains only the specified keyword and
// eventually the "const" or "volatile" type modifiers.
static void
cxxParserCleanupEnumStructClassOrUnionPrefixChain(CXXKeyword eKeyword,
                                                  CXXToken *pLastToken) {
...
static bool cxxParserParseEnumStructClassOrUnionFullDeclarationTrailer(
    unsigned int uKeywordState, CXXKeyword eTagKeyword,
    const char *szTypeName) {
  CXX_DEBUG_ENTER();

  cxxTokenChainClear(g_cxx.pTokenChain);

  CXX_DEBUG_PRINT("Parse enum/struct/class/union trailer, typename is '%s'",
                  szTypeName);
...
bool cxxParserParseEnum(void) {
  ...
  /*
          Spec is:
                  enum-key attr(optional) identifier(optional)
     enum-base(optional) { enumerator-list(optional) }	(1) enum-key
     attr(optional) identifier enum-base(optional) ; (2)	(since C++11)

                  enum-key	-	one of enum, enum class(since C++11), or
     enum struct(since C++11)
                  attr(C++11)	-	optional sequence of any number of
     attributes identifier	-	the name of the enumeration that's being
     declared. If present, and if this declaration is a re-declaration, it may
     be preceded by nested-name-specifier(since C++11): sequence of names and
     scope-resolution operators ::, ending with scope-resolution operator. The
     name can be omitted only in unscoped enumeration declarations

                  enum-base(C++11)	-	colon (:), followed by a
     type-specifier-seq that names an integral type (if it is cv-qualified,
     qualifications are ignored) enumerator-list	-	comma-separated
     list of enumerator definitions, each of which is either simply an
     identifier, which becomes the name of the enumerator, or an identifier with
     an initializer: identifier = constexpr. In either case, the identifier can
     be directly followed by an optional attribute specifier sequence. (since
     C++17)
  */
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeKeyword)) {
    ...
    if ((g_cxx.pToken->eKeyword == CXXKeywordSTRUCT) ||
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeEOF)) {
    ...
    CXX_DEBUG_LEAVE_TEXT(
        "EOF before enum block: treating as forward declaration");
    return true;
  }
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeParenthesisChain)) {
    if (uInitialKeywordState & CXXParserKeywordStateSeenTypedef) {
      CXX_DEBUG_LEAVE_TEXT(
          "Found parenthesis after typedef: parsing as generic typedef");
  ...
  // If we have found a semicolon then we might be in the special case of KnR
  // function declaration. This requires at least 5 tokens and has some
  // additional constraints. See cxxParserMaybeParseKnRStyleFunctionDefinition()
  ...
  CXX_DEBUG_ASSERT(
      cxxTokenTypeIsOneOf(g_cxx.pToken,
                          CXXTokenTypeSingleColon | CXXTokenTypeOpeningBracket),
      "We should be pointing to a : or a {");

  // check if we can extract a class name identifier now
  CXXToken *pEnumName =
      cxxTokenChainLastTokenOfType(g_cxx.pTokenChain, CXXTokenTypeIdentifier);
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeSingleColon)) {
    ...
    if (cxxTokenTypeIsOneOf(g_cxx.pToken,
      ...
      cxxParserNewStatement();
      CXX_DEBUG_LEAVE_TEXT(
          "EOF or semicolon before enum block: can't decode this");
      return true;
    }
  ...
  if (pEnumName) {
    ...
    if (cxxParserCurrentLanguageIsCPP()) {
      ...
      while (pNamespaceBegin != pEnumName) {
        ...
        if (cxxParserCurrentLanguageIsCPP()) {
          ...
          cxxScopePush(pNamespaceBegin, CXXScopeTypeClass,
    ...
    bAnonymous = true;
    CXX_DEBUG_PRINT("Enum name is %s (anonymous)",
  ...
  bool bRet = cxxParserParseEnumStructClassOrUnionFullDeclarationTrailer(
      uInitialKeywordState, CXXKeywordENUM, vStringValue(pScopeName));
...
static bool cxxParserParseClassStructOrUnionInternal(CXXKeyword eKeyword,
  ...
  /*
     ...
     member-specification }
                  ...
                  class-key	-	one of class or struct. The keywords are
     identical except for the default member access and the default base class
     access. attr(C++11)	-	optional sequence of any number of
     attributes, may include alignas specifier
                  class-head-name	-	the name of the class that's being
     defined. Optionally qualified, optionally followed by keyword final. The
     name may be omitted, in which case the class is unnamed (note that unnamed
     class cannot be final) base-clause	-	optional list of one or more
     parent classes and the model of inheritance used for each (see derived
     class)
                  member-specification	-	list of access specifiers,
     member object and member function declarations and definitions (see below)
  */
  ...
  unsigned int uTerminatorTypes =
      CXXTokenTypeEOF | CXXTokenTypeSingleColon | CXXTokenTypeSemicolon |
      CXXTokenTypeOpeningBracket | CXXTokenTypeSmallerThanSign;
  ...
  if (!pClassName) {
    ...
    if (cxxParserCurrentLanguageIsCPP() && (!g_cxx.bConfirmedCPPLanguage) &&
        (eKeyword != CXXKeywordCLASS) && (g_cxx.pTokenChain->iCount >= 3) &&
      ...
      CXX_DEBUG_PRINT("Found no class/struct/union name identifier but there "
                      "is '%s' which might look good",
  ...
  if ((uTagKind == CXXTagCPPKindCLASS) && (!g_cxx.bConfirmedCPPLanguage)) {
    CXX_DEBUG_PRINT(
        "Succeeded in parsing a C++ class: this really seems to be C++");
  ...
  if (tag) {
    ...
    if (g_cxx.pTokenChain->iCount > 0) {
      ...
      if (g_cxx.pTokenChain->iCount > 0) {
        cxxTokenChainCondense(g_cxx.pTokenChain,
                              CXXTokenChainCondenseNoTrailingSpaces);
        tag->extensionFields.inheritance =
            vStringValue(g_cxx.pTokenChain->pHead->pszWord);
      }
    }
  ...
  cxxScopePush(pClassName, uScopeType,
               (uTagKind == CXXTagCPPKindCLASS) ? CXXScopeAccessPrivate
  ...
  bRet = cxxParserParseEnumStructClassOrUnionFullDeclarationTrailer(
      uInitialKeywordState, eKeyword, vStringValue(pScopeName));
...
bool cxxParserParseClassStructOrUnion(CXXKeyword eKeyword,
                                      unsigned int uTagKind,
                                      unsigned int uScopeType) {
  // Trick for "smart" handling of public/protected/private keywords in .h files
  // parsed as C++. See the declaration of
  // cxxKeywordEnablePublicProtectedPrivate for more info.

  // Enable public/protected/private keywords and save the previous state
  bool bEnablePublicProtectedPrivateKeywords =
      cxxKeywordEnablePublicProtectedPrivate(true);

  bool bRet =
      cxxParserParseClassStructOrUnionInternal(eKeyword, uTagKind, uScopeType);
  ...
  if (g_cxx.bConfirmedCPPLanguage)
    bEnablePublicProtectedPrivateKeywords =
        true; // leave it on for good: we're (almost) sure it's C++
...
void cxxParserAnalyzeOtherStatement(void) {
  ...
  if (!cxxTokenTypeIsOneOf(t, CXXTokenTypeIdentifier | CXXTokenTypeKeyword)) {
    CXX_DEBUG_LEAVE_TEXT(
        "Statement does not start with an identifier or keyword");
    return;
  }
...
check_function_signature:
  ...
  if (cxxParserLookForFunctionSignature(g_cxx.pTokenChain, &oInfo, NULL)) {
    ...
    if (g_cxx.uKeywordState & CXXParserKeywordStateSeenFriend) {
      ...
      CXX_DEBUG_PRINT("But it has been preceded by the 'friend' keyword: this "
                      "is not a real prototype");
    } else {
      int iScopesPushed = cxxParserEmitFunctionTags(
          &oInfo, CXXTagKindPROTOTYPE, CXXEmitFunctionTagsPushScopes, NULL);
    ...
    if (oInfo.pTrailingComma) {
      ...
      // This might be a special case of multiple prototypes in a single
      // declaration.
      //
      ...
      //
      // We cannot rely on oInfo.pIdentifierStart after
      // cxxParserEmitFunctionTags() since it has been removed. Manually skip
      // the initial type name.

      CXXToken *pBegin = cxxTokenChainFirstTokenNotOfType(
          g_cxx.pTokenChain, CXXTokenTypeIdentifier | CXXTokenTypeKeyword);

      CXX_DEBUG_ASSERT(pBegin, "We should have found a begin token here!");
      cxxTokenChainDestroyRange(g_cxx.pTokenChain, pBegin,
                                oInfo.pTrailingComma);
  ...
  if (g_cxx.uKeywordState &
      (
          // Note that since C++-17 inline can be used as a modifier for
          // variables so don't be tempted to put it here.
          CXXParserKeywordStateSeenExplicit |
          CXXParserKeywordStateSeenOperator |
          CXXParserKeywordStateSeenVirtual)) {
    // must be function!
    CXX_DEBUG_LEAVE_TEXT("WARNING: Was expecting to find a function prototype "
...
bool cxxParserParseAccessSpecifier(void) {
  ...
  if ((eScopeType != CXXScopeTypeClass) && (eScopeType != CXXScopeTypeUnion) &&
      (eScopeType != CXXScopeTypeStruct)) {
    CXX_DEBUG_LEAVE_TEXT("Access specified in wrong context (%d)", eScopeType);

    if (!g_cxx.bConfirmedCPPLanguage) {
      CXX_DEBUG_LEAVE_TEXT("C++ is not confirmed and the scope is not right: "
                           "likely not access specifier");
      g_cxx.pToken->eType = CXXTokenTypeIdentifier;
      return true;
    }

    // this is a syntax error: we're in the wrong scope.
    CXX_DEBUG_LEAVE_TEXT("C++ language is confirmed: bailing out to avoid "
                         "reporting broken structure");
    return false;
  }
  ...
  if (!g_cxx.bConfirmedCPPLanguage) {
    if (g_cxx.pToken->pPrev) {
      ...
      CXX_DEBUG_LEAVE_TEXT("C++ is not confirmed and there is something "
                           "before: likely not access specifier");
      g_cxx.pToken->eType = CXXTokenTypeIdentifier;
      return true;
    }
  }
...
findColon:
  if (!cxxParserParseUpToOneOf(uExtraType | CXXTokenTypeSingleColon |
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeSingleColon)) {
    if (!pInitialToken->pPrev) {
      CXX_DEBUG_PRINT("The access specifier was the first token and I have "
                      "found a colon: this is C++");
      g_cxx.bConfirmedCPPLanguage = true;
    }
  }
...
bool cxxParserParseIfForWhileSwitchCatchParenthesis(void) {
  ...
  CXX_DEBUG_ASSERT(cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeKeyword),
                   "This function should be called only after encountering one "
                   "of the keywords");

  CXXKeyword eKeyword = g_cxx.pToken->eKeyword;

  if (!cxxParserParseUpToOneOf(CXXTokenTypeParenthesisChain |
                                   CXXTokenTypeSemicolon |
                                   CXXTokenTypeOpeningBracket | CXXTokenTypeEOF,
                               false)) {
    CXX_DEBUG_LEAVE_TEXT(
        "Failed to parse if/for/while/switch/catch up to parenthesis");
    return false;
  }

  if (cxxTokenTypeIsOneOf(g_cxx.pToken, CXXTokenTypeEOF |
                                            CXXTokenTypeSemicolon |
                                            CXXTokenTypeOpeningBracket)) {
    CXX_DEBUG_LEAVE_TEXT("Found EOF/semicolon/opening bracket while parsing "
                         "if/for/while/switch/catch");
    return true;
  }

  CXX_DEBUG_ASSERT(cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeParenthesisChain),
  ...
  if (!bOkToExtractVariables) {
    ...
    //   if(int i ...
    //
    bOkToExtractVariables =
        cxxTokenTypeIs(cxxTokenChainAt(pChain, 1), CXXTokenTypeKeyword);
    ...
    if (!bOkToExtractVariables) {
      ...
      CXXToken *pToken = cxxTokenChainFirstTokenOfType(
          pChain, CXXTokenTypeAnd | CXXTokenTypeMultipleAnds |
                      CXXTokenTypeStar | CXXTokenTypeSmallerThanSign |
      ...
      if (pToken) {
        ...
        case CXXTokenTypeSmallerThanSign:
          ...
          bOkToExtractVariables =
              (cxxTokenChainFirstTokenOfType(pChain,
                                             CXXTokenTypeAssignment |
                                                 CXXTokenTypeSemicolon)
                   ? true
                   : false); // ternary ?: needed because of MSVC
          break;
...
static rescanReason cxxParserMain(const unsigned int passCount) {
  ...
  cppInit((bool)(passCount > 1), false,
          true, // raw literals
          false, kind_for_define, role_for_macro_undef, kind_for_macro_param,
...
void cxxParserCleanup(langType language CTAGS_ATTR_UNUSED,
