...
bool cxxParserParseNamespace(void) {
  ...
  /*
          Spec is:
                  ...
                  inline namespace ns_name { declarations }	(2)	(since
     C++11) namespace { declarations }	(3) namespace name = qualified-namespace
     ;	(7) namespace ns_name::name	{ (8)	(since C++17)

          Note that the using clauses have their own parsing routine and do not
     end up here.
  */
  ...
  if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeIdentifier)) {
    ...
    case CXXTokenTypeAssignment: {
      ...
      if (!cxxTokenTypeIsOneOf(g_cxx.pToken, CXXTokenTypeIdentifier |
      ...
      if (tag) {
        // This is highly questionable but well.. it's how old ctags did, so we
        // do.
        ...
        cxxTagSetField(CXXTagCPPFieldAliasedName,
    ...
    case CXXTokenTypeMultipleColons:
      ...
      CXX_DEBUG_ASSERT(pFirstIdentifier != pLastIdentifier,
                       "We expected multiple identifiers here");

      if (!cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeOpeningBracket)) {
        if (!cxxParserParseUpToOneOf(CXXTokenTypeOpeningBracket |
        ...
        if (!cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeOpeningBracket)) {
          ...
          CXX_DEBUG_LEAVE_TEXT(
              "Found semicolon/EOF just after namespace declaration");
          return true;
        }
      }
      break;
    ...
    CXX_DEBUG_ASSERT(cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeOpeningBracket),
    ...
    while (t) {
      ...
      if (tag) {
        ...
        vString *pszProperties =
            uProperties ? cxxTagSetProperties(uProperties) : NULL;
    ...
    CXX_DEBUG_LEAVE_TEXT(
        "Some kind of syntax error after namespace declaration");
    return cxxParserSkipToSemicolonOrEOF();
  }
