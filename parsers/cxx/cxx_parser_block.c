...
bool cxxParserParseBlockHandleOpeningBracket(void) {
  ...
  CXX_DEBUG_ASSERT(g_cxx.pToken->eType == CXXTokenTypeOpeningBracket,
      ...
      (bIsCPP && (g_cxx.pToken->pPrev) &&
       ((
            ...
            (
                ...
                // case 4
                cxxTokenTypeIsOneOf(g_cxx.pToken->pPrev->pPrev,
                                    CXXTokenTypeSingleColon |
                                        CXXTokenTypeComma) ||
                // cases 1,2,3 but not 4
                (
                    // more parts of typename or maybe the "new" keyword before
                    // the identifier
                    cxxTokenTypeIsOneOf(g_cxx.pToken->pPrev->pPrev,
                                        CXXTokenTypeIdentifier |
                                            ...
                                            CXXTokenTypeKeyword) &&
                    // but no parenthesis (discard things like bool test()
                    // Q_DECL_NO_THROW { ... })
                    (!(pAux = cxxTokenChainPreviousTokenOfType(
                           g_cxx.pToken->pPrev->pPrev,
                           CXXTokenTypeParenthesisChain))))) &&
            // "override" is handled as identifier since it's a keyword only
            // after function signatures
            (strcmp(vStringValue(g_cxx.pToken->pPrev->pszWord), "override") !=
             0)) ||
        (
            // type var[][][]..[] { ... }
            // (but not '[] { ... }' which is a parameterless lambda)
            cxxTokenTypeIs(g_cxx.pToken->pPrev,
  ...
  if (bIsCPP && (pParenthesis = cxxParserOpeningBracketIsLambda())) {
    if (!cxxParserHandleLambda(pParenthesis)) {
      CXX_DEBUG_LEAVE_TEXT("Lambda handling failed");
      return false;
    }

    // Note that here we're leaving the token chain "alive" so further parsing
    // can be performed.
  ...
  if (eScopeType != CXXScopeTypeFunction) {
    ...
    // (note that {}-style initializers have been handled above and thus are
    // excluded)
  ...
  // then it's likely to be a function-try-block and should be followed by a
  // catch block that is in the same scope.

  if (oInfo.uFlags & CXXFunctionSignatureInfoFunctionTryBlock) {
    // look for the catch blocks.
    CXX_DEBUG_PRINT(
        "The function is a function-try-block: looking for catch blocks");
    ...
    for (;;) {
      ...
      // the standard requires a bracket here (catch block is always a compound
      // statement).
      ...
      if (!cxxParserParseNextToken()) {
        CXX_DEBUG_LEAVE_TEXT(
            "Found EOF while looking for catch() block: playing nice");
        break; // EOF (would be a syntax error!)
      }

      if (!cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeOpeningBracket)) {
        // Aargh...
        CXX_DEBUG_LEAVE_TEXT("Found something unexpected while looking for "
                             "catch() block: playing nice");
        cxxParserUngetCurrentToken();
...
static bool cxxParserParseBlockInternal(bool bExpectClosingBracket) {
  ...
  for (;;) {
    ...
    found_eof:
      ...
      if (bExpectClosingBracket) {
        CXX_DEBUG_LEAVE_TEXT("Syntax error: found EOF in block but a closing "
  ...
  process_token:
    ...
    CXX_DEBUG_PRINT("Token '%s' of type 0x%02x",
    ...
    case CXXTokenTypeKeyword: {
        ...
        if ((
            ...
            (
                ...
                (!cxxTokenChainPreviousTokenOfType(
                    g_cxx.pToken, CXXTokenTypeStar | CXXTokenTypeAnd |
                                      CXXTokenTypeKeyword)))) {
          ...
          // let's try to be flexible: treat the namespace keyword as an
          // identifier.
          if (!g_cxx.bConfirmedCPPLanguage) {
            CXX_DEBUG_LEAVE_TEXT("Found namespace in unexpected place, but "
                                 "we're not sure it's really C++ "
      ...
      case CXXKeywordCLASS:
        if (!cxxParserParseClassStructOrUnion(
                CXXKeywordCLASS, CXXTagCPPKindCLASS, CXXScopeTypeClass)) {
      ...
      case CXXKeywordCATCH:
        if (!cxxParserParseIfForWhileSwitchCatchParenthesis()) {
          CXX_DEBUG_LEAVE_TEXT(
              "Failed to parse if/for/while/switch/catch parenthesis");
          return false;
        }
        // Now we're just before the block that follows the parenthesis.
        cxxParserNewStatement();
        // Force the cpp preprocessor to think that we're in the middle of a
        // statement.
        cppBeginStatement();
        break;
      case CXXKeywordTRY:
      ...
      case CXXKeywordTHROW:
        ...
        // and may confuse us. We keep "throw" when used as exception
        // specification, and this is certainly outside of a function and when
        // the token chain already contains at least a type, an identifier and a
        // parenthesis. This check seems excessive but keep in mind that we deal
        // with broken input and we might also be wrong about the current scope.
        if ((cxxScopeGetType() == CXXScopeTypeFunction) ||
      ...
      case CXXKeywordEXTERN:
        ...
        if (cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeStringConstant)) {
          ...
          //   extern "C" { int x; } <-- a declaration and definition: x IS
          //   defined
          //                             here and is NOT extern.
          //
          // A variable in an extern "C" block has to be re-declared extern
          // again to be really treated as declaration only.
          //
    ...
    case CXXTokenTypeSemicolon: {
      if ((cxxParserCurrentLanguageIsC()) && cxxScopeIsGlobal() &&
          (!(g_cxx.uKeywordState & CXXParserKeywordStateSeenExtern)) &&
          (!(g_cxx.uKeywordState & CXXParserKeywordStateSeenTypedef))) {
        ...
        default:
          CXX_DEBUG_LEAVE_TEXT(
              "Failed to check for K&R style function definition");
          return false;
          break;
        }
      } else {
    ...
    case CXXTokenTypeSingleColon: {
      ...
      if ((g_cxx.pTokenChain->iCount == 2) &&
          cxxTokenTypeIs(cxxTokenChainFirst(g_cxx.pTokenChain),
                         CXXTokenTypeIdentifier)) {
        ...
        // what is this? (default: and similar things have been handled at
        // keyword level)
      }
    } break;
...
//
// This is the toplevel scanning function. It's a forward-only scanner that
// keeps accumulating tokens in the chain until either a characteristic token is
// found or the statement ends. When a characteristic token is found it usually
// enters a specialized scanning routine (e.g for classes, namespaces,
// structs...). When the statement ends without finding any characteristic token
// the chain is passed to an analysis routine which does a second scan pass.
//
