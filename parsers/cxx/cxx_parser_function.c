...
//      A K&R function declaration has possibly been extracted (but not
//      necessarily). Anyway, a new statement has been started.
//   0: The parser has NOT moved forward and the current statement hasn't been
//   cleared:
...
int cxxParserMaybeParseKnRStyleFunctionDefinition(void) {
  ...
  CXX_DEBUG_PRINT("Looking for K&R-style function in '%s'",
                  vStringValue(pChain));
  ...
  CXX_DEBUG_ASSERT(cxxParserCurrentLanguageIsC(),
                   "Should be called only when parsing C");
  CXX_DEBUG_ASSERT(cxxTokenChainLast(g_cxx.pTokenChain),
                   "At least one token should be there");
  CXX_DEBUG_ASSERT(cxxTokenTypeIs(cxxTokenChainLast(g_cxx.pTokenChain),
  ...
  CXXToken *pParenthesis = cxxTokenChainFirstTokenOfType(
      g_cxx.pTokenChain, CXXTokenTypeParenthesisChain);
  if (!pParenthesis)
  ...
  x = cxxTokenChainNextTokenNotOfType(
      pParenthesis, CXXTokenTypeIdentifier | CXXTokenTypeKeyword |
                        CXXTokenTypeSquareParenthesisChain | CXXTokenTypeStar |
                        CXXTokenTypeComma | CXXTokenTypeSingleColon |
                        CXXTokenTypeNumber);
  ...
  x = cxxTokenChainPreviousTokenNotOfType(
      x, CXXTokenTypeSquareParenthesisChain | CXXTokenTypeSingleColon |
             CXXTokenTypeNumber);
  ...
  CXX_DEBUG_ASSERT(pParenthesis->pChain,
  ...
  if ((pParenthesis->pChain->iCount == 3) &&
      cxxTokenTypeIs(cxxTokenChainAt(pParenthesis->pChain, 1),
  ...
  // Now check if the contents of the parenthesis chain look like a K&R
  // signature
  ...
  CXX_DEBUG_ASSERT(x,
                   "We should have found something in the parenthesis chain");
  ...
  for (;;) {
    ...
    x = x->pNext;
    CXX_DEBUG_ASSERT(x,
                     "We should have found at least the closing parenthesis");
  ...
  CXX_DEBUG_ASSERT(g_cxx.pTokenChain->pHead,
...
#define MAX_EXTRA_KNR_PARAMETERS 10
  ...
  while (iParameterCount > 0) {
    ...
    if (!cxxParserParseUpToOneOf(CXXTokenTypeSemicolon |
    ...
    if (iExtraStatementsInChain < MAX_EXTRA_KNR_PARAMETERS) {
      CXX_DEBUG_ASSERT(pCurrentTail->pNext,
  ...
  CXX_DEBUG_PRINT("Found K&R-style function '%s'",
  ...
  if (cxxTagKindEnabled(CXXTagKindPARAMETER)) {
    ...
    for (;;) {
      cxxParserExtractVariableDeclarations(
          g_cxx.pTokenChain, CXXExtractVariableDeclarationsKnRStyleParameters);
...
bool cxxParserTokenChainLooksLikeFunctionCallParameterSet(
  ...
  CXX_DEBUG_ASSERT(
      cxxTokenTypeIsOneOf(t, CXXTokenTypeOpeningParenthesis |
                                 CXXTokenTypeOpeningBracket),
      "The token chain should start with an opening parenthesis/bracket");
  CXX_DEBUG_ASSERT(
      cxxTokenTypeIsOneOf(pLast, CXXTokenTypeClosingParenthesis |
  ...
  bool bDealingWithParenthesisChain =
      (uTerminator == CXXTokenTypeClosingParenthesis);

  t = t->pNext;

  while (t != pLast) {
    if (bDealingWithParenthesisChain &&
        cxxTokenTypeIsOneOf(
            t, CXXTokenTypeNumber | CXXTokenTypeStringConstant |
                   CXXTokenTypeCharacterConstant | CXXTokenTypePointerOperator |
                   CXXTokenTypeDotOperator | CXXTokenTypeOperator |
    ...
    if (cxxTokenTypeIs(t, CXXTokenTypeKeyword)) {
      ...
      if (bDealingWithParenthesisChain && (cxxKeywordIsConstant(t->eKeyword) ||
      ...
      if ((t->eKeyword != CXXKeywordNEW) &&
          cxxTokenTypeIsOneOf(t->pNext, CXXTokenTypeKeyword | CXXTokenTypeStar |
                                            CXXTokenTypeAnd |
      ...
      if (cxxTokenTypeIsOneOf(t->pNext,
                              CXXTokenTypeKeyword | CXXTokenTypeIdentifier)) {
        // this is something like:
        // (a b...
        return false;
      }
    } else if (cxxTokenTypeIs(t, CXXTokenTypeGreaterThanSign)) {
      if (cxxTokenTypeIsOneOf(t->pNext, CXXTokenTypeAnd | CXXTokenTypeStar |
                                            CXXTokenTypeMultipleAnds |
    ...
    } else if (cxxTokenTypeIs(t, CXXTokenTypeParenthesisChain) &&
               cxxTokenTypeIsOneOf(
                   t->pPrev, CXXTokenTypeIdentifier | CXXTokenTypeKeyword |
                                 CXXTokenTypeStar | CXXTokenTypeAnd |
                                 CXXTokenTypeGreaterThanSign) &&
               cxxTokenTypeIs(t->pNext, CXXTokenTypeParenthesisChain) &&
               cxxTokenTypeIs(cxxTokenChainAt(t->pChain, 1),
                              CXXTokenTypeStar) &&
    ...
    if (cxxTokenTypeIs(t, CXXTokenTypeAssignment)) {
      ...
      t = cxxTokenChainNextTokenOfType(t, uTerminator | CXXTokenTypeComma);
...
bool cxxParserTokenChainLooksLikeConstructorParameterSet(
  ...
  if (pChain->iCount < 3) {
    CXX_DEBUG_ASSERT(pChain->iCount == 2, "This function should be called only "
                                          "on parenthesis and bracket chains");

    if (cxxTokenTypeIs(cxxTokenChainFirst(pChain),
                       CXXTokenTypeOpeningBracket)) {
      CXX_DEBUG_ASSERT(
    ...
    CXX_DEBUG_ASSERT(cxxTokenTypeIs(cxxTokenChainFirst(pChain),
                                    CXXTokenTypeOpeningParenthesis),
                     "This function should be called only on parenthesis and "
                     "bracket chains");
...
static bool cxxParserLookForFunctionSignatureCheckParenthesisAndIdentifier(
    CXXToken *pParenthesis, CXXTokenChain *pIdentifierChain,
    CXXToken *pIdentifierStart, CXXToken *pIdentifierEnd,
    CXXFunctionSignatureInfo *pInfo, CXXFunctionParameterInfo *pParamInfo) {
  CXX_DEBUG_ENTER();

  CXX_DEBUG_ASSERT(
      pParenthesis && pIdentifierChain && pIdentifierStart && pIdentifierEnd &&
          pInfo,
  ...
  CXX_DEBUG_ASSERT(cxxTokenTypeIs(pParenthesis, CXXTokenTypeParenthesisChain),
  ...
  if (
      ...
      cxxParserTokenChainLooksLikeFunctionParameterList(pInner->pChain,
                                                        pParamInfo)) {
    // __ARGS() case
...
//
//
bool cxxParserLookForFunctionSignature(CXXTokenChain *pChain,
  ...
  CXX_DEBUG_PRINT("Looking for function signature in '%s'",
  ...
  // Strategy:
  //
  //    Scan the toplevel token chain and look for the first identifier
  //    immediately followed by a parenthesis chain that looks like a (possibly
  //    empty) list of function parameters.
  //
  //    Since the identifier may be hidden within a parenthesis chain (and thus
  //    NOT be toplevel) we must scan the inner parenthesis chains in a sequence
  //    of special cases.
  //
  //    (Mainly) for this reason this loop first looks for a parenthesis chain
  //    (which is always present at toplevel) and then looks for a suitable
  //    identifier near or inside it.
  //
  //    Once we have found a suitable parenthesis-chain/identifier pair we
  //    continue scanning until one of { ; EOF : , is found.
  //
  //    We bail out if anything suspicious is found in the middle of the scan.
  //

  while (pToken) {
    CXX_DEBUG_PRINT("Token '%s' of type 0x%02x (%s)",
                    vStringValue(pToken->pszWord), pToken->eType,
                    cxxDebugTypeDecode(pToken->eType));

    // Check exit conditions first

    if (cxxTokenTypeIsOneOf(pToken, CXXTokenTypeOpeningBracket |
                                        CXXTokenTypeSemicolon |
                                        CXXTokenTypeEOF)) {
      // reached end
    ...
    if (cxxParserCurrentLanguageIsCPP() &&
        cxxTokenTypeIsOneOf(pToken, CXXTokenTypeSingleColon |
    ...
    // Check for tokens that should never appear at top level of a function
    // signature

    if (cxxTokenTypeIsOneOf(
            pToken, CXXTokenTypeOperator | CXXTokenTypePointerOperator |
      ...
      CXX_DEBUG_LEAVE_TEXT(
          "Found token that should never appear at toplevel of a signature");
      return false;
    }

    // Explicitly skip template-like angle brackets, which are not condensed
    // here and may confuse us
    ...
    // until an exit condition is found. Do not look for parenthesis+identifier
    // again.

    if (pInfo->pParenthesis) {
      CXX_DEBUG_PRINT("Already have a proper parenthesis: continuing loop to "
                      "find terminator");
      goto next_token;
    }
    ...
    if (cxxTokenIsKeyword(pToken, CXXKeywordOPERATOR)) {
      ...
      // int operator + (...)
      // void * operator new[] ()
      // template<typename T> cv::Affine3<T>::operator Eigen::Transform<T, 3,
      // Eigen::Affine, (Eigen::RowMajor)>() const
      ...
      while (pToken) {
        CXX_DEBUG_PRINT("Candidate token '%s' of type 0x%02x (%s)",
                        vStringValue(pToken->pszWord), pToken->eType,
                        cxxDebugTypeDecode(pToken->eType));
        ...
        if (cxxTokenTypeIs(pToken, CXXTokenTypeParenthesisChain)) {
          ...
          if ((!cxxTokenIsKeyword(pToken, CXXKeywordNEW)) &&
          ...
          if (pToken->pPrev == pIdentifierStart) {
            ...
            return false;
                       ...
                       CXXTokenTypeAnd | CXXTokenTypeAssignment |
                           ...
                           CXXTokenTypeAngleBracketChain |
                           CXXTokenTypeMultipleColons | CXXTokenTypeStar |
      ...
      CXX_DEBUG_ASSERT(cxxTokenTypeIs(pToken, CXXTokenTypeParenthesisChain),
                       "Must have found a parenthesis chain here");
      ...
      // Even if the check above failed we have nothing more to do with this
      // token
      goto next_token;
    }
    ...
    CXX_DEBUG_PRINT(
        "Found interesting parenthesis chain: check for identifier");
    ...
    if (cxxTokenTypeIs(pToken->pPrev, CXXTokenTypeIdentifier)) {
      ...
      if (cxxParserLookForFunctionSignatureCheckParenthesisAndIdentifier(
              pTopLevelParenthesis, pChain, pIdentifierStart, pIdentifierEnd,
              pInfo, pParamInfo)) {
        ...
        //
        // However, there are a couple of very common special cases that is nice
        ...
        // all macros for a large project. For this reason we use some
        // heuristics to handle these special cases automatically.
        if (
            ...
            cxxTokenTypeIs(pInfo->pParenthesis->pNext,
                           CXXTokenTypeIdentifier) &&
            // There is something after the identifier
            pInfo->pParenthesis->pNext->pNext &&
            (
                // The token following the identifier is again a parenthesis
                // chain
                cxxTokenTypeIs(pInfo->pParenthesis->pNext->pNext,
                               CXXTokenTypeParenthesisChain) ||
            ...
            (!cxxTokenChainFirstTokenOfType(pInfo->pParenthesis->pChain,
                                            CXXTokenTypeComma))) {
          CXX_DEBUG_PRINT(
              "Found special case of MACRO(return_type) function()/variable");
          pInfo->pParenthesis = NULL;
        }
    ...
    if (
      ...
      CXXToken *pSpecBegin =
          cxxTokenChainSkipBackToStartOfTemplateAngleBracket(pToken->pPrev);

      if (pSpecBegin && pSpecBegin->pPrev &&
    ...
    if (
        ...
        //    ret type (*baz(params))(params) <-- function returning function
        //    pointer ret type (*baz(params)) <-- function returning a pointer
        //    ret type (*baz(params))[2] <-- function returning a pointer to
        //    array
        (pIdentifierStart =
             cxxParserFindFirstPossiblyNestedAndQualifiedIdentifier(
                 pToken->pChain, &pIdentifierChain))) {
      CXX_DEBUG_PRINT("Got identifier '%s' inside parenthesis chain",
      ...
      if (pIdentifierStart->pNext &&
          cxxTokenTypeIs(pIdentifierStart->pNext,
                         CXXTokenTypeParenthesisChain) &&
          cxxParserTokenChainLooksLikeFunctionParameterList(
              pIdentifierStart->pNext->pChain, NULL)) {
        CXX_DEBUG_PRINT(
            "Identifier followed by a parameter-like parenthesis chain");
        pIdentifierEnd = pIdentifierStart;
        // correct our guess for parenthesis
        pTopLevelParenthesis = pIdentifierStart->pNext;

        if (cxxParserLookForFunctionSignatureCheckParenthesisAndIdentifier(
                pTopLevelParenthesis, pIdentifierChain, pIdentifierStart,
                pIdentifierEnd, pInfo, pParamInfo))
          goto next_token;

      } else {
        // Looks more like a function pointer or something else we can't figure
        // out
        CXX_DEBUG_LEAVE_TEXT(
            "Identifier NOT followed by a parameter-like parenthesis chain");
      }
  ...
  if (cxxParserCurrentLanguageIsCPP()) {
    ...
    while (pAux) {
      CXX_DEBUG_PRINT("Token '%s' of type 0x%02x", vStringValue(pAux->pszWord),
                      pAux->eType);
      ...
      if (!cxxTokenTypeIs(pAux, CXXTokenTypeIdentifier)) {
        ...
        if (cxxTokenTypeIs(pAux, CXXTokenTypeGreaterThanSign)) {
          ...
          CXXToken *pSmallerThan =
              cxxTokenChainSkipBackToStartOfTemplateAngleBracket(pAux);
          if (!pSmallerThan)
            break; // nope
          ...
          // same as above, but already condensed (though it should never
          // happen)
          if (!pAux->pPrev)
            break; // nope
      ...
      CXX_DEBUG_PRINT("Shifting scope start to '%s'",
    ...
    // Look for trailing const and other interesting things that may come after
    // the parenthesis.

    if (pTopLevelParenthesis->pNext) {
      CXX_DEBUG_PRINT("Top level parenthesis is followed by '%s' (%s)",
      ...
      CXXToken *pAssignment = cxxTokenChainNextTokenOfType(
          pTopLevelParenthesis, CXXTokenTypeAssignment);

      if (pAssignment && pAssignment->pNext) {
        if (cxxTokenTypeIs(pAssignment->pNext, CXXTokenTypeNumber) &&
      ...
      CXXToken *pIdentOrKeyword = cxxTokenChainNextTokenOfType(
          pTopLevelParenthesis, CXXTokenTypeIdentifier | CXXTokenTypeKeyword);

      while (pIdentOrKeyword) {
        // override is a keyword only in specific contexts so we handle it as
        // identifier
        if (cxxTokenTypeIs(pIdentOrKeyword, CXXTokenTypeKeyword)) {
          ...
          // The "final" keyword is actually disabled in most contexts so we
          // handle it as identifier. "override is always handled as identifier.
          if (strcmp(vStringValue(pIdentOrKeyword->pszWord), "final") == 0)
            pInfo->uFlags |= CXXFunctionSignatureInfoFinal;
          else if (strcmp(vStringValue(pIdentOrKeyword->pszWord), "override") ==
                   0)
            pInfo->uFlags |= CXXFunctionSignatureInfoOverride;
        }

        pIdentOrKeyword = cxxTokenChainNextTokenOfType(
            pIdentOrKeyword, CXXTokenTypeIdentifier | CXXTokenTypeKeyword);
      }
    } else {
      pInfo->pSignatureConst = NULL;
    }
  ...
  if (pInfo->pIdentifierChain != pChain) {
    // Nested parentheses. In this case the type name is the whole chain
    // (excluding the identifier and the signature).
    CXX_DEBUG_PRINT(
        "Nested parentheses, probably a function returning pointers");
    ...
    if (pToken->pPrev) {
      CXXToken *pParenthesisOrConst =
          pInfo->pSignatureConst ? pInfo->pSignatureConst : pInfo->pParenthesis;
      if (cxxParserCurrentLanguageIsCPP() &&
          cxxTokenTypeIs(pToken->pPrev, CXXTokenTypeKeyword) &&
          (pToken->pPrev->eKeyword == CXXKeywordAUTO) &&
          pParenthesisOrConst->pNext &&
          cxxTokenTypeIs(pParenthesisOrConst->pNext,
                         CXXTokenTypePointerOperator) &&
          pParenthesisOrConst->pNext->pNext &&
          (!cxxTokenTypeIsOneOf(pParenthesisOrConst->pNext->pNext,
                                CXXTokenTypeSemicolon |
                                    CXXTokenTypeOpeningBracket))) {
        ...
        while (pInfo->pTypeEnd->pNext &&
               (!cxxTokenTypeIsOneOf(pInfo->pTypeEnd->pNext,
                                     CXXTokenTypeSemicolon |
                                         CXXTokenTypeOpeningBracket)))
        ...
        if (cxxTokenTypeIs(pInfo->pTypeEnd, CXXTokenTypeParenthesisChain) &&
          ...
          pInfo->pTypeStart =
              cxxTokenChainFirst(pInfo->pTypeEnd->pChain)->pNext;
...
//
int cxxParserEmitFunctionTags(CXXFunctionSignatureInfo *pInfo,
                              unsigned int uTagKind, unsigned int uOptions,
  ...
  CXX_DEBUG_PRINT("Scope start is %x, push scope is %d", pInfo->pScopeStart,
                  bPushScopes);

  // We'll be removing the scope and identifier, fix type
  if (pInfo->pTypeStart && ((pInfo->pTypeStart == pInfo->pScopeStart) ||
                            (pInfo->pTypeStart == pInfo->pIdentifierStart)))
    pInfo->pTypeStart = pInfo->pIdentifierEnd->pNext;

  CXX_DEBUG_ASSERT(pInfo->pTypeEnd != pInfo->pIdentifierEnd,
                   "The type should never end at identifier");
    ...
    if (bPushScopes) {
      ...
      while (pInfo->pScopeStart != pInfo->pIdentifierStart) {
        ...
        pInfo->pScopeStart = cxxTokenChainNextTokenOfType(
            pInfo->pScopeStart, CXXTokenTypeMultipleColons);

        CXX_DEBUG_ASSERT(pInfo->pScopeStart,
                         "We should have found a next token here");

        pInfo->pScopeStart = pInfo->pScopeStart->pNext;

        cxxTokenChainDestroyRange(pInfo->pIdentifierChain, pScopeId->pNext,
        ...
        cxxScopePush(pScopeId, CXXScopeTypeClass,
                     // WARNING: We don't know if it's really a class! (FIXME?)
                     CXXScopeAccessUnknown);
        iScopesPushed++;
      }
    } else {
      cxxTokenChainDestroyRange(pInfo->pIdentifierChain, pInfo->pScopeStart,
                                pInfo->pIdentifierStart->pPrev);
    }
  }

  CXXToken *pIdentifier =
      cxxTokenChainExtractRange(pInfo->pIdentifierStart, pInfo->pIdentifierEnd,
                                // proper spacing has been already ensured
                                // by cxxParserLookForFunctionSignature()
                                0);

  cxxTokenChainDestroyRange(pInfo->pIdentifierChain, pInfo->pIdentifierStart,
                            pInfo->pIdentifierEnd);

  CXX_DEBUG_ASSERT(pIdentifier,
  ...
  if ((uTagKind == CXXTagKindFUNCTION) &&
    ...
    // the scope we're using is off by one level. This is the "friend
    // definition" trick:
    //
    //  class X
    //  {
    ...
    // parsed, but this is hard to do with the current implementation. We would
    // need to store this scope somewhere and push it back after the body of the
    // function has been parsed. It can be done, but it's expensive.
    //
    // Friend declarations are very rare and are implicitly inlined per C++
    // standard so "sane" such declarations are short and usually don't have
    // meaningful tags inside.
  ...
  if (tag) {
    ...
    vString *pszSignature =
        cxxTokenChainJoin(pInfo->pParenthesis->pChain, NULL, 0);
    ...
    if (pInfo->pTypeStart) {
      if (pInfo->bTypeContainsIdentifierScopeAndSignature) {
        ...
        if (pInfo->pTypeStart && pInfo->pTypeEnd) {
          ...
          cxxTokenChainTake(pInfo->pParenthesisContainerChain,
                            pInfo->pParenthesis);

          pTypeName =
              cxxTagCheckAndSetTypeField(pInfo->pTypeStart, pInfo->pTypeEnd);

          cxxTokenChainInsertAfter(pInfo->pParenthesisContainerChain,
    ...
    if (bGotTemplate && cxxTagFieldEnabled(CXXTagCPPFieldTemplate)) {
    ...
    if (cxxTagFieldEnabled(CXXTagFieldProperties)) {
      ...
      if (g_cxx.uKeywordState & CXXParserKeywordStateSeenExplicit)
        uProperties |= CXXTagPropertyExplicit; // FIXME: Handle
                                               // "CXXTagPropertyConstructor"?
      ...
      if ((pInfo->uFlags & CXXFunctionSignatureInfoTemplateSpecialization) ||
          bIsEmptyTemplate)
...
#ifdef CXX_DO_DEBUGGING
  if (uTagKind == CXXTagKindFUNCTION)
    CXX_DEBUG_PRINT("Emitted function '%s'",
                    vStringValue(pIdentifier->pszWord));
  else
    CXX_DEBUG_PRINT("Emitted prototype '%s'",
                    vStringValue(pIdentifier->pszWord));
#endif
  ...
  if (bGotTemplate && cxxTagKindEnabled(CXXTagCPPKindTEMPLATEPARAM))
...
int cxxParserExtractFunctionSignatureBeforeOpeningBracket(
    CXXFunctionSignatureInfo *pInfo, int *piCorkQueueIndex) {
  CXX_DEBUG_ENTER();
  ...
  CXX_DEBUG_ASSERT(g_cxx.pTokenChain->iCount > 0,
  ...
  if (!cxxParserLookForFunctionSignature(g_cxx.pTokenChain, pInfo,
                                         &oParamInfo)) {
  ...
  int iScopesPushed = cxxParserEmitFunctionTags(pInfo, CXXTagKindFUNCTION,
...
void cxxParserEmitFunctionParameterTags(CXXFunctionParameterInfo *pInfo) {
  ...
  while (i < pInfo->uParameterCount) {
    tagEntryInfo *tag =
        cxxTagBegin(CXXTagKindPARAMETER, pInfo->aIdentifiers[i]);
...
bool cxxParserTokenChainLooksLikeFunctionParameterList(
    CXXTokenChain *tc, CXXFunctionParameterInfo *pParamInfo) {
  CXX_DEBUG_ENTER();
  CXX_DEBUG_ASSERT(tc->iCount >= 2,
  ...
  for (;;) {
    ...
    if (!cxxTokenTypeIsOneOf(t, CXXTokenTypeIdentifier | CXXTokenTypeKeyword |
      ...
      CXX_DEBUG_LEAVE_TEXT("Token '%s' is something that is not a identifier, "
                           "keyword, :: or ...",
...
#define TOKENS_THAT_SHOULD_NOT_APPEAR_IN_SIGNATURE_BEFORE_ASSIGNMENT           \
  (CXXTokenTypePointerOperator | CXXTokenTypeOperator |                        \
   CXXTokenTypeDotOperator | CXXTokenTypeNumber | CXXTokenTypeStringConstant | \
   CXXTokenTypeCharacterConstant | CXXTokenTypeAngleBracketChain |             \
   CXXTokenTypeSingleColon)

  try_again:
    t = cxxTokenChainNextTokenOfType(
        t, CXXTokenTypeClosingParenthesis | CXXTokenTypeComma |
    ...
    if (cxxTokenTypeIs(t, CXXTokenTypeParenthesisChain)) {
      ...
      // Either part of function pointer declaration or a very ugly variable
      // decl Examples are:
      //    type (*name)(args)
      //    type ((*name)(args))
      //    type (*name)
      ...
      if ((!cxxTokenChainFirstTokenOfType(
              t->pChain,
              TOKENS_THAT_SHOULD_NOT_APPEAR_IN_SIGNATURE_BEFORE_ASSIGNMENT)) &&
          (cxxTokenChainFirstTokenOfType(
               t->pChain,
               CXXTokenTypeStar |
                   CXXTokenTypeAnd) || // part of (*name) or (&name)
           cxxParserTokenChainLooksLikeFunctionParameterList(t->pChain,
                                                             NULL) || // (args)
           !cxxTokenChainFirstTokenNotOfType(
               t->pChain,
               CXXTokenTypeOpeningParenthesis | CXXTokenTypeParenthesisChain |
                   CXXTokenTypeClosingParenthesis) // ((whatever)(whatever))
           ))
        goto try_again;

      CXX_DEBUG_LEAVE_TEXT("Found a parenthesis chain that doesn't belong to a "
    ...
    if (cxxTokenTypeIs(t, CXXTokenTypeSmallerThanSign)) {
      ...
      if (!t) {
        CXX_DEBUG_LEAVE_TEXT("Either not a function declaration or unbalanced "
    ...
    if (cxxTokenTypeIsOneOf(
            t, TOKENS_THAT_SHOULD_NOT_APPEAR_IN_SIGNATURE_BEFORE_ASSIGNMENT)) {
      CXX_DEBUG_LEAVE_TEXT(
      ...
      if (pParamInfo->uParameterCount < CXX_MAX_EXTRACTED_PARAMETERS) {
        ...
        if (cxxTokenTypeIs(t->pPrev, CXXTokenTypeIdentifier)) {
          ...
          CXXToken *pNonSquareParenthesis = cxxTokenChainPreviousTokenNotOfType(
              t, CXXTokenTypeSquareParenthesisChain);

          bool bPrevIsSquareParenthesis =
              (pNonSquareParenthesis && (pNonSquareParenthesis != t->pPrev));

          if (bPrevIsSquareParenthesis &&
              cxxTokenTypeIs(pNonSquareParenthesis, CXXTokenTypeIdentifier)) {
            // type var[]
            // type var[]...[]
            pIdentifier = pNonSquareParenthesis;
          } else if (bPrevIsSquareParenthesis &&
                     cxxTokenTypeIs(pNonSquareParenthesis,
                                    CXXTokenTypeParenthesisChain) &&
                     (pIdentifier = cxxTokenChainFirstTokenOfType(
                          pNonSquareParenthesis->pChain,
                          CXXTokenTypeIdentifier))) {
            // type (...var)[]
          } else if (cxxTokenTypeIs(t->pPrev, CXXTokenTypeNumber) &&
                     cxxTokenTypeIs(t->pPrev->pPrev, CXXTokenTypeIdentifier)) {
            // type var:bits
            pIdentifier = t->pPrev->pPrev;
          } else if (cxxTokenTypeIs(t->pPrev, CXXTokenTypeParenthesisChain) &&
                     ((
                          // type (*name)(args)
                          cxxTokenTypeIs(t->pPrev->pPrev,
                                         CXXTokenTypeParenthesisChain) &&
                          (pIdentifier =
                               cxxTokenChainLastPossiblyNestedTokenOfType(
                                   t->pPrev->pPrev->pChain,
                                   CXXTokenTypeIdentifier, NULL)) &&
                          pIdentifier->pPrev &&
                          cxxTokenTypeIs(pIdentifier->pPrev,
                                         CXXTokenTypeStar)) ||
                      (
                          // type (*&name)
                          (pIdentifier =
                               cxxTokenChainLastPossiblyNestedTokenOfType(
                                   t->pPrev->pChain, CXXTokenTypeIdentifier,
                                   NULL)) &&
                          pIdentifier->pPrev &&
                          cxxTokenTypeIsOneOf(pIdentifier->pPrev,
                                              CXXTokenTypeStar |
                                                  CXXTokenTypeAnd)))) {
            // type (*ptr)(args)
        ...
        if (pIdentifier) {
          ...
          CXX_DEBUG_PRINT("Found parameter '%s' in '%s'",
                          vStringValue(pIdentifier->pszWord),
                          vStringValue(pDecl->pszWord));
          cxxTokenDestroy(pDecl);
          CXX_DEBUG_ASSERT(cxxTokenChainFindToken(pParamInfo->pChain, pStart) >=
                               0,
    ...
    // assignment.
    if (!bIsCPP) {
      CXX_DEBUG_LEAVE_TEXT("Found assignment, this doesn't look like valid C "
    ...
    t = cxxTokenChainNextTokenOfType(t, CXXTokenTypeClosingParenthesis |
                                            CXXTokenTypeComma);
