...
bool cxxParserParseGenericTypedef(void) {
  ...
  for (;;) {
    ...
    // keyword
    if ((g_cxx.pToken->eKeyword == CXXKeywordEXTERN) ||
...
//
//    [typedef] int (*(*x)(int))[2];
//        x = int (*(*)(int))[2] <-- which is a function pointer taking an int
//        and
//                                   returning a pointer to an array of two
//                                   integers...
//
//    [typedef] blah (*x(k (*)(y *)))(z *);
//        x = blah (*(k (*)(y *)))(z *) <-- which is a function (!not a function
//        pointer!)
//                                   taking a function pointer A as argument and
//                                   returning a function pointer B. A = k (*)(y
//                                   *) and B = blah (*)(z *)
//
...
//
// In case of multiple declarations it seems that only the first part with
// identifiers and keywords is kept across types. Ex:
...
void cxxParserExtractTypedef(CXXTokenChain *pChain,
  ...
  if (bExpectTerminatorAtEnd) {
    ...
    CXX_DEBUG_ASSERT(cxxTokenTypeIs(t, CXXTokenTypeSemicolon),
  ...
  while (pChain->iCount >= 2) {
    //
    // Skip either to a comma or to the end, but keep track of the first
    // parenthesis.
    //
  ...
  skip_to_comma_or_end:
    ...
    if (pComma) {
      ...
      if (cxxTokenTypeIs(pComma, CXXTokenTypeSmallerThanSign)) {
        ...
        if (!t) {
          CXX_DEBUG_LEAVE_TEXT(
              "Mismatched < sign inside typedef: giving up on it");
          return;
        }
        // and go ahead
      ...
      CXX_DEBUG_ASSERT(cxxTokenTypeIs(pComma, CXXTokenTypeComma),
                       "Oops, expected a comma!");
    ...
    if (cxxTokenTypeIs(t, CXXTokenTypeIdentifier)) {
      ...
      CXX_DEBUG_PRINT("Identifier seems to be at end: %s",
      ...
      //
      if (pFirstParenthesis->pNext &&
          cxxTokenTypeIsOneOf(pFirstParenthesis->pNext,
                              CXXTokenTypeParenthesisChain |
                                  CXXTokenTypeSquareParenthesisChain)) {
        CXX_DEBUG_PRINT(
            "There are two parenthesis chains. Looking in the first one");
        t = cxxTokenChainLastPossiblyNestedTokenOfType(
            pFirstParenthesis->pChain, CXXTokenTypeIdentifier, &pTParentChain);
      } else {
        CXX_DEBUG_PRINT("There is one parenthesis chain. Looking just before");

        if (pFirstParenthesis->pPrev &&
      ...
      CXX_DEBUG_PRINT(
          "No identifier and no parenthesis chain, trying to scan backwards");
      pTParentChain = pChain;
    ...
    if (!t) {
      // Not found yet.
      // If we're in C++ mode but we haven't confirmed that the language is
      // really C++ then we might try to look for a C++ identifier here.
      ...
      if (!t) {
        CXX_DEBUG_LEAVE_TEXT(
            "Didn't find an identifier: something nasty is going on");
        return;
      }
    }
    ...
    if (tag) {
      ...
      if ((pTParentChain == pChain) && // not function pointer (see above)
          (pComma ? cxxTokenChainPreviousTokenOfType(
                        pComma, CXXTokenTypeParenthesisChain |
                                    CXXTokenTypeAngleBracketChain)
                  : cxxTokenChainLastTokenOfType(
                        pChain, CXXTokenTypeParenthesisChain |
                                    CXXTokenTypeAngleBracketChain))) {
        CXX_DEBUG_LEAVE_TEXT(
            "Wild parenthesis in type definition: not emitting typeref");
      } else {
        // other kind of typeref, use typename here.
        CXX_DEBUG_ASSERT(pChain->iCount > 0,
    ...
    // We must kill anything up to either an identifier, a keyword (type) or >
    // which is assumed to be part of a template.

    while (pComma->pPrev &&
           (!cxxTokenTypeIsOneOf(pComma->pPrev,
