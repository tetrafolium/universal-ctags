...
typedef void (*cxxDebugDumpCommonFunc)(void *, struct circularRefChecker *,
                                       struct circularRefChecker *, bool);
void cxxDebugDumpCommon(void *data,
                        void (*func)(void *, struct circularRefChecker *,
                                     struct circularRefChecker *, bool)) {
  ...
  if (!pTokenChecker) {
    ...
    DEFAULT_TRASH_BOX(pTokenChecker,
                      (TrashBoxDestroyItemProc)circularRefCheckerDestroy);
  }

  if (!pChainChecker) {
    pChainChecker = circularRefCheckerNew();
    DEFAULT_TRASH_BOX(pChainChecker,
                      (TrashBoxDestroyItemProc)circularRefCheckerDestroy);
  }
