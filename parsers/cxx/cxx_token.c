...
void cxxTokenAPIInit(void) {
  g_pTokenPool = objPoolNew(
      CXX_TOKEN_POOL_MAXIMUM_SIZE, (objPoolCreateFunc)createToken,
      (objPoolDeleteFunc)deleteToken, (objPoolClearFunc)clearToken, NULL);
}

void cxxTokenAPINewFile(void) { /* Stub */ }

void cxxTokenAPIDone(void) { objPoolDelete(g_pTokenPool); }

CXXToken *cxxTokenCreate(void) { return objPoolGet(g_pTokenPool); }
...
CXXToken *cxxTokenCreateKeyword(int iLineNumber, MIOPos oFilePosition,
                                CXXKeyword eKeyword) {
