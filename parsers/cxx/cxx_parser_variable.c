...
CXXToken *cxxParserFindFirstPossiblyNestedAndQualifiedIdentifier(
    CXXTokenChain *pChain, CXXTokenChain **pParentChain) {
  CXXToken *pId = cxxTokenChainFirstPossiblyNestedTokenOfType(
      pChain, CXXTokenTypeIdentifier, pParentChain);
...
//
// Notes:
// - Be aware that if this function returns true then the pChain very likely has
// been modified
...
//
bool cxxParserExtractVariableDeclarations(CXXTokenChain *pChain,
                                          unsigned int uFlags) {
  CXX_DEBUG_ENTER();
  ...
  CXX_DEBUG_PRINT("Looking for variable declarations in '%s'",
  ...
  //
  // Strategy:
  //   - verify that the chain starts with an identifier or keyword (always
  //   present)
  //   - run to one of : ; [] () {} = ,
  //   - ensure that the previous token is an identifier (except for special
  //   cases)
  ...
  if (cxxTokenTypeIs(t, CXXTokenTypeKeyword) &&
    ...
    CXX_DEBUG_LEAVE_TEXT(
        "Initial keyword can't appear in a variable declaration");
    return false;
  }
  ...
  while (t) {
    ...
    while (t) {
      if (cxxTokenTypeIsOneOf(
              t, CXXTokenTypeSingleColon | CXXTokenTypeParenthesisChain |
                     CXXTokenTypeSquareParenthesisChain |
                     CXXTokenTypeBracketChain | CXXTokenTypeAssignment |
      ...
      if (cxxTokenTypeIsOneOf(
        ...
        CXX_DEBUG_LEAVE_TEXT(
            ...
            vStringValue(t->pszWord), t->eType);
        return bGotVariable;
      }
    ...
    CXX_DEBUG_PRINT("Found notable token '%s' of type 0x%02x(%s)",
                    vStringValue(t->pszWord), t->eType,
    ...
    case CXXTokenTypeParenthesisChain: {
      ...
      if (
          ...
          ((cxxTokenTypeIs(t->pNext, CXXTokenTypeParenthesisChain) &&
            cxxParserTokenChainLooksLikeFunctionParameterList(t->pNext->pChain,
                                                              NULL)) ||
          ...
          // like void (*A(B))(C);
          ((!pIdentifier->pNext) ||
           (!cxxTokenTypeIs(pIdentifier->pNext,
        ...
        pTokenBefore = pIdentifier->pPrev;
          ...
          (
              ...
              pIdentifier = cxxParserVardefInParenthesis(
                  cxxTokenChainFirst(t->pChain), 0))) {
        CXX_DEBUG_LEAVE_TEXT(
            "Parenthesis seems to surround a variable definition");
        pTokenBefore = t->pPrev;
        t = t->pNext;
      } else {
    ...
    case CXXTokenTypeSingleColon:
      ...
      if (t->pNext &&
          cxxTokenTypeIsOneOf(t->pNext,
                              CXXTokenTypeNumber | CXXTokenTypeIdentifier)) {
        // ok, looks like a bit field
        if (cxxTokenTypeIs(t->pNext, CXXTokenTypeNumber) && t->pNext->pNext &&
            cxxTokenTypeIsOneOf(t->pNext->pNext, CXXTokenTypeComma |
    ...
    case CXXTokenTypeSquareParenthesisChain:
      ...
      if (!cxxTokenTypeIsOneOf(t->pNext, CXXTokenTypeComma |
                                             CXXTokenTypeSemicolon |
                                             CXXTokenTypeAssignment |
                                             CXXTokenTypeBracketChain)) {
        CXX_DEBUG_LEAVE_TEXT(
            "No comma, semicolon, = or {} after [] (\"%s\", %s)",
    ...
    if (!pTokenBefore) {
      ...
      //
      if (cxxTokenTypeIs(t, CXXTokenTypeParenthesisChain) && t->pNext &&
          cxxTokenTypeIs(t->pNext, CXXTokenTypeIdentifier) && t->pNext->pNext &&
          cxxTokenTypeIs(t->pNext->pNext, CXXTokenTypeSemicolon)) {
        CXX_DEBUG_PRINT(
            "Looks like the 'MACRO(whatever) variable;' special case");
        pIdentifier = t->pNext;
        pTokenBefore = t;
        t = t->pNext->pNext;
      } else {
    ...
    while (pTokenBefore->eType == CXXTokenTypeMultipleColons) {
      ...
      if (!pTokenBefore) {
        CXX_DEBUG_LEAVE_TEXT("Identifier preceded by multiple colons "
      ...
      if (cxxTokenTypeIs(pTokenBefore, CXXTokenTypeGreaterThanSign)) {
        CXXToken *pAux =
            cxxTokenChainSkipBackToStartOfTemplateAngleBracket(pTokenBefore);
        if ((!pAux) || (!pAux->pPrev)) {
          CXX_DEBUG_LEAVE_TEXT(
    ...
    if (!bGotVariable) {
      // now pTokenBefore should be part of a type (either the variable type or
      // return type of a function in case of a function pointer)
      if (!cxxTokenTypeIsOneOf(pTokenBefore,
        ...
        if (cxxTokenTypeIs(pTokenBefore, CXXTokenTypeGreaterThanSign)) {
          ...
          if (iLevel != 0) {
            CXX_DEBUG_LEAVE_TEXT("The > token is unbalanced and does not "
          ...
          CXX_DEBUG_LEAVE_TEXT("Token '%s' of type 0x%02x does not seem "
    ...
    if (pScopeStart) {
      // Push the scopes and remove them from the chain so they are not in the
      // way
      while (pScopeStart != pScopeEnd) {
        ...
        CXXToken *pPartEnd = cxxTokenChainNextTokenOfType(
            pScopeStart, CXXTokenTypeMultipleColons);
        CXX_DEBUG_ASSERT(pPartEnd,
                         "We should have found multiple colons here!");
        CXX_DEBUG_ASSERT(pPartEnd->pPrev,
                         "And there should be a previous token too");

        CXXToken *pScopeId =
            cxxTokenChainExtractRange(pScopeStart, pPartEnd->pPrev, 0);
    ...
    if (tag) {
      ...
      while (pPartOfType &&
             cxxTokenTypeIs(pPartOfType, CXXTokenTypeSquareParenthesisChain)) {
        CXXTokenChain *pAuxChain = pPartOfType->pChain;

        if (pAuxChain->iCount > 2) {
          if ((pAuxChain->iCount > 3) ||
              (!cxxTokenTypeIs(cxxTokenChainAt(pAuxChain, 1),
                               CXXTokenTypeNumber))) {
            cxxTokenChainDestroyRange(pAuxChain,
      ...
      CXXToken *pTypeToken =
          cxxTagCheckAndSetTypeField(cxxTokenChainFirst(pChain), t->pPrev);
    ...
    if (!cxxTokenTypeIsOneOf(t, CXXTokenTypeComma | CXXTokenTypeSemicolon |
                                    CXXTokenTypeOpeningBracket)) {
      // look for it, but also check for "<" signs: these usually indicate an
      // uncondensed template. We give up on them as they are too complicated in
      // this context. It's rather unlikely to have multiple declarations with
      // templates after the first one
      t = cxxTokenChainNextTokenOfType(
          t, CXXTokenTypeComma | CXXTokenTypeSemicolon |
      ...
      if (cxxTokenTypeIs(t, CXXTokenTypeSmallerThanSign)) {
        CXX_DEBUG_LEAVE_TEXT(
            "Found '<': probably a template on the right side of declaration");
        return bGotVariable;
      }
    }

    if (cxxTokenTypeIsOneOf(t, CXXTokenTypeSemicolon |
                                   CXXTokenTypeOpeningBracket)) {
...
static bool isConstVolatileOrStar(CXXToken *t, void *data) {
  ...
  if (cxxTokenTypeIs(t, CXXTokenTypeKeyword) &&
      ((t->eKeyword == CXXKeywordCONST) || (t->eKeyword == CXXKeywordVOLATILE)))
    return true;
...
static CXXToken *cxxParserVardefInParenthesis(CXXToken *pToken, int depth) {
  // ( const volatile * foo [] ) <- ended with
  // CXXTokenTypeSquareParenthesisChain ( const volatile * foo ) <- ended with
  // CXXTokenTypeClosingParenthesis
  ...
  if (cxxTokenTypeIs(t, CXXTokenTypeParenthesisChain))
    return cxxParserVardefInParenthesis(cxxTokenChainFirst(t->pChain),
           ...
           ((t->pNext && (cxxTokenTypeIsOneOf(
                             t->pNext, CXXTokenTypeSquareParenthesisChain |
                                           CXXTokenTypeClosingParenthesis))) ||
            (t->pNext && depth > 0)))
    return t;
  return NULL;
