...
bool cxxParserParseUsingClause(void) {
  ...
  CXXToken *pAssignment =
      cxxTokenChainFirstTokenOfType(g_cxx.pTokenChain, CXXTokenTypeAssignment);

  if (pAssignment) {
    CXXToken *pFirst = cxxTokenChainFirst(g_cxx.pTokenChain);

    if (cxxTokenTypeIs(pFirst, CXXTokenTypeIdentifier)) {
      CXX_DEBUG_PRINT("Found using clause '%s' which defines a type",
    ...
    if (g_cxx.pTokenChain->iCount > 0) {
      ...
      if (bUsingNamespace) {
        ...
        CXX_DEBUG_ASSERT(
            t, "Condensation of a non empty chain should produce a token!");

        CXX_DEBUG_PRINT("Found using clause '%s' which extends scope",
        ...
        CXX_DEBUG_PRINT("Found using clause '%s' which imports a name",
                        vStringValue(t->pszWord));
