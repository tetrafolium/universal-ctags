...
bool cxxParserTokenChainLooksLikeConstructorParameterSet(CXXTokenChain *pChain);
...
typedef struct _CXXFunctionSignatureInfo {
  ...
  // The identifier is always contained in the chain pointed by
  // pIdentifierChain.
...
typedef struct _CXXParserState {
  ...
  // Please note that the keywords appearing inside a () subchain are NOT
  // marked.
...
#define cxxParserCurrentLanguageIs(_eLanguage) (g_cxx.eLanguage == _eLanguage)
...
#define cxxParserCurrentLanguageIsC() cxxParserCurrentLanguageIs(CXXLanguageC)
