...
static CXXCharTypeData g_aCharTable[128] = {
    ...
    {CXXCharTypeOperator, 0, 0},
    // 038 (0x26) '&'
    {CXXCharTypeNamedSingleOrRepeatedCharToken, CXXTokenTypeAnd,
    ...
    {CXXCharTypeValidInNumber | CXXCharTypeNamedSingleOrRepeatedCharToken,
     CXXTokenTypeDotOperator, CXXTokenTypeMultipleDots},
    // 047 (0x2f) '/'
    {CXXCharTypeOperator, 0, 0},
    // 048 (0x30) '0'
    ...
    {CXXCharTypePartOfIdentifier | CXXCharTypeDecimalDigit |
     ...
     0, 0},
    // 058 (0x3a) ':'
    {CXXCharTypeNamedSingleOrRepeatedCharToken, CXXTokenTypeSingleColon,
    ...
    {CXXCharTypeNamedSingleCharToken, CXXTokenTypeGreaterThanSign, 0},
    // 063 (0x3f) '?'
    {CXXCharTypeOperator, 0, 0},
    // 064 (0x40) '@'
    {0, 0, 0},
...
static void cxxParserAnalyzeAttributeChain(CXXTokenChain *pChain) {
  ...
  while (pToken) {
    if (cxxTokenTypeIs(pToken, CXXTokenTypeIdentifier)) {
      ...
      if ((strcmp(vStringValue(pToken->pszWord), "always_inline") == 0) ||
      ...
      } else if ((strcmp(vStringValue(pToken->pszWord), "deprecated") == 0) ||
                 (strcmp(vStringValue(pToken->pszWord), "__deprecated__") ==
                  0)) {
...
static bool cxxParserParseNextTokenCondenseAttribute(void) {
  ...
  if (!cxxParserParseAndCondenseCurrentSubchain(
          CXXTokenTypeOpeningParenthesis |
              CXXTokenTypeOpeningSquareParenthesis | CXXTokenTypeOpeningBracket,
          false, false)) {
  ...
  if (pInner) {
    if (pInner->pNext &&
        cxxTokenTypeIs(pInner->pNext, CXXTokenTypeParenthesisChain))
...
// as __attribute__((...)). We move it out of the parser's way as it complicates
// parsing.
//
// Returns false if it finds an EOF. This is an important invariant required by
// cxxParserParseNextToken(), the only caller.
...
static bool cxxParserParseNextTokenCondenseCXX11Attribute(void) {
  ...
  CXX_DEBUG_ASSERT(
      (g_cxx.pToken->pChain->iCount == 3) &&
          cxxTokenTypeIs(cxxTokenChainAt(g_cxx.pToken->pChain, 1),
...
static bool cxxParserParseNextTokenSkipMacroParenthesis(CXXToken **ppChain) {
  ...
  if (!cxxParserParseAndCondenseCurrentSubchain(CXXTokenTypeOpeningParenthesis,
                                                false, false)) {
...
static void
cxxParserParseNextTokenApplyReplacement(const cppMacroInfo *pInfo,
  ...
  if (!pInfo->hasParameterList) {
    CXX_DEBUG_ASSERT(!pParameterChainToken,
  ...
  if (pInfo->hasParameterList && pParameterChainToken &&
      (pParameterChainToken->pChain->iCount >= 3)) {
    // kill parenthesis
    cxxTokenChainDestroyFirst(pParameterChainToken->pChain);
    cxxTokenChainDestroyLast(pParameterChainToken->pChain);

    pParameters = cxxTokenChainSplitOnComma(pParameterChainToken->pChain);

    aParameters =
        (const char **)eMalloc(sizeof(const char *) * pParameters->iCount);
    ...
    CXX_DEBUG_ASSERT(iParameterCount == pParameters->iCount,
                     "Bad number of parameters found");
  }

  vString *pReplacement =
      cppBuildMacroReplacement(pInfo, aParameters, iParameterCount);
  ...
  CXX_DEBUG_PRINT("Applying complex replacement '%s'",
                  vStringValue(pReplacement));
...
void cxxParserUngetCurrentToken(void) {
  ...
  CXX_DEBUG_ASSERT(!g_cxx.pUngetToken,
                   "The parser supports only one unget token at a time.");
  ...
  CXX_DEBUG_ASSERT(g_cxx.pUngetToken == g_cxx.pToken,
      ...
      if (pMacro) {
        ...
        if (pMacro->replacements) {
          ...
          if (iReplacementRecursionCount < 1024) {
            ...
            cxxParserParseNextTokenApplyReplacement(pMacro, pParameterChain);
        ...
        bool bRet = cxxParserParseNextToken();
        CXX_DEBUG_PRINT("Parsed inner token: %s type %d",
                        g_cxx.pToken->pszWord->buffer, g_cxx.pToken->eType);
        iReplacementRecursionCount--;
        return bRet;
      }
    }
