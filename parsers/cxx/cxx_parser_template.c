...
typedef enum _CXXParserParseTemplateAngleBracketsResult {
  ...
  // Succeeded, but the parsing was unbalanced and was terminated by an
  // 'unexpected' condition that detected the end of the template. If this is
  // function has been called from an upper template parsing level, the caller
  // should exit too.
...
static bool cxxTokenIsPresentInTemplateParameters(CXXToken *t) {
  ...
  return ptrArrayHasTest(g_cxx.pTemplateParameters, cxxTokenCompareWord,
...
cxxParserParseTemplateAngleBracketsInternal(bool bCaptureTypeParameters) {
  ...
  //
  // Without parentheses we still try to handle the << and >> shift operator
  // cases:
  // - << is always recognized as shift operator
  // - >> is recognized as shift unless it's non-nested. This is what C++11
  //   spec says and theoretically it should be also pseudo-compatible with
  //   C++03 which treats this case as a syntax error.
  //
  ...
  // around but without proper state (include headers, macro expansion, full
  // type database etc) we simply can't do the same. However, we try to recover
  // if we figure out we (or the programmer?) screwed up.
  //
  //
  ...
  // pslTypeParams list keeps all type parameters introduced in templated
  // prefixes. When deciding whether ">>" is an end marker of template prefixes
  // or a shift operator, this function looks up pslTypeParams list. If this
  // function can find A of "A >>" in the list, we can say ">>" is not an
  // operator.
  ...
  for (;;) {
    ...
    case CXXTokenTypeSmallerThanSign: {
      ...
      for (;;) {
        ...
        CXX_DEBUG_PRINT("< followed by token '%s' of type 0x%02x (%s)",
      ...
      if ((!bFirstFollowedBySpace) && (iSmallerThanCount == 2) &&
          cxxTokenTypeIsOneOf(g_cxx.pToken, CXXTokenTypeIdentifier |
                                                CXXTokenTypeParenthesisChain |
                                                CXXTokenTypeNumber |
                                                CXXTokenTypeKeyword) &&
          // note that pFirstGreaterThan->pPrev is certainly valid here
          cxxTokenTypeIsOneOf(pFirstSmallerThan->pPrev,
    ...
    case CXXTokenTypeGreaterThanSign: {
      ...
      if ((!bFirstFollowedBySpace) && (iGreaterThanCount == 2) &&
          cxxTokenTypeIsOneOf(g_cxx.pToken, CXXTokenTypeIdentifier |
                                                CXXTokenTypeParenthesisChain |
                                                CXXTokenTypeNumber |
                                                CXXTokenTypeKeyword) &&
          // note that pFirstGreaterThan->pPrev is certainly valid here
          cxxTokenTypeIsOneOf(pFirstGreaterThan->pPrev,
                              CXXTokenTypeIdentifier | CXXTokenTypeKeyword) &&
          // int >> ...
      ...
      if (iGreaterThanCount > iMaxGreaterThanCount) {
        ...
        CXX_DEBUG_ASSERT(iGreaterThanCount == (iMaxGreaterThanCount + 1),
                         "Bug");
      ...
      // The other cases can't be handled safely. We expect the user
      // to use parentheses.
      if ((iGreaterThanCount == 1) &&
          cxxTokenTypeIsOneOf(g_cxx.pToken, CXXTokenTypeNumber)) {
      ...
      if (cxxTokenIsNonConstantKeyword(g_cxx.pToken)) {
        ...
        CXX_DEBUG_PRINT("Found '> %s': assuming end of template",
                        vStringValue(g_cxx.pToken->pszWord));
    ...
    case CXXTokenTypeKeyword:
      ...
      if (cxxTokenIsKeyword(g_cxx.pToken, CXXKeywordTEMPLATE)) {
        ...
        if (!cxxTokenTypeIs(g_cxx.pToken, CXXTokenTypeSmallerThanSign)) {
          // aaargh...
          CXX_DEBUG_PRINT("Found unexpected token '%s' of type 0x%02x",
                          vStringValue(g_cxx.pToken->pszWord),
                          g_cxx.pToken->eType);
      ...
      if (bCaptureTypeParameters &&
          cxxKeywordMayBePartOfTypeName(g_cxx.pToken->eKeyword) &&
          g_cxx.pToken->pPrev &&
          cxxTokenTypeIsOneOf(g_cxx.pToken->pPrev, CXXTokenTypeSmallerThanSign |
                                                       CXXTokenTypeComma)) {
        // < typename X
        // , class Y
        // , int z
        ...
        CXX_DEBUG_PRINT("Adding '%s' to template parameter list",
    ...
    case CXXTokenTypeOpeningBracket:
      ...
      break;
    default:
      CXX_DEBUG_ASSERT(false, "Found unexpected token type 0x%02x",
                       g_cxx.pToken->eType);
      CXX_DEBUG_LEAVE_TEXT("Found unexpected token type 0x%02x",
                           g_cxx.pToken->eType);
      return CXXParserParseTemplateAngleBracketsFailed;
      break;
    }
...
bool cxxParserParseTemplateAngleBracketsToSeparateChain(void) {
  ...
  CXX_DEBUG_ASSERT(cxxParserCurrentLanguageIsCPP(),
                   "This should be called only in C++");
...
bool cxxParserParseTemplatePrefix(void) {
  ...
  if (cxxTokenTypeIsOneOf(g_cxx.pToken,
    ...
    CXX_DEBUG_LEAVE_TEXT(
        "Found EOF or semicolon: assuming this is unparseable");
    cxxParserNewStatement();
...
void cxxParserEmitTemplateParameterTags(void) {
  CXX_DEBUG_ASSERT(
      g_cxx.pTemplateTokenChain && (g_cxx.pTemplateTokenChain->iCount > 0) &&
          ...
          g_cxx.pTemplateParameters && // this should be ensured by chain
                                       // existence
  ...
  for (int i = 0; i < c; i++) {
    ...
    CXX_DEBUG_PRINT("Emitting template param tag for '%s'",
                    vStringValue(t->pszWord));

    tagEntryInfo *tag = cxxTagBegin(CXXTagCPPKindTEMPLATEPARAM, t);
