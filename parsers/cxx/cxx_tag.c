...
#define CXX_COMMON_KINDS(_langPrefix, _szMemberDescription, _syncWith)         \
  {true,                                                                       \
   ...
   "macro definitions",                                                        \
   .referenceOnly = false,                                                     \
   ATTACH_ROLES(_langPrefix##MacroRoles),                                      \
   .syncWith = _syncWith},                                                     \
      ...
      {true,                                                                   \
       ...
       .referenceOnly = true,                                                  \
       ATTACH_ROLES(_langPrefix##HeaderRoles),                                 \
       .syncWith = _syncWith},                                                 \
    ...
    false, 'D', "macroparam", "parameters inside macro definitions",           \
        .syncWith = _syncWith                                                  \
  }
...
static fieldDefinition g_aCXXCFields[] = {CXX_COMMON_FIELDS};
...
static fieldDefinition g_aCXXCPPFields[] = {
    ...
    {.name = "captures",
     ...
     .enabled = false},
    {.name = "name", .description = "aliased names", .enabled = true}};

static fieldDefinition g_aCXXCUDAFields[] = {CXX_COMMON_FIELDS};
...
void cxxTagInitForLanguage(langType eLangType) {
  ...
  if (g_cxx.eLangType == g_cxx.eCLangType) {
    ...
    g_cxx.uKindDefinitionCount =
        sizeof(g_aCXXCPPKinds) / sizeof(kindDefinition);
    ...
    g_cxx.uKindDefinitionCount =
        sizeof(g_aCXXCUDAKinds) / sizeof(kindDefinition);
    g_cxx.pFieldOptions = g_aCXXCUDAFields;
    g_cxx.uFieldOptionCount =
        sizeof(g_aCXXCUDAFields) / sizeof(fieldDefinition);
  } else {
    CXX_DEBUG_ASSERT(false,
                     "Invalid language passed to cxxTagInitForLanguage()");
  }
}

kindDefinition *cxxTagGetCKindDefinitions(void) { return g_aCXXCKinds; }

int cxxTagGetCKindDefinitionCount(void) {
  return sizeof(g_aCXXCKinds) / sizeof(kindDefinition);
}

kindDefinition *cxxTagGetCUDAKindDefinitions(void) { return g_aCXXCUDAKinds; }

int cxxTagGetCUDAKindDefinitionCount(void) {
  return sizeof(g_aCXXCUDAKinds) / sizeof(kindDefinition);
}

kindDefinition *cxxTagGetCPPKindDefinitions(void) { return g_aCXXCPPKinds; }
...
bool cxxTagKindEnabled(unsigned int uKind) {
  CXX_DEBUG_ASSERT(uKind < g_cxx.uKindDefinitionCount,
...
fieldDefinition *cxxTagGetCFieldDefinitionifiers(void) { return g_aCXXCFields; }
...
bool cxxTagFieldEnabled(unsigned int uField) {
  CXX_DEBUG_ASSERT(uField < g_cxx.uFieldOptionCount,
...
tagEntryInfo *cxxTagBegin(unsigned int uKind, CXXToken *pToken) {
  ...
  initTagEntry(&g_oCXXTag, vStringValue(pToken->pszWord), uKind);
...
static bool cxxTagCheckTypeField(CXXToken *pTypeStart, CXXToken *pTypeEnd) {
  CXX_DEBUG_ENTER();
    ...
    if (cxxTokenTypeIs(pTypeStart, CXXTokenTypeIdentifier)) {
      ...
      if (iConsecutiveIdentifiers > 4) {
        ...
        CXX_DEBUG_LEAVE_TEXT(
            "Too many consecutive identifiers for a type name");
        return false;
      }
    } else {
  ...
  if (iIdentifierOrKeywordCount < 1) {
    CXX_DEBUG_LEAVE_TEXT("Type does not seem to contains identifiers or "
                         "keywords, can't be a type name");
    return false;
  }
...
CXXToken *cxxTagCheckAndSetTypeField(CXXToken *pTypeStart, CXXToken *pTypeEnd) {
  ...
  CXXToken *pTypeName =
      cxxTokenChainExtractRangeFilterTypeName(pTypeStart, pTypeEnd);
...
void cxxTagSetCorkQueueField(int iIndex, unsigned int uField,
                             const char *szValue) {
  CXX_DEBUG_ASSERT(uField < g_cxx.uFieldOptionCount,
                   "The field must be associated to the current language!");

  CXX_DEBUG_ASSERT(g_cxx.pFieldOptions[uField].enabled,
                   "The field must be enabled!");

  attachParserFieldToCorkEntry(iIndex, g_cxx.pFieldOptions[uField].ftype,
                               szValue);
}
...
void cxxTagHandleTemplateField() {
  ...
  if (t) {
    cxxTagSetField(CXXTagCPPFieldTemplate, vStringValue(t->pszWord));
...
int cxxTagCommit(void) {
  ...
  CXX_DEBUG_PRINT("Emitting tag for symbol '%s', kind '%s', line %d",
                  g_oCXXTag.name,
                  getLanguageKindName(g_oCXXTag.langType, g_oCXXTag.kindIndex),
                  g_oCXXTag.lineNumber);
  if (g_oCXXTag.extensionFields.typeRef[0] &&
      g_oCXXTag.extensionFields.typeRef[1])
    CXX_DEBUG_PRINT("Tag has typeref %s %s",
  ...
  if (eScopeType == CXXScopeTypeEnum) {
    ...
    CXX_DEBUG_ASSERT(x,
                     "Scope with size >= 2 should have returned a value here");
  } else {
  ...
  CXX_DEBUG_PRINT("Emitting extra tag for symbol '%s', kind '%s', line %d",
                  g_oCXXTag.name,
                  getLanguageKindName(g_oCXXTag.langType, g_oCXXTag.kindIndex),
                  g_oCXXTag.lineNumber);
