...
bool cxxTokenChainTakeRecursive(CXXTokenChain *tc, CXXToken *t) {
  ...
  while (aux) {
    ...
    if (cxxTokenTypeIsOneOf(aux, CXXTokenTypeParenthesisChain |
...
void cxxTokenChainInsertAfter(CXXTokenChain *tc, CXXToken *before,
                              CXXToken *t) {
  if (!before) {
...
void cxxTokenChainJoinRangeInString(CXXToken *from, CXXToken *to, vString *s,
...
vString *cxxTokenChainJoinRange(CXXToken *from, CXXToken *to,
                                const char *szSeparator, unsigned int uFlags) {
  if (!from)
    return NULL;
...
void cxxTokenChainJoinInString(CXXTokenChain *tc, vString *s,
                               const char *szSeparator, unsigned int uFlags) {
  if (!tc)
    return;
...
vString *cxxTokenChainJoin(CXXTokenChain *tc, const char *szSeparator,
                           unsigned int uFlags) {
  if (!tc)
    return NULL;
...
void cxxTokenChainMoveEntryRange(CXXTokenChain *src, CXXToken *start,
                                 CXXToken *end, CXXTokenChain *dest) {
  if (!src || !dest || !start || !end)
    return;

  CXX_DEBUG_ASSERT(cxxTokenChainFindToken(src, start) >= 0,
                   "The start token must be in the source chain!");
  CXX_DEBUG_ASSERT(cxxTokenChainFindToken(src, end) >= 0,
                   "The end token must be in the source chain!");
  CXX_DEBUG_ASSERT(cxxTokenChainFindToken(src, start) <=
                       cxxTokenChainFindToken(src, end),
...
CXXTokenChain *cxxTokenChainSplitOnComma(CXXTokenChain *tc) {
  ...
  if (pStart) {
    ...
    CXXToken *pNew =
        cxxTokenChainExtractRange(pStart, cxxTokenChainLast(tc), 0);
    if (pNew)
      cxxTokenChainAppend(pRet, pNew);
  }
...
CXXToken *cxxTokenChainCondenseIntoToken(CXXTokenChain *tc,
                                         unsigned int uFlags) {
  if (!tc)
    return NULL;
  ...
  while (t) {
    ...
    if ((!(uFlags & CXXTokenChainCondenseNoTrailingSpaces)) &&
        t->bFollowedBySpace)
...
CXXToken *cxxTokenChainSkipToEndOfTemplateAngleBracket(CXXToken *t) {
  ...
  CXX_DEBUG_ASSERT(cxxTokenTypeIs(t, CXXTokenTypeSmallerThanSign),
...
CXXToken *cxxTokenChainSkipBackToStartOfTemplateAngleBracket(CXXToken *t) {
  if (!t)
    return NULL;
  CXX_DEBUG_ASSERT(t->eType == CXXTokenTypeGreaterThanSign,
                   "This function must be called when pointing to a >");
  int iLevel = 1;
  t = t->pPrev;
  while (t) {
...
CXXToken *cxxTokenChainFirstTokenOfType(CXXTokenChain *tc,
...
CXXToken *cxxTokenChainPreviousTokenOfType(CXXToken *t,
                                           unsigned int uTokenTypes) {
  if (!t)
    return NULL;
  t = t->pPrev;
...
CXXToken *cxxTokenChainPreviousTokenNotOfType(CXXToken *t,
                                              unsigned int uTokenTypes) {
  if (!t)
    return NULL;
  t = t->pPrev;
...
CXXToken *cxxTokenChainLastTokenOfType(CXXTokenChain *tc,
                                       unsigned int uTokenTypes) {
  if (!tc)
    return NULL;
  CXXToken *t = tc->pTail;
...
CXXToken *
cxxTokenChainLastPossiblyNestedTokenOfType(CXXTokenChain *tc,
                                           unsigned int uTokenTypes,
  ...
  while (t) {
    ...
    if (t->eType == CXXTokenTypeParenthesisChain) {
      CXXToken *tmp = cxxTokenChainLastPossiblyNestedTokenOfType(
          t->pChain, uTokenTypes, ppParentChain);
      if (tmp)
        return tmp;
    }
    t = t->pPrev;
...
CXXToken *
cxxTokenChainFirstPossiblyNestedTokenOfType(CXXTokenChain *tc,
                                            unsigned int uTokenTypes,
                                            CXXTokenChain **ppParentChain) {
  if (!tc)
    return NULL;
...
CXXToken *cxxTokenChainFirstTokenNotOfType(CXXTokenChain *tc,
                                           unsigned int uTokenTypes) {
  if (!tc)
    return NULL;
  CXXToken *t = tc->pHead;
...
CXXToken *cxxTokenChainNextTokenNotOfGeneric(
    CXXToken *t, bool (*predicator)(CXXToken *, void *), void *data) {
  if (!t)
    return NULL;
  t = t->pNext;
  while (t) {
...
CXXToken *cxxTokenChainNextTokenNotOfType(CXXToken *t,
                                          unsigned int uTokenTypes) {
  if (!t)
    return NULL;
  t = t->pNext;
...
CXXToken *cxxTokenChainLastTokenNotOfType(CXXTokenChain *tc,
                                          unsigned int uTokenTypes) {
  if (!tc)
    return NULL;
  CXXToken *t = tc->pTail;
...
CXXToken *cxxTokenChainPreviousKeyword(CXXToken *from, CXXKeyword eKeyword) {
  if (!from)
    return NULL;
...
CXXToken *cxxTokenChainNextKeyword(CXXToken *from, CXXKeyword eKeyword) {
  if (!from)
    return NULL;
...
int cxxTokenChainFirstKeywordIndex(CXXTokenChain *tc, CXXKeyword eKeyword) {
  if (!tc)
    return -1;
  if (tc->iCount < 1)
    return -1;
...
CXXToken *cxxTokenChainNextIdentifier(CXXToken *from,
...
void cxxTokenChainDestroyRange(CXXTokenChain *pChain, CXXToken *from,
                               CXXToken *to) {
  if (!from || !to)
    return;
  CXX_DEBUG_ASSERT(from,
                   "Bad from pointer passed to cxxTokenChainDestroyRange");
...
CXXToken *cxxTokenChainExtractRange(CXXToken *from, CXXToken *to,
                                    unsigned int uFlags) {
  if (!from)
    return NULL;
  ...
  if ((!(uFlags & CXXTokenChainExtractRangeNoTrailingSpaces)) &&
...
CXXToken *cxxTokenChainExtractRangeFilterTypeName(CXXToken *from,
                                                  CXXToken *to) {
  if (!from)
    return NULL;
...
CXXToken *cxxTokenChainExtractIndexRange(CXXTokenChain *tc, int iFirstIndex,
                                         int iLastIndex, unsigned int uFlags) {
  if (!tc)
    return NULL;
  if (iFirstIndex < 0)
    return NULL;
...
void cxxTokenChainNormalizeTypeNameSpacingInRange(CXXToken *pFrom,
                                                  CXXToken *pTo) {
  if (!pFrom || !pTo)
    return;
  ...
  for (;;) {
    if (cxxTokenTypeIsOneOf(t, CXXTokenTypeParenthesisChain |
                                   CXXTokenTypeSquareParenthesisChain)) {
      cxxTokenChainNormalizeTypeNameSpacing(t->pChain);
      t->bFollowedBySpace = false;
    } else if (cxxTokenTypeIsOneOf(
                   t, CXXTokenTypeIdentifier | CXXTokenTypeKeyword |
                          CXXTokenTypeGreaterThanSign | CXXTokenTypeAnd |
                          CXXTokenTypeMultipleAnds)) {
      t->bFollowedBySpace =
          t->pNext &&
          cxxTokenTypeIsOneOf(
              t->pNext, CXXTokenTypeParenthesisChain | CXXTokenTypeIdentifier |
                            CXXTokenTypeKeyword | CXXTokenTypeStar |
                            CXXTokenTypeAnd | CXXTokenTypeMultipleAnds);
    } else if (cxxTokenTypeIs(t, CXXTokenTypeStar)) {
      t->bFollowedBySpace =
          t->pNext &&
          (!cxxTokenTypeIsOneOf(t->pNext, CXXTokenTypeStar | CXXTokenTypeComma |
