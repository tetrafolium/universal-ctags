...
static void
dbusIntrospectFindTagsUnderMain(xmlNode *node, const char *xpath,
                                const struct sTagXpathRecurSpec *spec,
                                xmlXPathContext *ctx, void *userData);
static void makeTagForMainName(xmlNode *node, const char *xpath,
...
static void makeTagWithScope(xmlNode *node, const char *xpath,
                             const struct sTagXpathMakeTagSpec *spec,
                             struct sTagEntryInfo *tag, void *userData);
static int decideKindForMainName(xmlNode *node, const char *xpath,
                                 const struct sTagXpathMakeTagSpec *spec,
                                 void *userData);
...
static tagXpathTable dbusIntrospectXpathArgTable[] = {
    {
        ...
        {.recurSpec = {dbusIntrospectFindTagsUnderMain, -1}},
    },
};
...
static tagXpathTable dbusIntrospectXpathInterfaceTable[] = {
    {"method",
     ...
     {.recurSpec =
          {
              dbusIntrospectFindTagsUnderMain,
              TABLE_ARG,
          }}},
    {"signal",
     LXPATH_TABLE_DO_RECUR,
...
static tagXpathTable dbusIntrospectXpathMainNameTable[] = {
    {"@name",
     ...
     {.makeTagSpec = {KIND_GHOST_INDEX, ROLE_DEFINITION_INDEX,
                      makeTagForMainName, decideKindForMainName}}},
};
...
static void
makeTagForMainName(xmlNode *node CTAGS_ATTR_UNUSED,
                   const char *xpath CTAGS_ATTR_UNUSED,
                   const struct sTagXpathMakeTagSpec *spec CTAGS_ATTR_UNUSED,
                   struct sTagEntryInfo *tag, void *userData) {
  struct dbusIntrospectData *data = userData;
...
static int decideKindForMainName(
    xmlNode *node CTAGS_ATTR_UNUSED, const char *xpath CTAGS_ATTR_UNUSED,
    const struct sTagXpathMakeTagSpec *spec CTAGS_ATTR_UNUSED, void *userData) {
  return ((struct dbusIntrospectData *)userData)->kindForName;
}

static void findDbusIntrospectTags(void) {
...
extern parserDefinition *DbusIntrospectParser(void) {
  ...
  static xpathFileSpec xpathFileSpecs[] = {
      ...
      /* TODO: the following rule is too strong; parsers may conflicts each
       * other when we implement more xpath based parsers. */
      {
