...
#define isIdentChar(c) (isalnum(c) || (c) == '+' || (c) == '-' || (c) == '.')
...
typedef enum { KEYWORD_true, KEYWORD_false, KEYWORD_null } keywordId;

static langType Lang_json;

static kindDefinition JsonKinds[] = {
    {true, 'o', "object", "objects"},   {true, 'a', "array", "arrays"},
    {true, 'n', "number", "numbers"},   {true, 's', "string", "strings"},
    {true, 'b', "boolean", "booleans"}, {true, 'z', "null", "nulls"}};
...
static void pushScope(tokenInfo *const token, const tokenInfo *const parent,
...
static void skipToOneOf3(tokenInfo *const token, const tokenType type1,
                         const tokenType type2, const tokenType type3) {
  while (token->type != TOKEN_EOF && token->type != type1 &&
         token->type != type2 && token->type != type3) {
    readToken(token);
...
static void parseValue(tokenInfo *const token) {
  if (token->type == TOKEN_OPEN_CURLY) {
    ...
    readToken(token);
    while (token->type != TOKEN_EOF && token->type != TOKEN_CLOSE_SQUARE) {
      jsonKind tagKind;
...
static void initialize(const langType language) { Lang_json = language; }
