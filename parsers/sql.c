...
#define isReservedWord(token)                                                  \
  (SqlReservedWord[(token)->keyword].fn                                        \
...
static struct SqlReservedWord SqlReservedWord[SQLKEYWORD_COUNT] = {
    /*
     * RESERVED_BIT: MYSQL & POSTGRESQL&SQL2016&SQL2011&SQL92 & ORACLE11g&PLSQL
     * & SQLANYWERE
     */
...
static bool isOneOfKeyword(tokenInfo *const token,
...
static void findTokenOrKeywords(tokenInfo *const token, const tokenType type,
  ...
  while (!isType(token, type) &&
         !(isType(token, TOKEN_KEYWORD) &&
           isOneOfKeyword(token, keywords, kcount)) &&
...
static void parseSubProgram(tokenInfo *const token) {
  ...
  if (isCmdTerm(token)) {
    ...
    while (!isKeyword(token, KEYWORD_is) && !isKeyword(token, KEYWORD_begin) &&
           !isKeyword(token, KEYWORD_at) &&
           !isKeyword(token, KEYWORD_internal) &&
           !isKeyword(token, KEYWORD_external) &&
           !isKeyword(token, KEYWORD_url) && !isType(token, TOKEN_EQUAL) &&
           !isType(token, TOKEN_EOF) && !isCmdTerm(token)) {
...
static void parseRecord(tokenInfo *const token) {
  ...
  do {
    ...
    if (!isKeyword(token, KEYWORD_primary) &&
      ...
      if (isType(token, TOKEN_IDENTIFIER) || isType(token, TOKEN_STRING) ||
          (isType(token, TOKEN_KEYWORD) && (!isReservedWord(token)))) {
        makeSqlTag(token, SQLTAG_FIELD);
      }
    }

    while (!isType(token, TOKEN_COMMA) && !isType(token, TOKEN_CLOSE_PAREN) &&
           !isType(token, TOKEN_OPEN_PAREN) && !isType(token, TOKEN_EOF)) {
      readToken(token);
      /*
  ...
  } while (!isType(token, TOKEN_CLOSE_PAREN) && !isType(token, TOKEN_EOF));
}
  ...
  do {
    ...
    if (isType(token, TOKEN_BLOCK_LABEL_BEGIN))
      ...
      case KEYWORD_when:
         ...
         */
        while (!isKeyword(token, KEYWORD_then) && !isType(token, TOKEN_EOF))
          readToken(token);
      ...
      case KEYWORD_if:
        ...
        while (!isKeyword(token, KEYWORD_then) &&
               !isKeyword(token, KEYWORD_begin) && !isType(token, TOKEN_EOF)) {
          readToken(token);
        }
      ...
      case KEYWORD_for:
        ...
        if (isKeyword(token, KEYWORD_for)) {
          ...
          while (!isKeyword(token, KEYWORD_is) && !isType(token, TOKEN_EOF)) {
            /*
          ...
          while (!isKeyword(token, KEYWORD_do) && !isType(token, TOKEN_EOF))
            readToken(token);
        }
         ...
         *    END FOR [loop name];
         */
        if (isKeyword(token, KEYWORD_loop) || isKeyword(token, KEYWORD_case) ||
       ...
       */
      while (!stmtTerm && !isKeyword(token, KEYWORD_end) && !isCmdTerm(token) &&
             !isType(token, TOKEN_EOF)) {
    ...
    if (isCmdTerm(token) && !stmtTerm)
      stmtTerm = true;
           ...
           !isType(token, TOKEN_EOF) && !stmtTerm);
}
...
static void parseColumnsAndAliases(tokenInfo *const token) {
  ...
  readToken(token);
  do {
    if (isType(token, TOKEN_KEYWORD) && isKeyword(token, KEYWORD_is)) {
      readToken(token);
      if (isType(token, TOKEN_IDENTIFIER)) {
        /* Emit the alias */
        makeSqlTag(token, SQLTAG_FIELD);
...
static void parseTable(tokenInfo *const token) {
  ...
  if (isType(token, TOKEN_OPEN_PAREN)) {
    if (isType(name, TOKEN_IDENTIFIER) || isType(name, TOKEN_STRING) ||
        (isType(name, TOKEN_KEYWORD) && (!isReservedWord(name)))) {
      makeSqlTag(name, SQLTAG_TABLE);
      emitted = true;
...
static void parseTrigger(tokenInfo *const token) {
  ...
  while (!isKeyword(token, KEYWORD_on) && !isType(token, TOKEN_EOF) &&
         !isCmdTerm(token)) {
    readToken(token);
  }
...
static void parseKeywords(tokenInfo *const token) {
  ...
  case KEYWORD_with:
    readToken(token);
    break; /* skip next token */
  case KEYWORD_without:
    readToken(token);
    break; /* skip next token */
  default:
    break;
