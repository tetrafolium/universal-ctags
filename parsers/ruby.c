...
static bool notIdentChar(int c) { return !isIdentChar(c); }
...
static bool notOperatorChar(int c) { return !isOperatorChar(c); }

static bool isSigilChar(int c) { return (c == '@' || c == '$'); }

static bool isWhitespace(int c) { return c == 0 || isspace(c); }
 ...
 */
static bool canMatchKeywordWithAssign(const unsigned char **s,
                                      const char *literal) {
  const unsigned char *original_pos = *s;
...
static void emitRubyTag(vString *name, rubyKind kind) {
  ...
  if (unqualified_name && unqualified_name[1]) {
    if (unqualified_name > qualified_name) {
      ...
      vStringNCatS(scope, qualified_name, unqualified_name - qualified_name);
  ...
  if (vStringLength(scope) > 0) {
    Assert(0 <= parent_kind && (size_t)parent_kind < (ARRAY_SIZE(RubyKinds)));
...
static rubyKind parseIdentifier(const unsigned char **cp, vString *name,
  ...
  while (**cp != 0 &&
         (**cp == ':' || isIdentChar(**cp) || charIsIn(**cp, also_ok))) {
    char last_char = **cp;
