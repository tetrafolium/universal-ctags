...
typedef struct sConditionalInfo {
  ...
  int enterExternalParserBlockNestLevel; /* the parser state when entering this
                                            conditional: used only by cxx */
} conditionalInfo;
...
typedef struct sCppState {
  ...
  int *ungetPointer;   /* the current unget char: points in the middle of the
                          buffer */
...
static kindDefinition CPreProKinds[] = {
    {true, 'd', "macro", "macro definitions", .referenceOnly = false,
     ATTACH_ROLES(CPREPROMacroRoles)},
    {true, 'h', "header", "included header files", .referenceOnly = true,
     ATTACH_ROLES(CPREPROHeaderRoles)},
    {
        false,
        'D',
        "parameter",
...
typedef enum { F_MACRODEF, COUNT_FIELD } cPreProField;

static fieldDefinition CPreProFields[COUNT_FIELD] = {
    {.name = "macrodef", .description = "macro definition", .enabled = false},
};
...
void cppPushExternalParserBlock(void) { externalParserBlockNestLevel++; }

void cppPopExternalParserBlock(void) { externalParserBlockNestLevel--; }
...
extern bool cppIsBraceFormat(void) { return BraceFormat; }

extern unsigned int cppGetDirectiveNestLevel(void) {
  return Cpp.directive.nestLevel;
}

static void cppInitCommon(langType clientLang, const bool state,
                          const bool hasAtLiteralStrings,
                          const bool hasCxxRawLiteralStrings,
                          const bool hasSingleQuoteLiteralNumbers,
                          int defineMacroKindIndex, int macroUndefRoleIndex,
                          int macroParamKindIndex, int headerKindIndex,
  ...
  CXX_DEBUG_ASSERT(!Cpp.charOrStringContents,
                   "This string should be null when CPP is not initialized");
...
extern void cppInit(const bool state, const bool hasAtLiteralStrings,
  ...
  cppInitCommon(client, state, hasAtLiteralStrings, hasCxxRawLiteralStrings,
                hasSingleQuoteLiteralNumbers, defineMacroKindIndex,
                macroUndefRoleIndex, macroParamKindIndex, headerKindIndex,
...
extern void cppEndStatement(void) { Cpp.resolveRequired = false; }
...
void cppUngetString(const char *string, int len) {
  ...
  if (!Cpp.ungetPointer) {
    ...
    if (!Cpp.ungetBuffer) {
      ...
      Cpp.ungetBufferSize = 8 + len;
      Cpp.ungetBuffer =
          (int *)eRealloc(Cpp.ungetBuffer, Cpp.ungetBufferSize * sizeof(int));
    }
...
static void readFilename(int c, vString *const name) {
  ...
  while (c = cppGetcFromUngetBufferOrFile(),
         (c != EOF && c != c_end && c != '\n'))
    vStringPut(name, c);
}
...
static bool isIgnoreBranch(void) {
  ...
  /*
   * CXX: Force a single branch if the external parser (cxx) block nest level at
   * the beginning of this conditional is not equal to the current block nest
   * level (at exit of the first branch).
   *
  ...
  if ((Cpp.resolveRequired || (ifdef->enterExternalParserBlockNestLevel !=
...
static int makeDefineTag(const char *const name, const char *const signature,
                         bool undef) {
  ...
  if (!isLanguageKindEnabled(lang, Cpp.defineMacroKindIndex))
    return CORK_NIL;

  if (
      /* condition for definition tag */
      (!undef) || /* condition for reference tag */
...
static void makeIncludeTag(const char *const name, bool systemHeader) {
  ...
  role_index =
      systemHeader ? Cpp.headerSystemRoleIndex : Cpp.headerLocalRoleIndex;
...
static void regenreateSignatureFromParameters(vString *buffer, int from,
                                              int to) {
  vStringPut(buffer, '(');
...
static int directiveDefine(const int c, bool undef) {
  ...
  //        However we'd need a separate hash table for macros of the current
  //        file to avoid breaking the "global" ones.
    ...
    if (!isIgnore()) {
      ...
      if (p == '(') {
        ...
        if (p == ')') {
          ...
          r = makeDefineTag(vStringValue(Cpp.directive.name),
                            vStringValue(signature), undef);
...
static bool directiveIf(const int c) {
  DebugStatement(const bool ignore0 = isIgnore();) const bool ignore =
      pushConditional((bool)(c != '0'));
...
const vString *cppGetLastCharOrStringContents(void) {
  CXX_DEBUG_ASSERT(Cpp.charOrStringContents,
                   "Shouldn't be called when CPP is not initialized");
  ...
  if (c != '(' && !isCxxRawLiteralDelimiterChar(c)) {
    ...
    do {
      if (collectDelim) {
        ...
        while ((c = cppGetcFromUngetBufferOrFile()) != EOF && i < delimLen &&
               delim[i] == c)
          i++;
...
static void attachFields(int macroCorkIndex, unsigned long endLine,
  ...
  if (macrodef)
    attachParserFieldToCorkEntry(macroCorkIndex, Cpp.macrodefFieldIndex,
                                 macrodef);
}
  ...
  process:
    ...
    case EOF:
      ...
      if (macroCorkIndex != CORK_NIL) {
        attachFields(macroCorkIndex, getInputLineNumber(),
    ...
    case '<': {
      /*
         Quoted from
         http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2011/n3237.html:
         ------
    ...
    default:
      if (c == '@' && Cpp.hasAtLiteralStrings) {
         ...
         *
         * We need to know whether the previous character was an identifier or
         * not, because "R" has to be on its own, not part of an identifier.
    ...
    enter:
      ...
      if (directive) {
        ...
        if (Cpp.macrodefFieldIndex != FIELD_UNKNOWN &&
            macroCorkIndex != CORK_NIL && macrodef == NULL)
          macrodef = vStringNew();
      }
      break;
    }
...
static void findCppTags(void) {
  cppInitCommon(Cpp.lang, 0, false, false, false, KIND_GHOST_INDEX, 0,
                KIND_GHOST_INDEX, KIND_GHOST_INDEX, 0, 0, FIELD_UNKNOWN);
...
extern vString *
cppBuildMacroReplacement(const cppMacroInfo *macro,
...
static void saveIgnoreToken(const char *ignoreToken) {
  ...
  if (replacement) {
    cppMacroReplacementPartInfo *rep = (cppMacroReplacementPartInfo *)eMalloc(
        sizeof(cppMacroReplacementPartInfo));
  ...
  hashTablePutItem(defineMacroTable,
                   eStrndup(tokenBegin, tokenEnd - tokenBegin), info);
...
static void saveMacro(const char *macro) {
  ...
  CXX_DEBUG_PRINT("Macro identifier '%.*s'", identifierEnd - identifierBegin,
                  identifierBegin);
  ...
  if (*c == '(') {
    ...
    while (*c) {
      ...
      if (*c && (*c != ',') && (*c != ')')) {
        ...
        CXX_DEBUG_PRINT("Macro parameter %d '%.*s'", iParamCount,
...
#define ADD_CONSTANT_REPLACEMENT_NEW_PART(start, len)                          \
  do {                                                                         \
    ...
    rep->next = NULL;                                                          \
    CXX_DEBUG_PRINT("Constant replacement part: '%s'",                         \
                    vStringValue(rep->constant));                              \
...
#define ADD_CONSTANT_REPLACEMENT(start, len)                                   \
  do {                                                                         \
    if (lastReplacement && (lastReplacement->parameterIndex == -1)) {          \
      ...
      CXX_DEBUG_PRINT("Constant replacement part changed: '%s'",               \
      ...
      if (isalpha(*c) || (*c == '_')) {
        ...
        for (; i < iParamCount; i++) {
          ...
          if ((bIsVarArg && (paramLen == 3) &&
            ...
            CXX_DEBUG_PRINT("Parameter replacement part: %d (vararg %d)", i,
                            bIsVarArg);
      ...
      if (*c == '#') {
        ...
        if (*c == '#') {
          ...
          if (lastReplacement && (lastReplacement->parameterIndex == -1)) {
            ...
            CXX_DEBUG_PRINT("Last replacement truncated to '%s'",
                            vStringValue(lastReplacement->constant));
          }
        } else {
          // stringification
  ...
  hashTablePutItem(defineMacroTable,
                   eStrndup(identifierBegin, identifierEnd - identifierBegin),
                   info);
  CXX_DEBUG_LEAVE();
}
...
static hashTable *makeMacroTable(void) {
  return hashTableNew(1024, hashCstrhash, hashCstreq, free,
...
static void CpreProInstallIgnoreToken(const langType language CTAGS_ATTR_UNUSED,
...
static void CpreProInstallMacroToken(const langType language CTAGS_ATTR_UNUSED,
                                     const char *optname CTAGS_ATTR_UNUSED,
                                     const char *arg) {
  if (arg == NULL || arg[0] == '\0') {
    hashTableDelete(defineMacroTable);
...
static void CpreProSetIf0(const langType language CTAGS_ATTR_UNUSED,
                          const char *name, const char *arg) {
  doesExaminCodeWithInIf0Branch =
      paramParserBool(arg, doesExaminCodeWithInIf0Branch, name, "parameter");
}
