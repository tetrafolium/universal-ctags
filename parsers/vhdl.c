...
static bool isIdentifierMatch(const tokenInfo *const token,
  ...
  return (bool)(isType(token, TOKEN_IDENTIFIER) &&
                strcasecmp(vStringValue(token->string), vStringValue(name)) ==
                    0);
...
static bool isKeywordOrIdent(const tokenInfo *const token,
                             ...
                             const vString *const name) {
  return (bool)(isKeyword(token, keyword) || isIdentifierMatch(token, name));
}
...
getNextChar:
  ...
  default:
    if (!isIdentChar1(c))
      ...
      token->filePosition = getInputFilePosition();
      token->keyword =
          lookupCaseKeyword(vStringValue(token->string), Lang_vhdl);
...
static void initialize(const langType language) { Lang_vhdl = language; }
