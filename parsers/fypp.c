...
static bool fypp_start_cb(const char *line, const regexMatch *matches,
                          unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;

  TRACE_PRINT_PREFIX();
  TRACE_PRINT_FMT("%04d - %s", getInputLineNumber(), line);

  ulongArrayAdd(ctx->fypp_lines, getInputLineNumber());
  ctx->in_fypp_area++;
  ctx->if_cont = false;
...
static bool fypp_end_cb(const char *line, const regexMatch *matches,
                        unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
...
static bool fypp_line_cb(const char *line, const regexMatch *matches,
                         unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
...
static bool macro_start_cb(const char *line, const regexMatch *matches,
                           unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
  vString *macro = NULL;
  ...
  if (count > 0) {
    ...
    macro = vStringNew();
    vStringNCopyS(macro, line + matches[1].start, matches[1].length);
...
static bool macro_end_cb(const char *line, const regexMatch *matches,
                         unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
...
static bool if_start_cb(const char *line, const regexMatch *matches,
                        unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
  ...
  TRACE_PRINT("(count: %d, len: %d, ifCont: %d)", count, matches[1].length,
              ctx->if_cont);

  return true;
}

static bool if_else_cb(const char *line, const regexMatch *matches,
                       unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
...
static bool if_end_cb(const char *line, const regexMatch *matches,
                      unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;
...
static bool non_fypp_line_cb(const char *line, const regexMatch *matches,
                             unsigned int count, void *userData) {
  struct fyppParseCtx *ctx = userData;

  if ((ctx->in_fypp_area > 0 &&
       ((!ctx->if_tracker) || (!is_in_first_if_block(ctx)) || ctx->if_cont)) ||
      ctx->other_cont) {
    ulongArrayAdd(ctx->fypp_lines, getInputLineNumber());
    ...
    TRACE_PRINT_FMT("%04d - %s", getInputLineNumber(), line);
    TRACE_PRINT_PREFIX();
    TRACE_PRINT_FMT("(inFyppArea: %d, ifTracker: %p, inFirstIfBlock: %d, "
                    "ifCont: %d otherCont: %d/ ",
                    ctx->in_fypp_area, ctx->if_tracker,
                    ctx->if_tracker ? is_in_first_if_block(ctx) : -1,
                    ctx->if_cont, ctx->other_cont);
#ifdef DO_TRACING
    if (ctx->if_tracker) {
    ...
    TRACE_PRINT_FMT("(inFyppArea: %d, ifTracker: %p, ifCont: %d / ",
                    ctx->in_fypp_area, ctx->if_tracker, ctx->if_cont);
    TRACE_PRINT_FMT(")");
    TRACE_PRINT_NEWLINE();
  }
...
static void findFyppTags(void) {
  ...
  if (fyppGuestParser) {
    promise = makePromise(vStringValue(fyppGuestParser), 1, 0,
                          getInputLineNumber(), 0, 0);
    if (promise >= 0)
...
static void initializeFyppParser(langType language) {
  ...
  addLanguageCallbackRegex(
      language, "^[ \t]*#:[ \t]*enddef[ \t]*([a-zA-Z][a-zA-Z0-9_]*)[ \t]*$",
      "{exclusive}", macro_end_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*for[ \t].*$",
                           "{exclusive}", fypp_start_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*endfor.*$", "{exclusive}",
                           fypp_end_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*call[ \t].*$",
                           "{exclusive}", fypp_start_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*endcall.*$", "{exclusive}",
                           fypp_end_cb, NULL, &parseCtx);

  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*if[ \t].*$", "{exclusive}",
                           if_start_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*el(se|if)[ \t].*$",
                           "{exclusive}", if_else_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^[ \t]*#:[ \t]*endif.*", "{exclusive}",
                           if_end_cb, NULL, &parseCtx);

  addLanguageCallbackRegex(language, "^[ \t]*(#!|[#@$]:).*$", "{exclusive}",
                           fypp_line_cb, NULL, &parseCtx);
  addLanguageCallbackRegex(language, "^.*$", "{exclusive}", non_fypp_line_cb,
                           NULL, &parseCtx);
}
...
static parameterHandlerTable FyppParameterHandlerTable[] = {
    {
        .name = "guest",
        .desc = "parser run after Fypp parser parses the original input "
                "(\"NONE\" or a parser name [Fortran])",
