 ...
 *
 *   This module contains functions for generating tags for id="..." in a XML
 * file
 */
...
static void makeTagWithNotification(xmlNode *node, const char *xpath,
                                    const tagXpathMakeTagSpec *spec,
                                    tagEntryInfo *tag, void *userData);
...
static void findNsPrefix(xmlNode *node, const char *xpath,
...
static tagRegexTable XmlTagRegexTable[] = {
    {"<!DOCTYPE[[:space:]]+([a-zA-Z0-9]+)[[:space:]]+[^[]*\\[((.|[\n])+)\\]>",
     "\\1", ROOT_ELT_LETTER_STR, "{mgroup=1}{_guest=DTD,2start,2end}", NULL,
     true},
};

static int makeTagWithNotificationCommon(tagEntryInfo *tag, xmlNode *node) {
...
static void
makeTagWithNotification(xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
                        const tagXpathMakeTagSpec *spec CTAGS_ATTR_UNUSED,
                        tagEntryInfo *tag, void *userData CTAGS_ATTR_UNUSED) {
...
static void findXmlTags(void) {
  findXMLTagsFull(NULL, NULL, TABLE_MAIN, runAfter, NULL);
}

extern parserDefinition *XmlParser(void) {
