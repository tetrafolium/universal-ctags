...
static void findSvgTags(void) {
  scheduleRunningBaseparser(RUN_DEFAULT_SUBPARSERS);
}

static void makeTagEntryWithNodeNotify(xmlSubparser *s, xmlNode *node,
                                       tagEntryInfo *xmlTag) {
  if (node->type == XML_ATTRIBUTE_NODE &&
      (strcmp((char *)node->name, "id") == 0) && node->parent &&
      node->parent->parent && node->parent->parent->type == XML_ELEMENT_NODE &&
...
extern parserDefinition *SvgParser(void) {
