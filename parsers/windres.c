 ...
 *version.
 *
 *	This module contains functions for generating tags for Windows Resource
 *files.
 */
...
static ResParserState parseResDefinition(const unsigned char *line) {
  ...
  if (strcmp(vStringValue(type), "DIALOG") == 0 ||
      strcmp(vStringValue(type), "DIALOGEX") == 0) {
    makeResTag(name, K_DIALOG);
    state = P_STATE_IN_STATEMENTS;
  } else if (strcmp(vStringValue(type), "MENU") == 0 ||
             strcmp(vStringValue(type), "MENUEX") == 0) {
...
static ResParserState parseResLine(const unsigned char *line,
                                   ResParserState state) {
  while (*line !=
         '\0') /* readLineFromInputFile returns NULL terminated strings */
  {
