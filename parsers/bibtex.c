 ...
 *version.
 *
 *	 This module contains functions for generating identifiers of entries of
 *Bibtex language files.
 *
...
getNextChar:
  ...
  case '@':
    ...
    if (!isalpha(c))
      ...
      token->keyword =
          lookupCaseKeyword(vStringValue(token->string) + 1, Lang_bib);
...
static void initialize(const langType language) { Lang_bib = language; }
