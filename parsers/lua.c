...
static kindDefinition LuaKinds[] = {{true, 'f', "function", "functions"}};
...
static void extract_next_token(const char *begin, const char *end_sentinel,
                               vString *name) {
  bool found;
...
static void extract_prev_token(const char *end, const char *begin_sentinel,
                               vString *name) {
  const char *begin;
    ...
    if (q == NULL) {
      ...
      if (q)
        extract_next_token(p, q, name);
                ...
                '=')      /* ignore `if type(v) == "function" then ...' */
               && (q < p) /* ignore "function" ~=  */
    ) {
      p = (const char *)&line[0];
      if (p < q)
