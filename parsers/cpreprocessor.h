...
#define cppIsident(c) (cppIsalnum(c) || (c) == '_' || (c) == '$')
...
#define cppIsident1(c) (cppIsalpha(c) || (c) == '_' || (c) == '~' || (c) == '$')

#define cppIsspace(c) (cppIsascii(c) && isspace(c))
#define cppIsdigit(c) (cppIsascii(c) && isdigit(c))

#define RoleTemplateUndef                                                      \
  { true, "undef", "undefined" }

#define RoleTemplateSystem                                                     \
  { true, "system", "system header" }
#define RoleTemplateLocal                                                      \
  { true, "local", "local header" }
...
extern void cppInit(const bool state, const bool hasAtLiteralStrings,
                    ...
                    int headerKindIndex, int headerSystemRoleIndex,
                    int headerLocalRoleIndex, int macroParamKindIndex,
