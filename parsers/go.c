...
#define isStartIdentChar(c)                                                    \
  (isalpha(c) || (c) == '_' || (c) > 128) /* XXX UTF-8 */
...
static kindDefinition GoKinds[] = {
    ...
    {true, 'u', "unknown", "unknown", .referenceOnly = true,
     ATTACH_ROLES(GoUnknownRoles)},
...
static fieldDefinition GoFields[] = {
    ...
    {
        .name = "howImported",
        .description = "how the package is imported (\"inline\" for `.' or "
                       "\"init\" for `_')",
        .enabled = false,
    },
};
...
static void collectorAppendToken(collector *collector,
  ...
  do {
    ...
    if (c == '\n' &&
        (lastTokenType == TOKEN_IDENTIFIER || lastTokenType == TOKEN_STRING ||
         lastTokenType == TOKEN_OTHER || lastTokenType == TOKEN_CLOSE_PAREN ||
    ...
    if (collector && whitespace && firstWhitespace &&
        vStringLength(collector->str) < MAX_COLLECTOR_LENGTH) {
...
static void readToken(tokenInfo *const token) { readTokenFull(token, NULL); }
...
static int makeTagFull(tokenInfo *const token, const goKind kind,
                       ...
                       const char *typeref, const int role) {
...
static int makeTag(tokenInfo *const token, const goKind kind, const int scope,
                   const char *argList, const char *typeref) {
...
static void parseFunctionOrMethod(tokenInfo *const token, const int scope) {
  ...
  if (isType(token, TOKEN_IDENTIFIER)) {
    ...
    if (receiver_type_token) {
      void *p = hashTableGetItem(typeTable,
                                 vStringValue(receiver_type_token->string));
      if (p)
        func_scope = HT_PTR_TO_INT(p);
      else
        func_scope = makeTagFull(receiver_type_token, GOTAG_UNKNOWN, scope,
                                 NULL, NULL, R_GOTAG_UNKNOWN_RECEIVER);
    } else
      func_scope = scope;

    cork = makeTag(functionToken, GOTAG_FUNCTION, func_scope,
                   vStringValue(buffer), NULL);
    if (cork != CORK_NIL)
  ...
  while (!isType(token, TOKEN_EOF) && !isType(token, TOKEN_CLOSE_CURLY)) {
    if (isType(token, TOKEN_IDENTIFIER)) {
      ...
      if (isType(token, TOKEN_DOT)) {
        ...
        makeTag(headToken, GOTAG_METHODSPEC, scope, vStringValue(pbuf),
  ...
  while (!isType(token, TOKEN_EOF) && !isType(token, TOKEN_CLOSE_CURLY)) {
    ...
    while (!isType(token, TOKEN_EOF)) {
      if (isType(token, TOKEN_IDENTIFIER)) {
        if (first) {
          ...
          int cork;
          if (memberCandidate) {
            // if we are here, there was a comma and memberCandidate isn't an
            // anonymous field
    ...
    if (first && isType(token, TOKEN_STAR)) {
      ...
      readToken(token);
               ...
               (isType(token, TOKEN_DOT) || isType(token, TOKEN_STRING) ||
    ...
    if (vStringLength(typeForAnonMember) > 0) {
      ...
      // memberCandidate.
      while (isType(token, TOKEN_IDENTIFIER) || isType(token, TOKEN_DOT)) {
        if (isType(token, TOKEN_IDENTIFIER)) {
          if (!anonMember)
            anonMember = newToken();
    ...
    while (!isType(token, TOKEN_SEMICOLON) &&
           !isType(token, TOKEN_CLOSE_CURLY) && !isType(token, TOKEN_EOF)) {
      readToken(token);
      skipToMatched(token, NULL);
    }
...
static void parseConstTypeVar(tokenInfo *const token, goKind kind,
  ...
  do {
      ...
      if (isType(token, TOKEN_IDENTIFIER)) {
        if (kind == GOTAG_TYPE) {
          ...
          if (isKeyword(token, KEYWORD_struct))
            member_scope = makeTag(typeToken, GOTAG_STRUCT, scope, NULL, NULL);
          else if (isKeyword(token, KEYWORD_interface))
            member_scope =
                makeTag(typeToken, GOTAG_INTERFACE, scope, NULL, NULL);
          else
            member_scope = makeTag(typeToken, kind, scope, NULL, NULL);

          if (member_scope != CORK_NIL) {
            tagEntryInfo *e = getEntryInCorkQueue(member_scope);
            hashTablePutItem(typeTable, (char *)e->name,
    ...
    while (!isType(token, TOKEN_SEMICOLON) &&
           !isType(token, TOKEN_CLOSE_PAREN) && !isType(token, TOKEN_EOF)) {
      readToken(token);
      skipToMatched(token, NULL);
    }
  ...
  } while (!isType(token, TOKEN_EOF) && usesParens &&
...
static void parseImportSpec(tokenInfo *const token) {
  ...
  if (isType(token, TOKEN_IDENTIFIER)) {
    if (strcmp(vStringValue(token->string), "_") == 0)
      ...
      packageName_cork =
          makeTag(token, GOTAG_PACKAGE_NAME, CORK_NIL, NULL, NULL);
    }
    readToken(token);
  } else if (isType(token, TOKEN_DOT)) {
    how_imported = "inline";
  ...
  if (isType(token, TOKEN_STRING)) {
    ...
    if (package_cork != CORK_NIL && how_imported)
      attachParserFieldToCorkEntry(package_cork, GoFields[F_HOW_IMPORTED].ftype,
                                   how_imported);

    if (packageName_cork != CORK_NIL) {
      attachParserFieldToCorkEntry(packageName_cork, GoFields[F_PACKAGE].ftype,
      ...
      if (package_cork != CORK_NIL) {
        ...
        attachParserFieldToCorkEntry(package_cork,
                                     GoFields[F_PACKAGE_NAME].ftype, e->name);
      }
    }
  }
}
...
static void parseImport(tokenInfo *const token) {
  ...
  if (isType(token, TOKEN_OPEN_PAREN)) {
    do {
      ...
      readToken(token);
    } while (!isType(token, TOKEN_EOF) && !isType(token, TOKEN_CLOSE_PAREN));
  } else {
    parseImportSpec(token);
    return;
  }
...
static void findGoTags(void) {
  ...
  typeTable = hashTableIntNew(43, hashCstrhash, hashCstreq, NULL);
