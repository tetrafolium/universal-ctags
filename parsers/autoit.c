...
static kindDefinition AutoItKinds[] = {
    ...
    {true, 'S', "script", "included scripts", .referenceOnly = true,
     ATTACH_ROLES(AutoItIncludeRoles)},
};
...
static int makeAutoItTag(const NestingLevels *const nls,
  ...
  if (isInputLanguageKindEnabled(kindIndex) && name != NULL &&
      vStringLength(name) > 0) {
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (p[0] == '#') {
      ...
      if (match(p, "cs", NULL) || match(p, "comments-start", NULL))
        ...
        skipSpaces(&p);
        if (*p == '<' || *p == '"') {
          const AutoItIncludeRole role =
              (*p == '<') ? R_INCLUDE_SYSTEM : R_INCLUDE_LOCAL;
      ...
      if (*p == ';')
        ...
        if (match(p, "func", &p)) {
          ...
          if (k != CORK_NIL)
            attachParserFieldToCorkEntry(k, AutoItFields[F_PROPERTIES].ftype,
                                         "volatile");
        }
        ...
        nestingLevelsPop(nls);
                 ...
                 (isGlobal = match(p, "global", &p)) || match(p, "local", &p)) {
        /*
         * variable-identifier ::= "$[a-zA-Z_0-9]+"
         * scope-modifier ::= "Local" | "Global"
         * variable-declaration ::= "Static" [scope-modifier]
         * variable-identifier scope-modifier ["Static" | "Const"]
         * variable-identifier
         */
        skipSpaces(&p);
        if (isStatic) {
          /* skip optional modifiers */
          if ((isGlobal = match(p, "global", &p)) || match(p, "local", &p)) {
            skipSpaces(&p);
          }
        } else if ((isStatic = match(p, "static", &p)))
          skipSpaces(&p);
        ...
        if (*p == '$') {
          ...
          if (vStringLength(name) > 0) {
            int k = makeSimpleAutoItTag(nls, name,
                                        isGlobal ? K_GLOBALVAR : K_LOCALVAR);
