...
static enum token matchToken(const char *s, int len) {
  switch (len) {
  case 2:
    if (STREQN(s, "my"))
      return T_MY;
...
static const int kindMayHaveMethodPrefix = (1 << K_SUBMETHOD) | (1 << K_METHOD);
 ...
 */
static int trimIdentifier(enum perl6Kind kind, const char **ps, int len) {
  Assert(len > 0);
  const char *const end = *ps + len;
  const char *s = *ps;
  /* Trim the front if possible: */
  s += (kindMayHaveMethodPrefix & (1 << kind)) && validMethodPrefix[(int)*s];
...
static void makeTag(struct p6Ctx *ctx, int kind, const char *name, int len) {
  tagEntryInfo entry;
...
static void possiblyMakeTag(struct p6Ctx *ctx, const char *s, int len) {
...
static void initP6Ctx(struct p6Ctx *ctx) {
  ctx->n_tokens = 0;
  ctx->name = vStringNew();
  ctx->line = NULL;
}

static void deinitP6Ctx(struct p6Ctx *ctx) { vStringDelete(ctx->name); }
 ...
 */
static int getNonSpaceStr(struct p6Ctx *ctx, const char **ptok) {
...
static void findPerl6Tags(void) {
  struct p6Ctx ctx;
...
parserDefinition *Perl6Parser(void) {
