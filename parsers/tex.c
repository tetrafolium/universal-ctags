 ...
 *version.
 *
 *	 This module contains functions for generating tags for TeX language
 *files.
 *
...
#define isIdentChar(c)                                                         \
  (isalpha(c) || isdigit(c) || (c) >= 0x80 || (c) == '$' || (c) == '_' ||      \
   (c) == '#' || (c) == '-' || (c) == '.' || (c) == ':')
...
static bool readTokenFull(tokenInfo *const token,
                          const bool includeWhitespaces) {
  int c;
  int whitespaces = -1;
...
static bool parseTag(tokenInfo *const token, texKind kind) {
  ...
  if (isType(token, TOKEN_OPEN_SQUARE)) {
    ...
    while (!isType(token, TOKEN_CLOSE_SQUARE)) {
      if (enterSquare && isType(token, TOKEN_IDENTIFIER)) {
