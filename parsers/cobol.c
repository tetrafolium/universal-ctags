...
static kindDefinition CobolKinds[] = {
    ...
    {true, 'S', "sourcefile", "source code file", .referenceOnly = true,
     ATTACH_ROLES(CobolSourcefileRoles)},
};

static tagRegexTable cobolTagRegexTable[] = {
    {"......\\*.*", "", "", "{exclusive}"},
    {COBOL_REGEX_PREFIX "[FSR]D[ \t]+([A-Z0-9][A-Z0-9-]*)\\.", "\\1",
     "f,fd,file descriptions (FD, SD, RD)", "i"},
    {COBOL_REGEX_PREFIX "[0-9]+[ \t]+([A-Z0-9][A-Z0-9-]*)\\.", "\\1",
     "g,group,group items", "i"},
    {COBOL_REGEX_PREFIX "PROGRAM-ID\\.[ \t]+([A-Z0-9][A-Z0-9-]*)\\.", "\\1",
     "P,program,program ids", "i"},
    {COBOL_REGEX_PREFIX "([A-Z0-9][A-Z0-9-]*)[ \t]+SECTION\\.", "\\1",
     "s,section,sections", "i"},
    {COBOL_REGEX_PREFIX "([A-Z0-9][A-Z0-9-]*)[ \t]+DIVISION\\.", "\\1",
...
static void cobol_make_tag_maybe(const char *line, const regexMatch *matches,
                                 unsigned int count, langType cobol,
                                 int matchIndex, int kindIndex) {
  if (count > 0) {
    vString *name = vStringNew();

    vStringNCopyS(name, line + matches[matchIndex].start,
                  matches[matchIndex].length);
...
static bool make_tag_for_data_maybe(const char *line, const regexMatch *matches,
...
static void initializeCobolParser(langType language) {
  ...
  addLanguageCallbackRegex(
      ...
      ")",
      "{icase}", make_tag_for_data_maybe, NULL, &cobol);
  addLanguageCallbackRegex(cobol, COBOL_REGEX_PREFIX "([A-Z0-9][A-Z0-9-]*)\\.",
                           "{icase}", make_tag_for_paragraph_maybe, NULL,
                           &cobol);
  addLanguageCallbackRegex(cobol, "^[ \t]*COPY[ \t]+([A-Z0-9][A-Z0-9-]*)\\.",
                           "{icase}", make_tag_for_copied_in_sourcefile, NULL,
                           NULL);
}
