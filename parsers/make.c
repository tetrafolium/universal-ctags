...
static kindDefinition MakeKinds[] = {
    ...
    {true, 'I', "makefile", "makefiles", .referenceOnly = true,
     ATTACH_ROLES(MakeMakefileRoles)},
};
...
static void newMacro(vString *const name, bool with_define_directive,
                     bool appending) {
  subparser *s;
  ...
  foreachSubparser(s, false) {
    ...
    enterSubparser(s);
    if (m->newMacroNotify)
      m->newMacroNotify(m, vStringValue(name), with_define_directive,
                        appending);
    leaveSubparser();
  }
}
...
extern parserDefinition *MakefileParser(void) {
  ...
  static const char *const aliases[] = {/* the mode name in emacs */
                                        "makefile", NULL};
