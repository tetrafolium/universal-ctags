...
static kindDefinition Maven2Kinds[] = {
    {true, 'g', "groupId", "group identifiers", .referenceOnly = false,
     ATTACH_ROLES(Maven2GroupIdRoles)},
    {true, 'a', "artifactId", "artifact identifiers", .referenceOnly = false,
     ATTACH_ROLES(Maven2ArtifactIdRoles)},
    {true, 'p', "property", "properties"},
    {true, 'r', "repositoryId", "repository identifiers"},
};

static void makeTagWithScope(xmlNode *node, const char *xpath,
...
static void
makeTagForProperties(xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
...
static tagXpathTable maven2XpathMainTable[] = {
    {"/*[local-name()='project']/*[local-name()='groupId']",
     ...
     {.makeTagSpec = {K_GROUP_ID, ROLE_DEFINITION_INDEX, makeTagWithScope}}},
...
static tagXpathTable maven2XpathParentTable[] = {
    {"./*[local-name()='groupId']",
     LXPATH_TABLE_DO_MAKE,
     {.makeTagSpec = {K_GROUP_ID, R_GROUP_ID_PARENT, makeTagWithScope}}},
    {"./*[local-name()='artifactId']",
     LXPATH_TABLE_DO_MAKE,
     {.makeTagSpec = {K_ARTIFACT_ID, R_ARTIFACT_ID_PARENT, makeTagWithScope}}},
};

static tagXpathTable maven2XpathDependencyTable[] = {
    {"./*[local-name()='groupId']",
     LXPATH_TABLE_DO_MAKE,
     {.makeTagSpec = {K_GROUP_ID, R_GROUP_ID_DEPENDENCY, makeTagWithScope}}},
    {"./*[local-name()='artifactId']",
     LXPATH_TABLE_DO_MAKE,
...
static void makeTagWithScope(xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
                             const struct sTagXpathMakeTagSpec *spec,
                             struct sTagEntryInfo *tag, void *userData) {
  ...
  if ((tag->kindIndex == K_GROUP_ID) || (tag->kindIndex == K_ARTIFACT_ID))
    corkIndexes[spec->kind] = i;
}

static void findMaven2TagsForTable(enum maven2XpathTable tindex, xmlNode *node,
                                   xmlXPathContext *ctx) {
  int corkIndexes[] = {
      [K_GROUP_ID] = CORK_NIL,
...
static void findMaven2Tags(void) {
...
extern parserDefinition *Maven2Parser(void) {
