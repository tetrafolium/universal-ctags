...
static kindDefinition AsmKinds[] = {
    ...
    {true, 's', "section", "sections", .referenceOnly = true,
     ATTACH_ROLES(asmSectionRoles)},
};

static const keywordTable AsmKeywords[] = {{"align", OP_ALIGN},
...
static const opKind OpKinds[] = {
    ...
    {OP_MACRO, K_MACRO},     {OP_PROC, K_LABEL},
    {OP_RECORD, K_TYPE},     {OP_SECTIONS, K_NONE},
    {OP_SECTION, K_SECTION}, {OP_SET, K_DEFINE},
    {OP_STRUCT, K_TYPE}};
...
static AsmKind operatorKind(const vString *const operator, bool *const found) {
...
static bool isDefineOperator(const vString *const operator) {
  ...
  const bool result =
      (bool)(length > 0 && toupper((int)*op) == 'D' &&
...
static void makeAsmTag(const vString *const name, const vString *const operator,
                       const bool labelCandidate, const bool nameFollows,
                       const bool directive, unsigned int *lastMacroCorkIndex) {
  if (vStringLength(name) > 0) {
    ...
    if (found) {
      ...
      case K_PSUEDO_MACRO_END:
        ...
        break;
      case K_SECTION:
        makeSimpleRefTag(operator, kind_for_directive, ASM_SECTION_PLACEMENT);
        break;
      default:
...
static void findAsmTags(void) {
  ...
  while ((line = asmReadLineFromInputFile()) != NULL) {
    ...
    const bool isComment = (bool)(*cp != '\0' && strchr(";*@", *cp) != NULL);
...
static void initialize(const langType language) { Lang_asm = language; }
