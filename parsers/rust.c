...
static bool isAscii(int c) { return (c >= 0) && (c < 0x80); }
...
static void addTag(vString *ident, const char *arg_list, int kind,
                   unsigned long line, MIOPos pos, vString *scope,
                   int parent_kind) {
...
/* Skip tokens until one of the goal tokens is hit. Escapes when level = 0 if
 * there are no goal tokens.
 * Keeps track of balanced <>'s, ()'s, []'s, and {}'s and ignores the goal
 * tokens within those pairings */
static void skipUntil(lexerState *lexer, int goal_tokens[],
                      int num_goal_tokens) {
  int angle_level = 0;
  int paren_level = 0;
  int brace_level = 0;
  ...
  while (lexer->cur_token != TOKEN_EOF) {
    ...
    /* Has to be after the token switch to catch the case when we start with the
     * initial level token */
    if (num_goal_tokens == 0 && angle_level == 0 && paren_level == 0 &&
...
static void parseFn(lexerState *lexer, vString *scope, int parent_kind) {
  int kind =
      (parent_kind == K_TRAIT || parent_kind == K_IMPL) ? K_METHOD : K_FN;
  vString *name;
  vString *arg_list;
  unsigned long line;
  MIOPos pos;
...
static void parseMod(lexerState *lexer, vString *scope, int parent_kind) {
  ...
  addTag(lexer->token_str, NULL, K_MOD, lexer->line, lexer->pos, scope,
         parent_kind);
...
static void parseTrait(lexerState *lexer, vString *scope, int parent_kind) {
  ...
  addTag(lexer->token_str, NULL, K_TRAIT, lexer->line, lexer->pos, scope,
         parent_kind);
  addToScope(scope, lexer->token_str);
...
/* Impl format:
 * "impl" [<type_bounds>] <qualified_ident>[<type_bounds>] ["for"
 * <qualified_ident>[<type_bounds>]] "{" [<body>] "}"
 */
...
static void parseStatic(lexerState *lexer, vString *scope, int parent_kind) {
  ...
  addTag(lexer->token_str, NULL, K_STATIC, lexer->line, lexer->pos, scope,
         parent_kind);
}
...
static void parseType(lexerState *lexer, vString *scope, int parent_kind) {
  ...
  addTag(lexer->token_str, NULL, K_TYPE, lexer->line, lexer->pos, scope,
         parent_kind);
}

/* Structs and enums are very similar syntax-wise.
 * It is possible to parse variants a bit more cleverly (e.g. make tuple
 * variants functions and struct variants structs) but it'd be too clever and
 * the signature wouldn't make too much sense without the enum's definition
 * (e.g. for the type bounds)
 *
 * Struct/Enum format:
 * "struct/enum" <ident>[<type_bounds>] "{" [<ident>,]+ "}"
 * "struct/enum" <ident>[<type_bounds>] ";"
 * */
static void parseStructOrEnum(lexerState *lexer, vString *scope,
                              int parent_kind, bool is_struct) {
  ...
  addTag(lexer->token_str, NULL, kind, lexer->line, lexer->pos, scope,
         parent_kind);
  addToScope(scope, lexer->token_str);
  ...
  if (lexer->cur_token == '{') {
    ...
    while (lexer->cur_token != TOKEN_EOF) {
      ...
      if (lexer->cur_token == TOKEN_IDENT) {
        ...
        addTag(field_name, NULL, field_kind, lexer->line, lexer->pos, scope,
               kind);
        skipUntil(lexer, goal_tokens2, 2);
      }
      if (lexer->cur_token == '}') {
 ...
 */
static void parseMacroRules(lexerState *lexer, vString *scope,
                            int parent_kind) {
  advanceToken(lexer, true);
  ...
  addTag(lexer->token_str, NULL, K_MACRO, lexer->line, lexer->pos, scope,
         parent_kind);
 ...
 * any block
 */
static void parseBlock(lexerState *lexer, bool delim, int kind,
                       vString *scope) {
  int level = 1;
  if (delim) {
    if (lexer->cur_token != '{')
  ...
  while (lexer->cur_token != TOKEN_EOF) {
    if (lexer->cur_token == TOKEN_IDENT) {
      ...
      advanceToken(lexer, true);
      if (lexer->cur_token == TOKEN_IDENT &&
          strcmp(vStringValue(lexer->token_str), "static") == 0)
        advanceToken(lexer, true);
    } else {
      advanceToken(lexer, true);
    }
