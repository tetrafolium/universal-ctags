...
static kindDefinition FlexKinds[] = {
    ...
    {true, 'I', "import", "imports", .referenceOnly = true,
     ATTACH_ROLES(FlexImportRoles)},
    {true, 'x', "mxtag", "mxtags"}};
...
getNextChar:
  ...
  default:
    if (!isIdentChar(c))
      ...
      token->filePosition = getInputFilePosition();
      token->keyword =
          lookupCaseKeyword(vStringValue(token->string), Lang_flex);
...
#define IS_BINARY_OPERATOR(t)                                                  \
  ((t) == TOKEN_EQUAL_SIGN || (t) == TOKEN_COLON || (t) == TOKEN_PERIOD ||     \
   (t) == TOKEN_STAR || (t) == TOKEN_FORWARD_SLASH ||                          \
   (t) == TOKEN_QUESTION_MARK || (t) == TOKEN_LESS_THAN ||                     \
   (t) == TOKEN_GREATER_THAN || (t) == TOKEN_BINARY_OPERATOR)
...
static void parseFunction(tokenInfo *const token) {
  ...
  /* getter and setter */
  if (isKeyword(token, KEYWORD_get) || isKeyword(token, KEYWORD_set)) {
...
static bool parseStatement(tokenInfo *const token) {
  ...
  /* skip attributes */
  while (
      isKeyword(token, KEYWORD_public) || isKeyword(token, KEYWORD_protected) ||
      isKeyword(token, KEYWORD_private) || isKeyword(token, KEYWORD_override) ||
      isKeyword(token, KEYWORD_static) || isKeyword(token, KEYWORD_internal) ||
      isKeyword(token, KEYWORD_native) || isKeyword(token, KEYWORD_dynamic) ||
  ...
  do {
    ...
    if (isType(token, TOKEN_OPEN_MXML)) {
      ...
      readToken(token);
             ...
             isType(token, TOKEN_CLOSE_MXML) || isEOF(token)));
  return true;
}
  ...
  if (isKeyword(token, KEYWORD_script)) {
    ...
    do {
      readToken(token);
               ...
               isType(token, TOKEN_GREATER_THAN) || isEOF(token)));
