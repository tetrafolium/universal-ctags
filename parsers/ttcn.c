 ...
 *           Fix: skip tag generation from import definitions (import/except
 * lists) 0.3     Fix: expression parser no longer relies on optional semicolon
 *           after a const/var/timer/modulepar definition to find where the
 * initializer actually ends Fix: handle multiple module parameters not
 * separated by semicolons 0.31    Fix: lexer bug, missing character after a
 * charstring close-quote 0.4     Enhancement: tags for record/set/union
 * members, enum values, port instances within components
 */
...
} ttcnKeywords[] = {{T_ACTION, "action", K_NONE},
...
static struct s_ttcnOp {
  const ttcnTokenType_t id;
  const char name[3];
} ttcnOps[] = {{T_OP_TO, ".."},    {T_OP_EQ, "=="},  {T_OP_NE, "!="},
               {T_OP_LE, "<="},    {T_OP_GE, ">="},  {T_OP_ASS, ":="},
               {T_OP_ARROW, "->"}, {T_OP_SHL, "<<"}, {T_OP_SHR, ">>"},
               {T_OP_ROTL, "<@"},  {T_OP_ROTR, "@>"}};
...
static void findTTCNKeyword(ttcnToken_t *pTok) {
  ...
  if (!pTok || !(pTok->value) || !(vStringValue(pTok->value)))
    return;
  /* Binary search */
  k = bsearch(vStringValue(pTok->value), ttcnKeywords, ttcnKeywordCount,
              sizeof(*ttcnKeywords), ttcnKeywordsCompare);
  if (k) {
    pTok->type = k->id;
    pTok->kind = k->kind;
  } else {
...
static const ttcnTokenType_t ttcnExprOps[] = {
    T_OR,   T_XOR,   T_AND,   T_NOT,    T_OP_EQ,  T_OP_NE,   '>',
    '<',    T_OP_GE, T_OP_LE, T_OP_SHL, T_OP_SHR, T_OP_ROTL, T_OP_ROTR,
    T_OR4B, T_XOR4B, T_AND4B, T_NOT4B,  '+',      '-',       '&',
    '*',    '/',     T_MOD,   T_REM,    '.'};
