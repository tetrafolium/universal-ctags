...
static kindDefinition HtmlKinds[] = {
    ...
    {true, 'c', "class", "classes", .referenceOnly = true,
     ATTACH_ROLES(ClassRoles)},
    {true, 'h', "heading1", "H1 headings"},
    {true, 'i', "heading2", "H2 headings"},
    {true, 'j', "heading3", "H3 headings"},
    {true, 'C', "stylesheet", "stylesheets", .referenceOnly = true,
     ATTACH_ROLES(StylesheetRoles)},
    {true, 'I', "id", "identifiers"},
    {true, 'J', "script", "scripts", .referenceOnly = true,
     ATTACH_ROLES(ScriptRoles)},
};
...
#define E(X) [TOKEN_##X] = #X
    E(EOF),     E(NAME),     E(STRING), E(TEXT),    E(TAG_START), E(TAG_START2),
    E(TAG_END), E(TAG_END2), E(EQUAL),  E(COMMENT), E(OTHER),
#undef E
};
#endif
...
static void appendText(vString *text, vString *appendedText) {
  if (text != NULL && vStringLength(appendedText) > 0) {
    if (vStringLength(text) > 0 && vStringLast(text) == ' ' &&
        vStringLength(appendedText) > 0 &&
        vStringChar(appendedText, 0) == ' ') {
...
static bool readTagContent(tokenInfo *token, vString *text, long *line,
                           long *lineOffset, int depth) {
  tokenType type;
...
static bool skipScriptContent(tokenInfo *token, long *line, long *lineOffset) {
  ...
  do {
    ...
    } else if (found_start && type == TOKEN_NAME &&
               lookupKeyword(vStringValue(token->string), Lang_html) ==
                   KEYWORD_script) {
      found_script = true;
      *line = line_tmp[1];
...
static void makeClassRefTags(const char *classes) {
  ...
  do {
    if (*classes && !isspace(*classes))
      ...
      makeSimpleRefTag(klass, K_CLASS, CLASS_KIND_ATTRIBUTE_ROLE);
      vStringClear(klass);
    }
...
static void readTag(tokenInfo *token, vString *text, int depth) {
  ...
  if (token->type == TOKEN_NAME) {
    ...
    isHeading = (startTag == KEYWORD_h1 || startTag == KEYWORD_h2 ||
                 startTag == KEYWORD_h3);
    ...
    do {
      ...
      if (attribute == KEYWORD_class) {
        ...
        if (stylesheet_expectation && stylesheet &&
    ...
    if (!isVoid && token->type == TOKEN_TAG_END && depth < MAX_DEPTH) {
      ...
      tag_start2 =
          readTagContent(token, text, &endLineNumber, &endLineOffset, depth);
      if (tag_start2) {
        readToken(token, true);
        if (isHeading && textCreated && vStringLength(text) > 0) {
          ...
          keywordId endTag =
              lookupKeyword(vStringValue(token->string), Lang_html);
          if (startTag == endTag)
            makePromise("CSS", startLineNumber, startLineOffset, endLineNumber,
                        endLineOffset, startSourceLineNumber);
        }
...
static void initialize(const langType language) { Lang_html = language; }
