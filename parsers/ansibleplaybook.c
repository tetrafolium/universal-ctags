...
typedef enum { K_PLAY } ansiblePlaybookKind;
...
static void pushBlockType(struct sAnsiblePlaybookSubparser *ansible,
                          yaml_token_type_t t) {
...
static bool stateStackMatch(struct yamlBlockTypeStack *stack,
                            yaml_token_type_t *expectation, unsigned int length,
                            bool partial) {
  if (length == 0) {
    if (stack == NULL)
      return true;
...
static void
ansiblePlaybookPlayStateMachine(struct sAnsiblePlaybookSubparser *ansible,
                                yaml_token_t *token) {
  ...
  switch (token->type) {
  case YAML_KEY_TOKEN:
    if (stateStackMatch(ansible->type_stack, play_expect,
                        ARRAY_SIZE(play_expect), false))
  ...
  case YAML_SCALAR_TOKEN:
    if ((ansible->play_detection_state == DSTAT_PLAY_NAME_KEY) &&
      ...
      tagEntryInfo tag;
      initTagEntry(&tag, (char *)token->data.scalar.value, K_PLAY);
...
static void inputStart(subparser *s) {
  ((struct sAnsiblePlaybookSubparser *)s)->play_detection_state =
      DSTAT_PLAY_NAME_INITIAL;
...
static void findAnsiblePlaybookTags(void) { scheduleRunningBaseparser(0); }
