...
typedef enum { IMPL_UNDEFINED, IMPL_ABSTRACT, COUNT_IMPL } implType;
...
static kindDefinition PhpKinds[COUNT_KIND] = {
    {true, 'c', "class", "classes", ATTACH_SEPARATORS(PhpGenericSeparators)},
    ...
    {true, 'n', "namespace", "namespaces",
     ATTACH_SEPARATORS(PhpGenericSeparators)},
    {true, 't', "trait", "traits", ATTACH_SEPARATORS(PhpGenericSeparators)},
    {true, 'v', "variable", "variables",
     ATTACH_SEPARATORS(PhpGenericSeparators)},
    {true, 'a', "alias", "aliases", ATTACH_SEPARATORS(PhpGenericSeparators)},
};
...
static const keywordTable PhpKeywordTable[] = {
    ...
    {"catch", KEYWORD_catch},
    {"cfunction",
     KEYWORD_function}, /* nobody knows what the hell this is, but it seems to
                           behave much like "function" so bind it to it */
    {"class", KEYWORD_class},
    {"clone", KEYWORD_clone},
    {"const", KEYWORD_const},
    {"continue", KEYWORD_continue},
    {"declare", KEYWORD_declare},
    {"define", KEYWORD_define}, /* this isn't really a keyword but we handle it
...
static const char *accessToString(const accessType access) {
  static const char *const names[COUNT_ACCESS] = {"undefined", "private",
                                                  "protected", "public"};
...
static const char *implToString(const implType impl) {
  static const char *const names[COUNT_IMPL] = {"undefined", "abstract"};
...
static void initPhpEntry(tagEntryInfo *const e, const tokenInfo *const token,
  ...
  if (vStringLength(token->scope) > 0) {
    ...
    if (vStringLength(FullScope) > 0) {
      ...
      sep = phpScopeSeparatorFor(parentKind, K_NAMESPACE);
...
static void makeFunctionTag(const tokenInfo *const token,
                            ...
                            const vString *const rtype, const accessType access,
                            const implType impl) {
...
static void addToScope(tokenInfo *const token, const vString *const extra,
  ...
  if (vStringLength(token->scope) > 0) {
    ...
    sep = phpScopeSeparatorFor(token->parentKind, kindOfUpperScope);
...
static bool isOpenScriptLanguagePhp(int c) {
  ...
  if (c != '<' || tolower((c = getcFromInputFile())) != 's' ||
  ...
  if (tolower(c) != 'p' || tolower((c = getcFromInputFile())) != 'h' ||
      tolower((c = getcFromInputFile())) != 'p' ||
...
getNextChar:
  ...
  default:
    if (!isIdentChar(c))
      ...
      if (MayBeKeyword)
        token->keyword =
            lookupCaseKeyword(vStringValue(token->string), getInputLanguage());
      else
        token->keyword = KEYWORD_NONE;
...
static void enterScope(tokenInfo *const parentToken,
                       const vString *const extraScope, const int parentKind);
...
static bool parseClassOrIface(tokenInfo *const token, const phpKind kind,
  ...
  while (token->type == TOKEN_IDENTIFIER || token->type == TOKEN_BACKSLASH ||
         token->type == TOKEN_KEYWORD || token->type == TOKEN_COMMA) {
...
static bool parseUse(tokenInfo *const token) {
  ...
  if (token->type == TOKEN_KEYWORD &&
      (token->keyword == KEYWORD_function || token->keyword == KEYWORD_const)) {
    switch (token->keyword) {
    case KEYWORD_function:
      refType = PhpKinds[K_FUNCTION].name;
      break;
    case KEYWORD_const:
      refType = PhpKinds[K_DEFINE].name;
      break;
    default:
      break; /* silence compilers */
    }
    readNext = true;
  }
  ...
  do {
    ...
    if (token->type == TOKEN_IDENTIFIER) {
      if (vStringLength(CurrentNamesapce) > 0) {
        ...
        sep = phpScopeSeparatorFor(K_NAMESPACE, K_NAMESPACE);
...
static void enterScope(tokenInfo *const parentToken,
  ...
  while (token->type != TOKEN_EOF && token->type != TOKEN_CLOSE_CURLY) {
    ...
    case TOKEN_KEYWORD:
      ...
      case KEYWORD_class:
        readNext = parseClassOrIface(token, K_CLASS, NULL);
        break;
      case KEYWORD_interface:
        readNext = parseClassOrIface(token, K_INTERFACE, NULL);
        break;
      case KEYWORD_trait:
      ...
      case KEYWORD_private:
        CurrentStatement.access = ACCESS_PRIVATE;
        break;
      case KEYWORD_protected:
        CurrentStatement.access = ACCESS_PROTECTED;
        break;
      case KEYWORD_public:
      ...
      case KEYWORD_abstract:
        CurrentStatement.impl = IMPL_ABSTRACT;
        break;
...
static void findTags(bool startsInPhpMode) {
  ...
  do {
    enterScope(token, NULL, -1);
           ...
           TOKEN_EOF); /* keep going even with unmatched braces */
...
static void findPhpTags(void) { findTags(false); }

static void findZephirTags(void) { findTags(true); }
...
extern parserDefinition *PhpParser(void) {
  static const char *const extensions[] = {"php",  "php3",  "php4", "php5",
                                           "php7", "phtml", NULL};
