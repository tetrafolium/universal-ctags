 ...
 *
 *   This module contains functions for generating tags for reStructuredText
 * (reST) files.
 *
...
static void makeSectionRstTag(const vString *const name, const int kind,
                              const MIOPos filepos, char marker) {
  ...
  if (vStringLength(name) > 0) {
    ...
    if (parent && (parent->kindIndex < kind)) {
      ...
      /* TODO
         ...
         --- ./Units/rst.simple.d/expected.tags	2015-12-18 01:32:35.574255617
         +0900
         +++ /home/yamato/var/ctags-github/Units/rst.simple.d/FILTERED.tmp
         2016-05-05 03:05:38.165604756 +0900
         @@ -5,2 +5,2 @@
         -Subsection 1.1.1	input.rst	/^Subsection 1.1.1$/;"	S
         section:Section 1.1 -Subsubsection 1.1.1.1	input.rst
         /^Subsubsection 1.1.1.1$/;"	t	subsection:Subsection 1.1.1
         +Subsection 1.1.1	input.rst	/^Subsection 1.1.1$/;"	S
         section:Chapter 1.Section 1.1 +Subsubsection 1.1.1.1	input.rst
         /^Subsubsection 1.1.1.1$/;"	t
         subsection:Chapter 1.Section 1.1.Subsection 1.1.1
      */
...
static const unsigned char *is_markup_line(const unsigned char *line,
                                           char reftype) {
...
static int capture_markup(const unsigned char *target_line,
                          char defaultTerminator, rstKind kindex) {
...
static void findRstTags(void) {
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (line_len >= name_len && name_len > 0 && ispunct(line[0]) &&
        issame((const char *)line)) {
      char c = line[0];
      int kind = get_kind(c);
