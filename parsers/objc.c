...
static bool isUpperAlpha(char c) { return c >= 'A' && c <= 'Z'; }

static bool isAlpha(char c) { return isLowerAlpha(c) || isUpperAlpha(c); }

static bool isIdent(char c) { return isNum(c) || isAlpha(c) || c == '_'; }
...
static void pushEnclosingContextFull(const vString *parent, objcKind type,
...
static void popCategoryContext(void) { categoryCorkIndex = CORK_NIL; }
...
static void tillTokenOrFallBack(vString *const ident CTAGS_ATTR_UNUSED,
                                objcToken what) {
  if (what == waitedToken)
    toDoNext = comeAfter;
...
static void ignoreBalanced(vString *const ident CTAGS_ATTR_UNUSED,
                           objcToken what) {
...
static void tillTokenWithCapturingSignature(vString *const ident,
                                            objcToken what) {
  ...
  if (what != waitedToken) {
    if (what == Tok_Asterisk)
      ...
      if (!(vStringLast(signature) == ',' || vStringLast(signature) == '(' ||
...
static void parseMethodsNameCommon(vString *const ident, objcToken what,
                                   parseNext reEnter, parseNext nextAction) {
  unsigned int index;
  ...
  case Tok_PARL:
    ...
    if (!(vStringLength(prevIdent) == 0 && vStringLength(fullMethodName) == 0))
  ...
  case ObjcIDENTIFIER:
    if ((vStringLength(prevIdent) > 0
         ...
         the parameter. */
         && (vStringLast(signature) == ',' || vStringLast(signature) == '(')) ||
        (/* "- initWithObject: object;"
                        In this case no overwriting happens.
                        However, "id" for "object" is part
                        of signature. */
         vStringLength(prevIdent) == 0 && vStringLength(fullMethodName) > 0 &&
         vStringLast(signature) == '('))
  ...
  case Tok_semi:
    ...
    if (index != CORK_NIL) {
      ...
      if (categoryCorkIndex != CORK_NIL) {
        ...
        attachParserFieldToCorkEntry(index, ObjcFields[F_CATEGORY].ftype,
                                     e->name);
      }
    }
    break;
...
static void parseMethodsImplemName(vString *const ident, objcToken what) {
  parseMethodsNameCommon(ident, what, parseMethodsImplemName,
                         parseImplemMethods);
}
    ...
    if (parentCorkIndex != CORK_NIL) {
      ...
      if (e)
        attachParserFieldToCorkEntry(
            parentCorkIndex, ObjcFields[F_CATEGORY].ftype, vStringValue(ident));
...
static void parseInterfaceProtocolList(vString *const ident, objcToken what) {
  ...
  if (what == ObjcIDENTIFIER)
    ...
    attachParserFieldToCorkEntry(parentCorkIndex, ObjcFields[F_PROTOCOLS].ftype,
...
static void parseMethods(vString *const ident CTAGS_ATTR_UNUSED,
                         objcToken what) {
  switch (what) {
  case Tok_PLUS: /* + */
...
static void ignorePreprocStuff(vString *const ident CTAGS_ATTR_UNUSED,
                               objcToken what) {
  switch (what) {
  case Tok_Backslash:
...
extern parserDefinition *ObjcParser(void) {
  ...
  static const char *const aliases[] = {"objc", "objective-c", NULL};
  static selectLanguage selectors[] = {selectByObjectiveCAndMatLabKeywords,
                                       selectByObjectiveCKeywords, NULL};
