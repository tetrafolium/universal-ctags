 ...
 *version.
 *
 *	 This module contains functions for generating tags for JavaScript
 *language files.
 *
 *	 Reference:
...
static bool parseBlock(tokenInfo *const token,
                       const vString *const parentScope);
...
static void makeJsTagCommon(const tokenInfo *const token, const jsKind kind,
  ...
  if (JsKinds[kind].enabled) {
    ...
    if (!token->dynamicProp && kind != JSTAG_PROPERTY &&
        (p = strrchr(name, '.')) != NULL) {
    ...
    TRACE_PRINT("Emitting tag for symbol '%s' of kind %02x with scope '%s'",
                name, kind, vStringValue(fullscope));
...
static void makeClassTagCommon(tokenInfo *const token, vString *const signature,
...

  {
    ...
    if (!stringListHas(ClassNames, vStringValue(fulltag))) {
      ...
      makeJsTagCommon(token, JSTAG_CLASS, signature, inheritance, anonymous);
    }
...
static void makeFunctionTagCommon(tokenInfo *const token,
                                  vString *const signature, bool generator,
                                  bool anonymous) {
  {
    vString *fulltag = vStringNew();
    if (vStringLength(token->scope) > 0) {
    ...
    if (!stringListHas(FunctionNames, vStringValue(fulltag))) {
      ...
      makeJsTagCommon(token, generator ? JSTAG_GENERATOR : JSTAG_FUNCTION,
                      signature, NULL, anonymous);
    }
    vStringDelete(fulltag);
  }
}
...
static int handleUnicodeCodePoint(uint32_t point) {
  ...
  if (isConverting() && JSUnicodeConverter == (iconv_t)-2) {
    ...
    JSUnicodeConverter =
        iconv_open(getLanguageEncoding(Lang_js), INTERNAL_ENCODING);
  }
  if (isConverting() && JSUnicodeConverter != (iconv_t)-1) {
    ...
    if (iconv(JSUnicodeConverter, &input_ptr, &input_left, &output_ptr,
...
static bool readUnicodeEscapeSequenceValue(uint32_t *const value,
  ...
  if (d != 'u')
    ...
    *isUTF16 = (e != '{');
    if (e == '{') { /* Handles Unicode code point escapes: \u{ HexDigits }
                     * We skip the leading 0s because there can be any number of
                     * them and they don't change any meaning. */
      ...
      valid = ((cp_len > 0 || has_leading_zero) && cp_len < ARRAY_SIZE(cp) &&
               cp[cp_len] == '}' &&
               /* also check if it's a valid Unicode code point */
               (cp_len < 6 || (cp_len == 6 && strncmp(cp, "110000", 6) < 0)));
      if (!valid) /* put back the last (likely invalid) character */
        ungetcToInputFile(cp[cp_len]);
    } else { /* Handles Unicode escape sequences: \u Hex4Digits */
      do
...
static int readUnicodeEscapeSequence(const int fallback) {
  ...
  if (!readUnicodeEscapeSequenceValue(&value, &isUTF16))
    ...
    if (isUTF16 &&
        (value & 0xfc00) ==
            0xd800) { /* this is a high surrogate, try and read its low
                       * surrogate and emit the resulting code point */
      uint32_t low;
      int d = getcFromInputFile();

      if (d != '\\' || !readUnicodeEscapeSequenceValue(&low, &isUTF16))
        ungetcToInputFile(d);
      else if (!isUTF16) { /* not UTF-16 low surrogate but a plain code point */
        d = handleUnicodeCodePoint(low);
        ungetcToInputFile(d);
      } else if ((low & 0xfc00) !=
                 0xdc00) { /* not a low surrogate, so put back the escaped
                            * representation in case it was another high
                            * surrogate we should read as part
                            * of another pair. */
...
#define IS_STMT_SEPARATOR(t)                                                   \
  ((t) == TOKEN_SEMICOLON || (t) == TOKEN_EOF || (t) == TOKEN_COMMA ||         \
   (t) == TOKEN_OPEN_CURLY)
    /* these cannot be the start or end of a statement */
#define IS_BINARY_OPERATOR(t)                                                  \
  ((t) == TOKEN_EQUAL_SIGN || (t) == TOKEN_COLON || (t) == TOKEN_PERIOD ||     \
   (t) == TOKEN_STAR || (t) == TOKEN_BINARY_OPERATOR)

    if (!IS_STMT_SEPARATOR(LastTokenType) && !IS_STMT_SEPARATOR(token->type) &&
...
static void readTokenFullDebug(tokenInfo *const token, bool include_newlines,
  ...
  TRACE_PRINT("token '%s' of type %02x with scope '%s'",
              vStringValue(token->string), token->type,
...
static bool parseMethods(tokenInfo *const token, const tokenInfo *const class,
  ...
  TRACE_ENTER_TEXT(
      "token is '%s' of type %s in classToken '%s' of type %s (es6: %s)",
  ...
  do {
    ...
    if (!isType(token, TOKEN_KEYWORD) && !isType(token, TOKEN_SEMICOLON)) {
      bool is_generator = false;
      ...
      if (is_computed_name) {
        ...
        do {
          ...
          readTokenFull(token, false,
                        (is_dynamic_prop && depth != 0) ? dprop : NULL);
        } while (!isType(token, TOKEN_EOF) && depth > 0);
      }
...
static bool parseStatement(tokenInfo *const token, bool is_inside_class) {
   ...
   */
  if (isKeyword(token, KEYWORD_var) || isKeyword(token, KEYWORD_let) ||
      isKeyword(token, KEYWORD_const)) {
...
nextVar:
  ...
  copyToken(name, token, true);
  TRACE_PRINT("name becomes '%s' of type %s", vStringValue(token->string),
              tokenTypeName(token->type));

  while (!isType(token, TOKEN_CLOSE_CURLY) && !isType(token, TOKEN_SEMICOLON) &&
         !isType(token, TOKEN_EQUAL_SIGN) && !isType(token, TOKEN_COMMA) &&
         !isType(token, TOKEN_EOF)) {
    if (isType(token, TOKEN_OPEN_CURLY))
      parseBlock(token, NULL);
              ...
              while (!isType(method_body_token, TOKEN_SEMICOLON) &&
                ...
                if (isType(method_body_token, TOKEN_OPEN_PAREN))
                  skipArgumentList(method_body_token, false,
                                   vStringLength(signature) == 0 ? signature
                                                                 : NULL);
                else
                  readToken(method_body_token);
              }
  ...
  if (isType(token, TOKEN_SEMICOLON) || isType(token, TOKEN_EOF) ||
  ...
  if (isType(token, TOKEN_EQUAL_SIGN)) {
    ...
    if (isKeyword(token, KEYWORD_function)) {
      ...
      if (!isType(token, TOKEN_KEYWORD) && !isType(token, TOKEN_OPEN_PAREN)) {
        /*
      ...
      if (isType(token, TOKEN_OPEN_CURLY)) {
         ...
         */
        if (is_inside_class) {
          makeJsTag(name, is_generator ? JSTAG_GENERATOR : JSTAG_METHOD,
                    signature, NULL);
      ...
      if (isKeyword(token, KEYWORD_function) ||
          ...
          isKeyword(token, KEYWORD_capital_object) || is_var) {
  ...
  if (!isType(token, TOKEN_CLOSE_CURLY) && !isType(token, TOKEN_SEMICOLON)) {
    /*
...
static void parseUI5(tokenInfo *const token) {
  ...
  if (isType(token, TOKEN_PERIOD)) {
    readToken(token);
    while (!isType(token, TOKEN_OPEN_PAREN) && !isType(token, TOKEN_EOF)) {
      readToken(token);
    }
    readToken(token);
...
static bool parseLine(tokenInfo *const token, bool is_inside_class) {
  TRACE_ENTER_TEXT("token is '%s' of type %s", vStringValue(token->string),
                   tokenTypeName(token->type));
...
static const char *tokenTypeName(
    enum eTokenType e) { /* Generated by misc/enumstr.sh with cmdline
...
{   /* Generated by misc/enumstr.sh with cmdline "parsers/jscript.c" "eKeywordId" "keywordName" */
    ...
    case KEYWORD_capital_function:
        return "KEYWORD_capital_function";
    case   KEYWORD_capital_object:
        return "KEYWORD_capital_object";
    case            KEYWORD_catch:
