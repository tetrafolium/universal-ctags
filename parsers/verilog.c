...
static kindDefinition SystemVerilogKinds[] = {
    {true, 'c', "constant",
     "constants (define, parameter, specparam, enum values)"},
    {true, 'e', "event", "events"},
    {true, 'f', "function", "functions"},
    {true, 'm', "module", "modules"},
    {true, 'n', "net", "net data types"},
...
static tokenInfo *pushToken(tokenInfo *const token,
                            tokenInfo *const tokenPush) {
...
static verilogKind getKindForToken(tokenInfo *const token) {
  return (verilogKind)lookupKeyword(vStringValue(token->name),
                                    getInputLanguage());
}
...
static void dropEndContext(tokenInfo *const token) {
  ...
  if ((currentContext->kind == K_COVERGROUP &&
       strcmp(vStringValue(token->name), "endgroup") == 0) ||
      (currentContext->kind == K_BLOCK && currentContext->nestLevel == 0 &&
       strcmp(vStringValue(token->name), vStringValue(endTokenName)) == 0)) {
...
static void createTag(tokenInfo *const token) {
  ...
  if (isXtagEnabled(XTAG_QUALIFIED_TAGS) &&
      currentContext->kind != K_UNDEFINED) {
...
static void processBlock(tokenInfo *const token) {
  ...
  if (findBlockName(token)) {
    ...
    if (blockEnd && currentContext->kind == K_BLOCK &&
        currentContext->nestLevel <= 1) {
      verbose("Dropping context %s\n", vStringValue(currentContext->name));
      currentContext = popToken(currentContext);
    }
  }
...
static void processEnum(tokenInfo *const token) {
  ...
  if (isIdentifierCharacter(c)) {
    ...
    if (type->kind == K_UNDEFINED &&
        (typeQueue->scope == NULL || typeQueue->scope->kind != K_UNDEFINED)) {
