...
static kindDefinition ITclKinds[] = {
    ...
    {true, 'm', "method", "methods", ATTACH_SEPARATORS(ITclGenericSeparators)},
...
static const keywordTable ITclKeywordTable[] = {
    /* keyword			keyword ID */
    {"inherit", KEYWORD_INHERIT}, {"method", KEYWORD_METHOD},
    {"private", KEYWORD_PRIVATE}, {"protected", KEYWORD_PROTECTED},
    {"public", KEYWORD_PUBLIC},   {"variable", KEYWORD_VARIABLE},
    {"common", KEYWORD_COMMON},   {"proc", KEYWORD_PROC},
};
...
static void parseSubobject(tokenInfo *token, int parent, enum ITclKind kind,
                           keywordId protection) {
  int r = CORK_NIL;
...
static int commandNotify(tclSubparser *s, char *command, int parentIndex,
                         void *pstate) {
  ...
  if ((itcl->foundITclNamespaceImported && (strcmp(command, "class") == 0)) ||
