...
static kindDefinition AutomakeKinds[] = {
    {true, 'd', "directory", "directories", .referenceOnly = false,
     ATTACH_ROLES(AutomakeDirectoryRoles)},
    {true, 'P', "program", "programs", ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'M', "man", "manuals", ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'T', "ltlibrary", "ltlibraries",
     ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'L', "library", "libraries", ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'S', "script", "scripts", ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'D', "data", "datum", ATTACH_SEPARATORS(AutomakeSeparators)},
    {true, 'c', "condition", "conditions", .referenceOnly = true,
     ATTACH_ROLES(AutomakeConditionRoles)},
};
...
static int lookupAutomakeDirectory(hashTable *directories,
                                   vString *const name) {
...
static void addAutomakeDirectory(hashTable *directories, vString *const name,
                                 int corkIndex) {
...
static bool automakeMakeTag(struct sAutomakeSubparser *automake, char *name,
                            const char *suffix, bool appending, int kindex,
                            int rindex, struct sBlacklist *blacklist) {
  size_t expected_len;
  size_t len;
  char *tail;
  vString *subname;
...
static void valueCallback(makeSubparser *make, char *name) {
  ...
  if ((parent->kindIndex == K_AM_DIR) && (parent->extensionFields.roleBits)) {
    int roleIndex;
    for (roleIndex = 0; roleIndex < ARRAY_SIZE(AutomakeDirectoryRoles);
         roleIndex++)
...
static void refCondtionAM(vString *directive) {
  makeSimpleRefTag(directive, K_AM_CONDITION, R_AM_CONDITION_BRANCHED);
}
...
static void directiveCallback(makeSubparser *make CTAGS_ATTR_UNUSED,
                              char *directive) {
  int c;
...
static void newMacroCallback(makeSubparser *make, char *name,
                             bool with_define_directive, bool appending) {
  struct sAutomakeSubparser *automake = (struct sAutomakeSubparser *)make;
  ...
  (void)(0 ||
         automakeMakeTag(automake, name, "dir", appending, K_AM_DIR,
                         ROLE_DEFINITION_INDEX, am_blacklist) ||
         automakeMakeTag(automake, name, "_PROGRAMS", appending, K_AM_DIR,
                         R_AM_DIR_PROGRAMS, am_blacklist) ||
         automakeMakeTag(automake, name, "_MANS", appending, K_AM_DIR,
                         R_AM_DIR_MANS, am_blacklist) ||
         automakeMakeTag(automake, name, "_LTLIBRARIES", appending, K_AM_DIR,
                         R_AM_DIR_LTLIBRARIES, am_blacklist) ||
         automakeMakeTag(automake, name, "_LIBRARIES", appending, K_AM_DIR,
                         R_AM_DIR_LIBRARIES, am_blacklist) ||
         automakeMakeTag(automake, name, "_SCRIPTS", appending, K_AM_DIR,
                         R_AM_DIR_SCRIPTS, am_blacklist) ||
         automakeMakeTag(automake, name, "_DATA", appending, K_AM_DIR,
                         R_AM_DIR_DATA, am_blacklist));
}
...
static void inputStart(subparser *s) {
  ...
  automake->directories =
      hashTableNew(11, hashCstrhash, hashCstreq, eFree, eFree);
  automake->index = CORK_NIL;
}
...
static void findAutomakeTags(void) { scheduleRunningBaseparser(0); }
