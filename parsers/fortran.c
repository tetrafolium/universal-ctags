...
#define isSecondaryKeyword(token, k)                                           \
  (bool)((token)->secondary == NULL ? false                                    \
...
static void ancestorPush(tokenInfo *const token) {
  ...
  Ancestors.list[Ancestors.count].signature =
      token->signature ? vStringNewCopy(token->signature) : NULL;
  Ancestors.count++;
}
...
static const tokenInfo *ancestorScope(void) {
  ...
  for (i = Ancestors.count; i > 0 && result == NULL; --i) {
    ...
    if (token->type == TOKEN_IDENTIFIER && token->tag != TAG_UNDEFINED &&
        token->tag != TAG_INTERFACE && token->tag != TAG_ENUM)
      result = token;
  }
  return result;
}
...
static const char *implementationString(const impType imp) {
  static const char *const names[] = {"?", "abstract", "deferred",
                                      "non_overridable"};
...
static void makeFortranTag(tokenInfo *const token, tagType tag) {
  ...
  if (includeTag(token->tag)) {
    ...
    if (token->parentType != NULL && vStringLength(token->parentType) > 0 &&
    ...
    if (token->signature && vStringLength(token->signature) > 0 &&
        (token->tag == TAG_FUNCTION || token->tag == TAG_SUBROUTINE ||
...
static void ungetChar(const int c) { Ungetc = c; }
 ...
 */
static void skipOverPairsFull(tokenInfo *const token, tokenType topen,
                              tokenType tclose,
...
static void skipOverParensFull(tokenInfo *const token,
                               void (*token_cb)(tokenInfo *const, void *),
                               void *user_data) {
  skipOverPairsFull(token, TOKEN_PAREN_OPEN, TOKEN_PAREN_CLOSE, token_cb,
                    user_data);
}
...
static void skipOverSquaresFull(tokenInfo *const token,
                                void (*token_cb)(tokenInfo *const, void *),
                                void *user_data) {
  skipOverPairsFull(token, TOKEN_SQUARE_OPEN, TOKEN_SQUARE_CLOSE, token_cb,
                    user_data);
}
...
static void makeParentType(tokenInfo *const token, void *userData) {
  attachParentType((tokenInfo *const)userData, vStringNewCopy(token->string));
}
...
static void parseEntityDecl(tokenInfo *const token, tokenInfo *const st) {
...
static void parseEntityDeclList(tokenInfo *const token, tokenInfo *const st) {
...
static void parseCommonNamelistStmt(tokenInfo *const token, tagType type) {
  ...
  do {
    ...
    if (isType(token, TOKEN_COMMA))
      readToken(token);
  } while (!isType(token, TOKEN_STATEMENT_END) && !isType(token, TOKEN_EOF));
  skipToNextStatement(token);
}
 ...
 *   GENERIC [, access-spec ] :: generic-spec => binding-name1 [,
 * binding-name2]... access-spec: PUBLIC or PRIVATE generic-spec: 1. generic
 * name; 2. OPERATOR(op); 3. ASSIGNMENT(=) binding-name: type bound procedure
 */
...
static void parseComponentDefStmt(tokenInfo *const token) {
  ...
  Assert(isTypeSpec(token));
  if (isKeyword(token, KEYWORD_procedure) || isKeyword(token, KEYWORD_final) ||
 ...
 */
static vString *
parserParentIdentifierOfSubmoduleStatement(tokenInfo *const token) {
  vString *parentId;
...
static void parseSubprogramFull(tokenInfo *const token, const tagType tag) {
  ...
  if (isType(token, TOKEN_IDENTIFIER)) {
    ...
    if (tag == TAG_SUBROUTINE || tag == TAG_PROTOTYPE)
  ...
  do {
    if (isType(token, TOKEN_STATEMENT_END))
      ...
      case KEYWORD_subroutine:
        parseSubprogram(token);
        break;
      case KEYWORD_submodule:
        parseModule(token, true);
        break;
      case KEYWORD_module:
        parseModule(token, false);
        break;
...
static void initialize(const langType language) { Lang_fortran = language; }
