...
static kindDefinition XsltKinds[] = {
    {true, 's', "stylesheet", "stylesheets", .referenceOnly = true,
     ATTACH_ROLES(XsltStylesheetRoles)},
    {true, 'p', "parameter", "parameters", .referenceOnly = false,
     ATTACH_ROLES(XsltParameterRoles)},
    {true, 'm', "matchedTemplate", "matched template", .referenceOnly = false,
     ATTACH_ROLES(XsltMatchedTemplateRoles)},
    {true, 'n', "namedTemplate", "matched template", .referenceOnly = false,
     ATTACH_ROLES(XsltNamedTemplateRoles)},
    {true, 'v', "variable", "variables"},
};

static void
makeTagRecursivelyWithVersionVerification(xmlNode *node, const char *xpath,
                                          const tagXpathRecurSpec *spec,
                                          xmlXPathContext *ctx, void *userData);
static void makeTagRecursively(xmlNode *node, const char *xpath,
                               const tagXpathRecurSpec *spec,
                               xmlXPathContext *ctx, void *userData);

static void makeTagWithProvidingScope(xmlNode *node, const char *xpath,
...
#define xsltXpathTemplateTableEntry(PREFIX)                                    \
  {                                                                            \
    PREFIX "/*[local-name()='template']", LXPATH_TABLE_DO_RECUR, {             \
...
#define xsltXpathVariableTableEntry(PREFIX)                                    \
  {                                                                            \
    PREFIX "/*[local-name()='variable']", LXPATH_TABLE_DO_RECUR, {             \
...
#define xsltXpathParamTableEntry(PREFIX)                                       \
  {                                                                            \
    PREFIX "/*[local-name()='param']", LXPATH_TABLE_DO_RECUR, {                \
...
#define xsltXpathWithParamTableEntry(PREFIX)                                   \
  {                                                                            \
    PREFIX "/*[local-name()='with-param']", LXPATH_TABLE_DO_RECUR, {           \
...
static void verifyVersion(xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
...
static tagXpathTable xsltXpathVersionVerifyTable[] = {
    {"./@version", LXPATH_TABLE_DO_RECUR, {.recurSpec = {verifyVersion}}},
};
...
static tagXpathTableTable xsltXpathTableTable[] = {
    ...
    [TABLE_APPLIED_TEMPLATES] = {ARRAY_AND_SIZE(
        xsltXpathAppliedTemplatesTable)},
    ...
    [TABLE_CALL_TEMPLATE_INTERNAL] = {ARRAY_AND_SIZE(
        xsltXpathTemplateInternalTable)},
};

static void makeTagRecursivelyWithVersionVerification(
    xmlNode *node, const char *xpath CTAGS_ATTR_UNUSED,
    const tagXpathRecurSpec *spec CTAGS_ATTR_UNUSED, xmlXPathContext *ctx,
    void *userData) {
...
static void
makeTagWithProvidingScope(xmlNode *node CTAGS_ATTR_UNUSED,
                          const char *xpath CTAGS_ATTR_UNUSED,
                          const tagXpathMakeTagSpec *spec CTAGS_ATTR_UNUSED,
                          struct sTagEntryInfo *tag, void *userData) {
...
static void findXsltTags(void) {
  scheduleRunningBaseparser(RUN_DEFAULT_SUBPARSERS);
}

static void runXPathEngine(xmlSubparser *s, xmlXPathContext *ctx,
                           xmlNode *root) {
...
extern parserDefinition *XsltParser(void) {
