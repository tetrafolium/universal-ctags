...
static int makeSectionAsciidocTag(const vString *const name, const int kind,
                                  const bool two_line) {
...
static int process_leading_anchors(const unsigned char *const begin) {
  ...
  while (is_anchor(current) &&
         capture_anchor(current, &captured_len) != CORK_NIL) {
    /* minimum is "[#a]" */
  ...
  while ((line = readLineFromInputFile()) != NULL) {
    ...
    if (line[0] == '=' || line[0] == '-' || line[0] == '~' || line[0] == '^' ||
      ...
      if (n_same == line_len) {
        ...
        /* if its a =_~^+ and the same length +-2 as the line before then its a
           title */
        ...
        /* else if its 4 or more /+-.*_= (plus the -- special case) its a block
           start */
        else if (((line[0] == '/' || line[0] == '+' || line[0] == '-' ||
