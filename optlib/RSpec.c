/*
 * Generated by ./misc/optlib2c from optlib/RSpec.ctags, Don't edit this
 * manually.
 */
#include "field.h"
#include "general.h"
...
extern parserDefinition *RSpecParser(void) {
  ...
  static tagRegexTable RSpecTagRegexTable[] = {
      {"^[ \t]*RSpec\\.describe[ \t]+([^\"']+)[ \t]+do", "\\1", "d", NULL, NULL,
       false},
      {"^[ \t]*describe[ \t]+\"([^\"]+)\"[ \t]+do", "\\1", "d", NULL, NULL,
       false},
      {"^[ \t]*describe[ \t]+'([^']+)'[ \t]+do", "\\1", "d", NULL, NULL, false},
      {"^[ \t]*describe[ \t]+([^\"']+)[ \t]+do", "\\1", "d", NULL, NULL, false},
      {"^[ \t]*context[ \t]+\"([^\"]+)\"[ \t]+do", "\\1", "c", NULL, NULL,
       false},
      {"^[ \t]*context[ \t]+'([^']+)'[ \t]+do", "\\1", "c", NULL, NULL, false},
      {"^[ \t]*context[ \t]+([^\"']+)[ \t]+do", "\\1", "c", NULL, NULL, false},
  };
