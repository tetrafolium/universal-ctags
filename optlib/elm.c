...
extern parserDefinition *ElmParser(void) {
  ...
  static tagRegexTable ElmTagRegexTable[] = {
      {"^(port[[:blank:]]+)?module[[:blank:]]+([[:upper:]][[:alnum:]_.]*)",
       "\\2", "m", "{scope=push}{exclusive}", NULL, false},
      {"^import[[:blank:]]+[[:alnum:]_.]+[[:blank:]]+as[[:blank:]]+([[:alnum:]]"
       "+)",
       "\\1", "n", "{scope=clear}{exclusive}", NULL, false},
      {"^import[[:blank:]]+([[:alnum:]_.]+)[[:blank:]]exposing", "\\1", "m",
       "{scope=clear}{exclusive}{_role=imported}", NULL, false},
      {"^import[[:blank:]]+([[:alnum:]_.]+)", "\\1", "m",
       "{scope=clear}{exclusive}{_role=imported}", NULL, false},
      {"^port[[:blank:]]+([[:lower:]][[:alnum:]_]*).*", "\\1", "p",
       "{scope=clear}{exclusive}", NULL, false},
      {"^type +([[:upper:]][[:alnum:]_]*.*)", "\\1", "t",
       "{scope=set}{exclusive}", NULL, false},
      {"^[[:blank:]]+[|=][[:blank:]]+([[:upper:]][[:alnum:]_]*.*)$", "\\1", "c",
       "{scope=ref}{exclusive}", NULL, false},
      {"^type[[:blank:]]+alias[[:blank:]]+([[:upper:]][[:alnum:]_]*[[:blank:][:"
       "alnum:]_]*)",
       "\\1", "a", "{scope=set}{exclusive}", NULL, false},
      {"^([[:lower:]_][[:alnum:]_]*)[^=]*=$", "\\1", "f", "{scope=set}", NULL,
       false},
      {"^[[:blank:]]+([[:lower:]_][[:alnum:]_]*)[^=]*=$", "\\1", "f",
