...
public class PropertyEventsSample {
  ...
  public event MyDelegate1 Event1 {
    add { eventTable["Event1"] = (MyDelegate1) eventTable["Event1"] + value; }
    remove {
  ...
  public event MyDelegate1 Event2 {
    add { eventTable["Event2"] = (MyDelegate1) eventTable["Event2"] + value; }
    remove {
  ...
  public event MyDelegate2 Event3 {
    add { eventTable["Event3"] = (MyDelegate2) eventTable["Event3"] + value; }
    remove {
  ...
  public event MyDelegate3 Event4 {
    add { eventTable["Event4"] = (MyDelegate3) eventTable["Event4"] + value; }
    remove {
  ...
  public event MyDelegate3 Event5 {
    add { eventTable["Event5"] = (MyDelegate3) eventTable["Event5"] + value; }
    remove {
  ...
  public event MyDelegate4 Event6 {
    add { eventTable["Event6"] = (MyDelegate4) eventTable["Event6"] + value; }
    remove {
...
public interface I1 { event MyDelegate1 MyEvent; }

public delegate int MyDelegate2(string s);

public interface I2 { event MyDelegate2 MyEvent; }
...
public class ExplicitEventsSample : I1, I2 {
  ...
  {
    add { MyEvent2Storage += value; }
    remove { MyEvent2Storage -= value; }
  }
