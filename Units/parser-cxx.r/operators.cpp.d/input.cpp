...
public:
  ...
  bool operator==(const X &x) { return true; }

  inline X &operator-=(const X &x) { return *this; }
  ...
  inline void ***operator()() { return 0; }

  inline X &operator++() { return *this; }
  ...
  inline friend X operator*(const X &a, const X &b) { return X(); }
...
X X::operator&&(const X &a) { return *this; }

void *X::operator new(size_t) { return NULL; }
...
X::operator Type() const { return Type(); }

#ifdef DONT_CARE_ABOUT_COMPILATION
// This doesn't compile because it lacks the necessary definitions. But it's
// still extracted.
