...
// These declarations are required by C++ standard for the ones below to compile
// properly
...
int f01(int f01a01, int f01a02) { return 0; }

unsigned short int *f02(unsigned int &f02a01, ...) { return 0; }

auto f03(const int &f03a01, void *f03a02) -> const int & { return 0; }

auto f04() -> int (*)(int) { return 0; }

static inline std::string f05(const int ***f05a01) { return std::string(); }

auto n01::n02::f06(n01::c01 &&f06a01) -> n01::n02::type01 * { return 0; }
...
int f10(int f10a01, int f10a02[], int f10a03[2][3], int(f10a04)[],
        int(f10a05)[][5]) {
  return 0;
}
...
// Things that might look similar to function prototypes but are NOT function
// prototypes (but still valid C++)
std::string x01("test");

#if NOTVALIDCPP
// This is not really valid C++ because it appears in the wrong context.
// However we simulate the parser being wrong about the current state (it
// happens). This should be NOT marked as a prototype even if found out of a
// function.
throw std::string(MACRO_TEXT);
#endif
