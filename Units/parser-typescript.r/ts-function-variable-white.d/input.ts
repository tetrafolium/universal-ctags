...
function map<T, U>(a: T[], f: (x: T) => U): U[] {
  var result: U[] = [] for (var i = 0; i < a.length; i++) result.push(f(a[i]))
  return result
}
...
function f4(input = [ 1, 2 ]) { let [first, second] = input }
...
function d(p: string): void { const view = p as BigTableView }
