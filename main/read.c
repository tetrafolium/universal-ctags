...
static inputFile
    BackupFile; /* File is copied here when a nested parser is pushed */
...
extern unsigned long getInputLineNumber(void) { return File.input.lineNumber; }
...
extern MIOPos getInputFilePosition(void) { return File.filePosition.pos; }
...
extern bool isInputHeaderFile(void) { return File.input.isHeader; }
...
extern bool isInputLanguageRoleEnabled(int kindIndex, int roleIndex) {
  return isLanguageRoleEnabled(getInputLanguage(), kindIndex, roleIndex);
}
...
extern langType getSourceLanguage(void) { return sourceLang; }
...
static void appendLineFposMap(inputLineFposMap *lineFposMap, compoundPos *pos,
  ...
  if (lineFposMap->size == lineFposMap->count) {
    lineFposMap->size *= 2;
    lineFposMap->pos =
        xRealloc(lineFposMap->pos, lineFposMap->size, compoundPos);
  }
...
static int compoundPosForOffset(const void *oft, const void *p) {
  ...
  const compoundPos *next =
      (compoundPos *)(((char *)pos) + sizeof(compoundPos));

  if (offset < (pos->offset - pos->crAdjustment))
    return -1;
  else if (((pos->offset - pos->crAdjustment) <= offset) &&
           (pos->open || (offset < (next->offset - next->crAdjustment))))
    return 0;
  else
    return 1;
}
...
extern unsigned long getInputLineNumberForFileOffset(long offset) {
  ...
  p = bsearch(&offset, File.lineFposMap.pos, File.lineFposMap.count,
              sizeof(compoundPos), compoundPosForOffset);
  if (p == NULL)
    return 1; /* TODO: 0? */
  else
...
static void setInputFileParametersCommon(inputFileInfo *finfo,
  ...
  if (0)
    ...
    finfo->tagPath = vStringNewOwn(
        relativeFilename(vStringValue(fileName), getTagFileDirectory()));
  else if (Option.tagRelative == TREL_NEVER)
    finfo->tagPath = vStringNewOwn(absoluteFilename(vStringValue(fileName)));
  else if (Option.tagRelative == TREL_NO ||
...
static bool setSourceFileName(vString *const fileName) {
  const langType language =
      getLanguageForFilenameAndContents(vStringValue(fileName));
  bool result = false;
...
static bool parseLineDirective(char *s) {
  ...
  if (result) {
    ...
    if (lNum == 0)
      ...
      if (vStringLength(fileName) > 0 && lNum == 1)
...
extern MIO *getMio(const char *const fileName, const char *const openMode,
  ...
  size = st->size;
  eStatFree(st);
  if ((!memStreamRequired) && (size > MAX_IN_MEMORY_FILE_SIZE || size == 0))
...
static bool checkUTF8BOM(MIO *mio, bool skipIfFound) {
  bool r = false;
  if ((0xEF == mio_getc(mio)) && (0xBB == mio_getc(mio)) &&
      (0xBF == mio_getc(mio)))
    r = true;
...
extern bool openInputFile(const char *const fileName, const langType language,
  ...
  if (File.mio == NULL)
    ...
    verbose("OPENING%s %s as %s language %sfile [%s%s]\n",
            (File.bomFound ? "(skipping utf-8 bom)" : ""), fileName,
            getLanguageName(language), File.input.isHeader ? "include " : "",
            mio ? "reused" : "new", memStreamRequired ? ",required" : "");
  }
  return opened;
}
...
extern void *getInputFileUserData(void) { return mio_get_user_data(File.mio); }
...
static void fileNewline(bool crAdjustment) {
  ...
  if (BackupFile.mio == NULL)
    appendLineFposMap(&File.lineFposMap, &File.filePosition, crAdjustment);
...
/*  Do not mix use of readLineFromInputFile () and getcFromInputFile () for the
 * same file.
 */
...
/*  An alternative interface to getcFromInputFile (). Do not mix use of
 * readLineFromInputFile() and getcFromInputFile() for the same file. The
 * returned string does not contain the terminating newline. A NULL return value
 * means that all lines in the file have been read and we are at the end of
 * file.
 */
 ...
 *  "location".
 */
extern char *readLineFromBypass(vString *const vLine, MIOPos location,
...
extern void pushNarrowedInputStream(unsigned long startLine,
  ...
  if (isThinStreamSpec(startLine, startCharOffset, endLine, endCharOffset,
                       sourceLineOffset)) {
    File.thinDepth++;
  ...
  runModifiers(promise, startLine, startCharOffset, endLine, endCharOffset,
               mio_memory_get_data(subio, NULL), size);
  ...
  File.source.lineNumberOrigin =
      ((sourceLineOffset == 0) ? 0 : sourceLineOffset - 1);
}
...
extern langType popLanguage(void) { return popLangOnStack(&inputLang); }
...
static void langStackPush(langStack *langStack, langType type) {
  if (langStack->size == 0)
    ...
    langStack->languages =
        xRealloc(langStack->languages, ++langStack->size, langType);
...
extern bool isThinStreamSpec(unsigned long startLine, long startCharOffset,
                             ...
                             unsigned long sourceLineOffset) {
  return (startLine == 0 && startCharOffset == 0 && endLine == 0 &&
          endCharOffset == 0 && sourceLineOffset == 0);
}
