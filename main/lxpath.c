...
static void simpleXpathMakeTag(xmlNode *node, const char *xpath,
  ...
  if (spec->role == ROLE_DEFINITION_INDEX)
    ...
    initRefTagEntry(&tag, (char *)str, kind, spec->role);
  else
    goto out;
...
extern void addTagXpath(const langType language CTAGS_ATTR_UNUSED,
                        tagXpathTable *xpathTable) {
  ...
  if (!xpathTable->xpathCompiled)
    error(WARNING, "Failed to compile the Xpath expression: %s",
          xpathTable->xpath);
}

extern void removeTagXpath(const langType language CTAGS_ATTR_UNUSED,
                           tagXpathTable *xpathTable) {
  if (xpathTable->xpathCompiled) {
    ...
    if (set) {
      for (j = 0; j < xmlXPathNodeSetGetLength(set); ++j) {
        ...
        if (elt->specType == LXPATH_TABLE_DO_MAKE)
          simpleXpathMakeTag(node, elt->xpath, &(elt->spec.makeTagSpec),
                             userData);
        else
          elt->spec.recurSpec.enter(node, elt->xpath, &(elt->spec.recurSpec),
                                    ctx, userData);
      }
    }
...
extern void
findXMLTagsFull(xmlXPathContext *ctx, xmlNode *root, int tableTableIndex,
  ...
  if (ctx == NULL) {
    ...
    if (ctx == NULL)
      error(FATAL, "failed to make a new xpath context for %s",
            getInputFileName());
...
extern void removeTagXpath(const langType language CTAGS_ATTR_UNUSED,
                           tagXpathTable *xpathTable CTAGS_ATTR_UNUSED) {}
...
extern void findXMLTags(xmlXPathContext *ctx, xmlNode *root,
                        int tableTableIndex, void *userData) {
