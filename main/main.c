...
static bool createTagsForWildcardEntry(const char *const pattern,
...
static bool recurseIntoDirectory(const char *const dirName) {
  ...
  if (isRecursiveLink(dirName))
    ...
    verbose("not descending in directory \"%s\" (depth %u > %u)\n", dirName,
            recursionDepth, Option.maxRecursionDepth);
  else {
...
static void runMainLoop(cookedArgs *args) { (*mainLoop)(args, mainData); }

static void batchMakeTags(cookedArgs *args, void *user CTAGS_ATTR_UNUSED) {
  clock_t timeStamps[3];
  bool resize = false;
  bool files =
      (bool)(!cArgOff(args) || Option.fileList != NULL || Option.filter);
  ...
  while (fgets(buffer, sizeof(buffer), stdin)) {
    ...
    if (!strcmp("generate-tags", json_string_value(command))) {
      ...
      if (size == -1) { /* read from disk */
        if (iargs->sandbox) {
          error(FATAL, "invalid request in sandbox submode: reading file "
                       "contents from a file is limited");
          goto next;
        }
...
static bool isSafeVar(const char *var) {
  const char *safe_vars[] = {"BASH_FUNC_module()=", "BASH_FUNC_scl()=", NULL};
  const char *sv;
