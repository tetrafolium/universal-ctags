...
extern const char *getExecutableName(void) { return ExecutableName; }

extern const char *getExecutablePath(void) { return ExecutableProgram; }
...
extern void eFreeNoNullCheck(void *const ptr) { free(ptr); }
...
extern bool strToInt(const char *const str, int base, int *value) {
  ...
  if (!strToLong(str, base, &long_value) || long_value > INT_MAX ||
      long_value < INT_MIN)
    return false;
...
extern void setCurrentDirectory(void) /* TODO */
{
  ...
  if (buf == NULL)
    perror("");
  if (!isPathSeparator(
          CurrentDirectory[strlen(CurrentDirectory) - (size_t)1])) {
  ...
  if (file.name == NULL || strcmp(fileName, file.name) != 0) {
    ...
    if (lstat(file.name, &status) != 0)
      ...
      if (file.isSymbolicLink && stat(file.name, &status) != 0)
        ...
        file.isExecutable =
            (bool)((status.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0);
...
extern char *combinePathAndFile(const char *const path,
