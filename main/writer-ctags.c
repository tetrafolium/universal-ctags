...
static int writeCtagsEntry(tagWriter *writer CTAGS_ATTR_UNUSED, MIO *mio,
                           const tagEntryInfo *const tag, void *clientData);
static int writeCtagsPtagEntry(tagWriter *writer CTAGS_ATTR_UNUSED, MIO *mio,
                               const ptagDesc *desc, const char *const fileName,
                               const char *const pattern,
                               const char *const parserName, void *clientData);
static bool treatFieldAsFixed(int fieldType);
static void checkCtagsOptions(tagWriter *writer);

#ifdef WIN32
static enum filenameSepOp
overrideFilenameSeparator(enum filenameSepOp currentSetting);
#endif /* WIN32 */
...
static void *beginECtagsFile(tagWriter *writer CTAGS_ATTR_UNUSED,
                             MIO *mio CTAGS_ATTR_UNUSED,
...
static bool endECTagsFile(tagWriter *writer, MIO *mio CTAGS_ATTR_UNUSED,
                          const char *filename CTAGS_ATTR_UNUSED,
                          void *clientData CTAGS_ATTR_UNUSED) {
...
static bool hasTagEntryTabOrNewlineChar(const tagEntryInfo *const tag) {
  ...
  if (tag->lineNumberEntry) {
    if (Option.lineDirectives) {
      if (doesFieldHaveTabOrNewlineChar(FIELD_LINE_NUMBER, tag,
                                        NO_PARSER_FIELD))
        return true;
    }
  } else if (doesFieldHaveTabOrNewlineChar(FIELD_PATTERN, tag,
                                           NO_PARSER_FIELD)) {
    /* Pattern may have a tab char. However, doesFieldHaveTabOrNewlineChar
     * returns false because NO_PARSER_FIELD may not have doesContainAnyChar
     * handler.
     */
    return true;
  }
...
static const char *escapeFieldValueFull(tagWriter *writer,
                                        const tagEntryInfo *tag,
                                        fieldType ftype, int fieldIndex) {
  const char *v;
...
static const char *escapeFieldValue(tagWriter *writer, const tagEntryInfo *tag,
                                    fieldType ftype) {
  return escapeFieldValueFull(writer, tag, ftype, NO_PARSER_FIELD);
}

static int renderExtensionFieldMaybe(tagWriter *writer, int xftype,
                                     const tagEntryInfo *const tag, char sep[2],
                                     MIO *mio) {
  if (isFieldEnabled(xftype) && doesFieldHaveValue(xftype, tag)) {
    int len;
    len = mio_printf(mio, "%s\t%s:%s", sep, getFieldName(xftype),
...
static int addParserFields(tagWriter *writer, MIO *mio,
  ...
  for (i = 0; i < tag->usedParserFields; i++) {
    ...
    length += mio_printf(mio, "\t%s:%s", getFieldName(ftype),
                         escapeFieldValueFull(writer, tag, ftype, i));
  }
  return length;
}

static int writeLineNumberEntry(tagWriter *writer, MIO *mio,
                                const tagEntryInfo *const tag) {
  if (Option.lineDirectives)
    return mio_printf(mio, "%s",
                      escapeFieldValue(writer, tag, FIELD_LINE_NUMBER));
  else
    return mio_printf(mio, "%lu", tag->lineNumber);
}

static int addExtensionFields(tagWriter *writer, MIO *mio,
                              const tagEntryInfo *const tag) {
  ...
  const char *const kindKey =
      isKindKeyEnabled ? getFieldName(FIELD_KIND_KEY) : "";
  const char *const kindFmt = isKindKeyEnabled ? "%s\t%s:%s" : "%s\t%s%s";
  const char *const scopeKey =
      isScopeEnabled ? getFieldName(FIELD_SCOPE_KEY) : "";
  const char *const scopeFmt = isScopeEnabled ? "%s\t%s:%s:%s" : "%s\t%s%s:%s";
  ...
  if (kdef->name != NULL &&
    ...
    /* Use kind long name */
    str = kdef->name;
  } else if (kdef->letter != KIND_NULL_LETTER &&
  ...
  if (isFieldEnabled(FIELD_LINE_NUMBER) &&
      doesFieldHaveValue(FIELD_LINE_NUMBER, tag)) {
    length += mio_printf(mio, "%s\t%s:%ld", sep,
                         getFieldName(FIELD_LINE_NUMBER), tag->lineNumber);
    sep[0] = '\0';
  }
  ...
  if (isFieldEnabled(FIELD_TYPE_REF) &&
      doesFieldHaveValue(FIELD_TYPE_REF, tag)) {
    length += mio_printf(mio, "%s\t%s:%s", sep, getFieldName(FIELD_TYPE_REF),
  ...
  if (isFieldEnabled(FIELD_FILE_SCOPE) &&
      doesFieldHaveValue(FIELD_FILE_SCOPE, tag)) {
    length += mio_printf(mio, "%s\t%s:", sep, getFieldName(FIELD_FILE_SCOPE));
    sep[0] = '\0';
  }
  ...
  length +=
      renderExtensionFieldMaybe(writer, FIELD_IMPLEMENTATION, tag, sep, mio);
...
static int writeCtagsEntry(tagWriter *writer, MIO *mio,
                           const tagEntryInfo *const tag,
                           void *clientData CTAGS_ATTR_UNUSED) {
  if (writer->private) {
    struct rejection *rej = writer->private;
  ...
  int length =
      mio_printf(mio, "%s\t%s\t", escapeFieldValue(writer, tag, FIELD_NAME),
  ...
  if (tag->lineNumberEntry)
    ...
    if (Option.locate == EX_COMBINE)
      length +=
          mio_printf(mio, "%lu;", tag->lineNumber + (Option.backward ? 1 : -1));
...
#define OPT(X) ((X) ? (X) : "")
             ? mio_printf(mio, "%s%s%s%s\t%s\t%s\n", PSEUDO_TAG_PREFIX,
                          desc->name, PSEUDO_TAG_SEPARATOR, parserName,
                          OPT(fileName), OPT(pattern))
             : mio_printf(mio, "%s%s\t%s\t/%s/\n", PSEUDO_TAG_PREFIX,
                          desc->name, OPT(fileName), OPT(pattern));
#undef OPT
}
...
static void checkCtagsOptions(tagWriter *writer CTAGS_ATTR_UNUSED) {
  if (isFieldEnabled(FIELD_KIND_KEY) &&
      (!(isFieldEnabled(FIELD_KIND_LONG) || isFieldEnabled(FIELD_KIND)))) {
    error(
        WARNING,
        "though %c/%s field is enabled, neither %c nor %c field is not enabled",
        getFieldLetter(FIELD_KIND_KEY), getFieldName(FIELD_KIND_KEY),
        getFieldLetter(FIELD_KIND), getFieldLetter(FIELD_KIND_LONG));
    error(WARNING, "enable the %c field to make the %c/%s field printable",
          getFieldLetter(FIELD_KIND_LONG), getFieldLetter(FIELD_KIND_KEY),
          getFieldName(FIELD_KIND_KEY));
    enableField(FIELD_KIND_LONG, true, true);
  }
  if (isFieldEnabled(FIELD_SCOPE_KEY) && !isFieldEnabled(FIELD_SCOPE)) {
    error(WARNING, "though %c/%s field is enabled, %c field is not enabled",
          getFieldLetter(FIELD_SCOPE_KEY), getFieldName(FIELD_SCOPE_KEY),
          getFieldLetter(FIELD_SCOPE));
    error(WARNING, "enable the %c field to make the %c/%s field printable",
          getFieldLetter(FIELD_SCOPE), getFieldLetter(FIELD_SCOPE_KEY),
