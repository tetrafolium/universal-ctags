...
typedef struct sTagXpathMakeTagSpec {
  ...
  void (*make)(xmlNode *node, const char *xpath,
               const struct sTagXpathMakeTagSpec *spec, tagEntryInfo *tag,
               void *userData);
  int (*decideKind)(xmlNode *node, const char *xpath,
                    const struct sTagXpathMakeTagSpec *spec, void *userData);
  /* TODO: decideRole */
} tagXpathMakeTagSpec;

typedef struct sTagXpathRecurSpec {
  void (*enter)(xmlNode *node, const char *xpath,
                const struct sTagXpathRecurSpec *spec, xmlXPathContext *ctx,
                void *userData);
