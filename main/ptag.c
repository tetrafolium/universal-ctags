...
static bool ptagMakeHowSorted(ptagDesc *desc,
                              const void *data CTAGS_ATTR_UNUSED) {
  return writePseudoTag(desc,
                        Option.sorted == SO_FOLDSORTED
                            ? "2"
                            : (Option.sorted == SO_SORTED ? "1" : "0"),
                        "0=unsorted, 1=sorted, 2=foldcase", NULL);
}

static bool ptagMakeAuthor(ptagDesc *desc, const void *data CTAGS_ATTR_UNUSED) {
  return writePseudoTag(desc, AUTHOR_NAME, "", NULL);
}

static bool ptagMakeProgName(ptagDesc *desc,
                             const void *data CTAGS_ATTR_UNUSED) {
  return writePseudoTag(desc, PROGRAM_NAME, "Derived from Exuberant Ctags",
                        NULL);
}

static bool ptagMakeProgURL(ptagDesc *desc,
                            const void *data CTAGS_ATTR_UNUSED) {
  return writePseudoTag(desc, PROGRAM_URL, "official site", NULL);
}

static bool ptagMakeProgVersion(ptagDesc *desc,
                                const void *data CTAGS_ATTR_UNUSED) {
...
#ifdef HAVE_ICONV
static bool ptagMakeFileEncoding(ptagDesc *desc,
                                 const void *data CTAGS_ATTR_UNUSED) {
...
static ptagDesc ptagDescs[] = {
    {/* The prefix is not "TAG_".
     ...
     false, "JSON_OUTPUT_VERSION", "the version of json output stream format",
     ptagMakeJsonOutputVersion, true},
    {true, "TAG_FILE_FORMAT", "the version of tags file format", ptagMakeFormat,
     true},
    {true, "TAG_FILE_SORTED", "how tags are sorted", ptagMakeHowSorted, true},
    {true, "TAG_PROGRAM_AUTHOR", "the author of this ctags implementation",
     ptagMakeAuthor, true},
    {true, "TAG_PROGRAM_NAME", "the name of this ctags implementation",
     ptagMakeProgName, true},
    {true, "TAG_PROGRAM_URL",
     "the official site URL of this ctags implementation", ptagMakeProgURL,
     true},
    {true, "TAG_PROGRAM_VERSION", "the version of this ctags implementation",
     ptagMakeProgVersion, true},
#ifdef HAVE_ICONV
    {true, "TAG_FILE_ENCODING", "the encoding used in output tags file",
     ptagMakeFileEncoding, true},
#endif
    {false, "TAG_KIND_SEPARATOR", "the separators used in kinds",
     ptagMakeKindSeparators, false},
    {false, "TAG_KIND_DESCRIPTION",
     "the letters, names and descriptions of kinds in a parser",
     ptagMakeKindDescriptions, false},
    {true, "TAG_OUTPUT_MODE", "the output mode: u-ctags or e-ctags",
     ptagMakeCtagsOutputMode, true},
    {true, "TAG_OUTPUT_FILESEP",
     "the separator used in file name (slash or backslash)",
     ptagMakeCtagsOutputFilesep, true},
};
...
extern ptagDesc *getPtagDesc(ptagType type) {
  if (type == PTAG_UNKNOWN || type >= PTAG_COUNT)
    return NULL;
...
extern void printPtags(bool withListHeader, bool machinable, FILE *fp) {
  struct colprintTable *table =
      colprintTableNew("L:NAME", "L:ENABLED", "L:DESCRIPTION", NULL);
  for (unsigned int i = 0; i < PTAG_COUNT; i++) {
    ...
    colprintLineAppendColumnCString(line, ptagDescs[i].enabled ? "on" : "off");
