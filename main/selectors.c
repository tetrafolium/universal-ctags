...
static const char *
selectByLines(MIO *input, const char *(*lineTaster)(const char *, void *),
              const char *defaultLang, void *userData) {
  char line[0x800];
...
static const char *tastePerlLine(const char *line,
...
const char *
selectByPickingPerlVersion(MIO *input, langType *candidates CTAGS_ATTR_UNUSED,
                           unsigned int nCandidates CTAGS_ATTR_UNUSED) {
  /* Default to Perl 5 */
  return selectByLines(input, tastePerlLine, TR_PERL5, NULL);
}

static const char *tasteObjectiveCOrMatLabLines(const char *line,
                                                void *data CTAGS_ATTR_UNUSED) {
  if (startsWith(line, "% ") || startsWith(line, "%{"))
    return TR_MATLAB;
  else if (startsWith(line, "// ") || startsWith(line, "/* "))
    return TR_OBJC;
  else if (startsWith(line, "#include") || startsWith(line, "#import") ||
           startsWith(line, "#define ") || startsWith(line, "#ifdef "))
    return TR_OBJC;
  else if (startsWith(line, "@interface ") ||
           startsWith(line, "@implementation ") ||
           startsWith(line, "@protocol "))
    return TR_OBJC;
  else if (startsWith(line, "struct ") || startsWith(line, "union ") ||
...
const char *selectByObjectiveCAndMatLabKeywords(
    MIO *input, langType *candidates CTAGS_ATTR_UNUSED,
    unsigned int nCandidates CTAGS_ATTR_UNUSED) {
  return selectByLines(input, tasteObjectiveCOrMatLabLines, NULL, NULL);
}

static const char *tasteObjectiveC(const char *line,
                                   void *data CTAGS_ATTR_UNUSED) {
  if (startsWith(line, "#import") || startsWith(line, "@interface ") ||
...
const char *
selectByObjectiveCKeywords(MIO *input, langType *candidates CTAGS_ATTR_UNUSED,
                           unsigned int nCandidates CTAGS_ATTR_UNUSED) {
  ...
  return selectByLines(input, tasteObjectiveC, TR_CPP, NULL);
}

static const char *tasteR(const char *line, void *data CTAGS_ATTR_UNUSED) {
...
const char *selectByArrowOfR(MIO *input, langType *candidates CTAGS_ATTR_UNUSED,
                             unsigned int nCandidates CTAGS_ATTR_UNUSED) {
  ...
  return selectByLines(input, tasteR, NULL, NULL);
}

static const char *tasteREXXOrDosBatch(const char *line, void *data) {
  bool *in_rexx_comment = data;

  if (startsWith(line, ":"))
    return TR_DOSBATCH;
  else if (*in_rexx_comment && strstr(line, "*/"))
    return TR_REXX;
  else if (strstr(line, "/*")) {
...
const char *selectByRexxCommentAndDosbatchLabelPrefix(
    MIO *input, langType *candidates CTAGS_ATTR_UNUSED,
    unsigned int nCandidates CTAGS_ATTR_UNUSED) {
  /* TODO: Ideally opening input should be delayed till
     enable/disable based selection is done. */
  ...
  return selectByLines(input, tasteREXXOrDosBatch, NULL, &in_rexx_comment);
}
...
static void suppressWarning(void *ctx CTAGS_ATTR_UNUSED,
                            const char *msg CTAGS_ATTR_UNUSED, ...) {}

static xmlDocPtr xmlParseMIO(MIO *input) {
...
static bool matchXpathFileSpec(xmlDocPtr doc, xpathFileSpec *spec) {
  if (spec->rootElementName) {
    if (*spec->rootElementName == '\0') {
      ...
      if (doc->children && doc->children->name)
        return false;
    } else if (!(doc->children && doc->children->name &&
                 (strcmp(spec->rootElementName, (char *)doc->children->name) ==
                  0)))
      return false;
    else
  ...
  if (spec->nameInDTD) {
    if (*spec->nameInDTD == '\0') {
      if (doc->intSubset && doc->intSubset->name)
        return false;
    } else if (!(doc->intSubset && doc->intSubset->name &&
  ...
  if (spec->externalID) {
    if (*spec->externalID == '\0') {
      if (doc->intSubset && doc->intSubset->ExternalID)
        return false;
    } else if (!(doc->intSubset && doc->intSubset->ExternalID &&
                 (strcmp(spec->externalID,
                         (char *)doc->intSubset->ExternalID) == 0)))
      return false;
    else
  ...
  if (spec->systemID) {
    if (*spec->systemID == '\0') {
      if (doc->intSubset && doc->intSubset->SystemID)
        return false;
    } else if (!(doc->intSubset && doc->intSubset->SystemID &&
                 (strcmp(spec->systemID, (char *)doc->intSubset->SystemID) ==
                  0)))
      return false;
    else
  ...
  if (spec->rootNSPrefix) {
    if (*spec->rootNSPrefix == '\0') {
      if (doc->children && doc->children->ns && doc->children->ns->prefix)
        return false;
    } else if (!(doc->children && doc->children->ns &&
                 doc->children->ns->prefix &&
                 (strcmp(spec->rootNSPrefix,
    ...
    if (*spec->rootNSHref == '\0') {
      if (doc->children && doc->children->ns && doc->children->ns->href)
        return false;
                 ...
                 (strcmp(spec->rootNSHref, (char *)doc->children->ns->href) ==
                  0)))
      return false;
    else
...
static const char *selectParserForXmlDoc(xmlDocPtr doc, langType *candidates,
  ...
  verbose("		Xml[rootElementName]: %s\n",
          (doc->children && doc->children->name) ? ((char *)doc->children->name)
                                                 : "-");
  verbose("		Xml[nameInDTD]: %s\n",
...
const char *selectByXpathFileSpec(MIO *input, langType *candidates,
                                  unsigned int nCandidates) {
  xmlDocPtr doc;
  const char *r = NULL;
  ...
  if (r == NULL)
    ...
    mio_attach_user_data(input, doc, (MIODestroyNotify)xmlFreeDoc);
