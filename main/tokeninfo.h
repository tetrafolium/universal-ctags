...
void tokenUnreadFull(tokenInfo *token,
                     void *data); /* DATA passed to copy method internally. */
...
#define tokenIsKeyword(TKN, K)                                                 \
  ((TKN)->type == TKN->klass->typeForKeyword && (TKN)->keyword == KEYWORD_##K)
...
#define tokenLast(TKN)                                                         \
  (vStringIsEmpty((TKN)->string) ? '\0' : vStringLast((TKN)->string))
