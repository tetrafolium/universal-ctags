...
extern bool stringListHas(const stringList *const current,
                          const char *const string) {
  bool result;
  Assert(current != NULL);
...
static vString *
stringListFinds(const stringList *const current, const char *const string,
                bool (*test)(const char *s, vString *const vs)) {
  int i;
...
extern bool stringListHasInsensitive(const stringList *const current,
                                     const char *const string) {
  bool result;
  Assert(current != NULL);
  Assert(string != NULL);
...
extern bool stringListHasTest(const stringList *const current,
  ...
  for (i = 0; !result && i < ptrArrayCount(current); ++i)
    result =
        (*test)(vStringValue((vString *)ptrArrayItem(current, i)), userData);
  return result;
}

extern bool stringListDeleteItemExtension(stringList *const current,
...
extern bool stringListExtensionMatched(const stringList *const current,
                                       const char *const extension) {
#ifdef CASE_INSENSITIVE_FILENAMES
...
extern vString *stringListExtensionFinds(const stringList *const current,
                                         const char *const extension) {
#ifdef CASE_INSENSITIVE_FILENAMES
...
extern bool stringListFileMatched(const stringList *const current,
                                  const char *const fileName) {
  return stringListFileFinds(current, fileName) ? true : false;
}

extern vString *stringListFileFinds(const stringList *const current,
                                    const char *const fileName) {
  vString *vstr = NULL;
  bool matched = false;
  unsigned int i;
...
extern void stringListPrint(const stringList *const current, FILE *fp) {
  ...
  for (i = 0; i < ptrArrayCount(current); ++i)
    fprintf(fp, "%s%s", (i > 0) ? ", " : "",
            vStringValue((vString *)ptrArrayItem(current, i)));
}
