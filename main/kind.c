...
static void initRoleObject(roleObject *robj, roleDefinition *rdef,
                           freeRoleDefFunc freefunc, int roleId) {
#ifdef DEBUG
...
extern struct kindControlBlock *
allocKindControlBlock(parserDefinition *parser) {
  unsigned int i;
  ...
  if (parser->defaultScopeSeparator)
    kcb->defaultScopeSeparator.separator =
        eStrdup(parser->defaultScopeSeparator);
  ...
  if (parser->defaultRootScopeSeparator)
    kcb->defaultRootScopeSeparator.separator =
        eStrdup(parser->defaultRootScopeSeparator);
...
extern int defineKind(struct kindControlBlock *kcb, kindDefinition *def,
  ...
  verbose("Add kind[%d] \"%c,%s,%s\" to %s\n", def->id, def->letter, def->name,
          def->description, getLanguageName(kcb->owner));
...
extern int defineRole(struct kindControlBlock *kcb, int kindIndex,
  ...
  if (roleIndex == ROLE_MAX_COUNT) {
    ...
    error(FATAL,
          ...
          kcb->kind[kindIndex].def->name, getLanguageName(kcb->owner),
...
extern bool isRoleEnabled(struct kindControlBlock *kcb, int kindIndex,
                          int roleIndex) {
...
extern kindDefinition *getKindForLetter(struct kindControlBlock *kcb,
                                        int letter) {
  unsigned int i;
  kindDefinition *kdef;
...
extern kindDefinition *getKindForName(struct kindControlBlock *kcb,
                                      const char *name) {
  unsigned int i;
  kindDefinition *kdef;
...
extern roleDefinition *getRole(struct kindControlBlock *kcb, int kindIndex,
                               int roleIndex) {
  struct roleControlBlock *rcb = kcb->kind[kindIndex].rcb;
...
static void linkKinds(langType master, kindDefinition *masterKind,
                      kindDefinition *slaveKind) {
  kindDefinition *tail;
...
extern int defineScopeSeparator(struct kindControlBlock *kcb, int kindIndex,
  ...
  if (kindIndex == KIND_WILDCARD_INDEX) {
    if (parentKindIndex == KIND_WILDCARD_INDEX) {
      ...
      error(FATAL,
            "Don't specify a real kind as parent when defining a default scope "
            "separator: %d",
            parentKindIndex);
    return 0;
  }
...
static scopeSeparator *getScopeSeparatorDynamic(kindObject *kobj,
                                                int parentKindIndex) {
  scopeSeparator *sep;
...
static const scopeSeparator *getScopeSeparatorStatic(kindDefinition *kdef,
                                                     int parentKindIndex) {
...
extern struct colprintTable *kindColprintTableNew(void) {
  return colprintTableNew("L:LANGUAGE", "L:LETTER", "L:NAME", "L:ENABLED",
                          "L:REFONLY", "L:NROLES", "L:MASTER", "L:DESCRIPTION",
                          NULL);
}

static void kindColprintFillLine(struct colprintLine *line,
                                 const char *langName, kindDefinition *kdef) {
  ...
  colprintLineAppendColumnCString(
      line,
      kdef->name ? kdef->name : "ThisShouldNotBePrintedKindNameMustBeGiven");
  ...
  colprintLineAppendColumnCString(line, (kdef->master || kdef->slave)
                                            ? getLanguageName(kdef->syncWith)
                                            : RSV_NONE);
  colprintLineAppendColumnCString(
      line, kdef->description ? kdef->description : "NO DESCRIPTION GIVEN");
}
...
static int kindColprintCompareLines(struct colprintLine *a,
...
extern struct colprintTable *roleColprintTableNew(void) {
  return colprintTableNew("L:LANGUAGE", "L:KIND(L/N)", "L:NAME", "L:ENABLED",
                          "L:DESCRIPTION", NULL);
}
...
extern void roleColprintAddRoles(struct colprintTable *table,
  ...
  for (const char *c = kindspecs; *c != '\0'; c++) {
    ...
    for (unsigned int i = 0; i < countKinds(kcb); i++) {
      ...
      if ((kname && strlen(k->name) == kname_len &&
        ...
        for (unsigned int j = 0; j < nRoles; j++) {
          ...
          colprintLineAppendColumnCString(line, r->enabled ? "on" : "off");
...
static int roleColprintCompareLines(struct colprintLine *a,
                                    struct colprintLine *b) {
  int r;
