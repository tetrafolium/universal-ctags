...
int scandir(const char *directory_name, struct dirent ***array_pointer,
  ...
  if (array = (struct dirent **)eMalloc(allocated * sizeof(struct dirent *)),
      array == NULL)
    return -1;
  ...
  while (entry = readdir(directory), entry)
    if (select_function == NULL || (*select_function)(entry)) {
      ...
      if (copy = (struct dirent *)eMalloc(sizeof(struct dirent) + extra),
          copy == NULL) {
        closedir(directory);
        eFree(array);
        return -1;
      ...
      if (counter + 1 == allocated) {
        allocated <<= 1;
        array = (struct dirent **)eRealloc((char *)array,
...
int scanDirectory(const char *directory_name, struct dirent ***array_pointer,
                  int (*select_function)(const struct dirent *),
                  int (*compare_function)(const struct dirent **,
                                          const struct dirent **)) {
