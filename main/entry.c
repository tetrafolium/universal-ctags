...
static tagFile TagFile = {
    ...
    NULL,   /* vLine */
    .cork = false,
    .corkQueue = {.queue = NULL, .length = 0, .count = 0},
...
extern const char *tagFileName(void) { return TagFile.name; }
...
static void updateSortedFlag(const char *const line, MIO *const mio,
                             MIOPos startOfLine) {
  ...
  if (tab != NULL) {
    ...
    if (line[boolOffset] == '0' || line[boolOffset] == '1') {
      ...
      if (mio_getpos(mio, &nextLine) == -1 ||
          mio_setpos(mio, &startOfLine) == -1)
        ...
        if (c == '\t' && (d == '0' || d == '1') && d != (int)Option.sorted) {
          ...
          mio_putc(mio, Option.sorted == SO_FOLDSORTED
                            ? '2'
...
static void copyFile(const char *const from, const char *const to,
                     const long size) {
...
static void internalSortTagFile(void) {
  ...
  internalSortTags(TagsToStdout, mio,
 ...
 */
static size_t appendInputLine(int putc_func(char, void *),
                              const char *const line,
                              unsigned int patternLengthLimit, void *data,
                              bool *omitted) {
  size_t length = 0;
  const char *p;
  int extraLength = 0;
  ...
  *omitted = false;
  for (p = line; *p != '\0'; ++p) {
    ...
    if (patternLengthLimit != 0 && length >= patternLengthLimit &&
        /* Do not cut inside a multi-byte UTF-8 character, but safe-guard it not
         * to allow more than one extra valid UTF-8 character in case it's not
         * actually UTF-8.  To do that, limit to an extra 3 UTF-8 sub-bytes
         * (0b10xxxxxx). */
...
static char *
getFullQualifiedScopeNameFromCorkQueue(const tagEntryInfo *inner_scope) {
  ...
  n = vStringNew();
  sep = root_scope ? scopeSeparatorFor(root_scope->langType,
                                       root_scope->kindIndex, KIND_GHOST_INDEX)
                   : NULL;
  if (sep)
    vStringCatS(n, sep);
...
extern void getTagScopeInformation(tagEntryInfo *const tag, const char **kind,
                                   const char **name) {
  if (kind)
    *kind = NULL;
  if (name)
  ...
  if (tag->extensionFields.scopeKindIndex == KIND_GHOST_INDEX &&
    ...
    /* Make the information reusable to generate full qualified entry, and
     * xformat output*/
  ...
  if (tag->extensionFields.scopeKindIndex != KIND_GHOST_INDEX &&
    ...
    if (kind) {
      ...
      kindDefinition *kdef =
          getLanguageKind(lang, tag->extensionFields.scopeKindIndex);
      *kind = kdef->name;
    }
    if (name)
...
static int makePatternStringCommon(const tagEntryInfo *const tag,
  ...
  length += appendInputLine(putc_func, line, Option.patternLengthLimit, output,
                            &omitted);
...
static tagField *tagFieldNew(fieldType ftype, const char *value,
                             bool valueOwner) {
...
static void attachParserFieldGeneric(tagEntryInfo *const tag, fieldType ftype,
                                     const char *value, bool valueOwner) {
...
extern void attachParserField(tagEntryInfo *const tag, bool inCorkQueue,
                              fieldType ftype, const char *value) {
  Assert(tag != NULL);
...
extern void attachParserFieldToCorkEntry(int index, fieldType ftype,
                                         const char *value) {
  tagEntryInfo *tag;
...
extern const tagField *getParserField(const tagEntryInfo *tag, int index) {
  if (index < 0 || tag->usedParserFields <= ((unsigned int)index))
    return NULL;
...
static void copyParserFields(const tagEntryInfo *const tag,
  ...
  for (i = 0; i < tag->usedParserFields; i++) {
    ...
    attachParserFieldGeneric(slot, f->ftype, value, true);
  }
}

static void recordTagEntryInQueue(const tagEntryInfo *const tag,
                                  tagEntryInfo *slot) {
  *slot = *tag;
  ...
  if (slot->extensionFields.implementation)
    slot->extensionFields.implementation =
        eStrdup(slot->extensionFields.implementation);
  if (slot->extensionFields.inheritance)
    slot->extensionFields.inheritance =
        eStrdup(slot->extensionFields.inheritance);
  ...
  if (slot->extensionFields.typeRef[0])
    slot->extensionFields.typeRef[0] =
        eStrdup(slot->extensionFields.typeRef[0]);
  if (slot->extensionFields.typeRef[1])
    slot->extensionFields.typeRef[1] =
        eStrdup(slot->extensionFields.typeRef[1]);
#ifdef HAVE_LIBXML
...
static unsigned int queueTagEntry(const tagEntryInfo *const tag) {
  ...
  if (!(TagFile.corkQueue.count < TagFile.corkQueue.length)) {
    ...
    tmp = eRealloc(TagFile.corkQueue.queue, sizeof(*TagFile.corkQueue.queue) *
...
static bool isTagWritable(const tagEntryInfo *const tag) {
  ...
  if (tag->extensionFields.roleBits) {
    ...
    available_roles = countLanguageRoles(tag->langType, tag->kindIndex);
    if (tag->extensionFields.roleBits >= (makeRoleBit(available_roles)))
      return false;
    ...
    error(WARNING, "definition tag for refonly kind(%s) is made: %s",
          getLanguageKind(tag->langType, tag->kindIndex)->name, tag->name);
...
static void writeTagEntry(const tagEntryInfo *const tag) {
...
#ifdef WIN32
      if (getFilenameSeparator(Option.useSlashAsFilenameSeparator) ==
  ...
  if (includeExtensionFlags() && isXtagEnabled(XTAG_QUALIFIED_TAGS) &&
...
extern bool writePseudoTag(const ptagDesc *desc, const char *const fileName,
                           const char *const pattern,
                           const char *const parserName) {
  int length;

  length = writerWritePtag(TagFile.mio, desc, fileName, pattern, parserName);
  if (length < 0)
    return false;
...
extern size_t countEntryInCorkQueue(void) { return TagFile.corkQueue.count; }
...
extern int makeTagEntry(const tagEntryInfo *const tag) {
  ...
  if (tag->name[0] == '\0' && (!tag->placeholder)) {
    if (!doesInputLanguageAllowNullTag())
      error(WARNING, "ignoring null tag in %s(line: %lu)", getInputFileName(),
            tag->lineNumber);
    goto out;
  }
...
extern int makeQualifiedTagEntry(const tagEntryInfo *const e) {
  ...
  if (isXtagEnabled(XTAG_QUALIFIED_TAGS)) {
    ...
    bool in_subparser = isTagExtraBitMarked(&x, XTAG_SUBPARSER);
...
static void initTagEntryFull(tagEntryInfo *const e, const char *const name,
                             unsigned long lineNumber, langType langType_,
                             MIOPos filePosition, const char *inputFileName,
                             int kindIndex, roleBitsType roleBits,
...
extern void initTagEntry(tagEntryInfo *const e, const char *const name,
                         int kindIndex) {
  initTagEntryFull(e, name, getInputLineNumber(), getInputLanguage(),
                   getInputFilePosition(), getInputFileTagPath(), kindIndex, 0,
                   getSourceFileTagPath(), getSourceLanguage(),
...
extern void initRefTagEntry(tagEntryInfo *const e, const char *const name,
  ...
  initTagEntryFull(e, name, getInputLineNumber(), getInputLanguage(),
                   getInputFilePosition(), getInputFileTagPath(), kindIndex,
                   makeRoleBit(roleIndex), getSourceFileTagPath(),
                   getSourceLanguage(),
                   getSourceLineNumber() - getInputLineNumber());
}

static void markTagExtraBitFull(tagEntryInfo *const tag, xtagType extra,
                                bool mark) {
  unsigned int index;
  unsigned int offset;
  uint8_t *slot;
...
extern unsigned long numTagsAdded(void) { return TagFile.numTags.added; }
...
extern void invalidatePatternCache(void) { TagFile.patternCacheValid = false; }
