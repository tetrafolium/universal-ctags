...
static bool doesContainAnyCharInName(const tagEntryInfo *const tag,
                                     const char *value, const char *chars);
...
#define DEFINE_FIELD_FULL(L, N, V, H, A, DT, RE, RN, DCAC)                     \
  {                                                                            \
    .letter = L, .name = N, .description = H, .enabled = V, .render = RE,      \
    .renderNoEscaping = RN, .doesContainAnyChar = DCAC, .isValueAvailable = A, \
    .dataType = DT,                                                            \
  }
...
static fieldDefinition fieldDefinitionsFixed[] = {
    /* FIXED FIELDS */
    DEFINE_FIELD_FULL('N', "name", true, "tag name", NULL, FIELDTYPE_STRING,
                      renderFieldName, renderFieldNameNoEscape,
                      doesContainAnyCharInName),
    DEFINE_FIELD_FULL('F', "input", true, "input file", NULL, FIELDTYPE_STRING,
                      renderFieldInput, renderFieldInputNoEscape,
                      doesContainAnyCharInInput),
    DEFINE_FIELD('P', "pattern", true, "pattern",
                 FIELDTYPE_STRING | FIELDTYPE_BOOL, renderFieldPattern),
};

static fieldDefinition fieldDefinitionsExuberant[] = {
    DEFINE_FIELD('C', "compact", false,
                 "compact input line (used only in xref output)",
                 FIELDTYPE_STRING, renderFieldCompactInputLine),

    /* EXTENSION FIELDS */
    DEFINE_FIELD_FULL('a', "access", false,
                      "Access (or export) of class members",
                      isAccessFieldAvailable, FIELDTYPE_STRING,
                      renderFieldAccess, NULL, NULL),
    DEFINE_FIELD_FULL('f', "file", true, "File-restricted scoping",
                      isFileFieldAvailable, FIELDTYPE_BOOL, renderFieldFile,
                      NULL, NULL),
    DEFINE_FIELD_FULL('i', "inherits", false, "Inheritance information",
                      isInheritsFieldAvailable,
                      FIELDTYPE_STRING | FIELDTYPE_BOOL, renderFieldInherits,
                      NULL, NULL),
    DEFINE_FIELD('K', NULL, false, "Kind of tag in long-name form",
                 FIELDTYPE_STRING, renderFieldKindName),
    DEFINE_FIELD('k', NULL, true, "Kind of tag in one-letter form",
                 FIELDTYPE_STRING, renderFieldKindLetter),
    DEFINE_FIELD_FULL('l', "language", false,
                      "Language of input file containing tag",
                      isLanguageFieldAvailable, FIELDTYPE_STRING,
                      renderFieldLanguage, NULL, NULL),
    DEFINE_FIELD_FULL('m', "implementation", false,
                      "Implementation information",
                      isImplementationFieldAvailable, FIELDTYPE_STRING,
                      renderFieldImplementation, NULL, NULL),
    DEFINE_FIELD('n', "line", false, "Line number of tag definition",
                 FIELDTYPE_INTEGER, renderFieldLineNumber),
    DEFINE_FIELD_FULL('S', "signature", false,
                      "Signature of routine (e.g. prototype or parameter list)",
                      isSignatureFieldAvailable, FIELDTYPE_STRING,
                      renderFieldSignature, renderFieldSignatureNoEscape,
                      doesContainAnyCharInSignature),
    DEFINE_FIELD_FULL('s', NULL, true,
                      "[tags output] scope (kind:name) of tag definition, "
                      "[xref and json output] name of scope",
                      NULL, FIELDTYPE_STRING, renderFieldScope,
                      renderFieldScopeNoEscape, doesContainAnyCharInFieldScope),
    DEFINE_FIELD_FULL('t', "typeref", true,
                      "Type and name of a variable or typedef",
                      isTyperefFieldAvailable, FIELDTYPE_STRING,
                      renderFieldTyperef, NULL, NULL),
    DEFINE_FIELD('z', "kind", false,
                 "[tags output] prepend \"kind:\" to k/ (or K/) field output, "
                 "[xref and json output] kind in long-name form",
                 FIELDTYPE_STRING,
...
static fieldDefinition fieldDefinitionsUniversal[] = {
    DEFINE_FIELD('r', "roles", false, "Roles", FIELDTYPE_STRING,
                 renderFieldRoles),
    DEFINE_FIELD(
        'R', NULL, false,
        "Marker (R or D) representing whether tag is definition or reference",
        FIELDTYPE_STRING, renderFieldRefMarker),
    DEFINE_FIELD_FULL('Z', "scope", false,
                      "[tags output] prepend \"scope:\" key to s/scope field "
                      "output, [xref and json output] the same as s/ field",
                      NULL, FIELDTYPE_STRING,
                      /* Following renderer is for handling --_xformat=%{scope};
                         and is not for tags output. */
                      renderFieldScope, renderFieldScopeNoEscape,
                      doesContainAnyCharInFieldScope),
    DEFINE_FIELD_FULL('E', "extras", false, "Extra tag type information",
                      isExtrasFieldAvailable, FIELDTYPE_STRING,
                      renderFieldExtras, NULL, NULL),
    DEFINE_FIELD_FULL('x', "xpath", false, "xpath for the tag",
                      isXpathFieldAvailable, FIELDTYPE_STRING, renderFieldXpath,
                      NULL, NULL),
    DEFINE_FIELD('p', "scopeKind", false,
                 "[tags output] no effect, [xref and json output] kind of "
                 "scope in long-name form",
                 FIELDTYPE_STRING, renderFieldScopeKindName),
    DEFINE_FIELD_FULL('e', "end", false, "end lines of various items",
                      isEndFieldAvailable, FIELDTYPE_INTEGER, renderFieldEnd,
                      NULL, NULL),
};
...
extern void initFieldObjects(void) {
  ...
  fieldObjectAllocated = ARRAY_SIZE(fieldDefinitionsFixed) +
  ...
  for (i = 0; i < ARRAY_SIZE(fieldDefinitionsUniversal); i++) {
    ...
    if (fobj->def->name) {
      nameWithPrefix =
          eMalloc(sizeof CTAGS_FIELD_PREFIX + strlen(fobj->def->name) + 1);
      nameWithPrefix[0] = '\0';
...
extern fieldType getFieldTypeForNameAndLanguage(const char *fieldName,
                                                langType language) {
  ...
  for (i = 0; i < fieldObjectUsed; i++) {
    if (fieldObjects[i].def->name &&
        ...
        ((language == LANG_AUTO) || (fieldObjects[i].language == language)))
      return i;
  }
...
extern unsigned char getFieldLetter(fieldType type) {
  ...
  return fobj->def->letter == '\0' ? FIELD_NULL_LETTER_CHAR : fobj->def->letter;
}
...
static const char *
renderEscapedString(const char *s,
...
static const char *renderEscapedName(const bool isTagName, const char *s,
  ...
  if (unexpected_byte) {
    ...
    verbose("File: %s, Line: %lu, Lang: %s, Kind: %c\n", tag->inputFileName,
            tag->lineNumber, getLanguageName(tag->langType), kdef->letter);
...
static const char *
renderFieldSignatureNoEscape(const tagEntryInfo *const tag,
                             const char *value CTAGS_ATTR_UNUSED, vString *b) {
...
static bool doesContainAnyCharInSignature(const tagEntryInfo *const tag,
                                          const char *value CTAGS_ATTR_UNUSED,
                                          const char *chars) {
  return (tag->extensionFields.signature &&
          strpbrk(tag->extensionFields.signature, chars))
             ? true
             : false;
}
...
static const char *renderFieldInherits(const tagEntryInfo *const tag,
                                       const char *value CTAGS_ATTR_UNUSED,
                                       vString *b) {
  return renderEscapedString(
      WITH_DEFAULT_VALUE(tag->extensionFields.inheritance), tag, b);
}
...
static const char *renderFieldTyperef(const tagEntryInfo *const tag,
  ...
  vStringPut(b, ':');
  return renderEscapedName(
      false, WITH_DEFAULT_VALUE(tag->extensionFields.typeRef[1]), tag, b);
}

static const char *renderFieldCommon(fieldType type, const tagEntryInfo *tag,
                                     int index, bool noEscaping) {
...
extern const char *renderField(fieldType type, const tagEntryInfo *tag,
                               int index) {
  return renderFieldCommon(type, tag, index, false);
}

extern const char *renderFieldNoEscaping(fieldType type,
                                         const tagEntryInfo *tag, int index) {
  return renderFieldCommon(type, tag, index, true);
}

static bool
defaultDoesContainAnyChar(const tagEntryInfo *const tag CTAGS_ATTR_UNUSED,
                          const char *value, const char *chars) {
  return strpbrk(value, chars) ? true : false;
}

extern bool doesFieldHaveTabOrNewlineChar(fieldType type,
                                          const tagEntryInfo *tag, int index) {
  fieldObject *fobj = fieldObjects + type;
  const char *value;
  bool (*doesContainAnyChar)(const tagEntryInfo *const, const char *,
                             const char *) = fobj->def->doesContainAnyChar;

  Assert(tag);
  Assert(index == NO_PARSER_FIELD ||
         ((unsigned int)index) < tag->usedParserFields);
...
static const char *
renderFieldCompactInputLine(const tagEntryInfo *const tag,
                            const char *value CTAGS_ATTR_UNUSED, vString *b) {
  const char *line;
  static vString *tmp;
...
static const char *
renderFieldImplementation(const tagEntryInfo *const tag,
                          const char *value CTAGS_ATTR_UNUSED, vString *b) {
...
static const char *renderFieldXpath(const tagEntryInfo *const tag,
  ...
  if (tag->extensionFields.xpath)
    return renderEscapedString(tag->extensionFields.xpath, tag, b);
#endif
  return NULL;
}
  ...
  if (writerDoesTreatFieldAsFixed(type)) {
    if ((!state) && warnIfFixedField) {
      if (def->name && def->letter != NUL_FIELD_LETTER)
        error(WARNING, "Cannot disable fixed field: '%c'{%s}", def->letter,
              def->name);
      else if (def->name)
        error(WARNING, "Cannot disable fixed field: {%s}", def->name);
    ...
    if (isCommonField(type))
      ...
      verbose("enable field \"%s\"<%s>: %s\n", getFieldObject(type)->def->name,
              getLanguageName(getFieldOwner(type)), (state ? "yes" : "no"));
  }
  return old;
}
...
extern int countFields(void) { return fieldObjectUsed; }
...
static const char *
defaultRenderer(const tagEntryInfo *const tag CTAGS_ATTR_UNUSED,
                const char *value, vString *buffer CTAGS_ATTR_UNUSED) {
...
extern struct colprintTable *fieldColprintTableNew(void) {
  return colprintTableNew("L:LETTER", "L:NAME", "L:ENABLED", "L:LANGUAGE",
                          "L:JSTYPE", "L:FIXED", "L:DESCRIPTION", NULL);
}
...
static void fieldColprintAddLine(struct colprintTable *table, int i) {
  ...
  colprintLineAppendColumnChar(line, (fdef->letter == NUL_FIELD_LETTER)
  ...
  colprintLineAppendColumnCString(line, fobj->language == LANG_IGNORE
                                            ? RSV_NONE
  ...
  char typefields[] = "---";
  {
    ...
    for (bmask = 1, offset = 0; bmask < FIELDTYPE_END_MARKER;
         bmask <<= 1, offset++)
      if (type & bmask)
...
extern void fieldColprintAddLanguageLines(struct colprintTable *table,
                                          langType language) {
...
static int fieldColprintCompareLines(struct colprintLine *a,
  ...
  if ((strcmp(a_fixed, "yes") == 0) && (strcmp(b_fixed, "yes") == 0)) {
    ...
    if (a_index < b_index)
      ...
      return 1;
  } else if ((strcmp(a_fixed, "yes") == 0) && (strcmp(b_fixed, "yes") != 0))
    return -1;
  else if ((strcmp(a_fixed, "yes") != 0) && (strcmp(b_fixed, "yes") == 0))
    return 1;

  if (strcmp(a_parser, RSV_NONE) == 0 && strcmp(b_parser, RSV_NONE) != 0)
    return -1;
  else if (strcmp(a_parser, RSV_NONE) != 0 && strcmp(b_parser, RSV_NONE) == 0)
    return 1;
  else if (strcmp(a_parser, RSV_NONE) != 0 && strcmp(b_parser, RSV_NONE) != 0) {
    int r;
