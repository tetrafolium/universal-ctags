...
static int writeJsonEntry(tagWriter *writer CTAGS_ATTR_UNUSED, MIO *mio,
                          const tagEntryInfo *const tag, void *clientData);

static int writeJsonPtagEntry(tagWriter *writer CTAGS_ATTR_UNUSED, MIO *mio,
...
static const char *escapeFieldValueRaw(const tagEntryInfo *tag, fieldType ftype,
                                       int fieldIndex) {
  const char *v;
...
static void renderExtensionFieldMaybe(int xftype, const tagEntryInfo *const tag,
                                      json_t *response) {
  const char *fname = getFieldName(xftype);

  if (fname && doesFieldHaveRenderer(xftype, false) && isFieldEnabled(xftype) &&
      doesFieldHaveValue(xftype, tag)) {
    switch (xftype) {
    case FIELD_LINE_NUMBER:
      json_object_set_new(response, fname, json_integer(tag->lineNumber));
      break;
    case FIELD_FILE_SCOPE:
      json_object_set_new(response, fname, json_boolean(1));
      break;
    default:
...
static void addExtensionFields(json_t *response,
...
#define OPT(X) ((X) ? (X) : "")
  ...
  if (parserName) {
    response = json_pack("{ss ss ss ss ss}", "_type", "ptag", "name",
                         desc->name, "parserName", parserName, "path",
                         OPT(fileName), "pattern", OPT(pattern));
  } else {
    response = json_pack("{ss ss ss ss}", "_type", "ptag", "name", desc->name,
                         "path", OPT(fileName), "pattern", OPT(pattern));
  }
...
extern bool ptagMakeJsonOutputVersion(ptagDesc *desc,
                                      const void *data CTAGS_ATTR_UNUSED) {
  return writePseudoTag(desc, "0.0", "in development", NULL);
}
