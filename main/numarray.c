...
#define impNumArray(prefix, Prefix, type)                                      \
  ...
  extern void prefix##ArrayRemoveLast(prefix##Array *const current) {          \
    ...
    --current->count;                                                          \
                                                                               ...
                                                                               \
  extern void prefix##ArrayCombine(prefix##Array *const current,               \
                                   prefix##Array *const from) {                \
    unsigned int i;                                                            \
    Assert(current != NULL);                                                   \
    Assert(from != NULL);                                                      \
  ...
  extern unsigned int prefix##ArrayCount(const prefix##Array *const current) { \
    ...
    return current->count;                                                     \
                                                                               ...
                                                                               \
  extern type prefix##ArrayItem(const prefix##Array *const current,            \
  ...
  extern void prefix##ArrayDelete(prefix##Array *const current) {              \
    if (current != NULL) {                                                     \
      ...
      eFree(current);                                                          \
                                                                               ...
                                                                               \
  extern bool prefix##ArrayHasTest(                                            \
      const prefix##Array *const current,                                      \
      bool (*test)(const type num, void *userData), void *userData) {          \
    bool result = false;                                                       \
    unsigned int i;                                                            \
    Assert(current != NULL);                                                   \
  ...
  extern void prefix##ArrayReverse(const prefix##Array *const current) {       \
    ...
    for (i = 0, j = current->count - 1; i < (current->count / 2); ++i, --j) {  \
      ...
      current->array[j] = tmp;                                                 \
                                                                               ...
                                                                               \
  extern void prefix##ArrayDeleteItem(prefix##Array *const current,            \
                                      unsigned int indx) {                     \
  ...
  extern void prefix##ArraySort(prefix##Array *const current,                  \
                                bool descendingOrder) {                        \
    if (descendingOrder)                                                       \
      qsort(current->array, current->count, sizeof(type),                      \
            prefix##GreaterThan);                                              \
    else                                                                       \
...
impNumArray(char, Char, char) impNumArray(uchar, Uchar, unsigned char)
    impNumArray(int, Int, int) impNumArray(uint, Uint, unsigned int)
        impNumArray(long, Long, long) impNumArray(ulong, Ulong, unsigned long)
