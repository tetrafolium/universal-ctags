...
const char *specTypeName[] = {"none", "name", "extension", "pattern"};
...
typedef struct sParserObject {
  ...
  unsigned int
      dontEmit : 1; /* run but don't emit tags.
                                                           This parser was
                       disabled but a subparser on this parser makes this parser
                       run (to drive the subparser). */
  unsigned int
      pseudoTagPrinted : 1; /* pseudo tags about this parser
                                                                       is
                               emitted or not. */
  ...
  langType pretendingAsLanguage; /* OLDLANG in --_pretend-<NEWLANG>=<OLDLANG>
                                                                        is set
                                    here if this parser is NEWLANG.
                                                                        LANG_IGNORE
                                    is set if no pretending. */
  langType pretendedAsLanguage;  /* NEWLANG in --_pretend-<NEWLANG>=<OLDLANG>
                                                                        is set
                                    here if this parser is OLDLANG.  LANG_IGNORE
                                    is set if no being pretended. */
...
extern unsigned int countParsers(void) { return LanguageCount; }
...
extern const char *getLanguageKindName(const langType language,
                                       const int kindIndex) {
...
extern kindDefinition *getLanguageKindForLetter(const langType language,
                                                char kindLetter) {
  Assert(0 <= language && language < (int)LanguageCount);
  if (kindLetter == LanguageTable[language].fileKind->letter)
    ...
    return getKindForLetter(LanguageTable[language].kindControlBlock,
                            kindLetter);
}

extern kindDefinition *getLanguageKindForName(const langType language,
                                              const char *kindName) {
  Assert(0 <= language && language < (int)LanguageCount);
  Assert(kindName);
...
extern roleDefinition *getLanguageRole(const langType language, int kindIndex,
                                       int roleIndex) {
  return getRole(LanguageTable[language].kindControlBlock, kindIndex,
                 roleIndex);
}

extern roleDefinition *getLanguageRoleForName(const langType language,
                                              int kindIndex,
                                              const char *roleName) {
  return getRoleForName(LanguageTable[language].kindControlBlock, kindIndex,
                        roleName);
}

extern langType getNamedLanguageFull(const char *const name, size_t len,
                                     bool noPretending) {
  ...
  if (len == 0) {
    parserDefinition *def =
        (parserDefinition *)hashTableGetItem(LanguageHTable, name);
    if (def)
      result = def->id;
  } else
  ...
  if (result != LANG_IGNORE && (!noPretending) &&
...
extern langType getLanguageForCommand(const char *const command,
                                      langType startFrom) {
  const char *const tmp_command = baseFilename(command);
  char *tmp_spec;
  enum specType tmp_specType;

  return getNameOrAliasesLanguageAndSpec(
      tmp_command, startFrom, (const char **const) & tmp_spec, &tmp_specType);
}
...
static langType getPatternLanguageAndSpec(const char *const baseName,
  ...
  *spec = NULL;
  ...
  for (i = start_index; i < LanguageCount && result == LANG_IGNORE; ++i) {
    ...
    if (exts != NULL &&
        (tmp = stringListExtensionFinds(exts, fileExtension(baseName)))) {
      result = i;
      *spec = vStringValue(tmp);
...
extern langType getLanguageForFilename(const char *const filename,
                                       langType startFrom) {
  const char *const tmp_filename = baseFilename(filename);
  char *tmp_spec;
  enum specType tmp_specType;

  return getPatternLanguageAndSpec(
      tmp_filename, startFrom, (const char **const) & tmp_spec, &tmp_specType);
}

const char *scopeSeparatorFor(langType language, int kindIndex,
                              int parentKindIndex) {
  Assert(0 <= language && language < (int)LanguageCount);
  ...
  const scopeSeparator *sep =
      getScopeSeparator(kcb, kindIndex, parentKindIndex);
...
static bool processLangDefineScopesep(const langType language,
  ...
  if (parentKletter == '\0')
    ...
    error(FATAL,
          "the kind letter `F' in \"--%s\" option is reserved for \"file\" "
          "kind and no separator can be assigned to",
          option);
  else if (isalpha(parentKletter)) {
    kindDefinition *kdef =
        getKindForLetter(parser->kindControlBlock, parentKletter);
    if (kdef == NULL)
      error(FATAL,
            "the kind for letter `%c' specified in \"--%s\" option is not "
            "defined.",
  ...
  if (parentKindex == KIND_GHOST_INDEX)
    ...
    if (p[1] != '/')
      error(FATAL,
            "wrong separator specification in \"--%s\" option: no slash after "
            "parent kind letter `%c'",
  ...
  if (kletter == '\0')
    ...
    error(FATAL,
          "wrong separator specification in \"--%s\" option: don't specify "
          "slash char twice: %s",
          option, parameter);
  else if (kletter == ':')
    error(FATAL, "no child kind letter in \"--%s\" option", option);
  else if (kletter == KIND_WILDCARD_LETTER) {
    if (parentKindex != KIND_WILDCARD_INDEX && parentKindex != KIND_GHOST_INDEX)
      error(FATAL, "cannot use wild card for child kind unless parent kind is "
                   "also wild card or empty");
  ...
  if (parentKindex == KIND_GHOST_INDEX) {
    if (p[2] != ':')
      error(FATAL,
            "wrong separator specification in \"--%s\" option: cannot find a "
            "colon after child kind: %s",
            option, parameter);
    separator = p + 3;
  } else {
    if (p[3] != ':')
  ...
  defineScopeSeparator(parser->kindControlBlock, kindex, parentKindex,
                       separator);
  return true;
}

extern bool processScopesepOption(const char *const option,
                                  const char *const parameter) {
  langType language;
...
static unsigned int
nominateLanguageCandidatesForPattern(const char *const baseName,
                                     parserCandidate **candidates) {
  unsigned int count;
  langType i;
  const char *spec;
...
#define RING_SIZE 5
  ...
  int i, j;
  unsigned int k;
  const char *const prefix[] = {"vim:", "vi:", "ex:"};
...
static vString *extractMarkGeneric(
    MIO *input, vString *(*determiner)(const char *const, void *), void *data) {
  vString *const vLine = vStringNew();
  const char *const line = readLineRaw(vLine, input);
  vString *mode = NULL;
...
#define GLC_FOPEN_IF_NECESSARY(_glc_, _label_,                                 \
...
static langType tasteLanguage(struct getLangCtx *glc,
                              const struct taster *const tasters, int n_tasters,
                              langType *fallback);
 ...
 */
static int pickLanguageBySelection(selectLanguage selector, MIO *input,
                                   parserCandidate *candidates,
...
static void verboseReportCandidate(const char *header,
  ...
  for (i = 0; i < n_candidates; i++)
    verbose("			%u: %s (%s: \"%s\")\n", i,
            ...
            specTypeName[candidates[i].specType], candidates[i].spec);
}
...
static langType getSpecLanguageCommon(const char *const spec,
                                      ...
                                      unsigned int nominate(const char *const,
                                                            parserCandidate **),
                                      langType *fallback) {
  langType language;
  parserCandidate *candidates;
  ...
  verboseReportCandidate("candidates", candidates, n_candidates);

  n_candidates = sortAndFilterParserCandidates(candidates, n_candidates);
  verboseReportCandidate("candidates after sorting and filtering", candidates,
                         n_candidates);
  ...
  if (n_candidates == 1) {
    ...
    bool memStreamRequired =
        doesCandidatesRequireMemoryStream(candidates, n_candidates);

    GLC_FOPEN_IF_NECESSARY(glc, fopen_error, memStreamRequired);
    if (selector) {
      verbose("	selector: %p\n", selector);
      language = pickLanguageBySelection(selector, glc->input, candidates,
                                         n_candidates);
    } else {
...
static langType getSpecLanguage(const char *const spec, struct getLangCtx *glc,
                                langType *fallback) {
  return getSpecLanguageCommon(spec, glc, nominateLanguageCandidates, fallback);
}

static langType getPatternLanguage(const char *const baseName,
                                   struct getLangCtx *glc, langType *fallback) {
  return getSpecLanguageCommon(baseName, glc,
                               nominateLanguageCandidatesForPattern, fallback);
}
...
static langType tasteLanguage(struct getLangCtx *glc,
  ...
  for (i = 0; i < n_tasters; ++i) {
    ...
    if (NULL != spec) {
      ...
      language = getSpecLanguage(
          vStringValue(spec), glc,
...
getFileLanguageForRequestInternal(struct GetLanguageRequest *req) {
  ...
  verbose("	pattern: %s\n", baseName);
  language = getPatternLanguage(baseName, &glc, fallback + HINT_FILENAME);
...

  {
    ...
    if (templateBaseName) {
      ...
      mio_rewind(glc.input);
      language =
          getPatternLanguage(templateBaseName, &glc, fallback + HINT_TEMPLATE);
      if (language != LANG_IGNORE)
        goto cleanup;
    }
  }
  ...
  if (glc.input || ((fstatus = eStat(fileName)) && fstatus->exists)) {
    if ((fstatus && fstatus->isExecutable) || Option.guessLanguageEagerly) {
      GLC_FOPEN_IF_NECESSARY(&glc, cleanup, false);
      language = tasteLanguage(&glc, eager_tasters, 1, fallback + HINT_INTERP);
    }
    if (language != LANG_IGNORE)
      goto cleanup;
    ...
    if (Option.guessLanguageEagerly) {
      GLC_FOPEN_IF_NECESSARY(&glc, cleanup, false);
      language =
          tasteLanguage(&glc, eager_tasters + 1, ARRAY_SIZE(eager_tasters) - 1,
                        fallback + HINT_OTHER);
    }
  }
...
cleanup:
  ...
  for (i = 0; language == LANG_IGNORE && i < N_HINTS; i++) {
    language = fallback[i];
    if (language != LANG_IGNORE)
...
static langType getFileLanguageForRequest(struct GetLanguageRequest *req) {
  ...
  if (l == LANG_AUTO)
    ...
    error(FATAL, "%s parser specified with --language-force is disabled",
          getLanguageName(l));
...
extern void installLanguageMapDefault(const langType language) {
  ...
  if (parser->def->patterns == NULL)
    ...
    parser->currentPatterns = stringListNewFromArgv(parser->def->patterns);
  }
  if (parser->def->extensions == NULL)
    parser->currentExtensions = stringListNew();
  else {
    parser->currentExtensions = stringListNewFromArgv(parser->def->extensions);
  }
  BEGIN_VERBOSE(vfp);
  {
...
extern void installLanguageAliasesDefault(const langType language) {
  ...
  if (parser->def->aliases == NULL)
    ...
    parser->currentAliases = stringListNewFromArgv(parser->def->aliases);
  }
  BEGIN_VERBOSE(vfp);
  if (parser->currentAliases != NULL)
    for (unsigned int i = 0; i < stringListCount(parser->currentAliases); ++i)
      fprintf(vfp, " %s",
              vStringValue(stringListItem(parser->currentAliases, i)));
  putc('\n', vfp);
  END_VERBOSE();
}
...
extern bool removeLanguagePatternMap(const langType language,
                                     const char *const pattern) {
  bool result = false;
...
extern bool removeLanguageExtensionMap(const langType language,
                                       const char *const extension) {
  bool result = false;
...
static void initializeParserOne(langType lang) {
  ...
  Assert(
      !doesParserUseKind(parser->kindControlBlock, parser->fileKind->letter));
...
static void
linkDependenciesAtInitializeParsing(parserDefinition *const parser) {
  unsigned int i;
  parserDependency *d;
  langType upper;
  ...
  for (i = 0; i < parser->dependencyCount; i++) {
    ...
    linkDependencyAtInitializeParsing(
        d->type, upperParser->def, upperParser->slaveControlBlock,
        upperParser->kindControlBlock, parser,
        (LanguageTable + parser->id)->kindControlBlock, d->data);
  }
}
...
extern void initializeParsing(void) {
  ...
  LanguageHTable =
      hashTableNew(127, hashCstrcasehash, hashCstrcaseeq, NULL, NULL);
...
static void optlibRunBaseParser(void) { scheduleRunningBaseparser(0); }

static bool optlibIsDedicatedSubparser(parserDefinition *def) {
  return (def->dependencies && (def->dependencies->type == DEPTYPE_SUBPARSER) &&
          ((subparser *)def->dependencies->data)->direction &
...
static void pre_lang_def_flag_base_long(const char *const optflag,
  ...
  if (param[0] == '\0') {
    error(WARNING,
          "No base parser specified for \"%s\" flag of --langdef option",
          optflag);
    return;
  }
...
static void
pre_lang_def_flag_direction_long(const char *const optflag,
...
static void
pre_lang_def_flag_autoFQTag_long(const char *const optflag,
                                 const char *const param CTAGS_ATTR_UNUSED,
                                 void *data) {
  struct preLangDefFlagData *flag_data = data;
...
static flagDefinition PreLangDefFlagDef[] = {
    {'\0', "base", NULL, pre_lang_def_flag_base_long, "BASEPARSER",
     "utilize as a base parser"},
    {'\0', LANGDEF_FLAG_DEDICATED, NULL, pre_lang_def_flag_direction_long, NULL,
     "make the base parser dedicated to this subparser"},
    {'\0', LANGDEF_FLAG_SHARED, NULL, pre_lang_def_flag_direction_long, NULL,
     "share the base parser with the other subparsers"},
    {'\0', LANGDEF_FLAG_BIDIR, NULL, pre_lang_def_flag_direction_long, NULL,
     "utilize the base parser both 'dedicated' and 'shared' way"},
    {'\0', "_autoFQTag", NULL, pre_lang_def_flag_autoFQTag_long, NULL,
...
extern void processLanguageDefineOption(const char *const option,
  ...
  if (name[0] == '\0') {
    ...
    eFree(name);
    error(FATAL, "\"all\" is reserved; don't use it as the name for defining a "
                 "new language");
    ...
    /* name cannot be freed because it is used in the FATAL message. */
    /* We accept '_'.
     * We accept # and + because they are already used in C# parser and C++
     * parser.
...
extern bool isLanguageRoleEnabled(const langType language, int kindIndex,
                                  int roleIndex) {
  return isRoleEnabled(LanguageTable[language].kindControlBlock, kindIndex,
                       roleIndex);
}
...
static bool enableLanguageKindForLetter(const langType language, const int kind,
                                        const bool mode) {
  bool result = false;
...
static bool enableLanguageKindForName(const langType language,
                                      const char *const name, const bool mode) {
  bool result = false;
...
static void processLangKindDefinition(const langType language,
  ...
  while ((c = *p++) != '\0') {
    ...
    case '{':
      if (inLongName)
        error(FATAL, "unexpected character in kind specification: \'%c\'", c);
      inLongName = true;
      break;
    case '}':
      if (!inLongName)
      ...
      if (!r)
        error(WARNING, "Unsupported kind: '%s' for --%s option", k, option);
    ...
    default:
      if (inLongName)
        ...
        if (!r)
          error(WARNING, "Unsupported kind: '%c' for --%s option", c, option);
      }
      break;
    }
  }
...
static flagDefinition PreKindDefFlagDef[] = {
    {'\0', "_refonly", NULL, pre_kind_def_flag_refonly_long, NULL,
...
static bool processLangDefineKind(const langType language,
  ...
  if (!isalpha(letter))
    error(FATAL, "the kind letter given in \"--%s\" option is not an alphabet",
          option);
  else if (letter == KIND_FILE_DEFAULT_LETTER)
    error(
        FATAL,
        "the kind letter `F' in \"--%s\" option is reserved for \"file\" kind",
        option);
  else if (getKindForLetter(parser->kindControlBlock, letter)) {
    error(WARNING,
          "the kind for letter `%c' specified in \"--%s\" option is already "
          "defined.",
          letter, option);
    return true;
  }
  ...
  if (p[1] != ',')
    error(FATAL,
          "wrong kind definition in \"--%s\" option: no comma after letter",
          option);
  ...
  if (strncmp(name_start, KIND_FILE_DEFAULT_NAME, name_len) == 0)
    error(FATAL,
          "the kind name " KIND_FILE_DEFAULT_NAME
  ...
  if (getKindForName(parser->kindControlBlock, name)) {
    error(WARNING,
          "the kind for name `%s' specified in \"--%s\" option is already "
          "defined.",
          name, option);
    eFree(name);
    return true;
...
static bool processLangDefineRole(const langType language,
  ...
  if (!isalnum(kletter))
    error(FATAL,
          "the kind letter given in \"--%s\" option is not an alphabet or a "
          "number",
          option);
  else if (kletter == KIND_FILE_DEFAULT_LETTER)
    error(FATAL,
          "the kind letter `F' in \"--%s\" option is reserved for \"file\" "
          "kind and no role can be attached to",
          option);
  ...
  if (p[1] != '.')
    error(FATAL,
          "wrong role definition in \"--%s\" option: no period after kind "
          "letter `%c'",
          option, kletter);
  ...
  while (p != tmp_end) {
    if (!isalnum(*p))
      error(FATAL, "unacceptable char as part of role name in \"--%s\" option",
            option);
    p++;
  }
  ...
  if (getRoleForName(parser->kindControlBlock, kdef->id, name)) {
    error(WARNING,
          "the role for name `%s' specified in \"--%s\" option is already "
          "defined.",
          name, option);
    eFree(name);
    return true;
...
extern bool processKinddefOption(const char *const option,
                                 const char *const parameter) {
  langType language;
...
extern bool processRoledefOption(const char *const option,
                                 const char *const parameter) {
  langType language;
...
extern bool processKindsOption(const char *const option,
                               const char *const parameter) {
#define PREFIX "kinds-"
#define PREFIX_LEN strlen(PREFIX)
  ...
  if (dash != NULL &&
    ...
    if ((len == 3) && (strcmp(langName, RSV_LANG_ALL) == 0)) {
      error(WARNING, "\"--%s\" option is obsolete; use \"--kinds-%s\" instead",
            option, langName);
      ...
      if (language == LANG_IGNORE)
        error(WARNING, "Unknown language \"%s\" in \"%s\" option", langName,
              option);
      else
...
extern void printLanguageRoles(const langType language, const char *kindspecs,
  ...
  roleColprintTablePrint(table, (language != LANG_AUTO), withListHeader,
                         machinable, fp);
...
static void printParameters(struct colprintTable *table, langType language) {
  ...
  if (lang->parameterHandlerTable != NULL) {
    for (unsigned int i = 0; i < lang->parameterHandlerCount; ++i)
      paramColprintAddParameter(table, language,
                                lang->parameterHandlerTable + i);
  }
}
...
extern void printLanguageParameters(const langType language,
  ...
  paramColprintTablePrint(table, (language != LANG_AUTO), withListHeader,
                          machinable, fp);
  colprintTableDelete(table);
}
...
extern bool processAliasOption(const char *const option,
                               const char *const parameter) {
  langType language;
...
#define PREFIX "alias-"
  if (strcmp(option, "alias-" RSV_LANG_ALL) == 0) {
    if ((parameter[0] == '\0') ||
      ...
      error(WARNING,
            "Use \"%s\" option for reset (\"default\") or clearing (\"\")",
            option);
      return false;
    }
    return true;
...
static void printMaps(const langType language, langmapType type) {
  ...
  if (parser->currentPatterns != NULL && (type & LMAP_PATTERN))
    for (i = 0; i < stringListCount(parser->currentPatterns); ++i)
      printf(" %s", vStringValue(stringListItem(parser->currentPatterns, i)));
  if (parser->currentExtensions != NULL && (type & LMAP_EXTENSION))
    for (i = 0; i < stringListCount(parser->currentExtensions); ++i)
      printf(" *.%s",
             vStringValue(stringListItem(parser->currentExtensions, i)));
  putchar('\n');
}
...
static void mapColprintAddLanguage(struct colprintTable *table,
  ...
  if ((type & LMAP_PATTERN) &&
      (0 < (count = stringListCount(parser->currentPatterns)))) {
    for (i = 0; i < count; i++) {
  ...
  if ((type & LMAP_EXTENSION) &&
      (0 < (count = stringListCount(parser->currentExtensions)))) {
    for (i = 0; i < count; i++) {
      line = colprintTableGetNewLine(table);
...
extern void printLanguageMaps(const langType language, langmapType type,
  ...
  if (type & LMAP_TABLE_OUTPUT) {
    colprintTablePrint(table, (language == LANG_AUTO) ? 0 : 1, withListHeader,
                       machinable, fp);
    colprintTableDelete(table);
  }
}
...
static void aliasColprintAddLanguage(struct colprintTable *table,
  ...
  if (parser->currentAliases &&
      (0 < (count = stringListCount(parser->currentAliases)))) {
...
extern void printLanguageAliases(const langType language, bool withListHeader,
...
static void printLanguage(const langType language, parserDefinition **ltable) {
  ...
  if (lang->kindTable != NULL || (lang->method & METHOD_REGEX))
    printf("%s%s\n", lang->name,
           isLanguageEnabled(lang->id) ? "" : " [disabled]");
}
...
extern void printLanguageList(void) {
  ...
  qsort(ltable, LanguageCount, sizeof(parserDefinition *),
...
extern bool processExtradefOption(const char *const option,
                                  const char *const parameter) {
  langType language;

  language = getLanguageComponentInOption(option, "_"
                                                  "extradef-");
  if (language == LANG_IGNORE)
    return false;
...
extern bool processFielddefOption(const char *const option,
                                  const char *const parameter) {
  langType language;
...
static bool createTagsWithFallback1(const langType language,
  ...
  while ((whyRescan = createTagsForFile(language, ++passCount)) !=
         RESCAN_NONE) {
    if (useCork) {
      uncorkTagFile();
      corkTagFile();
...
extern bool
runParserInNarrowedInputStream(const langType language, unsigned long startLine,
                               long startCharOffset, unsigned long endLine,
                               long endCharOffset,
                               unsigned long sourceLineOffset, int promise) {
  bool tagFileResized;
  ...
  verbose("runParserInNarrowedInputStream: %s; "
          ...
          getLanguageName(language), getInputFileName(), startLine,
          startCharOffset, sourceLineOffset, endLine, endCharOffset);

  pushNarrowedInputStream(startLine, startCharOffset, endLine, endCharOffset,
                          sourceLineOffset, promise);
...
static bool createTagsWithFallback(const char *const fileName,
                                   const langType language, MIO *mio) {
  ...
  tagFileResized = createTagsWithFallback1(language, &exclusive_subparser);
  tagFileResized = forcePromises() ? true : tagFileResized;

  pushLanguage((exclusive_subparser == LANG_IGNORE) ? language
                                                    : exclusive_subparser);
...
extern bool processLanguageEncodingOption(const char *const option,
                                          const char *const parameter) {
  langType language;
...
extern bool doesParserRequireMemoryStream(const langType language) {
  ...
  if (lang->tagXpathTableCount > 0 || lang->useMemoryStreamInput) {
...
static bool parseMio(const char *const fileName, langType language, MIO *mio,
                     bool useSourceFileTagPath, void *clientData) {
  bool tagFileResized = false;
...
extern bool parseRawBuffer(const char *fileName, unsigned char *buffer,
                           size_t bufferSize, const langType language,
                           void *clientData) {
  MIO *mio = NULL;
  bool r;
...
static void matchLanguageMultilineRegexCommon(
...
extern void
processLanguageMultitableExtendingOption(langType language,
                                         const char *const parameter) {
  const char *src;
  char *dist;
  const char *tmp;
  ...
  if (tmp == parameter)
    error(FATAL, "the name of source table is empty in table extending: %s",
          parameter);

  src = tmp + 1;
  if (!*src)
    error(FATAL, "the name of dist table is empty in table extending: %s",
          parameter);
...
static bool
lregexQueryParserAndSubparsers(const langType language,
                               bool (*predicate)(struct lregexControlBlock *)) {
  bool r;
  subparser *tmp;
...
extern void addLanguageCallbackRegex(const langType language,
                                     const char *const regex,
                                     const char *const flags,
                                     const regexCallback callback,
                                     bool *disabled, void *userData) {
  addCallbackRegex((LanguageTable + language)->lregexControlBlock, regex, flags,
                   callback, disabled, userData);
}
...
extern bool hasLanguageScopeActionInRegex(const langType language) {
  ...
  hasScopeAction =
      lregexQueryParserAndSubparsers(language, hasScopeActionInRegex);
  popLanguage();

  return hasScopeAction;
}

extern void matchLanguageRegex(const langType language,
...
extern bool processTabledefOption(const char *const option,
                                  const char *const parameter) {
  langType language;
  ...
  if (lang->tagRegexTable != NULL) {
    for (i = 0; i < lang->tagRegexCount; ++i) {
      if (lang->tagRegexTable[i].mline)
        addTagMultiLineRegex(
            parser->lregexControlBlock, lang->tagRegexTable[i].regex,
            lang->tagRegexTable[i].name, lang->tagRegexTable[i].kinds,
            lang->tagRegexTable[i].flags, (lang->tagRegexTable[i].disabled));
      else
        addTagRegex(parser->lregexControlBlock, lang->tagRegexTable[i].regex,
                    lang->tagRegexTable[i].name, lang->tagRegexTable[i].kinds,
...
static void installKeywordTable(const langType language) {
  ...
  if (lang->keywordTable != NULL) {
    for (i = 0; i < lang->keywordCount; ++i)
      addKeyword(lang->keywordTable[i].name, language,
...
extern xpathFileSpec *getXpathFileSpec(const langType language,
                                       unsigned int nth) {
  parserDefinition *lang;
  ...
  for (i = 0; i < kindCount; ++i) {
    ...
    for (j = 0; j < kind->separatorCount; ++j) {
      ...
      if (sep->parentKindIndex == KIND_WILDCARD_INDEX) {
        ...
        upperKind = getLanguageKind(language, sep->parentKindIndex);
        if (!upperKind)
          continue;
      ...
      r = writePseudoTag(pdesc, vStringValue(sepval), name, lang->name) || r;
    }
  }
...
static bool makeKindDescriptionPseudoTag(kindDefinition *kind,
  ...
  data->written |= writePseudoTag(data->pdesc, vStringValue(letter_and_name),
                                  vStringValue(description), data->langName);
...
static void setupAnon(void) { parsersUsedInCurrentInput = ptrArrayNew(NULL); }

static void teardownAnon(void) { ptrArrayDelete(parsersUsedInCurrentInput); }
...
extern void scheduleRunningBaseparser(int dependencyIndex) {
  ...
  if (dependencyIndex == RUN_DEFAULT_SUBPARSERS)
    ...
    useSpecifiedSubparser(base_parser->slaveControlBlock, dep->data);
...
extern void printLanguageSubparsers(const langType language,
  ...
  if (language == LANG_AUTO) {
    for (int i = 0; i < (int)LanguageCount; i++) {
      ...
      subparserColprintAddSubparsers(table, parser->slaveControlBlock);
    }
  } else {
    parser = (LanguageTable + language);
    subparserColprintAddSubparsers(table, parser->slaveControlBlock);
  ...
  subparserColprintTablePrint(table, withListHeader, machinable, fp);
  colprintTableDelete(table);
}
...
extern void printLangdefFlags(bool withListHeader, bool machinable, FILE *fp) {
  ...
  flagsColprintAddDefinitions(table, PreLangDefFlagDef,
...
extern void printKinddefFlags(bool withListHeader, bool machinable, FILE *fp) {
  ...
  flagsColprintAddDefinitions(table, PreKindDefFlagDef,
...
extern void addLanguageTagMultiTableRegex(
    const langType language, const char *const table_name,
    const char *const regex, const char *const name, const char *const kinds,
    const char *const flags, bool *disabled) {
  parserObject *const parser = LanguageTable + language;
  addTagMultiTableRegex(parser->lregexControlBlock, table_name, regex, name,
                        kinds, flags, disabled);
}
...
#define pretendOptionPrefix "_pretend-"
  new_language =
      getLanguageComponentInOptionFull(option, pretendOptionPrefix, true);
  ...
  if (old_language == LANG_IGNORE)
    error(FATAL, "Unknown language \"%s\" in option \"--%s=%s\"", parameter,
          option, parameter);
  ...
  if (LanguageTable[new_language].pretendingAsLanguage != LANG_IGNORE) {
    error(FATAL, "%s parser pretends as %s already\n",
          ...
          getLanguageNameFull(LanguageTable[new_language].pretendingAsLanguage,
                              true));
  }
  if (LanguageTable[old_language].pretendedAsLanguage != LANG_IGNORE) {
    error(FATAL, "%s parser is pretended as %s already\n",
          ...
          getLanguageNameFull(LanguageTable[old_language].pretendedAsLanguage,
                              true));
  }
...
#if defined(DEBUG) && defined(HAVE_SECCOMP)
    {true, 'P', "callGetPPid",
     "trigger calling getppid(2) that seccomp sandbox disallows"},
#endif
    {false, 'd', "disabled", "a kind disabled by default",
     .referenceOnly = false, ATTACH_ROLES(CTST_DisabledKindRoles)},
    {true, 'e', "enabled", "a kind enabled by default", .referenceOnly = false,
     ATTACH_ROLES(CTST_EnabledKindRoles)},
    {true, 'r', "roles", "emit a tag with multi roles", .referenceOnly = true,
     ATTACH_ROLES(CTST_RolesKindRoles)},
    {false, 'R', "rolesDisabled",
     "emit a tag with multi roles(disabled by default)", .referenceOnly = true,
     ATTACH_ROLES(CTST_RolesDisabledKindRoles)},
};
      ...
      if ((c == CTST_Kinds[i].letter && i != K_NO_LETTER) ||
        ...
        case K_ENABLED: {
          ...
          if (found_enabled_disabled[i == K_DISABLED]++ == 0) {
            ...
            name = (i == K_DISABLED) ? "disable-kind-no-role"
                                     : "enabled-kind-no-role";
          } else if (found_enabled_disabled[i == K_DISABLED]++ == 1) {
            role = (i == K_DISABLED) ? R_DISABLED_KIND_DISABLED_ROLE
                                     : R_ENABLED_KIND_DISABLED_ROLE;
            name = (i == K_DISABLED) ? "disable-kind-disabled-role"
                                     : "enabled-kind-disabled-role";
          } else {
            role = (i == K_DISABLED) ? R_DISABLED_KIND_ENABLED_ROLE
                                     : R_ENABLED_KIND_ENABLED_ROLE;
            name = (i == K_DISABLED) ? "disable-kind-enabled-role"
