...
typedef void (*promiseInputModifier)(unsigned char *input, size_t size,
                                     unsigned long startLine,
                                     long startCharOffset,
...
static void attachPromiseModifier(int promise, promiseInputModifier modifier,
                                  promiseDestroyAttachedData destroyData,
                                  void *data);

int makePromise(const char *parser, unsigned long startLine,
                long startCharOffset, unsigned long endLine, long endCharOffset,
                unsigned long sourceLineOffset) {
  struct promise *p;
  int r;
  langType lang;
...
bool forcePromises(void) {
  ...
  for (i = 0; i < promise_count; ++i) {
    ...
    tagFileResized = runParserInNarrowedInputStream(
                         p->lang, p->startLine, p->startCharOffset, p->endLine,
                         p->endCharOffset, p->sourceLineOffset, i)
                         ? true
                         : tagFileResized;
  }
...
int getLastPromise(void) { return promise_count - 1; }

static unsigned char *fill_or_skip(unsigned char *input,
                                   unsigned char *input_end, bool filling) {
  if (!(input < input_end))
    return NULL;
...
static void line_filler(unsigned char *input, size_t size,
                        ...
                        unsigned long endLine, long endCharOffset, void *data) {
...
void promiseAttachLineFiller(int promise, ulongArray *lines) {
  attachPromiseModifier(promise, line_filler,
                        (promiseDestroyAttachedData)ulongArrayDelete, lines);
}
...
void runModifiers(int promise, unsigned long startLine, long startCharOffset,
  ...
  for (int i = ptrArrayCount(modifiers); i > 0; i--) {
    ...
    m->modifier(input, size, startLine, startCharOffset, endLine, endCharOffset,
                m->data);
  }
