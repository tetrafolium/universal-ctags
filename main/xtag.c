...
static xtagDefinition xtagDefinitions[] = {
    {true, 'F', "fileScope", "Include tags of file scope"},
    {false, 'f', "inputFile",
     "Include an entry for the base file name of every input file", NULL, NULL,
     enableFileKind},
    {false, 'p', "pseudo", "Include pseudo tags", isPseudoTagsEnabled,
     isPseudoTagsFixed},
    {false, 'q', "qualified",
     "Include an extra class-qualified tag entry for each tag"},
    {false, 'r', "reference", "Include reference tags"},
    {false, 'g', "guest", "Include tags generated by guest parsers"},
    {true, 's', "subparser", "Include tags generated by subparsers"},
    {true, '\0', "anonymous", "Include tags for non-named objects like lambda"},
};
...
typedef bool (*xtagPredicate)(xtagObject *pobj, langType language,
                              const void *user_data);
static xtagType getXtagTypeGeneric(xtagPredicate predicate, langType language,
                                   const void *user_data) {
...
static bool xtagEqualByNameAndLanguage(xtagObject *pobj, langType language,
                                       const void *user_data) {
...
static void xtagColprintAddLine(struct colprintTable *table, int xtype) {
  ...
  colprintLineAppendColumnChar(
      line, (xdef->letter == NUL_XTAG_LETTER) ? '-' : xdef->letter);
  colprintLineAppendColumnCString(line, xdef->name);
  colprintLineAppendColumnBool(line, isXtagEnabled(xdef->xtype));
  colprintLineAppendColumnCString(line, xobj->language == LANG_IGNORE
                                            ? RSV_NONE
...
extern int getXtagOwner(xtagType type) { return getXtagObject(type)->language; }
...
extern int countXtags(void) { return xtagObjectUsed; }
...
extern int defineXtag(xtagDefinition *def, langType language) {
  ...
  verbose("Add extra[%d]: %s,%s in %s\n", def->xtype, def->name,
          def->description, getLanguageName(language));
