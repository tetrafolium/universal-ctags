...
struct slaveControlBlock {
  slaveParser *slaveParsers; /* The parsers on this list must be initialized
                                when this parser is initialized. */
...
static void attachSubparser(struct slaveControlBlock *base_sb,
                            subparser *subparser) {
...
extern void freeSlaveControlBlock(struct slaveControlBlock *cb) { eFree(cb); }
...
extern void initializeDependencies(parserDefinition *parser,
  ...
  while (sp != NULL) {
    ...
    if (sp->type == DEPTYPE_KIND_OWNER ||
      ...
      if (sp->type == DEPTYPE_SUBPARSER && isXtagEnabled(XTAG_SUBPARSER)) {
...
extern void useSpecifiedSubparser(struct slaveControlBlock *controlBlock,
                                  subparser *s) {
...
extern subparser *
teardownSubparsersInUse(struct slaveControlBlock *controlBlock) {
  subparser *tmp;
  subparser *s = NULL;
...
extern bool doesSubparserRun(void) { return subparserDepth; }
...
static int subparserColprintCompareLines(struct colprintLine *a,
