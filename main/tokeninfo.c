...
retry:
  if (klass->pool)
    ...
    klass->pool = objPoolNew(klass->nPreAlloc, createToken, deleteToken,
                             clearToken, klass);
    goto retry;
  }
...
void tokenDelete(tokenInfo *token) { objPoolPut(token->klass->pool, token); }
...
void tokenRead(tokenInfo *token) { tokenReadFull(token, NULL); }
...
bool tokenSkipToTypeFull(tokenInfo *token, tokenType t, void *data) {
  while (!(tokenIsEOF(token) || token->type == t))
...
void tokenUnread(tokenInfo *token) { tokenUnreadFull(token, NULL); }
