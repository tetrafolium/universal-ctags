...
static void eFree(void *const ptr) { free(ptr); }
 ...
 *
 * A #MIO object is created using mio_new_file(), mio_new_memory() or
 * mio_new_mio(), depending on whether you want file or in-memory operations.
 * Its life is managed by reference counting. Just after calling one of
 * functions for creating, the count is 1. mio_ref() increments the counter.
 * mio_unref() decrements it. When the counter becomes 0, the #MIO object will
 * be destroyed in mio_unref(). There is also some other convenient API to
 * create file-based #MIO objects for more complex cases, such as
 * mio_new_file_full() and mio_new_fp().
 *
 ...
 *
 * Returns: Error code obtained from the registered MIOFCloseFunc or 0 on
 * success.
 */
    ...
    if (new_size == ULONG_MAX) {
      ...
      if (new_size > mio->impl.mem.size) {
        if (new_size <= mio->impl.mem.allocated_size) {
          ...
          newsize =
              MAX(mio->impl.mem.allocated_size + MIO_CHUNK_SIZE, new_size);
 ...
 * @user_data_free_func: a call back function to destroy the data object passed
 * as @user_data
 *
 * Attach any data object to a #MIO object. Attached data can be got with
 * mio_get_user_data(). The attached data is destroyed when new data is attached
 * to the #MIO object, or #the MIO object itself is destroyed. For destroying
 * the data
 ...
 *
 */
void *mio_get_user_data(MIO *mio) { return mio->udata.d; }
