...
extern bool openConverter(const char *inputEncoding,
  ...
  if (iconv_fd == (iconv_t)-1) {
    error(FATAL, "failed opening encoding from '%s' to '%s'", inputEncoding,
          outputEncoding);
    return false;
  }
  return true;
...
extern bool isConverting() { return iconv_fd != (iconv_t)-1; }
