...
static void deleteChar(void *c) { eFree(c); }
...
static void uugcActivate(void) {
  ...
  if (uugcCharPool == NULL) {
    uugcCharPool = objPoolNew(256, newChar, deleteChar, NULL, NULL);
...
extern unsigned long uwiGetLineNumber(void) { return uugcGetLineNumber(); }

extern MIOPos uwiGetFilePosition(void) { return uugcGetFilePosition(); }
...
extern void uwiPushMarker(void) {
  ...
  if (uwiCurrentMarker)
    ...
    uwiCurrentMarker = uwiMarkerStack;
...
extern void uwiPopMarker(const int upto, const bool revertChars) {
  ...
  if (uwiCurrentMarker == uwiMarkerStack)
    uwiCurrentMarker = NULL;
  else
    uwiCurrentMarker--;
}
...
extern void uwiDropMaker() { uwiPopMarker(0, false); }
