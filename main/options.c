...
static optionDescription LongOptionDescription[] = {
    ...
    {1, "      Add a pattern detecting a name, can be used as an alternative "
        "name"},
    {1, "      for LANG."},
    {1, "  --append=[yes|no]"},
    ...
    {1, "  --fields=[+|-]flags"},
    {1, "      Include selected extension fields (flags: "
        "\"aCeEfFikKlmnNpPrRsStxzZ\") [fks]."},
    ...
    {1, "       o emacs mode specification at the beginning and end of input "
        "file, and"},
    ...
    {1, "       For each line, associated language name is printed when "
        "\"all\" is"},
    ...
    {1, "       Output list of flags which can be used in a multiline regex "
        "parser definition."},
    {1, "  --list-params=[language|all]"},
    {1, "       Output list of language parameters. This works with "
        "--machinable."},
    {0, "  --list-pseudo-tags"},
    {0, "       Output list of pseudo tags."},
    {1, "  --list-regex-flags"},
    {1, "       Output list of flags which can be used in a regex parser "
        "definition."},
    {1, "  --list-roles=[[language|all].[kindspecs|*]]"},
    {1, "       Output list of all roles of tag kind(s) specified for "
        "language(s)."},
    ...
    {1, "       Unlike --langmap, this doesn't take a list; only one file name "
        "pattern"},
    {1, "       or one file extension can be specified at once."},
    {1, "       Unlike --langmap the change with this option affects mapping "
        "of <LANG> only."},
    {1, "  --maxdepth=N"},
#ifdef RECURSE_SUPPORTED
    ...
    {1, "       Define multiline regular expression for locating tags in "
        "specific language."},
    {1, "  --options=path"},
    {1, "       Specify file(or directory) from which command line options "
        "should be read."},
    {1, "  --options-maybe=path"},
    {1, "       Do the same as --options but this doesn't make an error for "
        "non-existing file."},
    {1, "  --optlib-dir=[+]DIR"},
    {1, "      Add or set DIR to optlib search path."},
#ifdef HAVE_ICONV
    {1, "  --output-encoding=encoding"},
    {1, "      The encoding to write the tag file in. Defaults to UTF-8 if "
        "--input-encoding"},
    ...
    {1, "       Set <LANG> specific parameter. Available parameters can be "
        "listed with --list-params."},
    {0, "  --pattern-length-limit=N"},
    {0, "      Cutoff patterns of tag entries after N characters. Disable by "
        "setting to 0. [96]"},
    {0, "  --print-language"},
    {0, "       Don't make tags file but just print the guessed language name "
        "for"},
    {0, "       input file."},
    {0, "  --pseudo-tags=[+|-]ptag"},
    {0, "  --pseudo-tags=*"},
    ...
    {0, "       Should paths be relative to location of tag file [no; yes when "
        "-e]?"},
    {0, "       always: be relative even if input files are passed in with "
        "absolute paths"},
    {0, "       never:  be absolute even if input files are passed in with "
        "relative paths"},
...
static optionDescription ExperimentalLongOptionDescription[] = {
    ...
    {1, "       Define new extra for <LANG>. \"--extras-<LANG>=+{name}\" "
        "enables it."},
    {1, "  --_fatal-warnings"},
...
#ifdef HAVE_SECCOMP
    {0, "       Enter file I/O limited interactive mode if sandbox is "
        "specified. [default]"},
#endif
#endif
    ...
    {1, "       Output list of flags which can be used in a multitable regex "
        "parser definition."},
    ...
    {1, "       Define multitable regular expression for locating tags in "
        "specific language."},
    ...
    {1,
     "       Define new role for kind specified with <kind_letter> in <LANG>."},
    {1,
...
static const char *const StageDescription[] = {
    ...
    [OptionLoadingStageXdg] =
        "file(s) under $XDG_CONFIG_HOME and $HOME/.config",
...
static bool parseAllConfigurationFilesOptionsInDirectory(
    const char *const fileName, stringList *const already_loaded_files);
...
static bool isFalse(const char *parameter) {
  return (bool)(strcasecmp(parameter, "0") == 0 ||
...
static bool isTrue(const char *parameter) {
  return (bool)(strcasecmp(parameter, "1") == 0 ||
...
static void processExcludeOption(const char *const option CTAGS_ATTR_UNUSED,
                                 const char *const parameter) {
...
static void processExtraTagsOption(const char *const option,
  ...
  while ((c = *p++) != '\0') {
    ...
    case '{':
      if (inLongName)
        error(FATAL, "unexpected character in extra specification: \'%c\'", c);
      inLongName = true;
      break;
    case '}':
      if (!inLongName)
      ...
      if (t == XTAG_UNKNOWN)
        error(WARNING, "Unsupported parameter '{%s}' for \"%s\" option", x,
              option);
      else
        enableXtag(t, mode);
...
static void processFieldsOption(const char *const option,
  ...
  while ((c = *p++) != '\0')
    ...
    case '{':
      if (inLongName)
        error(FATAL, "unexpected character in field specification: \'%c\'", c);
      inLongName = true;
      break;
    case '}':
      if (!inLongName)
...
static void
processFilterTerminatorOption(const char *const option CTAGS_ATTR_UNUSED,
                              const char *const parameter) {
...
#ifdef HAVE_ICONV
static void
processInputEncodingOption(const char *const option CTAGS_ATTR_UNUSED,
                           const char *const parameter) {
...
static void
processOutputEncodingOption(const char *const option CTAGS_ATTR_UNUSED,
                            const char *const parameter) {
  if (Option.outputEncoding)
...
static void
processListExcludesOption(const char *const option CTAGS_ATTR_UNUSED,
  ...
  colprintTablePrint(table, 0, localOption.withListHeader,
                     localOption.machinable, stdout);
...
static void
processListFeaturesOption(const char *const option CTAGS_ATTR_UNUSED,
                          const char *const parameter CTAGS_ATTR_UNUSED) {
  int i;

  struct colprintTable *table =
      colprintTableNew("L:NAME", "L:DESCRIPTION", NULL);
...
static void processListFieldsOption(const char *const option CTAGS_ATTR_UNUSED,
  ...
  fieldColprintTablePrint(table, localOption.withListHeader,
                          localOption.machinable, stdout);
  colprintTableDelete(table);
  exit(0);
}
...
static void
processHelpOptionCommon(const char *const option CTAGS_ATTR_UNUSED,
                        const char *const parameter CTAGS_ATTR_UNUSED,
...
#ifdef HAVE_JANSSON
static void processInteractiveOption(const char *const option CTAGS_ATTR_UNUSED,
                                     const char *const parameter) {
  ...
  if (parameter && (strcmp(parameter, "sandbox") == 0)) {
    ...
    error(FATAL, "Unknown option argument \"%s\" for --%s option", parameter,
          option);
 ...
 */
static char *extractMapFromParameter(const langType language, char *parameter,
                                     char **tail, bool *pattern_p,
                                     char *(*skip)(char *)) {
  char *p = NULL;
...
static char *addLanguageMap(const langType language, char *map_parameter,
  ...
  map = extractMapFromParameter(language, map_parameter, &p, &pattern_p,
                                skipPastMap);
...
extern bool processMapOption(const char *const option,
                             const char *const parameter) {
  langType language;
  const char *spec;
  char *map_parameter;
  ...
  if (clear) {
    ...
    verbose("    %s %s %s %s language map:", op == '+' ? "Adding" : "Removing",
            spec, op == '+' ? "to" : "from", getLanguageName(language));
...
extern bool processParamOption(const char *const option,
...
static void
processLicenseOption(const char *const option CTAGS_ATTR_UNUSED,
                     const char *const parameter CTAGS_ATTR_UNUSED) {
  printProgramIdentification();
  puts("");
  puts(License1);
...
static void processListAliasesOption(const char *const option,
                                     const char *const parameter) {
  if (parameter[0] == '\0' || strcasecmp(parameter, RSV_LANG_ALL) == 0)
    printLanguageAliases(LANG_AUTO, localOption.withListHeader,
                         localOption.machinable, stdout);
  else {
    langType language = getNamedLanguage(parameter, 0);
    if (language == LANG_IGNORE)
      ...
      printLanguageAliases(language, localOption.withListHeader,
                           localOption.machinable, stdout);
  }
  exit(0);
}
...
static void
processListExtrasOption(const char *const option CTAGS_ATTR_UNUSED,
                        const char *const parameter CTAGS_ATTR_UNUSED) {
  ...
  xtagColprintTablePrint(table, localOption.withListHeader,
                         localOption.machinable, stdout);
  colprintTableDelete(table);
  exit(0);
}
...
static void processListKindsOption(const char *const option,
  ...
  if (parameter[0] == '\0' || strcasecmp(parameter, RSV_LANG_ALL) == 0)
    printLanguageKinds(LANG_AUTO, print_all, localOption.withListHeader,
                       localOption.machinable, stdout);
  else {
    langType language = getNamedLanguage(parameter, 0);
    if (language == LANG_IGNORE)
      ...
      printLanguageKinds(language, print_all, localOption.withListHeader,
                         localOption.machinable, stdout);
  }
  exit(0);
}
...
static void processListParametersOption(const char *const option,
                                        const char *const parameter) {
  if (parameter[0] == '\0' || strcasecmp(parameter, RSV_LANG_ALL) == 0)
    printLanguageParameters(LANG_AUTO, localOption.withListHeader,
                            localOption.machinable, stdout);
  else {
    langType language = getNamedLanguage(parameter, 0);
    if (language == LANG_IGNORE)
      ...
      printLanguageParameters(language, localOption.withListHeader,
                              localOption.machinable, stdout);
  }
  exit(0);
}
...
static void
processListMapsOptionForType(const char *const option CTAGS_ATTR_UNUSED,
                             const char *const parameter, langmapType type) {
  if (parameter[0] == '\0' || strcasecmp(parameter, RSV_LANG_ALL) == 0)
    printLanguageMaps(LANG_AUTO, type, localOption.withListHeader,
                      localOption.machinable, stdout);
  else {
    langType language = getNamedLanguage(parameter, 0);
    if (language == LANG_IGNORE)
      ...
      printLanguageMaps(language, type, localOption.withListHeader,
                        localOption.machinable, stdout);
  }
  exit(0);
}
...
static void processListMapExtensionsOption(const char *const option,
                                           const char *const parameter) {
  processListMapsOptionForType(option, parameter,
                               LMAP_EXTENSION | LMAP_TABLE_OUTPUT);
}

static void processListMapPatternsOption(const char *const option,
                                         const char *const parameter) {
  processListMapsOptionForType(option, parameter,
                               LMAP_PATTERN | LMAP_TABLE_OUTPUT);
}

static void
processListMapsOption(const char *const option CTAGS_ATTR_UNUSED,
                      const char *const parameter CTAGS_ATTR_UNUSED) {
  processListMapsOptionForType(option, parameter, LMAP_ALL);
}

static void
processListLanguagesOption(const char *const option CTAGS_ATTR_UNUSED,
                           const char *const parameter CTAGS_ATTR_UNUSED) {
  printLanguageList();
  exit(0);
}
...
static void
processListPseudoTagsOptions(const char *const option CTAGS_ATTR_UNUSED,
                             const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void
processListRegexFlagsOptions(const char *const option CTAGS_ATTR_UNUSED,
                             const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void processListMultilineRegexFlagsOptions(
    ...
    const char *const parameter CTAGS_ATTR_UNUSED) {
  printMultilineRegexFlags(localOption.withListHeader, localOption.machinable,
                           stdout);
  exit(0);
}
...
static void processListMultitableRegexFlagsOptions(
    const char *const option CTAGS_ATTR_UNUSED,
    const char *const parameter CTAGS_ATTR_UNUSED) {
  printMultitableRegexFlags(localOption.withListHeader, localOption.machinable,
                            stdout);
  exit(0);
}

static void
processListLangdefFlagsOptions(const char *const option CTAGS_ATTR_UNUSED,
                               const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void
processListKinddefFlagsOptions(const char *const option CTAGS_ATTR_UNUSED,
                               const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void processListRolesOptions(const char *const option CTAGS_ATTR_UNUSED,
  ...
  if (parameter == NULL || parameter[0] == '\0') {
    printLanguageRoles(LANG_AUTO, "*", localOption.withListHeader,
                       localOption.machinable, stdout);
    exit(0);
  }
  ...
  printLanguageRoles(lang, kindspecs, localOption.withListHeader,
                     localOption.machinable, stdout);
  exit(0);
}

static void
processListSubparsersOptions(const char *const option CTAGS_ATTR_UNUSED,
                             const char *const parameter) {
  langType lang;

  if (parameter == NULL || parameter[0] == '\0' ||
      (strcmp(parameter, RSV_LANG_ALL) == 0)) {
    printLanguageSubparsers(LANG_AUTO, localOption.withListHeader,
                            localOption.machinable, stdout);
    exit(0);
  }
  ...
  printLanguageSubparsers(lang, localOption.withListHeader,
                          localOption.machinable, stdout);
  exit(0);
}
...
static vString *expandOnOptlibPathList(const char *leaf) {
  ...
  return expandOnSearchPathList(OptlibPathList, leaf, doesFileExist);
}
...
static void processOutputFormat(const char *const option CTAGS_ATTR_UNUSED,
  ...
  if (strcmp(parameter, "u-ctags") == 0)
    ...
    error(FATAL, "unknown output format name supplied for \"%s=%s\"", option,
          parameter);
}
...
static void
processAnonHashOption(const char *const option CTAGS_ATTR_UNUSED,
                      const char *const parameter CTAGS_ATTR_UNUSED) {
  if (parameter == NULL || parameter[0] == '\0')
...
static void
processDumpKeywordsOption(const char *const option CTAGS_ATTR_UNUSED,
                          const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void
processForceInitOption(const char *const option CTAGS_ATTR_UNUSED,
                       const char *const parameter CTAGS_ATTR_UNUSED) {
...
static void
processVersionOption(const char *const option CTAGS_ATTR_UNUSED,
                     const char *const parameter CTAGS_ATTR_UNUSED) {
  printProgramIdentification();
  exit(0);
}
...
static parametricOption ParametricOptions[] = {
    ...
    {"list-mline-regex-flags", processListMultilineRegexFlagsOptions, true,
     STAGE_ANY},
    ...
    {"_list-mtable-regex-flags", processListMultitableRegexFlagsOptions, true,
     STAGE_ANY},
#ifdef DO_TRACING
...
static booleanOption BooleanOptions[] = {
    ...
    {"file-scope", ((bool *)XTAG_FILE_SCOPE), false, STAGE_ANY,
     setBooleanToXtag},
    {"file-tags", ((bool *)XTAG_FILE_NAMES), false, STAGE_ANY,
     setBooleanToXtag},
    ...
    {"use-slash-as-filename-separator",
     (bool *)&Option.useSlashAsFilenameSeparator, false, STAGE_ANY},
#endif
...
static void checkOptionOrder(const char *const option, bool longOption) {
  if (NonOptionEncountered)
    error(FATAL, "-%s%s option may not follow a file name",
          longOption ? "-" : "", option);
}
  ...
  for (i = 0; i < count && !found; ++i) {
    ...
    if (strcmp(option, entry->name) == 0) {
      ...
      if (!(entry->acceptableStages & (1UL << Stage))) {
        error(WARNING, "Cannot use --%s option in %s", option,
              StageDescription[Stage]);
        break;
      }
      if (entry->initOnly)
...
static void enableLanguageField(langType language, const char *field,
                                bool mode) {
...
#define PREFIX_LEN strlen(PREFIX)
  ...
  else if (len == strlen(RSV_LANG_ALL) &&
           (strncmp(lang, RSV_LANG_ALL, len) == 0))
    language = LANG_AUTO;
  else
...
extern int scanDirectory(const char *directory_name,
                         ...
                         int (*compare_function)(const struct dirent **,
                                                 const struct dirent **));

static void
parseConfigurationFileOptionsInDirectoryWithLeafname(const char *directory,
                                                     const char *leafname) {
...
static bool parseAllConfigurationFilesOptionsInDirectory(
    const char *const dirName, stringList *const already_loaded_files) {
  struct dirent **dents;
  int i, n;
...
static void parseConfigurationFileOptions(void) { preload(preload_path_list); }
...
static void
processDumpOptionsOption(const char *const option CTAGS_ATTR_UNUSED,
                         const char *const parameter CTAGS_ATTR_UNUSED) {
  fprintf(stdout, "# %s\n", "ParametricOptions");
