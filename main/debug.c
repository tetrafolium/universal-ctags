...
extern void debugEntry(const tagEntryInfo *const tag) {
  ...
  if (debug(DEBUG_PARSE)) {
    ...
    if (tag->extensionFields.scopeKindIndex != KIND_GHOST_INDEX &&
      ...
      printf(" [%s:%s]", scopeKindDef->name, tag->extensionFields.scopeName);
...
extern void debugAssert(const char *assertion, const char *file,
  ...
  if (getInputFileName()) {
    fprintf(stderr, "ctags: %s:%u: parsing %s:%lu as %s\n", file, line,
            getInputFileName(), getInputLineNumber(), getInputLanguageName());
  }
  fflush(stderr);
  abort();
}
...
extern void debugInc(void) { debugScopeDepth++; }
