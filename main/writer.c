...
static tagWriter *writerTable[WRITER_COUNT] = {
    [WRITER_U_CTAGS] = &uCtagsWriter, [WRITER_E_CTAGS] = &eCtagsWriter,
    [WRITER_ETAGS] = &etagsWriter,    [WRITER_XREF] = &xrefWriter,
    [WRITER_JSON] = &jsonWriter,      [WRITER_CUSTOM] = NULL,
};
...
extern void writerSetup(MIO *mio, void *clientData) {
  ...
  if (writer->preWriteEntry)
    writer->private = writer->preWriteEntry(writer, mio, writer->clientData);
  else
    writer->private = NULL;
}
...
extern bool writerTeardown(MIO *mio, const char *filename) {
  if (writer->postWriteEntry) {
    bool r;
    r = writer->postWriteEntry(writer, mio, filename, writer->clientData);
    writer->private = NULL;
    return r;
  }
  return false;
...
extern int writerWriteTag(MIO *mio, const tagEntryInfo *const tag) {
  return writer->writeEntry(writer, mio, tag, writer->clientData);
}

extern int writerWritePtag(MIO *mio, const ptagDesc *desc,
  ...
  return writer->writePtagEntry(writer, mio, desc, fileName, pattern,
                                parserName, writer->clientData);
}
...
extern bool ptagMakeCtagsOutputMode(ptagDesc *desc,
  ...
  return writePseudoTag(desc, mode, "u-ctags or e-ctags", NULL);
}
