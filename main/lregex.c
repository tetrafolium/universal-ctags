 ...
 */
static int getTableIndexForName(const struct lregexControlBlock *const lcb,
                                const char *name);
...
extern struct lregexControlBlock *
allocLregexControlBlock(parserDefinition *parser) {
...
static void initRegexTag(tagEntryInfo *e, const vString *const name,
                         int kindIndex, int roleIndex, int scopeIndex,
                         int placeholder, unsigned long line, MIOPos *pos,
                         int xtag_type) {
 ...
 */
static bool parseTagRegex(enum regexParserType regptype, char *const regexp,
                          char **const name, char **const kinds,
                          char **const flags) {
  bool result = false;
...
static void pre_ptrn_flag_exclusive_short(char c CTAGS_ATTR_UNUSED,
                                          void *data) {
  bool *exclusive = data;
  *exclusive = true;
}
...
static void
pre_ptrn_flag_exclusive_long(const char *const s CTAGS_ATTR_UNUSED,
                             const char *const unused CTAGS_ATTR_UNUSED,
                             void *data) {
  pre_ptrn_flag_exclusive_short('x', data);
}

static flagDefinition prePtrnFlagDef[] = {
    {'x', "exclusive", pre_ptrn_flag_exclusive_short,
     pre_ptrn_flag_exclusive_long, NULL,
...
static void placeholder_ptrn_flag_eval(const char *const f CTAGS_ATTR_UNUSED,
                                       const char *const v CTAGS_ATTR_UNUSED,
                                       void *data) {
  unsigned int *bfields = data;
...
static flagDefinition scopePtrnFlagDef[] = {
    {'\0', "scope", NULL, scope_ptrn_flag_eval, "ACTION",
     "use scope stack: ACTION = ref|push|pop|clear|set"},
    {'\0', "placeholder", NULL, placeholder_ptrn_flag_eval, NULL,
     "don't put this tag to tags file."},
};

static kindDefinition *kindNew(char letter, const char *name,
...
static void pre_ptrn_flag_mgroup_long(const char *const s, const char *const v,
                                      void *data) {
  ...
  if (!strToInt(v, 10, &mgroup->forLineNumberDetermination)) {
    ...
    error(WARNING, "out of range(0 ~ %d) %s specification: %s",
          (BACK_REFERENCE_COUNT - 1), s, v);
    mgroup->forLineNumberDetermination = NO_MULTILINE;
  }
}

static void pre_ptrn_flag_advanceTo_long(const char *const s,
                                         const char *const v, void *data) {
  struct mGroupSpec *mgroup = data;
  char *vdup;
  char *tmp;
  ...
  if (!strToInt(vdup, 10, &(mgroup->forNextScanning))) {
    ...
    mgroup->nextFromStart = false;
  } else if (mgroup->forNextScanning < 0 ||
             mgroup->forNextScanning >= BACK_REFERENCE_COUNT) {
    error(WARNING, "out of range(0 ~ %d) %s specification: %s",
...
static void pre_ptrn_flag_guest_long(const char *const s, const char *const v,
                                     void *data) {
  ...
  if ((tmp - v) == 0) {
    if (type == REG_PARSER_MULTI_LINE) {
      error(WARNING,
            "using placeholder for guest name field is not allowed in "
            "multiline regex spec: %s",
            v);
      goto err;
    }
    ...
    *(char *)n_tmp = '\0';
    if (!strToInt(n, 10, &(guest->lang.spec.patternGroup))) {
      ...
      error(WARNING,
            "wrong guest name specification (back reference count is too "
            "large): %d",
    ...
    *(char *)n_tmp = c;
    if (*n_tmp != ',') {
      error(WARNING,
            "wrong guest specification (garbage at the end of end guest spec): "
            "%s",
            v);
      goto err;
    }

    guest->lang.type = (*v == '\\') ? GUEST_LANG_PTN_GROUP_FOR_LANGNAME
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (tmp[0] == ((i == BOUNDARY_START) ? ',' : '\0')) {
      if (type == REG_PARSER_MULTI_LINE)
        error(WARNING,
              "using placeholder for %s field is not allowed in multiline "
              "regex spec: %s",
              current_field_str, v);
      ...
      *tmp = '\0';
      if (!strToInt(n, 10, &(current->patternGroup))) {
        error(WARNING,
              "wrong guest area specification (patternGroup of %s, number "
              "expected): %s:%s",
      ...
      *tmp = c;
      if (*tmp == '\0') {
        error(WARNING,
              "wrong guest area specification (patternGroup of %s, nether "
              "start nor end given): %s",
              current_field_str, v);
        goto err;
    ...
    if (i == 0) {
      if (*tmp != ',') {
        error(WARNING,
              "wrong guest area specification (separator between start and end "
              "boundaries): %s",
              v);
        goto err;
      }
      ...
      error(WARNING,
            "wrong guest area specification (garbage at the end of end "
            "boundary spec): %s",
            v);
      goto err;
    }
...
static flagDefinition multilinePtrnFlagDef[] = {
    {'\0', "mgroup", NULL, pre_ptrn_flag_mgroup_long, "N",
     "a group in pattern determining the line number of tag"},
    {'\0', "_advanceTo", NULL, pre_ptrn_flag_advanceTo_long, "N[start|end]",
...
static void common_flag_msg_long(const char *const s, const char *const v,
                                 void *data) {
...
static void common_flag_extra_long(const char *const s, const char *const v,
                                   void *data) {
  struct commonFlagData *cdata = data;
  ...
  if (cdata->ptrn->xtagType == XTAG_UNKNOWN)
    error(WARNING, "no such extra \"%s\" in %s", v,
          getLanguageName(cdata->owner));
}

static struct fieldPattern *fieldPatternNew(fieldType ftype,
                                            const char *template) {
...
static void common_flag_field_long(const char *const s, const char *const v,
                                   void *data) {
  struct commonFlagData *cdata = data;
  regexPattern *ptrn = cdata->ptrn;
  ...
  if (ftype == FIELD_UNKNOWN) {
    error(WARNING, "no such field \"%s\" in %s", fname,
          getLanguageName(cdata->owner));
    eFree(fname);
    return;
  }
  ...
  if (ptrn->fieldPatterns) {
    for (unsigned int i = 0; i < ptrArrayCount(ptrn->fieldPatterns); i++) {
      ...
      if (fp->ftype == ftype) {
        error(WARNING, "duplicated field specification \"%s\" in %s", fname,
              getLanguageName(cdata->owner));
        eFree(fname);
        return;
      }
...
static void common_flag_role_long(const char *const s, const char *const v,
                                  void *data) {
  struct commonFlagData *cdata = data;
  regexPattern *ptrn = cdata->ptrn;
  roleDefinition *role;
  ...
  role = getLanguageRoleForName(cdata->owner, ptrn->u.tag.kindIndex, v);
  if (!role) {
...
static void common_flag_anonymous_long(const char *const s, const char *const v,
                                       void *data) {
  struct commonFlagData *cdata = data;
  regexPattern *ptrn = cdata->ptrn;
  ...
  if (ptrn->u.tag.kindIndex == KIND_GHOST_INDEX) {
    error(WARNING, "use \"%s\" regex flag only with an explicitly defined kind",
          s);
    return;
  }
...
static flagDefinition commonSpecFlagDef[] = {
    {'\0', "fatal", NULL, common_flag_msg_long, "\"MESSAGE\"",
     "print the given MESSAGE and exit"},
    {'\0', "warning", NULL, common_flag_msg_long, "\"MESSAGE\"",
     "print the given MESSAGE at WARNING level"},
#define EXPERIMENTAL "_"
    {'\0', EXPERIMENTAL "extra", NULL, common_flag_extra_long, "EXTRA",
     "record the tag only when the extra is enabled"},
    {'\0', EXPERIMENTAL "field", NULL, common_flag_field_long, "FIELD:VALUE",
     "record the matched string(VALUE) to parser own FIELD of the tag"},
    {'\0', EXPERIMENTAL "role", NULL, common_flag_role_long, "ROLE",
     "set the given ROLE to the roles field"},
    {'\0', EXPERIMENTAL "anonymous", NULL, common_flag_anonymous_long, "PREFIX",
     "make an anonymous tag with PREFIX"},
};

static void pre_ptrn_flag_mtable_long(const char *const s, const char *const v,
                                      void *data) {
  struct commonFlagData *cdata = data;
  regexPattern *ptrn = cdata->ptrn;
  struct mTableActionSpec *taction;
  ...
  if (taking_table) {
    ...
    if (taction->action == TACTION_ENTER && (continuation = strchr(v, ','))) {
      char *tableEnterTo;
      ...
      if (t_cont < 0)
        error(FATAL, "table for continuation is not defined: %s",
              continuation + 1);
...
static flagDefinition multitablePtrnFlagDef[] = {
    {'\0', "tenter", NULL, pre_ptrn_flag_mtable_long, "TABLE[,CONT]",
     "enter to given regext table (with specifying continuation)"},
    {'\0', "tleave", NULL, pre_ptrn_flag_mtable_long, NULL,
     "leave from the current regext table"},
    {'\0', "tjump", NULL, pre_ptrn_flag_mtable_long, "TABLE",
     "jump to another regext table(don't push the current table to state "
     "stack)"},
    {'\0', "treset", NULL, pre_ptrn_flag_mtable_long, "TABLE",
     "clear the state stack and jump to given regex table"},
    {'\0', "tquit", NULL, pre_ptrn_flag_mtable_long, NULL,
...
static void setKind(regexPattern *ptrn, const langType owner,
  ...
  if (kdef) {
    if (strcmp(kdef->name, kindName) &&
        (strcmp(kindName, KIND_REGEX_DEFAULT_NAME)))
      /* When using a same kind letter for multiple regex patterns, the name of
         kind should be the same. */
      error(WARNING,
            "Don't reuse the kind letter `%c' in a language %s (old: \"%s\", "
            "new: \"%s\")",
            kdef->letter, getLanguageName(owner), kdef->name, kindName);
...
static void patternEvalFlags(struct lregexControlBlock *lcb, regexPattern *ptrn,
                             enum regexParserType regptype, const char *flags) {
  struct commonFlagData commonFlagData = {
      .owner = lcb->owner, .lcb = lcb, .ptrn = ptrn};

  if (regptype == REG_PARSER_SINGLE_LINE)
    flagsEval(flags, prePtrnFlagDef, ARRAY_SIZE(prePtrnFlagDef),
              &ptrn->exclusive);

  flagsEval(flags, commonSpecFlagDef, ARRAY_SIZE(commonSpecFlagDef),
            &commonFlagData);

  if (regptype == REG_PARSER_SINGLE_LINE || regptype == REG_PARSER_MULTI_TABLE)
    flagsEval(flags, scopePtrnFlagDef, ARRAY_SIZE(scopePtrnFlagDef),
              &ptrn->scopeActions);
  ...
  if (regptype == REG_PARSER_MULTI_LINE || regptype == REG_PARSER_MULTI_TABLE) {
    ...
    flagsEval(flags, multilinePtrnFlagDef, ARRAY_SIZE(multilinePtrnFlagDef),
              &ptrn->mgroup);
  }
  ...
  flagsEval(flags, guestPtrnFlagDef, ARRAY_SIZE(guestPtrnFlagDef),
            &guestPtrnFlagData);

  if (regptype == REG_PARSER_MULTI_TABLE)
    flagsEval(flags, multitablePtrnFlagDef, ARRAY_SIZE(multitablePtrnFlagDef),
              &commonFlagData);
}

static regexPattern *
addCompiledTagPattern(struct lregexControlBlock *lcb, int table_index,
                      enum regexParserType regptype, regex_t *const pattern,
                      const char *const name, char kindLetter,
                      const char *kindName, char *const description,
                      const char *flags, bool kind_explicitly_defined,
                      bool *disabled) {
  regexPattern *ptrn =
      addCompiledTagCommon(lcb, table_index, pattern, regptype);
...
static regexPattern *
addCompiledCallbackPattern(struct lregexControlBlock *lcb,
                           regex_t *const pattern, const regexCallback callback,
                           const char *flags, bool *disabled, void *userData) {
  regexPattern *ptrn;
  bool exclusive = false;
  flagsEval(flags, prePtrnFlagDef, ARRAY_SIZE(prePtrnFlagDef), &exclusive);
  ptrn = addCompiledTagCommon(lcb, TABLE_INDEX_UNUSED, pattern,
...
static void regex_flag_basic_long(const char *const s CTAGS_ATTR_UNUSED,
                                  const char *const unused CTAGS_ATTR_UNUSED,
                                  void *data) {
...
static void regex_flag_extend_long(const char *const c CTAGS_ATTR_UNUSED,
                                   const char *const unused CTAGS_ATTR_UNUSED,
                                   void *data) {
...
static void regex_flag_icase_long(const char *s CTAGS_ATTR_UNUSED,
                                  const char *const unused CTAGS_ATTR_UNUSED,
                                  void *data) {
...
static flagDefinition regexFlagDefs[] = {
    {'b', "basic", regex_flag_basic_short, regex_flag_basic_long, NULL,
     "interpreted as a Posix basic regular expression."},
    {'e', "extend", regex_flag_extend_short, regex_flag_extend_long, NULL,
     "interpreted as a Posix extended regular expression (default)"},
    {'i', "icase", regex_flag_icase_short, regex_flag_icase_long, NULL,
...
static regex_t *compileRegex(enum regexParserType regptype,
  ...
  flagsEval(flags, regexFlagDefs, ARRAY_SIZE(regexFlagDefs), &cflags);
...
static bool parseKinds(const char *const kindSpec, char *const kindLetter,
...
static void fillEndLineFieldOfUpperScopes(struct lregexControlBlock *lcb,
...
static void matchTagPattern(struct lregexControlBlock *lcb, const char *line,
                            const regexPattern *const patbuf,
                            const regmatch_t *const pmatch, off_t offset) {
  vString *const name =
      (patbuf->u.tag.name_pattern[0] != '\0')
          ? substitute(line, patbuf->u.tag.name_pattern, BACK_REFERENCE_COUNT,
                       pmatch)
          : (patbuf->anonymous_tag_prefix)
                ? anonGenerateNew(patbuf->anonymous_tag_prefix,
                                  patbuf->u.tag.kindIndex)
                : vStringNewInit("");
  bool placeholder =
      !!((patbuf->scopeActions & SCOPE_PLACEHOLDER) == SCOPE_PLACEHOLDER);
  ...
  if (vStringLength(name) == 0 && (placeholder == false)) {
    ...
    initRegexTag(&e, name, kind, ROLE_DEFINITION_INDEX, scope, placeholder, ln,
                 ln == 0 ? NULL : &pos, patbuf->xtagType);
    ...
    if (patbuf->fieldPatterns) {
      for (unsigned int i = 0; i < ptrArrayCount(patbuf->fieldPatterns); i++) {
        ...
        if (isFieldEnabled(fp->ftype)) {
          vString *const value =
              substitute(line, fp->template, BACK_REFERENCE_COUNT, pmatch);
    ...
    if (roleBits) {
      ...
      for (roleIndex = 0; roleIndex < countLanguageRoles(e.langType, kind);
           roleIndex++) {
...
static bool matchCallbackPattern(const vString *const line,
                                 const regexPattern *const patbuf,
...
static void printMessage(const langType language,
                         const regexPattern *const ptrn, const off_t offset,
  ...
  msg = substitute(line, ptrn->message.message_string, BACK_REFERENCE_COUNT,
                   pmatch);

  error(ptrn->message.selection, "%sMessage from regex<%s>: %s (%s:%lu)",
        (ptrn->message.selection == FATAL ? "Fatal: " : ""),
        getLanguageName(language), vStringValue(msg), getInputFileName(),
...
static bool isGuestRequestConsistent(struct guestRequest *guest_req) {
  return (guest_req->lang != LANG_IGNORE) &&
         (guest_req->boundary[BOUNDARY_START].offset <
          guest_req->boundary[BOUNDARY_END].offset);
}

static bool fillGuestRequest(const char *start, const char *current,
  ...
  if (guest_spec->lang.type == GUEST_LANG_UNKNOWN)
    ...
    const char *name =
        current + pmatch[guest_spec->lang.spec.patternGroup].rm_so;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (!boundary_spec->placeholder) {
      boundary->offset = current - start +
                         (boundary_spec->fromStartOfGroup
...
static bool matchRegexPattern(struct lregexControlBlock *lcb,
  ...
  match = regexec(patbuf->pattern, vStringValue(line), BACK_REFERENCE_COUNT,
                  pmatch, 0);
  if (match == 0) {
    result = true;
    ...
    if (patbuf->type == PTRN_TAG) {
      ...
      if (guest->lang.type != GUEST_LANG_UNKNOWN) {
        ...
        if (fillGuestRequest(vStringValue(line) - current, vStringValue(line),
                             pmatch, guest, lcb->guest_req)) {
...
static bool matchMultilineRegexPattern(struct lregexControlBlock *lcb,
  ...
  do {
    match = regexec(patbuf->pattern, current, BACK_REFERENCE_COUNT, pmatch, 0);
    if (match != 0) {
      entry->statistics.unmatch++;
      break;
    }
    ...
    if (hasMessage(patbuf))
      printMessage(lcb->owner, patbuf, (current + pmatch[0].rm_so) - start,
                   current, pmatch);

    offset =
        (current + pmatch[mgroup->forLineNumberDetermination].rm_so) - start;
    ...
    delta = (mgroup->nextFromStart ? pmatch[mgroup->forNextScanning].rm_so
 ...
 * on pattern matched.
 */
extern bool matchRegex(struct lregexControlBlock *lcb,
                       const vString *const line) {
  bool result = false;
  unsigned int i;
  for (i = 0; i < ptrArrayCount(lcb->entries[REG_PARSER_SINGLE_LINE]); ++i) {
    regexTableEntry *entry =
        ptrArrayItem(lcb->entries[REG_PARSER_SINGLE_LINE], i);
    regexPattern *ptrn = entry->pattern;

    Assert(ptrn);

    if ((ptrn->xtagType != XTAG_UNKNOWN) && (!isXtagEnabled(ptrn->xtagType)))
      continue;
...
extern void findRegexTags(void) { findRegexTagsMainloop(fileReadLineDriver); }
...
static regexPattern *
addTagRegexInternal(struct lregexControlBlock *lcb, int table_index,
                    enum regexParserType regptype, const char *const regex,
  ...
  if (kindLetter == fileKind->letter)
    error(FATAL,
          "Kind letter \'%c\' used in regex definition \"%s\" of %s language "
          "is reserved in ctags main",
          kindLetter, regex, getLanguageName(lcb->owner));
  else if (!isalpha((unsigned char)kindLetter))
    error(FATAL, "Kind letter must be an alphabetical character: \"%c\"",
          kindLetter);

  if (strcmp(kindName, fileKind->name) == 0)
    error(FATAL,
          "Kind name \"%s\" used in regex definition \"%s\" of %s language is "
          "reserved in ctags main",
          kindName, regex, getLanguageName(lcb->owner));

  const char *option_bsae =
      (regptype == REG_PARSER_SINGLE_LINE
           ? "regex"
           : regptype == REG_PARSER_MULTI_LINE
                 ? "mline-regex"
                 : regptype == REG_PARSER_MULTI_TABLE ? "_mtable-regex" : NULL);
  Assert(option_bsae);
    ...
    if (p == kindName) {
      if (!isalpha(*p))
        error(FATAL,
              ...
              "'%s' in \"--%s-%s\" option",
              kindName, option_bsae, getLanguageName(lcb->owner));
    } else {
      /*
  ...
  if (*name == '\0') {
    if (rptr->exclusive || rptr->scopeActions & SCOPE_PLACEHOLDER ||
        rptr->anonymous_tag_prefix || regptype == REG_PARSER_MULTI_TABLE ||
...
extern void addTagRegex(struct lregexControlBlock *lcb, const char *const regex,
                        const char *const name, const char *const kinds,
                        const char *const flags, bool *disabled) {
  addTagRegexInternal(lcb, TABLE_INDEX_UNUSED, REG_PARSER_SINGLE_LINE, regex,
                      name, kinds, flags, disabled);
}

extern void addTagMultiLineRegex(struct lregexControlBlock *lcb,
                                 const char *const regex,
                                 const char *const name,
                                 const char *const kinds,
                                 const char *const flags, bool *disabled) {
  addTagRegexInternal(lcb, TABLE_INDEX_UNUSED, REG_PARSER_MULTI_LINE, regex,
                      name, kinds, flags, disabled);
}
...
extern void addTagMultiTableRegex(struct lregexControlBlock *lcb,
  ...
  addTagRegexInternal(lcb, table_index, REG_PARSER_MULTI_TABLE, regex, name,
                      kinds, flags, disabled);
}

extern void addCallbackRegex(struct lregexControlBlock *lcb,
                             const char *const regex, const char *const flags,
                             const regexCallback callback, bool *disabled,
                             void *userData) {
  Assert(regex != NULL);
...
static void addTagRegexOption(struct lregexControlBlock *lcb,
  ...
  if (parseTagRegex(regptype, regex_pat, &name, &kinds, &flags))
    addTagRegexInternal(lcb, table_index, regptype, regex_pat, name, kinds,
                        flags, NULL);
...
extern void printRegexFlags(bool withListHeader, bool machinable, FILE *fp) {
  ...
  flagsColprintAddDefinitions(table, prePtrnFlagDef,
                              ARRAY_SIZE(prePtrnFlagDef));
  flagsColprintAddDefinitions(table, guestPtrnFlagDef,
                              ARRAY_SIZE(guestPtrnFlagDef));
  flagsColprintAddDefinitions(table, scopePtrnFlagDef,
                              ARRAY_SIZE(scopePtrnFlagDef));
  flagsColprintAddDefinitions(table, commonSpecFlagDef,
                              ARRAY_SIZE(commonSpecFlagDef));

  flagsColprintTablePrint(table, withListHeader, machinable, fp);
  colprintTableDelete(table);
}

extern void printMultilineRegexFlags(bool withListHeader, bool machinable,
                                     FILE *fp) {
  struct colprintTable *table;
  ...
  flagsColprintAddDefinitions(table, regexFlagDefs, ARRAY_SIZE(regexFlagDefs));
  flagsColprintAddDefinitions(table, multilinePtrnFlagDef,
                              ARRAY_SIZE(multilinePtrnFlagDef));
  flagsColprintAddDefinitions(table, guestPtrnFlagDef,
                              ARRAY_SIZE(guestPtrnFlagDef));
  flagsColprintAddDefinitions(table, commonSpecFlagDef,
                              ARRAY_SIZE(commonSpecFlagDef));
...
extern void printMultitableRegexFlags(bool withListHeader, bool machinable,
                                      FILE *fp) {
  struct colprintTable *table;
  ...
  flagsColprintAddDefinitions(table, multilinePtrnFlagDef,
                              ARRAY_SIZE(multilinePtrnFlagDef));
  flagsColprintAddDefinitions(table, multitablePtrnFlagDef,
                              ARRAY_SIZE(multitablePtrnFlagDef));
  flagsColprintAddDefinitions(table, guestPtrnFlagDef,
                              ARRAY_SIZE(guestPtrnFlagDef));
  flagsColprintAddDefinitions(table, scopePtrnFlagDef,
                              ARRAY_SIZE(scopePtrnFlagDef));
...
extern void freeRegexResources(void) { /* TODO: SHOULD BE REMOVED */ }
...
extern bool matchMultilineRegex(struct lregexControlBlock *lcb,
                                const vString *const allLines) {
  bool result = false;
  ...
  for (i = 0; i < ptrArrayCount(lcb->entries[REG_PARSER_MULTI_LINE]); ++i) {
    regexTableEntry *entry =
        ptrArrayItem(lcb->entries[REG_PARSER_MULTI_LINE], i);
    Assert(entry && entry->pattern);
...
extern void addRegexTable(struct lregexControlBlock *lcb, const char *name) {
  ...
  for (c = name; *c; c++)
    if (!(isalnum(*c) || *c == '_'))
      error(FATAL, "`%c' in \"%s\" is not acceptable as part of table name", *c,
            name);
...
static void
printMultitableMessage(const langType language, const char *const tableName,
                       const unsigned int index, const regexPattern *const ptrn,
                       const off_t offset, const char *const current,
                       const regmatch_t *const pmatch) {
  vString *msg;
  ...
  msg = substitute(current, ptrn->message.message_string, BACK_REFERENCE_COUNT,
                   pmatch);

  error(ptrn->message.selection,
        "%sMessage from mtable<%s/%s[%2u]>: %s (%s:%lu)",
        (ptrn->message.selection == FATAL ? "Fatal: " : ""),
        getLanguageName(language), tableName, index, vStringValue(msg),
        getInputFileName(), getInputLineNumberForFileOffset(offset));

  vStringDelete(msg);
}

static struct regexTable *
matchMultitableRegexTable(struct lregexControlBlock *lcb,
                          struct regexTable *table, const vString *const start,
                          unsigned int *offset) {
...
restart:
  ...
  for (unsigned int i = 0; i < ptrArrayCount(table->entries); i++) {
    ...
    match = regexec(ptrn->pattern, current, BACK_REFERENCE_COUNT, pmatch, 0);
    ...
    if (match == 0) {
      ...
      if (ptrn->type == PTRN_TAG) {
        ...
        matchTagPattern(
            lcb, current, ptrn, pmatch,
            (current + pmatch[ptrn->mgroup.forLineNumberDetermination].rm_so) -
                cstart);
        BEGIN_VERBOSE(vfp);
        {
        ...
        if (hasMessage(ptrn))
          printMultitableMessage(lcb->owner, table->name, i, ptrn, *offset,
                                 current, pmatch);
        ...
        *offset += delta;
        ...
        case TACTION_ENTER:
          ...
          ptrArrayAdd(lcb->tstack, taction->continuation_table
          ...
          BEGIN_VERBOSE(vfp);
          {
            if (taction->continuation_table)
              fprintf(vfp, "action: [enter] to {%s}, cont: {%s}, stack: /",
                      next->name, taction->continuation_table->name);
            else
        ...
        case TACTION_LEAVE:
          ...
          if (ptrArrayCount(lcb->tstack) == 0) {
            error(WARNING, "leave is specified as regex table action but the "
        ...
        case TACTION_JUMP:
          ...
          BEGIN_VERBOSE(vfp);
          {
            fprintf(vfp, "action: [jump] from {%s} to {%s}, stack: /",
...
extern void extendRegexTable(struct lregexControlBlock *lcb, const char *src,
                             const char *dist) {
  ...
  if (i < 0)
    error(FATAL, "no such regex table in %s: %s", getLanguageName(lcb->owner),
          src);
  ...
  for (unsigned int i = 0; i < ptrArrayCount(lcb->tables); i++) {
    ...
    for (unsigned int j = 0; j < ptrArrayCount(table->entries); j++) {
      ...
      fprintf(stderr, "%10u/%-10u%-40s ref: %d\n", entry->statistics.match,
              ...
              entry->pattern->pattern_string, entry->pattern->refcount);
    }
    fputc('\n', stderr);
  }
}

extern bool matchMultitableRegex(struct lregexControlBlock *lcb,
                                 const vString *const allLines) {
  if (ptrArrayCount(lcb->tables) == 0)
    return false;
  ...
  while (table) {
    ...
    if (motionless_counter > MTABLE_MOTIONLESS_MAX) {
      error(WARNING,
            "mtable<%s/%s>: the input cursor stays at %u in %s so long though "
            "the tables are switched",
            getLanguageName(lcb->owner), table->name, offset,
            getInputFileName());
      break;
    }
...
static int makePromiseForAreaSpecifiedWithOffsets(const char *parser,
  ...
  return makePromise(parser, startLine, startOffset - startLineOffset, endLine,
                     endOffset - endLineOffset, startOffset - startLineOffset);
}
...
static void guestRequestDelete(struct guestRequest *r) { eFree(r); }

static bool guestRequestIsFilled(struct guestRequest *r) {
  return (r->lang_set && (r->boundary + 0)->offset_set &&
          (r->boundary + 1)->offset_set);
}
...
static void guestRequestSubmit(struct guestRequest *r) {
  ...
  verbose("guestRequestSubmit: %s; "
          "range: %" PRId64 " - %" PRId64 "\n",
          langName, (int64_t)r->boundary[BOUNDARY_START].offset,
