...
static int writeEntry(tagWriter *writer, MIO *mio,
...
/* we need to be able to provide an error printer which doesn't crash Geany on
 * error */
...
/* we need to be able to enable all kinds for all languages (some are disabled
 * by default) */
...
/* we need to be able to initialize ctags like in main() but skipping some
 * things */
...
static const char *ctagsGetLangName(int lang) { return getLanguageName(lang); }
...
static unsigned int ctagsGetLangCount(void) { return countParsers(); }

/*******************************************************************************
 * So let's just use what we have for our great client...
 ******************************************************************************/

/* our internal tag representation - this is all the tag information we use in
 * Geany */
typedef struct {
  char *name;
  char *signature;
 ...
 * tell us the number of valid tags so remove all the extra tags and shrink the
 * array */
static void rescanFailed(tagWriter *writer, unsigned long validTagNum,
                         void *clientData) {
...
extern int main(int argc, char **argv) {
   ...
   * NOTE: Geany doesn't use the ptrArray type - it is used just for the purpose
   * of this demo */
  ...
  if (argc == 1) /* parsing contents of a buffer */
  {
     ...
     */
    parseRawBuffer("whatever", (unsigned char *)program, strlen(program), lang,
                   tagArray);
