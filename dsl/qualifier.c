...
struct sCode {
  ...
  const char *helpstr;
    ...
    {"$input", value_input, NULL, MEMORABLE, 0UL, .helpstr = "input file name"},
    ...
    {"$roles", value_roles, NULL, MEMORABLE, 0UL, .helpstr = "<list>"},
    {"$pattern", value_pattern, NULL, MEMORABLE, 0UL},
    {"$inherits", value_inherits, NULL, MEMORABLE, 0UL, .helpstr = "<list>"},
...
static EsObject *eval0(EsObject *object, tagEntry *entry) {
  if (es_null(object))
    ...
    return es_object_autounref(
        es_cons(eval(es_car(object), entry), eval0(es_cdr(object), entry)));
}
static EsObject *eval(EsObject *object, tagEntry *entry) {
  EsObject *r;
  Code *code;
...
DEFINE_OP_WITH_CHECK(lt, es_number_get(a) < es_number_get(b), es_number_p,
                     NUMBER_REQUIRED, es_symbol_intern("<"));
DEFINE_OP_WITH_CHECK(gt, es_number_get(a) > es_number_get(b), es_number_p,
                     NUMBER_REQUIRED, es_symbol_intern(">"));
DEFINE_OP_WITH_CHECK(le, es_number_get(a) <= es_number_get(b), es_number_p,
                     NUMBER_REQUIRED, es_symbol_intern("<="));
DEFINE_OP_WITH_CHECK(ge, es_number_get(a) >= es_number_get(b), es_number_p,
                     NUMBER_REQUIRED, es_symbol_intern(">="));
...
static EsObject *builtin_prefix(EsObject *args, tagEntry *entry) {
  ...
  if ((!es_string_p(target)) || (!es_string_p(prefix)))
    throw(WRONG_TYPE_ARGUMENT, es_symbol_intern("prefix?"));
...
static EsObject *builtin_suffix(EsObject *args, tagEntry *entry) {
  ...
  if ((!es_string_p(target)) || (!es_string_p(suffix)))
    throw(WRONG_TYPE_ARGUMENT, es_symbol_intern("suffix?"));
...
static EsObject *builtin_substr(EsObject *args, tagEntry *entry) {
  ...
  if ((!es_string_p(target)) || (!es_string_p(substr)))
...
static EsObject *value_xget_csvlist(tagEntry *entry, const char *field) {
  ...
  if (inherits == NULL)
    ...
    while ((t = strchr(h, ','))) {
      *t = '\0';
      s = es_cons(es_object_autounref(es_string_new(h)), s);
...
static EsObject *builtin_entry_ref(EsObject *args, tagEntry *entry) {
  ...
  if (es_error_p(key))
    ...
    throw(WRONG_TYPE_ARGUMENT, es_symbol_intern("$"));
  else
...
enum QRESULT q_is_acceptable(QCode *code, tagEntry *entry) {
  ...
  if (es_object_equal(r, es_false))
    ...
    fprintf(stderr, "GOT ERROR in QUALIFYING: %s: ", es_error_name(r));
