...
static const char *const Usage =
    ...
    "    -            Treat arguments after this as NAME even if they start "
    "with -.\n"
...
static QCode *convertToQualifier(const char *exp) {
  ...
  if (es_error_p(sexp)) {
    fprintf(stderr, "Failed to read the expression of qualifier: %s\n", exp);
  ...
  if (qcode == NULL) {
    fprintf(stderr, "Failed to compile the expression of qualifier: %s\n", exp);
    exit(1);
  }
