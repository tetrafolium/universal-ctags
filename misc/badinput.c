/* badinput.c: do bisect-quest to find minimal input which breaks the target
   command execution
   ...
   Here is an example:
        ...
        $ misc/badinput "timeout 1 ./ctags -o - --language-force=Ada %s >
   /dev/null" Test/1880687.js /tmp/output.txt
   ...
   The size of original Test/1880687.js is 2258 bytes.
        ...
        $ misc/badinput "timeout 1 ./ctags -o - --language-force=Ada %s >
   /dev/null" Test/1880687.js /tmp/output.txt [0, 2448]...31744 [0, 0]...0
   ...
   New shorter input, only 38 bytes, which can reproduce the issue is reported
   at the end. This new input is useful for debugging.

   The result is shown in stdout and is recorded to the file specified as
   OUTPUT. */
...
static void print_help(const char *prog, FILE *fp, int status) {
  fprintf(fp, "Usage:\n");
  fprintf(fp, "	%s --help|-h\n", prog);
...
static void load(const char *input_file, char **input, size_t *len) {
  int input_fd;
...
static int test(char *cmdline, char *input, off_t start, size_t len,
                int output_fd) {
  int r;
...
int main(int argc, char **argv) {
  ...
  if (test(cmdline, input, 0, len, output_fd) == 0) {
    fprintf(
        stderr,
        "the target command line exits normally against the original input\n");
    exit(1);
  }

  if (test(cmdline, input, 0, 0, output_fd) != 0) {
    fprintf(stderr,
            "the target command line exits normally against the empty input\n");
    exit(1);
  }
